...
inline const QHash<QString, QString>& notes() {
	static const QHash<QString, QString> kNotes = {
		...
		{"D13", QObject::tr(R"note(The same shared Cornell box, but the camera itself moves during the exposure - keyframed from one side to the other via real AnimatedTransform interpolation (pbrt-v4's motion-blur technique), with each ray sampling its own time within the shutter interval instead of a single fixed camera position. CPU only for now; GPU renders a static frame at the first keyframe.)note")},
