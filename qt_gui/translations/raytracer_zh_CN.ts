...
（如果已有渲染正在进行，则改为加入队列排在其后）</translation>
    ...
    <message>
        ...
        <translation>&lt;h3&gt;Ray Tracer&lt;/h3&gt;&lt;p&gt;一款基于物理的路径追踪器，拥有并行的 CPU 与 GPU(OptiX)后端，从 &lt;i&gt;Ray Tracing in One Weekend&lt;/i&gt; 系列逐步扩展为具备 pbrt-v4 风格特性集的完整渲染器。&lt;/p&gt;&lt;p&gt;149 个场景、丰富的 BxDF 材质库、多种光源与相机类型、三角网格与纹理支持、BVH 加速结构、体积渲染效果，以及在标准路径追踪之外可选的多种积分器——SPPM 光子映射、双向路径追踪(BDPT)、Metropolis 光传输(MLT)，以及若干参考/调试用积分器(RandomWalk、Ambient Occlusion、SimplePath、SimpleVolPath、LightPath)。&lt;/p&gt;&lt;p&gt;本窗口以子进程方式驱动 &lt;code&gt;%1&lt;/code&gt;。&lt;/p&gt;</translation>
    </message>
    <message>
        ...
        <translation type="vanished">&lt;h3&gt;Ray Tracer&lt;/h3&gt;&lt;p&gt;一款基于物理的路径追踪器，拥有并行的 CPU 与 GPU(OptiX)后端，从 &lt;i&gt;Ray Tracing in One Weekend&lt;/i&gt; 系列逐步扩展为具备 pbrt-v4 风格特性集的完整渲染器。&lt;/p&gt;&lt;p&gt;123 个场景、丰富的 BxDF 材质库、多种光源与相机类型、三角网格与纹理支持、BVH 加速结构、体积渲染效果，以及在标准路径追踪之外可选的多种积分器——SPPM 光子映射、双向路径追踪(BDPT)、Metropolis 光传输(MLT)，以及若干参考/调试用积分器(RandomWalk、Ambient Occlusion、SimplePath、SimpleVolPath、LightPath)。&lt;/p&gt;&lt;p&gt;本窗口以子进程方式驱动 &lt;code&gt;%1&lt;/code&gt;。&lt;/p&gt;</translation>
    </message>
    <message>
...
%1</translation>
    ...
    <message>
        ...
        <translation type="vanished">⚠ 此场景的文件推荐使用 %1,但“渲染选项”标签页目前仍设为默认值——不会自动应用,如果想匹配场景自身的设置,请在那里手动更改。</translation>
    </message>
    <message>
...
Use the pause/stop controls if a category turns out to take too long.</source>
        <translation>为当前视图中每个已就绪待渲染、但尚未保存缩略图的场景创建一张小型预览图。
仅在 CPU 上以低分辨率运行——对于“非常慢”的整体环境场景，仍可能需要一些时间，
因为加载并构建数百万三角形网格的 BVH 所花费的时间与缩略图本身的小尺寸无关。
...
Please check the render log for ffmpeg errors.</source>
        <translation>帧序列已成功渲染（%1 个文件），但未生成视频文件。
...
渲染可能未能成功生成输出。</translation>
    ...
    <message>
        ...
        <translation type="vanished">无法加载 scene_metadata.dylib/.so，因此场景列表为空。请确保 scene_metadata.dylib/.so 与 RayTracerGUI 位于同一目录下。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">选择要渲染的场景。场景按类别分组并可搜索；切换到网格视图可查看缩略图预览。在此处选择场景，还会在下方带出该场景推荐的相机/设置提示（如果有的话）。</translation>
    </message>
    <message numerus="yes">
...
Clear it (the small &quot;x&quot; inside the field) to see every scene in the current category again.</source>
        <translation type="vanished">按名称、ID 或描述中的子字符串匹配来缩小下方场景列表/网格的范围——这是在上方的可用性和分类标签页基础上的叠加,而非替代。
...
This app ships with dozens of built-in scenes covering the basics (a simple Cornell box) up through complex conductor/dielectric materials, volumetric fog, and real photogrammetry-scale models - pick one to render, or browse by category using the tabs above.</source>
        <translation type="vanished">每次渲染都从一个场景开始 - 它描述了世界中的一切：几何体（形状与网格）、材质（表面由什么构成）、光源，以及相机。
...
scene not already cached. CPU-only, low resolution - takes a while the first time.</source>
        <translation type="vanished">为每个尚未缓存的自包含“基础/材质/纹理/相机”场景渲染一张小型预览图。
仅使用 CPU、分辨率较低 - 首次运行需要一些时间。</translation>
    ...
    <message>
        ...
        <translation type="vanished">将采样器/积分器/光源采样器(渲染选项标签页)设置为此场景自身推荐的值。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">在此处选择输出模式（单张图像、视频或实时预览）以及渲染器（GPU 或 CPU），再往下还可选择质量/分辨率预设或手动覆盖。仅用于视频和实时预览的字段在图像模式下仍会保持可见并可编辑，只是会变暗并附带提示——这样你就可以在切换模式之前预先配置好它们。</translation>
    </message>
    <message>
...
Generate Video cannot be combined with an alternate Integrator - see the warning below if that combination is picked.</source>
        <translation type="vanished">决定本次渲染生成的是单帧静态画面,还是拼接成视频的一系列帧。
...
Generate Video renders a camera path frame by frame and assembles an MP4.</source>
        <translation type="vanished">“单张图像”渲染一帧画面。
...
including the handful the GPU backend does not implement.</source>
        <translation type="vanished">GPU：基于 OptiX 的硬件光线追踪 — 通常快上几个数量级。
CPU：重要性采样路径追踪器 — 支持所有场景和材质，
...
GPU rendering is not available in this build.</source>
        <translation type="vanished">重要性采样 CPU 路径追踪器 — 支持所有场景和材质。
...
GPU (OptiX) uses NVIDIA&apos;s dedicated ray-tracing cores to trace thousands of rays in parallel, typically far faster. CPU uses ordinary processor cores instead: much slower, but works on any machine and supports every material this app implements, including a couple the GPU path hasn&apos;t caught up to yet.</source>
        <translation type="vanished">两者追踪的是完全相同的光线，生成完全相同的图像 - 区别只在于速度和所用硬件，而非物理原理。
...
Only applies when Renderer is set to GPU.</source>
        <translation type="vanished">递归式：每像素一个线程，是默认的 GPU 路径追踪器 — 覆盖面广，经过充分验证。
Wavefront：将每次反弹拆分为独立的、通过队列传递的内核启动 — 在复杂/发散度高的
场景上能更好地利用 GPU，但属于较新、测试较少的代码路径。
...
Recursive traces one ray per thread from start to finish, bouncing recursively - simple and battle-tested. Wavefront instead groups all rays currently doing the same kind of work (e.g. &quot;just hit glass&quot;) into a batch and processes them together - better use of the GPU&apos;s parallel hardware on complex scenes with lots of different materials, at the cost of being a newer, less-tested code path.</source>
        <translation type="vanished">在 GPU 上组织同一份光线追踪工作的两种不同方式。
...
default Path Tracer - see each control&apos;s own tooltip.</source>
        <translation type="vanished">选择使用哪种渲染算法。路径追踪器(默认)是经过充分验证的通用选择——下方的备选项则以牺牲通用性为代价换取特定技术(光子映射、双向/Metropolis 光传输,或若干无偏的参考/调试积分器)。除 SPPM 外,所有备选项均仅限 CPU,且都无法与生成视频模式组合使用。
...
Hover any item in the dropdown for details on that specific integrator.</source>
        <translation type="vanished">渲染算法本身,而不仅是渲染速度。路径追踪器(默认)是本应用其他地方通用的、经过充分验证的选择。

SPPM(随机渐进光子映射)能处理路径追踪难以应对的强焦散/玻璃场景。BDPT 与基于 BDPT 构建的 MLT 会同时从相机和光源追踪光路并将两者连接——对某些困难光照效果更有效,仅支持面光源。RandomWalk、Ambient Occlusion、SimplePath、SimpleVolPath 与 LightPath 是参考/调试用积分器——更简单,通常噪点更多或适用范围更窄(例如 Ambient Occlusion 根本不是一次真正的光照渲染),便于单独观察某项具体技术的贡献。
...
“实时预览”使用可环绕的相机连续渲染——仅限 GPU。</translation>
    ...
    <message>
        ...
        <translation type="vanished">NVIDIA OptiX 硬件光线追踪。通常比 CPU 快上几个数量级,但需要支持 CUDA 的 NVIDIA GPU,且尚未实现 CPU 路径支持的全部材质。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">完整的重要性采样路径追踪器。可在任何机器上运行,支持本应用实现的所有场景和材质,包括少数几种 GPU 后端尚未跟进实现的材质——代价是速度慢得多。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">每像素一个线程,逐次反弹递归式地追踪每条光线。默认的 GPU 路径追踪器——覆盖面广,场景和材质均经过充分验证。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">将每次反弹拆分为独立的、通过队列传递的内核启动,把执行同类工作的光线归批处理。在复杂、发散度高的场景上能更好地利用 GPU——但这是一条较新、测试较少的代码路径。</translation>
    </message>
    <message>
...
Render time scales roughly linearly with samples per pixel.</source>
        <translation type="vanished">每像素采样数 / 最大光线深度：
  草稿    25 spp， 深度 10
  预览    50 spp， 深度 20
  良好   100 spp， 深度 50
  高质量 500 spp， 深度 50
...
Clear it (the small &quot;x&quot; inside the field) to see every scene in the current category again.</source>
        <translation>根据你输入的内容，筛选出名称、ID 或描述中包含它的场景，显示在下方的列表/网格中——这是在上方的可用性和分类标签基础上进一步筛选，而不是取代它们。
...
This app ships with dozens of built-in scenes, ranging from simple starter setups (a plain box-shaped room) up through scenes with realistic metal and glass, fog and smoke effects, and highly detailed 3D-scanned models - pick one to render, or browse by category using the tabs above.</source>
        <translation>每一次渲染都从一个场景开始——场景描述了虚拟世界里有什么：各种形状和物体、它们表面的材质、灯光，以及一台相机。
...
take a while the first time you do this.</source>
        <translation type="vanished">为“基础/材质/纹理/相机”分类下所有尚未生成缩略图的可渲染场景，各生成一张小型预览图。
仅使用 CPU、以较低分辨率渲染——第一次执行时可能需要一些时间。</translation>
    </message>
    <message>
...
Generate Video cannot be combined with an alternate rendering method (Integrator) - see the warning below if that combination is picked.</source>
        <translation>这次渲染是生成一张静态图片，还是一段由多张图片拼接成的视频，或者是在 GPU 上进行的实时交互式预览。

“单张图像”会用此标签页设置的相机对场景拍摄一次。“生成视频”则会让相机沿一条路径移动（路径设置见本标签页下方的“视频生成设置”），每一步渲染一张图片，再把它们拼接成 MP4 视频——耗时大约是单张图像的“帧数”倍。“实时预览”会以固定的小分辨率持续渲染，你可以通过点击拖动/滚轮来环绕相机，实时看着画面逐渐变清晰——它不会写入任何输出文件。
...
Live Preview renders continuously with a camera you can freely orbit - GPU only.</source>
        <translation>“单张图像”渲染一张图片。
“生成视频”会逐帧渲染一条移动的相机路径，并合成为 MP4 视频。
...
including the handful the GPU option does not implement.</source>
        <translation>GPU：使用你显卡的光线追踪硬件——通常速度快得多。
CPU：功能最完整的渲染方式——支持所有场景和材质，
包括少数 GPU 选项尚未实现的材质。</translation>
    </message>
    <message>
        <source>The full-featured CPU rendering method — supports every scene and material.
GPU rendering is not available in this build.</source>
        <translation type="vanished">功能最完整的 CPU 渲染方式——支持所有场景和材质。
此构建版本不支持 GPU 渲染。</translation>
    </message>
    <message>
...
GPU was found on this Mac.</source>
        <translation>功能最完整的 CPU 渲染方式——支持所有场景和材质。
GPU 渲染当前不可用（上方显示为灰色）——未在此 Mac 上
...
GPU uses your NVIDIA graphics card&apos;s dedicated ray-tracing hardware to process thousands of light rays at once, so it&apos;s typically far faster. CPU uses your computer&apos;s regular processor instead: much slower, but works on any machine and supports every material this app implements, including a couple the GPU option hasn&apos;t caught up to yet.</source>
        <translation>这两个选项进行的计算和得到的图像完全相同——唯一的区别是速度快慢，以及由哪种硬件来完成计算，而不是背后的物理原理。
...
Only applies when Renderer is set to GPU.</source>
        <translation>递归式：默认的 GPU 渲染方式——经过广泛测试，适用范围最广。
Wavefront：将相似的光线归组处理，在复杂场景中能更充分利用显卡，
但比递归式更新，测试也更少。
...
renderer at all, so Renderer above can only ever be CPU.</source>
        <translation>此构建版本不支持——GPU 渲染需要 Windows 及兼容的
NVIDIA 显卡。此平台的构建版本完全没有 GPU
...
Recursive follows each light ray from start to finish, one ray at a time - simple and thoroughly tested. Wavefront instead groups together all the rays currently doing the same kind of work (e.g. &quot;just hit a glass surface&quot;) and processes them as a batch - this can make better use of the graphics card on complex scenes with lots of different materials, at the cost of being a newer, less-tested option.</source>
        <translation>这是在你的显卡上组织同一份渲染工作的两种不同方式——最终图像完全相同，只是计算过程不同。
...
the samples takes roughly twice as long.</source>
        <translation>每像素采样数（决定画面干净程度）/ 最大光线深度（允许的反弹次数）：
  草稿      25 spp， 深度 10
  预览      50 spp， 深度 20
  良好     100 spp， 深度 50
  高       500 spp， 深度 50
...
Each step up roughly doubles the render time in exchange for a cleaner, less noisy image - Draft is for quickly checking a scene looks right, Ultra/Maximum are for a final image you&apos;d actually want to look at closely.</source>
        <translation>一个可同时设置“每像素采样数”和“最大光线深度”的快捷方式，因为这两者正是用渲染时间换取图像质量的两个旋钮。
...
Every path starts from the camera position set below.</source>
        <translation>相机在整个帧序列中的运动方式：
  环绕    —— 绕场景转一整圈，始终注视场景中心
  直线    —— 从场景旁直线掠过
  8 字形  —— 沿一条 8 字形路径运动，中途会反复穿过场景中部
  螺旋    —— 一边环绕一边持续靠近
  漫游    —— 一边左右摆动、一边向前滑行并环顾四周，如同走进一个房间参观
  展示    —— 一次平滑的转动，先推近、再先升后降，如同产品广告镜头
...
输出：帧图像将保存到 output/frames/

需要 ffmpeg：把渲染出的帧图像合成视频，需要一个叫 ffmpeg 的免费程序，必须先安装到电脑上并能从命令行调用——如果渲染日志提示找不到它，可以到 ffmpeg.org 下载。
...
Looking for the image-quality settings (ReSTIR, Exposure, Samples per Frame, Max Bounces, Firefly Clamp)? Those now live on the Render Options tab&apos;s own Live Preview Settings group, next to the Denoiser section.</source>
        <translation>调整实时预览中鼠标环绕/缩放，以及键盘 WASD/上/下移动加左右/+/- 的响应灵敏度。只有当上方的输出模式为“实时预览（交互式）”时才会生效，但在任何模式下都可以编辑。
...
Type or Browse to a .exr path instead of .png/.ppm to get a high-dynamic-range file that stores the full range of brightness values without compressing them for a normal screen - useful if you plan to edit the image further in other software. If Denoise is also on and GPU Backend is Recursive, two extra helper files (_albedo.exr and _normal.exr, storing surface color and surface direction) are saved alongside it automatically to help with that cleanup (Wavefront doesn&apos;t produce these yet).</source>
        <translation>渲染文件的保存位置。视频模式会自动附加正确的扩展名；实时预览完全不使用这个设置，因为它从不写入文件。
...
it inline.</source>
        <translation>渲染图像的保存位置。系统总是会在保存原始 .ppm 文件的同时，
附带保存一份 .png 副本，“预览”标签页显示的正是这份 .png。

改为输入 .exr 路径，则只会得到一个高动态范围文件，不再生成 PNG 副本——
...
Choosing a .exr path instead skips both: it writes one file that stores the full range of brightness values with no adjustment for a normal screen - the format professional photo/video editing tools expect, and the only way to get the raw brightness data out of this app instead of an image already adjusted to look right on a regular monitor.</source>
        <translation>最终图像的保存位置。

系统总会写入一份原始 .ppm 文件，并自动生成一份 .png 副本——“预览”标签页始终显示的是这份 .png，因为大多数看图软件（包括本应用自己的预览功能）都无法直接打开 .ppm。
...
Every path starts from the camera position set below.</source>
        <translation type="vanished">相机在帧序列中的运镜方式：
...
Orbit circles fully around the scene, always facing its center - the classic &quot;turntable&quot; shot. Linear sweeps past in a straight line. Figure-8 traces a lemniscate, crossing back through the middle. Spiral orbits while steadily moving closer. Tour sways side to side and glides forward while its look-at point drifts too, like an actual visitor walking through and looking around a room. Showcase turns once around the subject with an eased push-in and a gentle rise-and-fall, like a product advertisement&apos;s hero shot. Every path starts from wherever the camera is positioned further down this tab.</source>
        <translation type="vanished">相机在整个帧序列中的运镜方式。
...
Tips: use GPU mode for faster rendering. Lower samples/pixel (10-50) for quick previews, higher (100-500) for production quality. Typical render time is 1-5 minutes on GPU, 15-60 minutes on CPU.</source>
        <translation type="vanished">视频时长：%1 秒（%2）
...
提示：使用 GPU 模式可获得更快的渲染速度。较低的每像素采样数（10-50）适合快速预览，较高（100-500）适合成片质量。典型渲染时间为 GPU 上 1-5 分钟，CPU 上 15-60 分钟。</translation>
    ...
    <message>
        ...
        <translation type="vanished">调节实时预览中鼠标环绕/缩放以及键盘 WASD/上/下移动 + 左/右/+/- 的响应灵敏度。仅在上方输出模式为“实时预览（交互式）”时生效，但在任何模式下都可编辑。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">仅限 OptiX AI 降噪器。在有噪点的输入与完全降噪的输出之间混合（0.0 = 100% 降噪，1.0 = 原始噪点图像），含义与“图像和视频”子分区自身的混合控件相同。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">仅限 OptiX AI 降噪器。按原样显示每个已降噪的帧，而不是将其与之前的帧累积平均。这会放弃累积更多样本本可达到的额外画质，换来始终只反映最新一帧的画面——在用 WASD 四处飞行时很有用，因为累积的旧帧来自你已经离开的相机位置。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">时域混合率的下限——数值越低，保留历史信息越久（噪点更少，但在场景变化时滞后更多）；数值越高，适应越快（噪点更多，滞后更少）。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">限制一个已收敛像素最多能累积多少帧历史——用于限定其“粘滞”程度的上限。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">在历史长度低于此值时，方差会先根据相邻像素做空间预滤波，而不是单独信任自身——这有助于一个新出现或刚解除遮挡的像素，在积累起足够自身时域历史之前先获得合理的边缘保持权重。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">上方方差预滤波所用的框半径（以像素为单位）——半径 3 表示一个 7x7 的框。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">对着色法线差异的边缘保持灵敏度——数值越高，越小的法线差异也会被拒绝，从而防止在曲面或轮廓边缘处产生模糊。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">对深度差异的边缘保持灵敏度，相对于局部深度梯度而言——数值越高，在把邻近像素判定为不同表面之前所能容忍的深度变化越大。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">对亮度差异的边缘保持灵敏度，相对于该像素自身估计的噪声水平而言——数值越高，能在越大的亮度差异范围内进行模糊处理。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">每次 A-trous 滤波的滤波器覆盖半径——限制在 [0,2] 之间（半径 2 = 5x5），因为该滤波器自身的核权重表只有 3 项。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">在用颜色除以反照率（去调制）之前施加的下限——防止反照率接近零的像素数值爆炸，或来回折算成黑色。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">A-trous 滤波遍数（每一遍步长翻倍：1、2、4、8……）——遍数越多，能覆盖的有效半径越大，但 GPU 开销也相应更高。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">ReSTIR DI/GI、辐射缓存、曝光、每帧采样数/最大反弹次数，以及萤火虫钳制——均独立于下方仅适用于图像/视频的“高级参数”组，也独立于上方的降噪器部分。仅在上方输出模式为“实时预览（交互式）”时生效，但在任何模式下都可编辑。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">重采样的单次反弹间接光照（ReSTIR GI）——与上方启用哪种降噪器无关。禁用它会回退到经典的单样本间接估计，噪点更多但每帧开销更低。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">重采样的直接光照采样（ReSTIR DI）——与上方 ReSTIR GI 相互独立（那重采样的是单次反弹的间接光照；这重采样的是经典下一事件估计原本会从单个全局别名表样本中做出的直接光照抽取）。禁用它会回退到那种经典的单样本抽取，在光源较多的场景中噪点更多，但每帧开销更低。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">色调映射前施加的固定亮度倍数，含义与“渲染选项”标签页自身的“曝光”控件相同，但为实时预览独立设置。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">实时预览每次调用时渲染的每像素采样数——实时预览拥有独立于下方“高级参数”组的自身取值，后者仅适用于图像/视频。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">实时预览的最大光线深度——独立于下方仅适用于图像/视频的“高级参数”组。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">限制单个样本值可达到的最大亮度以抑制萤火虫噪点，代价是会截断真正明亮的高光。数值越低，钳制越激进。</translation>
    </message>
    <message>
...
switches Quality to Custom.</source>
        <translation>每个像素要平均多少个随机光照采样。这是控制画质/耗时的主要旋钮：
采样数越多，画面越干净，但收益会递减——要把噪点减半，大约需要 4 倍的
...
surfaces look the same well below the maximum.</source>
        <translation>一条光线在被渲染器放弃追踪之前，最多允许在表面之间反弹多少次。
数值太低会让玻璃和镜面显得偏暗，因为它们需要很多次反弹才能正确成像；
而只有纯粹哑光表面的场景，即使远低于最大值，看起来也不会有明显差别。</translation>
    ...
    <message>
        ...
        <translation type="vanished">渲染文件的保存位置。视频模式会自动附加正确的扩展名；实时预览则完全忽略此设置，因为它从不写入文件。</translation>
    </message>
    <message>
        <source>Where the rendered image is written. A .png is always saved alongside
the raw .ppm, and it is the .png the Preview tab displays.</source>
        <translation type="vanished">渲染图像的写入位置。系统总会在原始 .ppm 文件旁
额外保存一份 .png，预览标签页显示的正是这份 .png。</translation>
    ...
    <message>
        ...
        <translation type="vanished">PNG 图像 (*.png);;PPM 图像 (*.ppm)</translation>
    </message>
    <message>
        <source>Where the finished image is saved.
...
A raw .ppm file is always written, and a .png copy is generated alongside it automatically - the Preview tab always shows the .png, since most image viewers (and this app&apos;s own preview) can&apos;t open .ppm directly.</source>
        <translation type="vanished">渲染完成图像的保存位置。
...
Paired with Height below to set the resolution manually, overriding whatever the Quality preset on the Basic tab would otherwise use.</source>
        <translation type="vanished">图像的像素宽度。
...
Paired with Width above - together they set the resolution manually, overriding the Basic tab&apos;s Quality preset.</source>
        <translation type="vanished">图像的像素高度。
...
the render time. Setting it here switches Quality to Custom.</source>
        <translation type="vanished">每像素追踪的光线数。这是控制质量/时间的主要旋钮：噪点会随该值的
平方根下降，因此将噪点减半大约需要 4 倍的渲染时间。在此处修改会将
...
More samples means a more accurate average, which shows up as less speckly &quot;noise&quot; in the image - but each extra sample costs render time. Doubling this value roughly halves the noise, but takes about twice as long to render.</source>
        <translation>光线追踪通过发射大量随机光线并对结果取平均来估算每个像素的颜色,就像向很多人做民意调查再取平均一样。
...
of plain diffuse surfaces look the same well below the maximum.</source>
        <translation type="vanished">光线在被终止前最多可以反弹的次数。数值过低会使玻璃和镜面变暗,因为它们需要多次反弹才能正确成像;而纯漫反射表面构成的场景,远低于最大值时看起来就已经没有差别。</translation>
    </message>
    <message>
...
Each extra bounce lets light travel one more surface before giving up, which is what makes glass, mirrors, and soft indirect lighting look correct. Most scenes look &quot;finished&quot; well before the maximum - beyond that, extra depth mostly traces light too dim to matter.</source>
        <translation>深度为 1 表示光线只能看到它直接击中的物体,完全没有反弹光——就像一个没有反射或间接光照的场景。
...
Generate Video cannot be combined with an alternate Integrator - see the warning below if that combination is picked.</source>
        <translation type="vanished">此渲染是生成一帧静态画面、一段拼接成视频的帧序列，还是一个交互式 GPU 预览。
...
Choosing &quot;Custom&quot; unlocks the X/Y/Z fields below so you can fly the camera anywhere you like instead.</source>
        <translation>为该场景手工挑选的几个相机位置,每个都刻意取景以展示某个特定效果(例如从正面向内看,或是从康奈尔盒式封闭空间的内部观察)。
...
Only editable when the preset above is set to Custom - the camera always looks toward the scene&apos;s own fixed look-at point, so moving X/Y/Z changes the viewing angle and distance, not just a straight left-right pan.</source>
        <translation>相机在世界坐标系 X 轴(左/右)上的位置。
...
Same Custom-preset-only editing rule as Camera X - the camera keeps looking at the scene&apos;s fixed look-at point as you move it.</source>
        <translation>相机在世界坐标系 Y 轴(上/下)上的位置。
...
Same Custom-preset-only editing rule as Camera X/Y.</source>
        <translation>相机在世界坐标系 Z 轴(前/后,即深入或离开场景的方向)上的位置。
...
The quickest way to zoom in or pull back once you&apos;ve already found an angle you like via the X/Y/Z fields or a preset.</source>
        <translation>沿相机当前朝向的方向,直接将相机推近或拉远场景的注视点,而不改变相机所指的方向。
...
当你已经通过 X/Y/Z 字段或预设找到满意的角度后,这是放大或拉远画面最快捷的方式。</translation>
    ...
    <message>
        ...
        <translation type="vanished">选择光传输算法——默认的路径追踪器，或者选择 SPPM/BDPT/MLT/AO 等备选算法，一旦选定会在下方显示其专属子选项。备选积分器仅限 CPU，且不能与视频模式组合使用。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">SPPM 运行的相机遍历 + 光子遍历轮数。迭代次数越多,结果收敛得越干净,渲染时间大致呈线性增长。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">每次迭代从光源发射的光子数量。光子数越多,间接光照/焦散中的噪点越少,但光子遍历阶段也会相应变慢。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">BDPT 连接的两条子路径(相机侧与光源侧)各自允许的最大反弹次数。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">MLT 在每个深度上预先采样的候选光路数量,用于为其马尔可夫链提供初始分布——数值越大,初始分布的信息量越充分。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">所有链条上 Metropolis 变异的总次数——是控制渲染时间/质量的主要参数,类似于默认路径追踪器中的每像素采样数。</translation>
    </message>
    <message>
        <source>Same meaning as BDPT&apos;s max path depth (MLT is built directly on BDPT&apos;s subpath machinery).</source>
        <translation type="vanished">含义与 BDPT 的最大路径深度相同(MLT 直接构建在 BDPT 的子路径机制之上)。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">一条遮蔽测试光线在被判定为未遮蔽之前可以到达的最远距离。默认值(100 亿)实际上相当于无限远——调低该值可让只有附近的几何体才计入遮蔽。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">默认情况下,遮蔽光线按朝向表面法线加权采样(余弦半球分布),这与朗伯表面实际受光的方式一致。均匀半球分布则改为在半球上均匀撒点——是一种不同的、无权重的估计方式。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">作用于下方遮蔽颜色的整体倍率。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">环境光遮蔽用于可视化的颜色——这不是一次真实光照渲染,因此这只是一个可视化选择,而非光源颜色。默认值为白色 (1, 1, 1)。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">默认开启。直接光源采样会在每次反弹时显式地将阴影光线指向光源,能大幅降低小/亮光源场景中的噪点。关闭后将退化为仅凭随机反弹碰巧找到光源,与纯粹无偏的路径追踪器行为一致。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">默认开启。会按表面材质实际反射光线更集中的方向,对每次反弹的新方向进行加权采样。关闭后将退化为均匀半球采样。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">采样器算法、光谱渲染、自适应采样，以及作为固定采样数替代方案的时间限制。这些控制的是采样“如何”进行，与“数量多少”（设置标签页）相互独立。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">一种基于 Sobol 序列的低差异序列,按像素打乱(scrambled)。是最佳的通用默认选择——收敛快,且不会出现可见的结构性伪影。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">一种沿 Morton(Z 序)曲线重新排序的 Sobol 变体。收敛速度至少与普通 Sobol 相当,在自适应/渐进式采样下表现更好。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">带有额外填充维度的 Sobol 序列,当某像素所需的随机维度超出基础 Sobol 能舒适覆盖的范围时(例如反弹次数很多的路径),可避免出现相关性伪影。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">将每个像素划分为一个子网格,每个格子取一个样本。覆盖方式简单、可预测——不如 Sobol/Halton 精细,但可用作参考/对比用的采样器。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">渐进式多重抖动(0,2)序列,采用蓝噪声排序。相邻像素之间的采样点在空间上的分布(蓝噪声)尤其均匀。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">一种经典的低差异序列,每个维度采用不同的质数作为基数构建。经过充分验证,能避免普通分层采样可能出现的轴对齐聚簇现象。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">纯粹不相关的伪随机数,完全不具备低差异结构。加入它是为了忠实还原加载的 .pbrt 场景自身的 Sampler 指令,而非作为推荐选项本身。</translation>
    </message>
    <message>
...
or under BDPT/MLT/SPPM/the debug integrators.</source>
        <translation type="vanished">决定驱动随机决策所用的采样器(除 Independent 外均为低差异序列)。仅对 CPU 默认路径追踪器生效——对 GPU,以及 BDPT/MLT/SPPM/调试积分器均无影响。</translation>
    </message>
    <message>
...
Grayed out? This only exists on the CPU renderer&apos;s default path tracer - switch Renderer to CPU on the Settings tab to use it.</source>
        <translation type="vanished">普通渲染将光记录为三个数值——红、绿、蓝——就像屏幕显示颜色的方式一样。
...
becomes a ceiling, not a fixed count. CPU default path tracer only.</source>
        <translation type="vanished">一旦某个像素已收敛就提前停止对它采样，而不是
...
Lower = cleaner but slower. 0.01 matches Blender Cycles&apos; own default.</source>
        <translation type="vanished">判定一个像素已收敛所需的目标相对噪声水平。
...
Grayed out? This only affects the CPU renderer&apos;s default path tracer - switch Renderer to CPU on the Settings tab to use it.</source>
        <translation type="vanished">蒙特卡洛路径追踪器的噪点来自随机性——有些像素（比如一面明亮且均匀受光的墙）只需几个样本就能收敛到干净的估计值，而另一些像素（比如仅由一扇小窗照亮的昏暗角落）则需要多得多的样本，噪点才会平息下来。对两者花费相同的固定样本数，会在已经收敛完毕的像素上浪费时间。
...
tracer only.</source>
        <translation type="vanished">经过这么多秒后即停止渲染，而不是
...
budget above.</source>
        <translation type="vanished">停止前的渲染秒数，与上方的“采样数”
预算无关。</translation>
    </message>
    <message>
...
Grayed out? This only affects the CPU renderer&apos;s default path tracer - switch Renderer to CPU on the Settings tab to use it.</source>
        <translation type="vanished">适用于固定的预览或渲染农场时间预算，无需猜测一个恰好能按时完成的样本数——让上方的“采样数”保持为一个宽裕的上限，而由此项决定何时真正停止。
...
显示为灰色？此选项仅影响 CPU 渲染器的默认路径追踪器——请在“设置”标签页将“渲染器”切换为 CPU 以使用它。</translation>
    ...
    <message>
        ...
        <translation type="vanished">色调映射曲线、是否打印渲染统计信息，以及 OptiX 校验模式——这些是关于最终图像如何被处理和汇报的行为开关，而非渲染什么内容。降噪功能在此标签页下方有自己专属的分区。</translation>
    </message>
    <message>
...
can over-smooth.</source>
        <translation type="vanished">在有噪点的输入与完全降噪的输出之间混合
...
Grayed out? This needs the GPU recursive backend - switch Renderer to GPU (and GPU Backend to Recursive) on the Settings tab to use it.</source>
        <translation type="vanished">光线追踪本质上是有噪点的——较低的采样数会留下颗粒状、斑驳的图像，这正是为什么样本越多通常意味着画面越干净。
...
SVGF Denoiser: an alternative, experimental spatiotemporal filter - tracks per-pixel variance over time and uses it to drive an edge-aware spatial filter, which holds up better during camera movement than the AI denoiser + running-mean combination. Always shows the latest filtered frame rather than accumulating (see the SVGF Advanced Tuning group below for its own tunable constants).</source>
        <translation type="vanished">无：原始累积样本，不进行降噪。
...
SVGF 降噪器：一种备选的实验性时空滤波器——随时间跟踪每像素方差，并用它来驱动一个边缘感知的空间滤波器，在相机移动时比“AI 降噪器 + 滑动平均”的组合表现更稳定。它始终显示最新的滤波结果，而不是累积平均（其自身可调常数见下方“SVGF 高级调优”组）。</translation>
    ...
    <message>
        ...
        <translation type="vanished">世界空间辐照度探针缓存——缓存并重采样两次或更多次反弹深度的漫反射光照，与上方 ReSTIR GI 相互独立（那只重采样第一次间接反弹；这覆盖它之外的每一次反弹）。需要经过多帧才能收敛，因此启用它或将相机移入新区域后的最初几秒内，画面可能显得斑驳或有噪点。禁用它会使每次反弹都回退到经典的下一事件估计，在深层间接光较多的场景中噪点更多，但没有收敛延迟。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">针对光泽/金属表面的反弹方向，依据一个粗略的、增量学习得到的入射光实际位置估计进行重要性采样，而不是完全依赖材质自身的反射波瓣采样。需要同时开启上方的“辐射缓存”——它复用该缓存自身的网格和更新流程，若辐射缓存关闭则此功能不起作用。与辐射缓存一样，需要经过数秒才能收敛；禁用它会回退到材质自身的无偏反射采样。</translation>
    </message>
    <message>
...
SVGF Denoiser: an alternative, experimental noise-reduction filter. Instead of blending many frames together, it tracks how much each pixel&apos;s brightness has been changing over time and smooths it out along natural edges - it holds up better than the AI Denoiser while you&apos;re actively moving the camera. Always shows the latest smoothed frame rather than gradually sharpening over time (see the SVGF Advanced Tuning group below for its own fine-tuning options).</source>
        <translation>“无”：完全按渲染出的原样显示图像，保留所有天然噪点——不做任何平滑处理。

“OptiX AI 降噪器”：使用与上方“图像和视频”分区处理最终渲染结果相同的 AI 平滑技术，改善实时预览在你移动镜头时那种带噪点、细节模糊的画面。这样预览画面能立刻显得比较干净，而不用等它自己慢慢变清晰。每帧会多花一点点 GPU 时间。
...
“SVGF 降噪器”：一种可选的实验性降噪滤镜。它不是把很多帧混合在一起，而是跟踪每个像素的亮度随时间的变化情况，并沿着物体的自然边缘进行平滑处理——在你持续移动相机时，效果比 AI 降噪器更稳定。它始终显示最新的平滑帧，而不是随时间逐渐变得更清晰（更多微调选项见下方的“SVGF 高级调节”分组）。</translation>
    ...
    <message>
        ...
        <translation type="vanished">通过一个确定性的亚像素抖动序列，在多帧之间重建出更清晰的图像，而不是让实时预览原生的低分辨率画面被简单拉伸以填满窗口。每帧的渲染成本不变——2倍/4倍只是改变了需要多少帧才能变清晰（相机移动稳定后分别为4帧或16帧）。4倍比2倍（约40-50 MB）占用明显更多的内存（约150-200 MB）。与上方“降噪器”下拉菜单的SVGF模式以及“显示最新帧”选项互斥——两者同时开启时以那些设置优先。</translation>
    </message>
    <message>
...
Grayed out? This is GPU-only - switch Renderer to GPU on the Settings tab to use it.</source>
        <translation type="vanished">启用 GPU 光线追踪管线内部的额外正确性检查，可捕获某些原本会悄无声息地产生错误图像或随机崩溃的错误类别。
...
Grayed out? This only affects the CPU renderer&apos;s default path tracer - switch Renderer to CPU on the Basic Settings tab to use it.</source>
        <translation type="vanished">光线追踪需要大量随机数——例如光线该朝哪个方向反弹、该在光源上取样哪一点等等——而这些&quot;随机&quot;数是如何生成的,会直接影响图像收敛到干净结果的速度。

朴素的随机数生成器会产生聚簇和空隙;这里的大多数采样器(Sobol、Halton 等)都是低差异序列,刻意让采样点在采样空间中分布得更均匀,因此在相同采样数下收敛速度比真正的随机数更快。Independent 是个例外——纯粹不相关的随机数,加入它是为了忠实还原加载的 .pbrt 场景自身的 Sampler 指令,而非作为推荐选项。
...
显示为灰色不可用?这是因为该选项只影响 CPU 渲染器的默认路径追踪器——请在基础设置标签页中将渲染器切换为 CPU 以使用它。</translation>
    ...
    <message>
        ...
        <translation type="vanished">对场景中的光源构建空间层次结构,并根据光功率和与着色点的距离对每个光源进行加权,按每次反弹自适应调整,而不是使用单一的全局权重。这是 pbrt-v4 自身的默认选项——通常能带来最好的收敛效果,只需付出少量额外的簿记开销。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">使用该场景自身的积分器 “string lightsampler” 参数所请求的值（如果未请求则为 BVH，或请求了本项目未实现的算法），而不是固定选择——与 CLI 自身的 --lightsampler auto 相匹配。选定这一项并保持不变，是在不同场景推荐各异的情况下唯一始终保持正确的选择。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">按光源的总发光功率加权来选取光源——越亮的光源被采样的概率越高。在光源亮度差异较大的场景中,收敛速度比均匀采样更快,但忽略了距离和遮挡。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">从场景中的所有光源里均匀随机地选取一个光源,不考虑其亮度或距离远近。简单且无偏,但在包含许多亮度差异悬殊的光源的场景中收敛较慢——暗光源被采样的频率与亮光源相同。</translation>
    </message>
    <message>
...
or under BDPT/MLT/SPPM/the debug integrators.</source>
        <translation type="vanished">决定每次下一事件估计反弹时选用哪种策略来挑选
...
Grayed out? This only affects the CPU renderer&apos;s default path tracer - switch Renderer to CPU on the Basic Settings tab to use it.</source>
        <translation type="vanished">每一次漫反射/光泽反弹都需要从(可能有很多个)光源中挑选一个,直接对它进行下一事件估计采样——挑中哪一个光源、挑得有多“公平”,会影响图像收敛的快慢,但永远不会改变它最终收敛到的结果。
...
this checkbox only ever adds the request, never removes it.</source>
        <translation type="vanished">在路径的第一次非镜面反弹后加宽粗糙 BSDF 的 GGX alpha 值——
可以抑制来自高难度焦散路径的萤火虫噪点,
代价是引入一些模糊。仅对 CPU 和 GPU 的默认路径追踪器
生效。已经自行请求该效果的场景不受影响——
此复选框只会额外添加该请求,不会移除它。</translation>
    </message>
    <message>
...
Off by default, matching pbrt-v4&apos;s own default. If a loaded .pbrt scene&apos;s file already requests this itself, it&apos;s applied either way - this checkbox can only add the request on top, never take it away.</source>
        <translation type="vanished">某些光路对路径追踪器来说天生就很难被干净地采样到——光线在一个粗糙(但并非完全镜面)的表面上反弹,再穿过另一个粗糙表面,最终射入一个很小很亮的光源。这类路径会在图像中表现为明亮、孤立的斑点(“萤火虫噪点”),需要极长时间才能被平均掉。

路径正则化会在光路每经过一次非镜面反弹后,故意让表面粗糙度略微增大一些——这会引入少量偏差(严格来说是不准确的结果),但换来萤火虫噪点收敛速度大幅加快,这通常是更划算的取舍,能让渲染结果的实际观感更好。

默认关闭,与 pbrt-v4 自身的默认设置一致。如果加载的 .pbrt 场景文件本身已经请求了该效果,无论这里是否勾选都会生效——这个复选框只能在此基础上追加请求,不能取消它。</translation>
    </message>
    <message>
...
per-sample-total.</source>
        <translation type="vanished">将任何最亮通道超过下方数值的像素样本进行截断,
并同时按比例缩小所有通道以保持色相不变。
...
Off by default (effectively unbounded, matching pbrt-v4&apos;s own default). CPU default path tracer only.</source>
        <translation type="vanished">蒙特卡洛路径追踪器偶尔会采样到一条本身正确、但极其明亮的光路——比如一条光线恰好以某个角度擦过一个很小但很强的光源——这样的单个样本可能在很长一段时间内主导某个像素的平均值,直到有足够多的其他样本进来将其平滑掉。这类样本会在图像中表现为明亮、孤立的斑点(“萤火虫噪点”)。

该截断会限制单个样本最亮通道在被计入平均值之前所能达到的最大亮度,以少量可控的偏差换取在相同样本数下明显更干净的画面——数值越低,清理效果越激进,但也越容易让真正明亮的小型光源被明显压暗,而不仅仅是压制离群噪点。

默认关闭(实际上不设上限,与 pbrt-v4 自身的默认设置一致)。仅对 CPU 默认路径追踪器生效。</translation>
    </message>
    <message>
...
BDPT/MLT/SPPM/the debug integrators.</source>
        <translation type="vanished">决定各类随机选择所用的低差异采样器。
仅对 CPU 默认路径追踪器生效——对 GPU,以及
...
Grayed out? This only affects the CPU renderer&apos;s default path tracer - switch Renderer to CPU on the Basic Settings tab to use it.</source>
        <translation type="vanished">光线追踪需要大量的随机数——光线该往哪个方向反弹、在光源上采样哪个点等等——而这些“随机”数是如何生成的,直接决定了画面收敛为干净结果的速度。

朴素的随机数生成器会出现扎堆和空白区域;这里的采样器(Sobol、Halton 等)都是低差异序列,刻意让采样点更均匀地覆盖采样空间,在相同样本数下比真正的随机数收敛得更快、画面更干净。
...
Grayed out? This only affects the CPU renderer&apos;s default path tracer - switch Renderer to CPU on the Settings tab to use it.</source>
        <translation type="vanished">光线追踪需要大量随机数——光线该往哪个方向反弹、该在光源上采样哪个点，等等——而这些“随机”数是如何生成的，会改变图像收敛到干净结果的速度。
...
Grayed out? This only affects the CPU renderer&apos;s default path tracer - switch Renderer to CPU on the Settings tab to use it.</source>
        <translation type="vanished">每次漫反射/光泽反弹都需要从（可能很多的）光源中挑选一个，直接对其采样以进行下一事件估计——选中哪个光源，以及选取的公平程度，会影响图像收敛的速度，但绝不会影响最终收敛到什么结果。
...
than silently rendering wrong colors. Noticeably slower per-sample.</source>
        <translation type="vanished">使用真正的主波长(hero-wavelength)光谱渲染,而非平面 RGB。
仅限 CPU 默认路径追踪器。仅支持 lambertian、metal、dielectric、
rough_dielectric、conductor 和 diffuse_light 材质——使用其他材质的场景
...
Grayed out? This only exists on the CPU renderer&apos;s default path tracer - switch Renderer to CPU on the Basic Settings tab to use it.</source>
        <translation type="vanished">普通渲染用红、绿、蓝三个数值来表示光,这与屏幕显示颜色的方式相同。

而真实的光是连续的波长光谱,一些物理现象(比如棱镜把白光分解成彩虹)之所以会发生,正是因为不同波长的折射程度不同——单靠 RGB 无法表现这一点。光谱渲染让每条光线携带若干个真实波长,而不仅仅是 RGB,代价是单样本噪点更多、速度更慢。
...
Both CPU and GPU default path tracer only.</source>
        <translation type="vanished">在色调映射之前对线性颜色施加的统一乘数(1.0 表示不生效)。
...
1.0 leaves the image unchanged; below 1.0 darkens it, above 1.0 brightens it - useful for a scene that&apos;s rendering correctly but is just too dark or too bright to see clearly, without changing any actual light in the scene.</source>
        <translation>对整幅图像施加的统一亮度乘数,作用和相机的曝光设置一样。
...
1.0 保持画面不变;小于 1.0 会使画面变暗,大于 1.0 会使画面变亮——适用于场景本身渲染正确、只是太暗或太亮而看不清细节的情况,而无需改动场景中任何真实的光照。</translation>
    ...
    <message>
        ...
        <translation type="vanished">仅限 CPU 的空间加速结构与分割启发式方法，用于加速光线与场景的相交测试。适用于每一种积分器，而不仅仅是默认路径追踪器——默认设置在几乎所有场景中都表现良好。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">保留已加载 .pbrt 场景自身的 Accelerator 指令不变（如果未指定则为 bvh，即真正 pbrt-v4 自身的默认值）。对原生（非 .pbrt）场景则无论如何都没有影响。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">一种包围体层次结构，由下方选择的分割方法构建——本项目原有的默认加速结构，与 pbrt-v4 本身一致。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">使用 k-d 树代替 BVH——pbrt-v4 另一种真实存在的加速结构选项。它没有自己的分割方法概念（选择此项时会忽略下方的下拉框）。在带有物体运动模糊的场景中会回退到 BVH/sah（本项目的 kd 树封装没有逐光线时间通道）——发生这种情况时会打印一条警告。</translation>
    </message>
    <message>
...
scene (has no Accelerator directive to override - see the log).</source>
        <translation type="vanished">哪种加速结构承载场景的几何体。所有
...
Only a loaded .pbrt scene has an Accelerator directive to override at all - a native (non-.pbrt) scene always uses its own fixed BVH regardless of this choice (a warning is printed if you pick something else anyway).</source>
        <translation type="vanished">一份三角形/球体的平铺列表会让每条光线都去测试场景中的每一个图元——而加速结构正是让光线可以跳过场景中的大部分内容，只测试它实际经过路径附近的少数图元。
...
只有已加载的 .pbrt 场景才有可覆盖的 Accelerator 指令——原生（非 .pbrt）场景无论如何都始终使用自身固定的 BVH（如果你仍选择了其他选项，会打印一条警告）。</translation>
    ...
    <message>
        ...
        <translation type="vanished">表面积启发式——评估若干候选分割方案的遍历成本，并选择最便宜的一个。构建速度比 Middle/Equal 慢，但在大多数场景中能得到遍历效率最高的树——本项目原有的 BVH 构建方式。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">在包围盒最长轴的中点处分割每个节点。构建成本低，完全不做成本估算——在分布不均匀的几何体上可能遍历效果不佳。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">将每个节点分割成两侧图元数量相等，而不考虑它们的空间范围。构建成本低；对聚集的几何体可能产生形状很差的节点。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">分层线性 BVH——基于 Morton 码自底向上构建，是这四种方法中在超大三角形数量下构建速度最快的一种，但遍历质量比 SAH 略逊一筹。</translation>
    </message>
    <message>
...
warning is printed when that happens.</source>
        <translation type="vanished">上方 BVH 的构建方式（当加速结构设为
...
打印一条警告。</translation>
    ...
    <message>
        ...
        <translation type="vanished">仅当上方加速结构解析为 BVH 时才会参考此项（Kd 树没有分割方法的概念，因此会忽略）。这四种方法都以不同策略构建出同一大类树形——SAH 花更多时间构建以换取遍历效果更好的树；Middle/Equal 是简单、廉价的后备方案；HLBVH 则以牺牲部分遍历质量换取在超大场景下最快的构建速度。</translation>
    </message>
    <message>
...
effect under BDPT/MLT/SPPM/the debug integrators.</source>
        <translation type="vanished">在应用 sRGB 曲线之前使用哪种色调映射算子。
对 CPU 和 GPU(递归式与波前式)均生效——对
...
ACES rolls off bright highlights gently, the way film does; Reinhard is a simpler, older compression; None just clips anything too bright to flat white, which can look harsh.</source>
        <translation type="vanished">光线追踪场景的真实亮度值是无上限的——一个灯泡可能比墙壁亮上百倍——但屏幕只能显示有限的亮度范围。色调映射就是把这个巨大范围压缩到可显示范围内所使用的曲线。
...
never changes the rendered image.</source>
        <translation type="vanished">渲染结束时在“日志”标签页打印一小段统计信息(发射光线数、
反弹次数、阴影光线数、每秒样本数)。仅供观察——
...
Purely informational: it never changes the rendered image, just tells you what the renderer actually did.</source>
        <translation type="vanished">渲染完成后打印一段简短的统计摘要——发射了多少光线、发生了多少次反弹、追踪了多少条阴影光线,以及每秒样本数。
...
Grayed out? This needs the GPU recursive backend - switch Renderer to GPU (and GPU Backend to Recursive) on the Basic Settings tab to use it.</source>
        <translation type="vanished">光线追踪本质上是有噪点的——样本数低时画面会显得颗粒感强、布满斑点,这也是样本数越多画面通常越干净的原因。

降噪器是一个经过训练的机器学习模型,能够识别这种斑点噪声模式并在事后将其抹平,而无需追踪更多光线——这是一种更快获得干净画面的方式,代价是会损失一些细节。
...
real per-launch cost. GPU only, for debugging, not routine use.</source>
        <translation type="vanished">启用 OptiX 验证模式——增加设备端检查,
...
Grayed out? This is GPU-only - switch Renderer to GPU on the Basic Settings tab to use it.</source>
        <translation type="vanished">在 GPU 光线追踪管线内部开启额外的正确性检查,能够捕获某些原本会悄悄产生错误画面或不可预知崩溃的 bug 类别。

这是为开发渲染器自身 GPU 代码的人准备的调试辅助工具,普通渲染并不会从中受益——它会带来实实在在的性能开销,也不会改变正确渲染结果的外观。
...
显示为灰色?此功能仅限 GPU——请在“基本设置”标签页中将渲染器切换为 GPU 以使用它。</translation>
    ...
    <message>
        ...
        <translation type="vanished">仅渲染完整画面中的一个矩形子区域，以归一化的 0-1 坐标给出——适合快速测试渲染场景的某一区域，而不必为整张图像付出代价。</translation>
    </message>
    <message>
...
path tracer only.</source>
        <translation type="vanished">将渲染范围限制在画面的一个矩形区域内,该区域以
完整图像的 [0,1] 区间内的分数形式给出。仅对 CPU 和
GPU 的默认路径追踪器生效。</translation>
    </message>
    <message>
...
Off by default (the full frame). If a loaded .pbrt scene&apos;s file already requests its own cropwindow/pixelbounds, checking this overrides it with the rectangle below; leaving it unchecked lets the scene&apos;s own request (if any) stand.</source>
        <translation type="vanished">只渲染完整画面中的一个矩形区域——区域之外的部分保持黑色——而不是渲染整张图像。该矩形以完整画面宽/高的四个分数给出,从 0(左/上边缘)到 1(右/下边缘),因此无论分辨率如何,形状都保持不变。

适合在体积大、渲染慢的场景中,针对某个有问题的局部快速迭代——当采样只需覆盖画面的一角而非整个画面时,相同的总采样数能收敛得快得多。

默认关闭(渲染整个画面)。如果加载的 .pbrt 场景文件本身已经请求了自己的 cropwindow/pixelbounds,勾选此项会用下方的矩形将其覆盖;不勾选则保留场景自身的请求(如果有的话)。</translation>
    </message>
    <message>
...
Sampler/Spectral/Exposure/Tonemap/Stats below only affect the
default Path Tracer - see each control&apos;s own tooltip.</source>
        <translation>使用哪种渲染方式。“路径追踪器”（默认选项）是经过充分测试的通用选择——下方的备选方式则放弃了这种通用性，换取某种特定技术（比如光子映射，或是同时从相机和光源两端追踪光线再将其连接起来），也有的是用于测试和对比的简化版本。除 SPPM 外，所有备选方式都只能在 CPU 上运行，且都不能与“生成视频”模式同时使用。
...
Hover any item in the dropdown for details on that specific mode.</source>
        <translation>这个选项决定的是渲染方式本身，而不仅仅是渲染速度。“路径追踪器”（默认选项）是通用、经过充分测试的选择，本应用其他地方用的也是它。

SPPM（一种光子映射技术）能处理普通路径追踪难以应付的复杂玻璃和聚光效果。BDPT 和建立在 BDPT 之上的 MLT 会同时从相机和光源两端追踪光线，再将它们在中途连接起来——这有助于处理一些棘手的光照场景，但只支持面光源。RandomWalk、环境光遮蔽、SimplePath、SimpleVolPath 和 LightPath 都是参考/调试模式——更简单，通常噪点更多或者呈现的内容更有限（比如环境光遮蔽根本不会生成真正带光照的图像），适合用来单独观察某一种技术对最终画面的具体贡献。
...
Grayed out? This only affects the CPU renderer&apos;s default path tracer - switch Renderer to CPU on the Settings tab to use it.</source>
        <translation>渲染过程需要大量随机数——光线该往哪个方向反弹、该瞄准光源上的哪个点等等——而这些“随机”选择具体是怎么生成的，会影响画面变干净的速度。

普通随机数往往会在某些地方扎堆，又在另一些地方留下空白。这里的大多数采样器（Sobol、Halton 等）都改用刻意均匀分布的模式，更全面地覆盖所有可能性，在相同采样数下比真正的随机数更快让画面变干净。“独立采样”是个例外——它是普通的、不做任何相关性处理的随机数，保留它是为了让明确要求使用它的 .pbrt 场景文件能被如实还原，而不是作为推荐选项。
...
Grayed out? This only affects the CPU renderer&apos;s default path tracer - switch Renderer to CPU on the Settings tab to use it.</source>
        <translation>每次在哑光或半光泽表面上发生反弹时，都需要从场景中可能存在的多个光源里挑出一个，直接朝它发射光线——挑选哪个光源、挑得是否公平合理，会影响画面变干净的速度，但不会改变最终的画面效果。

BVH（默认选项）把场景中的光源整理成可快速查找的索引，并针对每个着色点调整权重——亮的光源和距离近的光源都会被优先考虑。“自动”则使用加载的场景文件自身所要求的方法（如果没有指定，则回退到 BVH）。“按功率”只依据亮度挑选，忽略位置——更简单，但在光源距离差异很大的场景中效果较差。“均匀”则两者都不考虑——不论亮度或距离，每个光源被选中的概率都相同，主要用于对比和调试。
...
Grayed out? This only exists on the CPU renderer&apos;s default path tracer - switch Renderer to CPU on the Settings tab to use it.</source>
        <translation>普通渲染只用三个数值——红、绿、蓝——来记录光，就像屏幕显示颜色的方式一样。

而真实的光其实是一段连续的波长光谱，一些物理现象（比如棱镜把白光分解成彩虹）之所以会发生，正是因为不同波长的光弯折程度不同——单纯的红/绿/蓝无法表现这一点。这个选项会为每条光线追踪若干个真实的单独波长，代价是画面噪点更多、渲染更慢。
...
Grayed out? This only affects the CPU renderer&apos;s default path tracer - switch Renderer to CPU on the Settings tab to use it.</source>
        <translation>渲染是通过大量随机采样逐步构建出图像的，所以画面一开始会有噪点，然后逐渐变干净——但不同像素变干净的速度并不一样。一面明亮、受光均匀的墙上的像素可能只需几个采样就足够干净，而只有一扇小窗照亮的昏暗角落，则需要多得多的采样才能让噪点平息下来。对两者都花费同样固定的采样数，会在已经足够干净的像素上白白浪费时间。

自适应采样会持续跟踪每个像素还剩多少噪点，一旦低于下方设置的阈值就提前停止为它增加采样，让“采样数”变成一个上限，而不是每个像素都必须用满的固定数量——这与 Blender Cycles 自身的自适应采样是同一个思路。
...
Grayed out? This only affects the CPU renderer&apos;s default path tracer - switch Renderer to CPU on the Settings tab to use it.</source>
        <translation>适合用于固定时长的预览，或是在共享电脑上限定渲染时间，而不用去猜一个恰好能按时完成的采样数——可以让上方的“采样数”保持一个宽松的上限，由这里来决定实际何时停止。

它是逐行停止的：时间用完时正在处理的那些行会正常完成；还没开始处理的行则会保留为黑色，而不是被跳过，因此你依然能得到一张有效（虽然不完整）的图像，而不是一个损坏的文件。

正在生成视频？这个时间预算是针对整段视频的，而不是每一帧——后面的帧会使用剩余的时间，一旦时间用完，剩下未渲染的帧会被直接跳过。
...
request, never removes it.</source>
        <translation>在光路发生第一次非镜面式反弹之后，稍微柔化粗糙、反光或玻璃表面的清晰度——这能抑制难以追踪的光路所产生的萤火虫噪点（孤立的亮斑），代价是画面会稍微多一点模糊。仅对 CPU 和 GPU 上的默认路径追踪器生效。如果场景文件本身已经要求这么做，则不受此选项影响——这个选框只会额外附加这个请求，绝不会取消它。</translation>
    </message>
    <message>
...
Off by default. If a loaded .pbrt scene file already asks for this itself, it&apos;s applied either way - this checkbox can only add the request on top, never take it away.</source>
        <translation>有些光路对路径追踪器来说确实很难干干净净地找到——比如光先在一个粗糙（但非完美镜面）的表面反弹，再穿过另一个粗糙表面，最终命中一个又小又亮的光源。这类光路会表现为萤火虫噪点：某条光线恰好以合适的角度捕捉到一个很亮的小光源，形成一个孤立的亮斑，需要极长时间的平均才能消除。

这项设置会在光路每经过一次非镜面式反弹后，都刻意让表面的粗糙度稍微增加一点——这会带来一点技术上并不严谨的偏差，但作为交换，萤火虫噪点会消退得快得多，通常能让最终画面看起来更好。
...
rather than per whole sample).</source>
        <translation>如果某个采样最亮的颜色通道超过下方设定的数值，就会把这个采样的所有颜色通道按比例一起缩小，使色彩/色调保持不变。CPU 和两种 GPU 后端都支持——递归式 GPU 模式与 CPU 完全一致，而 Wavefront GPU 模式的应用方式略有不同（是按每次反弹而不是按整个采样来限制）。</translation>
    </message>
    <message>
...
Off by default (effectively unlimited). CPU default path tracer only.</source>
        <translation>光线追踪渲染偶尔会得到一个理论上正确、但极其明亮的采样——比如一条光线恰好以合适的角度捕捉到一个很小但极强的光源——这样一个采样在足够多其他采样到来把它拉平之前，会长时间主导某个像素的平均值。这些就表现为萤火虫噪点：在画面中格外突兀的孤立亮斑。

这项钳制会限制单个采样最亮的颜色通道在被计入平均值之前允许达到的最大亮度，用一点可控的误差换取相同采样数下明显更干净的画面——数值越低，清理效果越激进，但也越可能把真正明亮的小光源一并压暗，而不只是消除杂散噪点。
...
Only a scene loaded from a .pbrt file has its own accelerator choice to override at all - any other scene always uses its own fixed BVH-style index regardless of this setting (a warning is printed if you pick something else anyway).</source>
        <translation>如果渲染器必须为每条光线逐一检查场景中的每个物体，那么即使是简单的场景也会慢得让人受不了。这种索引结构能让光线快速跳过明显不可能命中的物体，只检测真正位于其传播路径附近的少数几个物体。

BVH（把相邻物体分组装进层级化的包围盒）和 Kd 树（划分空间本身）是组织同一份几何体的两种不同的实际策略——两者渲染出的图像完全相同，只是根据场景形状的不同，构建和渲染速度会有差异。
...
ACES rolls off bright highlights gently, the way film does, giving a soft, filmic look; Reinhard is a simpler, older way of compressing brightness; None just clips anything too bright straight to flat white, which can look harsh.</source>
        <translation>渲染出的场景，其真实亮度值是没有上限的——一个灯泡可能比一面墙亮上百倍——但屏幕只能显示一个有限的亮度范围。色调映射就是用来把这个巨大的亮度范围压缩到屏幕真正能显示的范围内的曲线。
...
Purely informational: it never changes the rendered image, it just tells you what the renderer actually did.</source>
        <translation>渲染完成后打印一份简短摘要——发射了多少条光线、发生了多少次反弹、追踪了多少条阴影光线（用于检测某个点是否能看到光源的光线），以及每秒采样数。
...
debugging, not routine use.</source>
        <translation>开启额外的 GPU 端正确性检查，每次 GPU 运行时都会带来实际的性能开销。仅限 GPU，供调试使用，不建议日常渲染时开启。</translation>
    </message>
    <message>
...
Grayed out? This is GPU-only - switch Renderer to GPU on the Settings tab to use it.</source>
        <translation>在 GPU 渲染过程内部开启额外的正确性检查，用来捕捉某些原本会悄悄产生错误图像或不可预测崩溃的问题。

这是为编写渲染器自身 GPU 代码的人准备的调试辅助工具，普通渲染用不上它——它会带来实际的性能开销，也不会改变一次正确渲染的最终效果。

显示为灰色？这项功能仅限 GPU——请到设置标签页把渲染器切换为 GPU 才能使用它。</translation>
    </message>
    <message>
...
have their own denoiser).</source>
        <translation>对渲染完成的图像运行 AI 降噪器以消除噪点，它会借助每个像素的基础颜色和表面朝向等额外信息，比单纯的模糊处理效果更好。仅限 GPU，两种 GPU 模式（递归式和 Wavefront）各自都有自己的降噪器。</translation>
    </message>
    <message>
...
Grayed out? This needs the GPU recursive backend - switch Renderer to GPU (and GPU Backend to Recursive) on the Settings tab to use it.</source>
        <translation>当只使用少量采样时，渲染画面天生就会带有噪点，这也是为什么采样数越多画面通常越干净（但渲染也越慢）。

降噪器是一个经过训练、能识别这种噪点并在渲染之后把它抹平的 AI 模型，不需要额外追踪光线——这是一种更快得到干净画面的方式，代价是会损失一些细节。它右侧的数值用来在带噪点的原始图像和完全降噪的结果之间混合——0 表示完全降噪（默认值）；调高该数值会保留一部分原始噪点，适合在完整强度的降噪把你想保留的纹理也一并抹掉时使用。

显示为灰色？这需要 GPU 的递归式后端——请到设置标签页把渲染器切换为 GPU（并把 GPU 后端设为“递归式”）才能使用它。</translation>
    </message>
    <message>
...
default path tracer only.</source>
        <translation>把渲染范围限定在画面中的一个矩形区域内，该区域用整幅图像 0 到 1 之间的比例来表示。仅对 CPU 和 GPU 上的默认路径追踪器生效。</translation>
    </message>
    <message>
...
Off by default (the full frame). If a loaded .pbrt scene file already requests its own crop region, checking this overrides it with the rectangle below; leaving it unchecked lets the scene&apos;s own request (if any) stand.</source>
        <translation>只渲染整个画面中的一个矩形区域——区域之外的部分保持黑色——而不是渲染整幅图像。这个矩形区域用整个画面宽/高的四个比例值来表示，从 0（左/上边缘）到 1（右/下边缘），因此无论分辨率如何，形状都保持不变。

适合在一个庞大、渲染缓慢的场景中，针对某个棘手的局部快速迭代——同样总数的采样，只需覆盖画面的一角而不是整幅图像时，会干净得快得多。
...
Off by default (the scene&apos;s own camera, unchanged). Only affects scenes loaded from a scene file - the built-in demo gallery&apos;s scenes keep their own author-chosen fixed camera regardless of this setting.</source>
        <translation>对于任何从场景文件加载的场景，薄透镜式景深虚化本来就已经完全支持——只需要文件里的 Camera 指令包含 &quot;lensradius&quot;/&quot;focaldistance&quot; 参数即可。这个选项让你无需手动编辑文件，就能设置或修改这些参数：“光圈”是以场景单位表示的镜头直径（0 表示针孔般的清晰，没有虚化），“对焦距离”则是清晰对焦平面所在的距离。
...
same seed. Both CPU and GPU default path tracer only.</source>
        <translation>使用相同种子重新渲染时,结果逐字节完全一致。
仅对 CPU 和 GPU 的默认路径追踪器生效。</translation>
    </message>
    <message>
...
Off by default (genuinely random every render).</source>
        <translation>渲染通常每次都会使用不同的随机序列,因此即使设置完全相同,同一场景的两次渲染结果也不会逐像素完全一致。勾选此项会固定随机种子,使同一个种子值始终生成完全相同的图像——适合用来对比场景编辑前后的效果,或者判断某个视觉差异究竟是代码改动造成的,还是仅仅是随机噪点。
...
拖动以环绕，滚轮或 +/- 缩放，WASD 移动，上/下飞行</translation>
    ...
    <message>
        <source>Live Preview: %1</source>
        <translation type="vanished">实时预览：%1</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">当前正在渲染的任务的实时状态——是哪个任务、完成百分比，以及已用/预计时间。暂停、停止和放弃操作专门作用于这个任务。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">排在当前正在渲染的任务后面排队的任务——在已有任务进行中时点击“渲染”，会在此处新增一项，而不是打断当前任务。会按顺序自动渲染，每完成一个就接着下一个。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">⚠ 只有当“基本设置”标签页中的输出模式设为“生成视频”时,这些设置才会生效。</translation>
    </message>
    <message>
...
other controls on this tab afterward is fine - they simply stop matching
the preset, the same as if you had built the same settings by hand.</source>
        <translation type="vanished">预先调好的经典光线追踪参考场景与运镜方式,让你无需手动设置
场景、相机路径、帧数、帧率和速度。选择其中一项也会同步
更改“基本设置”标签页中的场景。之后再更改本标签页的其他控件也没问题——
...
Picking one fills in every field below (and the scene on the Basic tab) - you can still change anything afterward, it just stops matching the preset once you do.</source>
        <translation type="vanished">一套现成的组合,包含场景、相机路径、帧数、帧率与速度,经过调优使生成的视频效果良好,无需你逐项手动挑选设置。
...
Every path starts from the camera position on the Advanced tab.</source>
        <translation type="vanished">相机在整个帧序列中的运动方式:
  环绕      —— 绕场景转满一圈,始终注视场景中心
  直线      —— 从场景旁直线掠过
  8 字形    —— 双纽线路径,反复穿过场景中部
  螺旋      —— 一边环绕一边持续靠近
...
Paired with Frames Per Second below to determine the video&apos;s total length in seconds.</source>
        <translation type="vanished">构成视频的独立图像帧数——每一帧都是一次完整、独立的渲染,因此该值会直接乘算总渲染时间(在相同设置下,100 帧所需时间大约是单张图像的 100 倍)。
...
Doesn&apos;t change how many frames get rendered (that&apos;s Frame Count above) - only how fast they play back, and therefore how many seconds long the finished video is (Frame Count divided by FPS).</source>
        <translation type="vanished">每秒视频播放多少帧已渲染的图像。
...
Speed 0.5x renders twice as many frames to cover the same journey more slowly and smoothly; speed 2x renders half as many frames, covering the same journey faster.</source>
        <translation>相机完整路径所分布的帧数的一个乘数——不会改变路径本身,路径始终完整走完同样的全程。
...
速度为 0.5x 时会渲染两倍的帧数,让同样的路程走得更慢、更平滑;速度为 2x 时渲染的帧数减半,让同样的路程走得更快。</translation>
    ...
    <message>
        ...
        <translation type="vanished">%1 帧(基础 %2 × 1/%3x 速度)%4</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">&lt;b&gt;视频时长:&lt;/b&gt; %1 秒(%2)&lt;br&gt;&lt;b&gt;相机路径:&lt;/b&gt; %3,无论速度如何都会完整走完全程&lt;br&gt;&lt;b&gt;输出:&lt;/b&gt; 帧图像将保存到 &lt;code&gt;output/frames/&lt;/code&gt;</translation>
    </message>
    <message>
        <source>ℹ️ Requirements</source>
        <translation type="vanished">ℹ️ 依赖要求</translation>
    </message>
    <message>
        ...
        <translation type="vanished">&lt;b&gt;需要 ffmpeg:&lt;/b&gt; 视频编码使用 ffmpeg(libx264),必须已安装并配置到 PATH 中。&lt;br&gt;&lt;small&gt;如果渲染日志提示缺失,请前往 &lt;a href=&quot;https://ffmpeg.org/download.html&quot;&gt;ffmpeg.org&lt;/a&gt; 获取。&lt;/small&gt;&lt;br&gt;&lt;br&gt;&lt;b&gt;自动合成:&lt;/b&gt; 所有帧渲染完成后,视频会被自动合成并打开。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">&lt;b&gt;第 1 步:&lt;/b&gt; 配置视频设置(相机路径、帧数、帧率)&lt;br&gt;&lt;b&gt;第 2 步:&lt;/b&gt; 在“基本设置”/“高级设置”标签页中配置画质设置&lt;br&gt;&lt;b&gt;第 3 步:&lt;/b&gt; 点击“开始视频渲染”并等待&lt;br&gt;&lt;b&gt;第 4 步:&lt;/b&gt; 完成后视频会自动合成并打开!&lt;br&gt;&lt;br&gt;&lt;b&gt;小贴士:&lt;/b&gt;&lt;br&gt;• 使用 GPU 模式可加快渲染速度&lt;br&gt;• 快速预览可降低每像素样本数(10-50)&lt;br&gt;• 正式成片可提高每像素样本数(100-500)&lt;br&gt;• 典型渲染时间:1-5 分钟(GPU)、15-60 分钟(CPU)</translation>
    </message>
    <message>
...
The alternates below trade that generality for a specific technique - photon mapping, bidirectional/Metropolis light transport, or a handful of unbiased reference and debug integrators. All are CPU-only except SPPM, and none can be combined with Generate Video mode. Sampler/Spectral/Exposure/Tonemap/Stats above only affect this default Path Tracer.</source>
        <translation type="vanished">本应用其他地方通用的重要性采样路径追踪器——下一事件估计加上 BSDF 重要性采样,并通过幂启发式(MIS)进行组合。这是经过充分验证的默认选项,除非有特殊理由,否则应从这里开始。
...
GPU: capability-checked per scene - Lambertian/DiffuseLight, RoughDielectric, Metal, Dielectric, Conductor, RoughMetal, and DiffuseTransmission are supported (area lights only); an unsupported scene falls back to an error - use CPU SPPM instead.</source>
        <translation type="vanished">随机渐进光子映射(pbrt-v4 风格)。最适合普通路径追踪难以解决的强焦散/玻璃场景。

CPU:已在康奈尔毛玻璃场景上完成端到端验证;其他场景尚未验证,且仅支持 lambertian 与 delta-BSDF 材质。
...
CPU only. Area lights only (no punctual/sky-light NEE yet). Verified end-to-end on the Cornell Box scene only; other scenes are unverified.</source>
        <translation type="vanished">双向路径追踪——同时从相机和光源追踪子路径并将每一对路径连接起来,对某些单靠路径追踪难以处理的困难光照配置更有效。
...
CPU only. Same area-lights-only scope and single-scene (Cornell Box) verification as BDPT.</source>
        <translation type="vanished">Metropolis 光传输,直接构建在 BDPT 的子路径机制之上——使用马尔可夫链将采样集中到已经产生贡献的光路上,适用于存在难以找到的明亮光路的场景。
...
CPU only.</source>
        <translation type="vanished">pbrt-v4 的无偏参考路径追踪器——采用均匀球面采样,不使用下一事件估计或多重重要性采样。比默认路径追踪器更简单也更嘈杂;可作为验证其他积分器的基准参考。
...
CPU only.</source>
        <translation type="vanished">一种可视化/调试模式,而非真实光照渲染——衡量每个点被附近几何体遮蔽的程度,完全忽略材质颜色与间接光照。
...
CPU only. NEE, when enabled, is area-lights-only, the same scope as BDPT/MLT.</source>
        <translation type="vanished">pbrt-v4 的标准参考路径追踪器——可选的下一事件估计与可选的 BSDF 重要性采样,两者默认均为开启(见下方开关)。
...
CPU only.</source>
        <translation type="vanished">pbrt-v4 中最简单的体积路径追踪器——纯粹的 delta tracking,不使用 NEE/MIS/表面 BSDF。

在本集成中可用但缺少介质支持,因此在普通实体几何场景中大多渲染为全黑,只有相机光线直接命中光源的地方例外——这与 pbrt-v4 上游在无介质场景下的行为一致。
...
CPU only. Area lights only.</source>
        <translation type="vanished">一种纯粹的光源追踪器——与其他所有积分器方向相反:每个样本都从光源出发,并将与相机的连接贡献投射到胶片上,而不是从相机出发。
...
The alternates below trade that general-purpose approach for a specific technique - simulating light as bouncing particles, tracing extra paths starting from the light itself, or a handful of plain reference/debug modes used mainly for testing. All of them run on the CPU only except one (SPPM), and none can be combined with Generate Video mode. The Sampler/Spectral/Exposure/Tonemap/Stats settings above only affect this default Path Tracer.</source>
        <translation>这是本应用模拟光照的通用方式，程序中其他地方用的也是它。每次反弹时，它既会让一条光线直接瞄准光源（这样简单直接的光照能很快变干净），也会朝着符合表面反射特性的方向发射一条光线，然后把两个结果混合起来，使画面比单用其中任何一种方式都更快变得干净、噪点更少。这是经过充分测试的默认选项；除非有特别的理由，否则从这里开始就好。
...
GPU: only certain materials are supported, checked per scene - matte surfaces and plain light sources, plus the Rough Dielectric, Metal, Dielectric, Conductor, Rough Metal, and Diffuse Transmission material types (and only with lights that have a physical size or shape, not point or sky lights). A scene using anything else shows an error - use the CPU version of this mode instead.</source>
        <translation>把光模拟成一团在场景中四处弹射、最终聚集到相机附近的粒子。它特别擅长渲染透过玻璃或水面看到的那种明亮、聚焦的光斑图案（比如泳池底部的粼粼波光）——这类场景普通路径追踪很难处理干净。

CPU：已确认在“康奈尔粗糙玻璃”场景上能正确工作；其他场景尚未验证，且只支持哑光表面，以及完全镜面式或玻璃式的材质。
...
CPU only. Only works with lights that have a physical size or shape (point lights and a sky/environment light aren&apos;t supported yet). Confirmed to work correctly on the Cornell Box scene only; other scenes haven&apos;t been checked.</source>
        <translation>同时从相机和光源两端构建局部光路，再把每一对光路两两连接起来。这样处理某些棘手的光照场景（比如光线要挤过一道狭窄缝隙）时，效果比单纯从相机一端追踪要好。
...
CPU only. Same lights-must-have-a-physical-size-or-shape limitation and single-scene (Cornell Box) verification as Bidirectional Path Tracing above.</source>
        <translation>建立在与上方“双向路径追踪”相同的路径连接方式之上，但一旦它找到一条确实有贡献的光路，就会在附近不断进行微小的随机试探，寻找更多类似的光路。适合大部分光都只能通过少数几条难以找到的路径到达的场景。
...
CPU only.</source>
        <translation>一个最基础的渲染方式，光线在表面上完全随机地反弹，不使用默认路径追踪器用来加快去噪的任何技巧。它更简单，但画面保持带噪点的时间要长得多——主要用作可靠的参考基准，用来核实其他模式的结果是否正确。
...
CPU only.</source>
        <translation>这是一种可视化/调试模式，而不是一张成品图像——它根据附近物体的遮挡情况，显示表面上每个点被包围或暴露的程度，效果类似于房间角落里那种柔和的阴影。它会完全忽略材质颜色和任何反弹光照。
...
CPU only. When &quot;aim at lights&quot; is on, it only works with lights that have a physical size or shape - the same limitation as Bidirectional Path Tracing and Metropolis Light Transport above.</source>
        <translation>一个简单直接的参考路径追踪器，带有两个可选的加速手段，二者默认都是开启的（参见下方的开关）：让部分光线直接瞄准光源以加快去噪，以及让反弹方向偏向对表面反射最重要的角度。
...
CPU only.</source>
        <translation>渲染烟雾、雾气等半透明体积效果的最简单模式——它在空白空间中一步步前进，直到随机撞上某个东西为止。它不会让光线瞄准光源，也不使用默认路径追踪器那种加快去噪的混合方式，而且完全不处理实体表面。

本应用目前还没有可供它渲染的烟雾/雾气效果，所以用在普通的实体物体场景上，多半只会得到一张黑色图像，除非相机光线恰好直视一个光源——这与底层渲染器在无物可渲染时的正常表现一致。
...
CPU only. Only works with lights that have a physical size or shape.</source>
        <translation>与这里的其他模式方向相反：它不是从相机出发追踪每条光线，而是从光源出发向外追踪，每当一条光路碰巧连回相机时，就把它的贡献计入图像。
...
<context>
    ...
    <message>
        ...
        <translation type="vanished">该场景基于共享的康奈尔盒数据构建:五面 lambertian 漫反射墙壁、一块作为 diffuse_light 的天花板矩形光源、一个旋转过的纯白 lambertian 轴对齐盒子,以及一个使用 dielectric BSDF、进行完整菲涅尔加权反射/折射的球体。它是渲染器的标准收敛测试场景——墙壁间的漫反射互反射、用于验证 BSDF 采样正确性的镜面/透射物体,以及用来检验下一事件估计(NEE)的矩形面光源,因此大多数跨材质或跨光源采样的问题都会最先在这里暴露出来。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">经典的《Ray Tracing in One Weekend》压轴场景:一个棋盘格纹理的 lambertian 地面大球,上方是由 BVH 加速的一大片小球,大多数是漫反射球体,部分球体带有随时间偏移的第二个球心,使球体本身带有线性运动模糊,一些是带随机模糊半径的金属球,还有少量是电介质(dielectric)球。前方摆放三个大型代表球体——一个清晰的电介质球、一个纯 lambertian 漫反射球,以及一个零模糊、如镜面般完美的金属球——作为镜面/漫反射的参照端点。这个场景主要用于压力测试 BVH 的构建,以及运动模糊光线与球体相交检测在模糊金属反射模型下的表现。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">两个巨大的球体(半径为 10,一个位于原点上方,一个位于下方)共用同一张通过球面 UV 坐标采样的程序化棋盘格纹理,并以纯 lambertian 方式着色——图案是根据三维位置解析生成的,而非查表得到的图像纹理。下方球体的顶部还摆放了几个较小的点缀球体(漫反射、带轻微模糊的光泽金属,以及一个电介质球),既作为比例参照,也为画面带来真实的镜面/折射效果。这个场景主要测试程序化纹理的三维到棋盘格映射,在大曲率球面上是否依然表现正确。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">地面与一个大球均使用基于 Perlin 湍流的程序化大理石纹理,直接根据世界空间位置计算并输入 lambertian 材质——不涉及任何图像纹理。另外两个较小的球体以更高的噪声频率复用同一技术作为对比,由一块通过下一事件估计(NEE)采样的 diffuse_light 矩形光源照亮。该场景测试的是 Perlin 噪声湍流/大理石输出能否在真实 NEE 光照(而非平坦环境光)下正确接入 BSDF 管线。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">五个矩形图元,各自通过不同的基向量呈现不同朝向(朝前、朝侧、朝上),每个都使用纯色 lambertian 材质——这里测试的是矩形可命中体(quad hittable)的平面相交与各朝向下法线/UV 的计算,而非着色的复杂度。房间中还悬浮着一块独立的 diffuse_light 矩形光源,作为场景中唯一的发光体,为下一事件估计提供了具体的重要性采样目标。该场景主要用于在真实面光源采样下,检验矩形几何体及其取决于朝向的法线计算是否正确。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">复用了大理石纹理场景中的 Perlin 噪声地面/球体设置,但真正的主题是发光:一个赋予 diffuse_light 材质(暖色调)的球体充当了弯曲、非平面的面光源发光体,与之并存的还有一块独立的冷色调 diffuse_light 矩形光源。让球形和矩形两种发光体共存于同一场景,是为了测试光源采样是否能在不同发光体几何形状下正确处理立体角采样,而不仅仅是常见的平面矩形情形——冷暖色温的区分也让人很容易看出哪盏光在哪里起作用。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">重建了标准的康奈尔盒墙壁,并配有自己那块超大的天花板 diffuse_light 光源,然后将常见的实心盒子替换为两个经过旋转/平移、包裹着 constant_medium 的盒子——这是一种低密度、带有自身色调的均匀各向同性散射参与介质,而非经典的单色烟雾。这是一个体积介质测试场景:在其他场景中被用作实心 lambertian 表面的盒子几何体,在这里则改为定义各向同性内散射体积的边界,用以检验介质采样与边界形状相交计算能否正确配合。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">这是该系列中功能最密集的场景:由高度随机的 lambertian 盒子排列成起伏地面(以 BVH 构建),一块矩形天花板面光源,一个带运动模糊的 lambertian 球体,一个电介质玻璃球,一个模糊金属球,一个包裹在电介质球内部的彩色 constant_medium(一个自成一体的体积雾团),另一个包裹着巨大外壳球体、密度接近零的 constant_medium 作为微弱的全局大气效果,一个使用 mipmap 图像纹理的地球贴图球体,一个 Perlin 噪声大理石纹理球体,以及一簇打包进旋转 BVH 子树中的小白球群。这个场景在一次渲染中同时考验了运动模糊、电介质/金属/lambertian 三种 BSDF、有界与全局参与介质、图像纹理的 mipmap,以及多层级 BVH 构建。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">一排五个球体,依次呈现同一种金色调 rough_metal 材质(GGX/Trowbridge-Reitz 微表面分布,配合简单的 RGB 色调反射率,而非完整的复数折射率菲涅尔计算)在不同粗糙度下的表现。场景中只有一块地面和一盏面光源,画面中唯一变化的变量就是随着粗糙度增加,镜面高光是如何扩散、变暗的。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">使用与“粗糙金属球体”场景相同的 rough_metal GGX BxDF(拉丝铝盒、粗糙度不同的拉丝金球),但被放入了熟悉的康奈尔盒场景,而不是开放式的工作室布光,因此这种光泽微表面反射需要在真实的间接反弹光照和墙壁颜色渗透下依然保持正确,而不只是单一顶光下的表现。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">康奈尔盒中的玻璃球从完美电介质换成了 rough_dielectric——反射和透射两个波瓣都应用了 GGX 微表面模型,因此折射光会散射成磨砂般的模糊效果,而不是遵循斯涅耳定律的清晰光线,同时每个微表面上仍然遵循菲涅尔的反射/透射分配比例。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">抛光的金和铝表面使用 conductor 材质,基于实测的 eta/k 光谱数据、按通道计算真实的复数菲涅尔反射率,而非简单的 RGB 反照率近似——因此金属的颜色来自真实的反射率光谱,而不是美术人员手动挑选的色调。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">一个使用 coated_diffuse 材质的涂层漫反射球体与盒子:在 Lambertian 漫反射基底上叠加一层粗糙电介质涂层。这里考验的是分层 BSDF 的求解——光线可以在涂层上发生镜面反射,也可以透射进入并带上漫反射基底的颜色,两层之间还存在内部反射。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">一块使用 thin_dielectric 材质的竖直玻璃隔板,将康奈尔盒一分为二:这是零厚度玻璃,采用解析的多次反弹菲涅尔公式,而非真实的折射界面,因此透射光会直接穿过而不发生弯折。隔板被刻意倾斜放置,因为折射率为 1.5 的菲涅尔反射率只有在接近掠射角时才明显可见。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">通过 coated_conductor 呈现漆面金属效果:一个金球和一个铜盒,均是在按通道计算复数菲涅尔的 GGX 导体(conductor)基底上,叠加一层粗糙电介质涂层。这是该类别中层叠最多的 BxDF——它把光谱化的金属反射率置于一层无色光泽涂层之下,呈现出涂层的菲涅尔光泽叠加在金属自身色调之上的效果。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">一个使用 diffuse_transmission 材质的蜡质球体,反射率与透射率分别设置为不同颜色——光线会同时向同侧半球(反射)和对侧半球(透射)发生漫散射,这是一种廉价的双参数半透明近似方案,无需体积随机游走,与“次表面平板”场景为达到同样视觉目标所采用的方法不同。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">一个使用 normalized_fresnel 材质的水晶球体——一种菲涅尔加权的漫射出射模型,其 BSDF 值在接近掠射角时升高(此时菲涅尔透射率较低,因此有更多内部散射的光得以逸出)。采样方式仍是余弦半球采样,但散射 PDF 本身携带了菲涅尔加权,并经过归一化处理,以确保在 MIS(多重重要性采样)下总反射能量依然正确。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">一排七个球体,逐一扫过 principled(原则化)材质的参数空间:哑光漫反射、低粗糙度塑料、带清漆层的塑料、半金属、粗糙近金属、光滑全金属,以及带清漆层的全金属——分别独立改变金属度、粗糙度和清漆强度,展示单一的统一 BSDF 如何跨越从电介质到导体的整个范围,而这个范围在本类别的其他场景中是由多种独立材质分别覆盖的。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">五个使用 hair 材质(HairBxDF)的球体,各自具有不同的吸收/粗糙度参数,针对的是一种强烈偏向前向/镜面散射的纤维 BSDF,而非漫反射式的 BSDF——由于头发 BSDF 的峰值响应在正常面光源强度下会过曝,这里的光照强度不得不比该系列其他场景暗得多。该材质被应用在球体表面而非真正的曲线几何体上,以便将 BxDF 的着色行为与曲线相交计算机制分离开来(曲线相交部分见“曲线纤维”场景)。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">在纯 Lambertian 表面上应用了两种扰动技术:凹凸贴图(bump mapping)根据 Perlin 大理石噪声纹理偏移着色法线(后墙与盒子),而法线贴图(normal mapping)则根据棋盘格图案扰动法线(球体)——几何表面本身仍是平面/球面,但用于光照计算的着色法线被扭曲,从而伪造出精细的表面细节。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">这并非真正的 BSSRDF(次表面散射反射分布函数)——进入乳白蜡质平板和玉石球体的光,会先穿过一层电介质外壳,进入一个 constant_medium(具有自身密度与色调的均匀参与介质),因此这种半透明的辉光效果来自物体内部真实的体积随机游走,其边界由表面的折射菲涅尔效应界定,而不是基于扩散模型的次表面项。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">该场景考验的是 pbrt-v4 的实测 BRDF 数据管线——一排五个球体使用的是合成生成的表格化 NDF、sigma、VNDF、亮度以及按波长的光谱表(而非真实采集的材质数据)。BSDF 的实际求值被简化为带纯色调的余弦半球采样,而非完整的五维重要性采样映射链,因此该场景更多是在测试表格化数据的接入管线,而非追求逼真的实测材质渲染效果。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">一个真正的三棱玻璃棱镜,使用色散电介质材质——其折射率依据柯西方程随波长变化,与真实的冕牌玻璃一致——而非固定折射率。白光进入棱镜后,出射时会按波长分开,像真实的玻璃棱镜一样在承接屏上散开成扇形,但这一效果只有在 --spectral(它让渲染器按波长逐一追踪光线)开启时才会出现;若不开启,每个 RGB 通道只是按各自固定且略有差异的折射率折射,而不会形成连续的色散分布。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">与 B23 相同的色散棱镜场景,但改用磨砂玻璃(rough_dielectric)而非光滑玻璃。在 --spectral 下,承接屏幕上会出现同样的彩虹扇形,只是因表面粗糙度而变得模糊,不再锐利——其背后的物理原理(折射率随波长变化)完全相同,只是叠加了微表面散射的差异。要让磨砂玻璃正确产生色散,不能只是简单地对 B23 换皮:与光滑玻璃不同,粗糙表面需要通过显式的下一事件估计才能采样到该场景中偏轴的定向光源,因此采样过程本身也必须具备波长感知能力,而不仅仅是初始折射。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">pbrt 的“mix”材质会在每个着色点上对两种材质进行混合,采用真正的逐样本随机选择——每次命中都按概率加权做出选择,而不是简单地混合颜色——因此哑光漫反射与金属导体的混合会呈现出真实的斑驳颗粒感:部分样本落在纯漫反射上,部分落在纯镜面反射上,经过大量样本后正确地平均出结果。该场景在 CPU、GPU 递归式和 GPU 波前式后端上表现一致。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">这个场景集中了本应用其他地方都没有一起出现过的四种材质:thindielectric(零厚度玻璃片,会发生折射但不会使光线产生位移,不同于普通电介质)、coatedconductor(金属基底上覆盖一层透明涂层)、diffusetransmission(光既发生反射也会透射过去,类似薄叶片或纸张的效果),以及通过命名预设(“Marble”,大理石)实现的次表面散射——无需外部实测数据文件即可获得真实的次表面散射参数。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">pbrt 的 CoatedDiffuse 材质(在透明电介质涂层下是一个漫反射基底,类似上过漆的木材),其反射率从真实的图像纹理中读取,而非使用纯色——这种组合过去在两个后端上都会悄悄忽略纹理,回退为纯灰色。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">一个由显式 RGB eta/k(赋予金属带色调、随波长变化反射率的复数折射率)驱动的 conductor 材质,而非使用命名预设——同时,coatedconductor 的金属基底层也应用了相同的复数折射率计算,从而产生准确的彩色镜面高光,而不是单一的镜面色调。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">DiffuseTransmission(光既发生漫反射也发生漫透射,是薄叶片或灯罩之类物体的模型)材质,其反射率和透射率分别绑定到真实纹理,而非纯色,在两个后端上均是如此。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">pbrt 的“hair”材质——Marschner/Chiang 纤维散射模型,与真实电影级生产渲染器所使用的基于物理的头发 BSDF 相同——在这里被应用到普通球体上,以便与本项目自带的“毛发纤维”演示场景进行受控的并排对比。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">一张 pbrt 棋盘格纹理,其两种颜色本身各自绑定到一张真实图像(即在棋盘格/mix 纹理内部再嵌套一层 imagemap 纹理),而不只是简单的纯色——测试纹理引用是否可以嵌套组合,而不仅仅是独立使用。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">pbrt 的 NamedMaterial 被直接引用到某个形状上(只声明一次、按名称复用,而不只是作为“mix”材质的子成分出现),并且该材质的其中一个参数绑定了纹理,同时场景中还有一个设置了 twosided 标志的 AreaLightSource,使其两面都能发光。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">七个球体，分别对应一个命名玻璃折射率(IOR)预设(BK7、BAF10、FK51A、LASF9、F5、F10、F11)，通过 FindGlassPreset() 在渲染的参考波长下解析为固定的电介质折射率——这与真实光学玻璃目录所使用的查找表完全相同，此前只能从单元测试中触达，在这个场景之前从未被真正渲染过。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">四个四边形，使用相同的源图像和 UV 缩放，各自单独测试一个 imagemap 纹理选项：默认的 sRGB 解码加环绕重复(wrap-repeat)作为参考情形，一个线性解码四边形读取相同的字节数据但显得更亮(跳过了通常会压缩中间调的伽马曲线)，一个钳制环绕(clamp-wrap)四边形在超出 [0,1] 范围时保持边缘纹素而不是平铺，以及一个反转通道的四边形——这些路径在其他任何内置场景的默认纹理设置下都无法触达。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">四个四边形，从左到右分别由一个不同的 pbrt-v4 程序化纹理类驱动——这些类都已接入构建器，但此前从未被任何内置场景使用过：windy(无参数湍流)、wrinkled(可调节倍频/粗糙度的 Perlin 湍流)、dots(由 Perlin 噪声边界分隔的两种纯色)，以及 bilerp(四个角的颜色在 UV 上平滑过渡混合)。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">两个四边形对比了嵌套两层的纹理(外层棋盘格自身的 tex1 是另一个棋盘格，而后者的 tex1 才是真正的图像)与旁边同时呈现的更简单的单层情形。在 CPU 上是真实计算的——同一张照片的两种不同棋盘格频率确实相互嵌套平铺；GPU 则有意将整棵嵌套树展平为单一平均颜色并给出警告，这是一种有文档记录的近似处理而非缺陷，因此用两种后端分别渲染这个场景会显示出真实的差异。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">一个开放场景,完全由程序化生成的 HDR 天空渐变(蓝色天顶渐变为暖色地平线)照亮,该渐变以基于图像的无限光形式接入,而非任何局部发光体。场景中放置了一个漫反射球体、一个近乎镜面的金属球体和一个电介质球体,用以展示这种环境光照在不同 BSDF 下的表现,从类似环境光遮蔽的柔和阴影,到清晰反射/折射出的天空渐变副本。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">光秃秃的康奈尔盒墙壁,完全由一盏聚光灯式的点光源照亮,该光源从天花板正对向下照射,总锥角较宽,内层衰减起始角较窄,因而形成柔和的半影,而非边缘生硬的光斑。与使用面光源的康奈尔盒场景不同,这是一个没有表面可供采样的 delta 光源——锥形与衰减正是这个场景要展示的重点。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">光秃秃的康奈尔盒墙壁,由一盏平行、类似太阳光的远光源(distant light)照亮:它是一个固定方向向量,没有 1/r² 衰减,因此辐亮度的比例尺直接就是入射辐照度,而不是为距离做补偿的强度值。这是一个刻意设计的对照案例,用于验证点光源管线在处理这种方向性、不衰减的光源时,能否与会衰减的点光源/聚光灯/goniometric 光源一样正确工作。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">康奈尔盒墙壁由头顶上方一盏各向同性的点光源照亮,具有经典的 1/r² 强度衰减——这是该类别中最简单的点光源:没有锥形限制、没有方向性分布、没有平行光线,只是从天花板附近一个位置发出、随距离衰减的光。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">康奈尔盒墙壁由一盏 goniometric(测光)光源照亮:这是一种点光源,其发光强度按方向变化,依据一张表格化图像而非均匀发光,同时叠加与普通点光源相同的 1/r² 衰减,并显式设置了旋转以在世界空间中调整该强度分布的朝向。这展示的是任意方向强度分布的情形,与聚光灯简单的锥形分布或点光源的均匀发光形成对比。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">康奈尔盒墙壁由一盏投影光源(projection light)照亮:这是一种点光源,通过透视视锥投射出一张棋盘格幻灯片图像,像真实的幻灯投影仪一样在场景上投出清晰的图案光束,而不是简单地发出锥形或均匀球形的光。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">一个康奈尔盒风格的房间,天花板上根本没有矩形光源——取而代之的是后墙上开出的一个真实矩形洞口,天空光只有透过这扇“窗户”才可见。该场景考验的是通过“传送门”(portal)采样的无限光照:环境光的采样被限制在透过传送门孔径实际可见的方向范围内,而非整个球面方向,这是无限光下一事件估计中更困难、更容易出错的情形。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">该场景集齐了 pbrt-v4 全部五种点光源(delta 分布)类型:点光源(point)、聚光灯(spot)、远光源(distant)、goniometric 光源,以及投影光源(projection)——是一个用于并排比较每种零面积光源类型的单一参考场景。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">pbrt 的 goniometric 光源和投影光源现在具备真正的图像解码功能——此前它们曾悄悄忽略自身的 filename 参数,回退为均匀光束;这个场景就是用来验证它们现在确实会读取并应用图像的回归测试。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">pbrt 的“blackbody L”光源颜色,以开尔文温度而非 RGB 三元组来指定,并通过真实的普朗克轨迹黑体转 RGB 数学模型进行换算。两块除色温外完全相同的面板并排放置,一块设为暖色、类似白炽灯的色温,另一块设为冷色、类似阴天天空的色温,一旦颜色计算出现回归错误,两块面板都会立刻变成平淡的纯白色,非常容易察觉。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">在非三角形形状(球体或矩形)上设置的 AreaLightSource,既通过 filename 使用图像纹理(其发光图案来自图像,而非纯色),又设置了 twosided(两面都发光,而非仅朝表面法线方向的那一面)——两个后端上均是如此。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">pbrt 的“infinite”(无限)光源——一种没有任何形状的恒定颜色光源,同时从各个方向照亮场景,是天空最简单的替代方案。需要开放式的几何结构才能看到效果,因为封闭的房间无论如何都会从各个方向挡住它。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">圆盘与圆柱形状作为真正的下一事件估计(NEE)光源使用——每次反弹都朝光源方向进行立体角采样,而不是单纯依靠光线偶然命中——在两个 GPU 后端上都能像 CPU 路径一样干净地收敛,而不是像非 NEE 光源那样,靠“碰运气让光线命中”的方式收敛,噪点会更多。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">一个场景中包含两个独立的球形面光源——这是一个最小化的用例,曾经用来定位一个真实的 GPU bug:光源类型查找表当时只按一盏光源的大小分配,导致第一盏之后的每一盏光源都悄悄读错了自己的类型,渲染结果因此出错。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">一个形状为不规则五三角形扇面的面光源——它不是一个简单的平行四边形,也无法被渲染器的“矩形合并”优化(该优化会把两个三角形重新拼合为一个矩形以降低采样成本)重新拼合——因此这个场景考验的是 GPU 上真正的逐三角形光源采样,而非更常见的矩形捷径。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">pbrt 的 &quot;ColorSpace&quot; 指令,用于选择&quot;blackbody&quot;色温所转换到的工作 RGB 色彩空间(此处使用色域更宽的 Rec.2020,而非默认的 sRGB)。与 Blackbody Light 示例相同的几何结构、相同的 2500K/9000K 色温——唯一的区别在于所声明的色彩空间——因此这两个场景的面板颜色可以直接并排比较,从而证明该指令确实被应用,而不只是被解析后忽略。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">真正的 pbrt-v4 PortalImageInfiniteLight：一个等面积环境贴图无限光，被限制在一扇窗口四边形之内，因此只有那扇开口能看到真实的天空细节，而不是让整个环境光从四面八方透进来。原生的“传送门无限光”场景(C7)只是在一面墙上、一个平坦 sky_light 背后开出一个几何洞口，从未真正构建过这个类——这是第一个真正做到这一点的内置场景。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">三个原本完全相同的球体，分别只由一种光源类型照亮——点光源、聚光灯和面光源——各自以 &quot;float power&quot;(以瓦特为单位的总光通量)而非通常直接给出的强度/辐亮度来指定亮度。渲染器必须针对每种光源类型，通过不同的立体角公式将功率反向换算为强度(点光源为 4*pi，聚光灯为其锥体立体角，朗伯发光体的面光源为 area*pi)——因此任何一种类型换算错误，都会表现为该球体亮度明显不匹配，而不是整个场景均匀出错。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">一张故意做成非正方形(8x4，而非其他投影光场景通常使用的正方形 uv-checker.bmp)的投影光幻灯片图像，是第一个真正考验图像与光源自身视场之间宽高比不匹配情形的内置场景。正确保持宽高比的投影会投射出一个明显偏宽的矩形光斑，与图像本身 2:1 的形状相符；若这里出现问题，光斑就会被压缩成正方形，或被拉伸以填满光源的视场。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">三盏饱和、近距离的彩色光源照射在一个纯漫反射表面上，其摆放位置使重叠区域恰好落在 --spectral 渲染下 sRGB 色域边界附近。这正是此前真实存在的一个逐样本 XYZ 到 RGB 色域钳制缺陷会导致错误变暗、褪色的确切条件——修复方案将该转换推迟到逐像素而非逐样本进行，因此这里的重叠区域应该平滑地过渡到白色/粉彩色，而不会出现色带或浑浊的欠饱和色斑。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">一个开放场景(棋盘格地面加上位于不同深度的若干球体,一个玻璃、一个金属),使用默认透视相机内置的薄透镜离焦模糊渲染,因此只有处于对焦距离上的球体是清晰的,而近处和远处的球体则会模糊——这是最直接的“光圈加对焦距离”景深路径,不涉及其他相机类别。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">一排位于棋盘格地面上方的球体,通过显式的正交(orthographic)相机渲染。光线以平行方式投射,而非从单一视点汇聚发出,因此球体不会出现透视缩短效果——等大的球体无论深度如何都保持等大,这正是平行投影最典型的视觉特征。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">围绕一个中心发光球体排列的一圈彩色球体,由显式的球面(等距圆柱投影)相机拍摄,因此整个 360 度周边环境都被映射进一张全景图像,而非有限视场角的视锥。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">五个深度依次递增的球体位于棋盘格地面上方,通过显式的“真实”(realistic)相机渲染,该相机基于真实的多镜片处方(每个镜片表面的曲率、厚度、折射率与光圈)构建,而非理想化的薄透镜模型——光线会逐个镜片元件地穿过整套镜片组进行追踪,产生真实的镜头虚化(bokeh)与暗角效果,而不是闭式解的离焦圆盘近似。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">使用与普通康奈尔盒场景以及其他相机对比康奈尔盒场景相同的经典几何结构,采用默认透视相机的薄透镜离焦模糊渲染,而非其他相机类别。这样一来,在与其他相机对比场景相同的房间中,景深就成为唯一的变量,使该技术的效果可以直接对比,而不会与不同的几何结构混淆。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">与其他相机对比场景共用同一个康奈尔盒,通过显式的正交相机渲染,视角沿盒子的深度轴正对拍摄。由于是平行投影而非透视投影,盒子的边缘会一直保持平行延伸到画面边缘,而不会像透视/景深版本那样发生汇聚——特意使用同一个房间,是为了让这种对比清晰可辨。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">同一个共享的康奈尔盒,由放置在盒子中心的显式球面(等距圆柱投影)相机环视拍摄。全景图从一个内部视点出发,将全部五面墙壁、天花板光源和玻璃球都包裹进一张 360 度图像中——这与独立的“球面相机”场景采用的是同一种技术,只是这里应用到了共享的对比几何结构上。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">同一个共享的康奈尔盒,使用与独立“真实相机”场景完全相同的多镜片镜头处方进行渲染,但光圈按比例放大以适配盒子大得多的尺度——这样即便场景尺度不同,离焦锥的角度大小、进而可见的镜头虚化效果仍能保持可比性。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">与之前相同的康奈尔盒场景,但相机本身会在曝光期间移动——通过真正的 AnimatedTransform 插值(pbrt-v4 的运动模糊技术)在两侧位置之间设置关键帧,每条光线都在快门时间区间内采样各自独立的时间点,而非使用单一固定相机位置。仅在默认路径追踪器与 SPPM 下生效(BDPT/MLT/调试积分器会改为渲染一帧静止的首关键帧画面)。GPU 同样只渲染首关键帧的静止画面。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">透视相机的薄透镜景深效果——镜头半径与对焦距离——直接从 pbrt 文件的 Camera 指令中加载,两个后端上均是如此。这与本项目自带的“景深”场景使用的是同一种光学模型,只是这里考验的是文件加载路径。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">从文件中加载的 pbrt 正交(平行投影)相机——所有光线都是平行的,而不是汇聚于一个视点,因此两个大小相同、深度不同的球体在屏幕上呈现出相同的大小,而不是近处的显得更大,这与普通透视缩短效果正好相反。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">从文件中加载的 pbrt 球面(等面积)相机——光线从单一点向各个方向发散,而不是穿过一个平面成像平面,因此一次渲染就能捕捉完整的 360 度环绕视野。该相机被放置在一个封闭房间内,以确保各个方向都确实有可看的内容。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">pbrt 的“真实”(realistic)相机——一套真正的多镜片系统,而非其他相机场景所使用的理想化薄透镜/针孔模型——从镜头处方文件中加载,让光线穿过实际的玻璃镜片元件,以获得真实的景深和镜头像差,其中也包含了内置编译场景从未测试过的镜头文件加载路径。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">在康奈尔盒内填充了一种均匀参与介质——一个具有统一散射系数和 Henyey-Greenstein 相函数的边界盒。边界内每一点的密度都相同,因此这个场景测试的是基础的自由程采样/单一散射系数介质路径,而非任何空间变化的密度模型。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">一个真正的异质体积,其密度是逐点通过多倍频 Perlin 噪声、叠加丝缕状扰动和高度衰减程序化计算出来的,而不是查表读取预先烘焙的体素数据。渲染使用 delta 追踪(空碰撞采样)来处理这种非均匀密度场,呈现出如同真实云朵一样底部更浓密、顶部逐渐变薄的效果。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">把“电介质加内部介质”这一组合作为独立主题展示:多个玻璃球,每个都由一个折射性电介质表面构成,内部填充着密度各不相同的均匀参与介质。该场景考验的是散射介质由真正的折射表面(而非不透明外壳)界定的情形,因此光线必须先折射进入,穿过内部雾气发生散射/吸收,再折射出来。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">一种异质介质,其真实的逐体素散射数据被烘焙进一个三维网格,红/绿/蓝三个通道以不同频率独立生成,使颜色在空间上互不相关,而不是呈现为均匀染色的云雾——采样时使用了“主导网格”(majorant grid)加速的 delta 追踪。与逐点解析计算的程序化介质不同,这里是离散的体素数据,从渲染空间的盒子映射到网格自身的坐标空间中。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">pbrt 的 MakeNamedMedium “cloud”——一种异质散射体积,其密度来自三维 Perlin 噪声,这与真实云朵渲染所采用的噪声驱动方法相同,用以避免呈现出一块平淡、明显假的雾块——从文件中加载,两个后端上均支持。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">一种均匀参与介质(统一的雾气/散射密度)附加在一个圆柱形状上——这种组合过去在 GPU 后端上会悄悄渲染成普通的空几何体,完全没有雾气效果;这个场景就是用来验证介质效果现在在 GPU 上也真实生效的回归测试。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">pbrt 的 MakeNamedMedium “rgbgrid”——一个三维体素网格,其中每个体素单元都带有自己的 RGB 散射颜色,而不仅仅是一个密度标量——渲染效果呈现为柔和的、带彩色的星云状体积,两个后端上均是如此。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">pbrt 的 MakeNamedMedium “uniformgrid”——一个单通道密度体素网格,不像 RGB 网格介质场景那样带有逐体素颜色——渲染效果呈现为一团柔和发光的团状物,是那个场景更简单的姊妹版本。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">pbrt 的 MakeNamedMedium “nanovdb”：从外部 .nvdb 文件读取的真正 NanoVDB 格式稀疏密度网格，渲染为一个柔和的雾状体积球体——这是本项目所填补的单项最大 pbrt-v4 缺口，因为稀疏体素数据所需的查找方式与其他介质场景所用的致密网格截然不同。仅限 CPU；GPU 完全没有实现 NanoVDB 查找，而是回退为均匀的平坦雾效，这是一种有文档记录的近似处理而非缺陷。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">pbrt-v4 的相机介质用法：在 Camera 指令之前发出的 MediumInterface 指令，会让相机本身置身于雾中而没有任何边界形状——这与其他所有内置介质场景真正不同，因为它们都是把 MediumInterface 附加到相机从外部观察的某个形状上。此处的实现方式是在相机代码中作为一个显式的“命中后”步骤，而不是作为 BVH 可求交对象，因为这个渲染器本身并没有光线携带的介质状态。在 CPU 和 GPU 递归式后端上是真实实现的；GPU Wavefront 完全没有实现它，会悄悄地在没有雾效的情况下渲染，这是一个真实存在的后端差距，严重到需要在 CPU/GPU 一致性测试中为其单独设置容差豁免。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">测试双线性面片(bilinear patch)图元——这是一种真正的四角弯曲曲面,有自己专属的相交计算方法,而不是由一对三角形拼成——具体做法是放置两个面片,通过让四角高度错开,分别形成一个双曲抛物面鞍形和一个弯曲坡面。这里刻意使用了带有轻微粗糙度的金属材质,以便着色渐变能够描绘出曲面的连续曲率——如果用镜面材质,则只会显示单一的清晰高光,掩盖了曲面并非平面这一事实。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">构建了一个程序化生成的正二十面体,作为真正的带索引三角网格,考验的是渲染器中每个加载网格所使用的“无缝”(watertight)光线-三角形相交路径。该网格没有提供逐顶点法线,因此每个三角形会退化为其平坦的逐面几何法线,呈现出多面体式的低多边形观感,而非平滑着色的插值法线效果。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">该场景考验的是真正的曲线几何体及其专属的光线-曲线相交测试,不同于另一个“毛发纤维”场景那样,只是用 hair BSDF 给普通球体着色,而没有真正让表面本身弯曲。带有从根部到梢部渐细效果的贝塞尔发丝以散点方式扎根在表面上,因此这个场景测试的是曲线图元真实的扫掠带状几何,而非头发着色模型本身。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">pbrt 的 Shape “plymesh” 携带的真实逐顶点 UV 数据,已贯通两个后端——此前 GPU 递归式后端会把这个场景渲染成一片纯黑,原因就是这些 UV 数据被悄悄丢弃了。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">pbrt 的 Shape “plymesh” 完整加载一个真实的外部 .ply 网格文件,包括对源文件中一个尚未三角化的面进行扇形三角化处理——加载器不能假定每个面都已预先三角化。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">pbrt 的 Shape “curve”——真正的三次贝塞尔纤维几何体,在底层被镶嵌为适合 GPU 处理的三角形,而不是任何三角网格近似——与本项目自带的、以同样方式构建的“曲线毛簇”演示场景进行了直接对比。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">这是本项目中首次将真正的曲线几何体与 Material “hair” 材质结合使用——正是这种组合当初暴露出一个 bug:头发 BSDF 使用的是平面法线,而非纤维自身的切线方向,而切线方向正是让每根发丝上的高光看起来像“一根发丝”、而不是一团哑光绒毛的关键所在。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">pbrt 的 Shape “trianglemesh” 的 “point2 uv” 参数——三角网格上显式的逐顶点纹理坐标——已贯通两个后端;在这条数据通路建立之前,GPU 递归式后端会把这个场景渲染成一片纯黑。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">pbrt 的 PixelFilter 指令选用了盒式滤波器(box filter)——像素内的每个样本被等权重计入,在像素边界处硬性截断——而非更平滑的默认高斯滤波器。可将此处更明显锯齿化、边缘更生硬的轮廓,与其他任何使用默认滤波的场景渲染结果进行对比。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">一个球体的 ActiveTransform StartTime/EndTime 区间，为同一个形状烘焙进两个不同的世界空间中心，由相机自身的快门区间逐条光线采样——运动中的球体应当渲染成一道柔和的方向性拖影，而不是清晰的单一副本或重影，与旁边完全没有区间设置的静态参考球体形成对比。在全部三个后端(CPU、GPU 递归式、GPU Wavefront)上都是真实实现的，这一点不同于下面的圆盘/圆柱情形。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">与“物体运动模糊”场景相同的 ActiveTransform 运动模糊技术，但应用到了 Shape &quot;disk&quot;/&quot;cylinder&quot; 而非球体——这在内部走的是一条不同的代码路径(烘焙进形状自身的起止变换，而不是球体的 center1/center2 一对参数)。仅限 CPU：两个 GPU 后端都会将这些形状固定渲染在其起始姿态上并给出警告，而不会产生模糊效果，这一点不同于 GPU 确实支持的球体运动模糊。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">两个四边形共享完全相同的点数组和索引绕序，这本身会产生一个背对相机的法线——以这种绕序构成的单面面光源从正面是不可见的。紧接在第二个四边形之前发出的裸 ReverseOrientation 指令，会将其有效法线翻转为朝向相机，因此一个四边形保持暗淡，另一个则被点亮；ReverseOrientation 中的缺陷会表现为两个四边形都暗淡或都被点亮，而绝不会是恰好一明一暗。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">四个形状测试本项目自有的 pbrt-v3 兼容性扩展(Shape &quot;cone&quot;/&quot;paraboloid&quot;，并非真正 pbrt-v4 的一部分)：一个普通漫反射圆锥和抛物面作为基础形状，一个用作 AreaLightSource 发光体(而非普通材质)的圆锥，以及一个用作 MediumInterface 雾边界的抛物面，其开口边缘朝向相机，使光线从开口端而非实体表面进入介质。仅限 CPU——GPU(两个后端)会发出警告并完全丢弃任何圆锥/抛物面形状。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">经典的 pbrt-v4 “killeroo” 雕像示例场景——这是一个完全从自己的 .pbrt 文件端到端加载的场景(几何体、材质、光源、相机全部由文件驱动),而非直接编译进本渲染器,使用的是与用户自己的自定义 pbrt 场景相同的文件格式和加载路径。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">与 A1 相同的康奈尔盒场景,但故意以较低的每像素 16 次采样(而非通常的 100 次)进行渲染——采样数过少,难以干净收敛。在渲染选项标签页切换采样器(Sobol、Z-Sobol、Padded Sobol、Stratified、PMJ02BN、Halton)后重新渲染:每种选择都是在像素与光源上分布这 16 个样本的不同低差异序列,因此即便场景其他部分完全不变,柔和阴影半影区中的噪点/聚簇模式也会随之改变。仅对 CPU 默认路径追踪器生效——采样器控件对 GPU 没有任何影响。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">与 B23(光谱色散)相同的玻璃棱镜,在此重新用作&quot;光谱渲染&quot;复选框自身的演示场景。关闭光谱渲染(渲染选项标签页)时,渲染器只跟踪红/绿/蓝三通道,每种波长在玻璃中的弯折角度都完全相同;开启后,渲染器会跟踪每条光线真实的波长,棱镜的折射率会随波长真正发生变化,将白光散开成一道可见光谱,与真实棱镜的效果一致。仅对 CPU 默认路径追踪器生效。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">与 C1(HDRI 天空)相同的 HDR 天空渐变——一个明亮的程序化天空背景,前方是一个投下自身阴影的漫反射球体——亮度范围真实覆盖从接近纯黑到远超单张图像所能显示的极亮区间。渲染选项标签页中的两个控件以不同方式作用于这一范围:曝光是在其他一切处理之前应用的整体倍率,提高曝光会让整幅图像(球体与天空)均匀变亮;色调映射则是在该倍率之后应用的曲线,用于将结果压缩到可显示范围内——ACES 会柔和地压低天空最亮部分的高光,Reinhard 压缩得更猛烈,而 None 则直接将其裁剪为纯白。可尝试在几个不同曝光值下分别切换各种色调映射选项,观察这两个控件如何独立发挥作用。CPU 与 GPU 均支持。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">与 A1 相同的康奈尔盒场景,但故意以较低的每像素 32 采样渲染,使其在做任何后处理之前就带有明显的颗粒感——分别在关闭和开启 OptiX AI 降噪器(“渲染选项”标签页)的情况下各渲染一次。降噪器是一个机器学习模型,能够识别这种斑点噪声模式(由场景自身的反照率与法线缓冲引导)并将其抹平,而无需追踪哪怕一条额外的光线——以损失少量细节为代价,在相同采样数下换来明显更干净的画面。仅限 GPU——递归式与 Wavefront 两种后端各自都有自己的降噪器。旁边的“OptiX 验证模式”复选框也是这里值得了解的另一类控件:它只是开启额外的、有实际性能开销的设备端正确性检查——绝不会改变渲染出的图像,所以这一项没有什么画面可供直观对比。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">与 B3(康奈尔粗糙玻璃)相同的磨砂玻璃康奈尔盒，重新设计为“积分器”下拉列表自身的 SPPM 演示。以相同的采样数分别用默认路径追踪器和 SPPM(渲染选项标签页)各渲染一次：粗糙电介质球体下方地板上的焦散——正是普通路径追踪难以收敛的、高频率的间接光照困难情形——在 SPPM 的光子映射方法下应当明显更快地变干净，因为这种方法正是为处理这类光传输而设计的。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">与 A1 相同的康奈尔盒，重新设计为“积分器”下拉列表自身的 BDPT/MLT 演示。尝试使用 BDPT 或 MLT(渲染选项标签页)而非默认路径追踪器：两者都会同时从相机和光源追踪光路并在中途将它们连接起来，而不是像默认积分器和 SPPM 那样只从相机追踪——这是一种根本不同的策略，在这类具有明显间接光照的场景上可能收敛得不同(有时更好，有时呈现不同的噪点特征)。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">与 A1 相同的康奈尔盒，重新设计为“积分器”下拉列表参考/调试模式的巡览。RandomWalk 在半球上均匀采样下一个反弹方向，完全不进行光源采样，也不使用 MIS——是收敛最慢但偏差最小的基准。SimplePath 为这两个要素各自添加了独立的复选框：仅开启“采样光源”(NEE)时，直接光照获取成本低，但朝向明亮但偏轴光源的反弹仍然依赖运气；仅开启“采样 BSDF”时则相反；两者都开启时，即为将两者结合的多重重要性采样(MIS)——这也正是默认路径追踪器已经在做的事情。以相同的低采样数分别渲染，观察它们在相同几何体上收敛方式的差异。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">专为本示例构建的康奈尔盒：天花板上有五盏灯，而不是 A1 的一盏，且刻意设置了悬殊的功率(约为 1:2:6:15:80，而不仅仅是尺寸不同)。Uniform(光源采样器，渲染选项标签页)会将每次下一事件估计采样中的五分之一均匀分配给每盏灯，而不考虑其实际对图像的贡献——因此大部分采样都花在了几乎无关紧要的四盏灯上，而那盏真正主导的灯反而保留了不必要的噪点。Power 和 BVH 都会将选择权重偏向那盏主导灯(BVH 还会考虑距离/方向，Power 仅考虑总发光通量)——以相同的低采样数渲染这三种方式，比较明亮角落那盏灯周围的噪点消退速度。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">与 A1 相同的康奈尔盒，重新设计为“积分器”下拉列表自身的 Ambient Occlusion(环境光遮蔽)演示。AO 刻意完全不是一个有光照的渲染：切换到该模式(渲染选项标签页)后，每个表面自身的颜色以及场景中的每一个真实光源都会被完全忽略，取而代之的是单一的灰度遮蔽项——即每个点上方的半球有多少被附近的几何体遮挡。旋转盒子自身的接触阴影以及两面墙相交的角落是仅剩的结构，这正是 AO 作为快速预览场景几何复杂度(与其光照隔离开来)的有用之处。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">与 B3 和 I5 相同的磨砂玻璃康奈尔盒——粗糙电介质球体的地板焦散是一个教科书式的硬焦散案例，这种光路(类镜面散射进入一个很小的立体角，然后落在漫反射地板上)在普通路径追踪下会产生孤立的明亮像素“萤火虫”噪点。Regularize 和萤火虫噪点截断(--maxcomponentvalue，均在渲染选项标签页)以两种不同的方式驯服它：Regularize 在第一次非镜面反弹之后加宽粗糙 BSDF 自身的粗糙度，使得焦散本身从一开始就不那么病态地尖锐、难以采样；而截断则允许萤火虫噪点发生，然后将任何最亮通道超过阈值的样本进行缩放，以损失一点能量为代价换取一个有保证的上限。先不启用任何选项渲染一次，然后分别单独启用每一项渲染，使用相同的采样数进行比较。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation>康奈尔盒是计算机图形学中最著名的测试场景之一：一个小房间，两侧墙壁分别涂有颜色，天花板上嵌着一块发光面板，房间里放着一个纯白的盒子和一个既能反射又能折射周围光线的玻璃球。留意红墙和绿墙是如何把反弹的颜色染到附近的白色表面上的，以及天花板上的灯光是如何产生柔和干净的阴影，而不是带噪点的斑驳效果的。正因为它把反弹光照、光源和玻璃物体都塞进了一个小房间里，这个场景往往是任何光照或材质问题最先暴露出来的地方。</translation>
    </message>
    <message>
        ...
        <translation>向经典教程《一个周末学会光线追踪》的最终场景致敬：一片棋盘格地面向地平线延伸，上面散布着许多哑光、金属和玻璃质感的小球体。仔细看会发现有些球体带着轻微的拖影——那是它们在拍摄过程中正在画面里移动，就像真实相机捕捉运动模糊那样。金属球体的表现从镜面般锐利到柔和拉丝质感不等，取决于表面的粗糙程度。前方三个较大的球体——一个透明玻璃球、一个纯哑光球和一面完美的镜子——组成一组参照物，方便并排比较不同的材质效果。</translation>
    </message>
    <message>
        ...
        <translation>两个巨大的球体上下叠放，表面都覆盖着同样的蓝白棋盘格图案——这个图案不是贴上去的，而是通过数学方式生成的，能够干净利落地包裹住弯曲表面，不会出现拉伸或接缝。顶上停着几个较小的球体——一个哑光、一个带柔和模糊感的金属、一个玻璃——既能作为比例参照，也展示了同样的光照如何在不同材质上产生不同的效果。</translation>
    </message>
    <message>
        ...
        <translation>地面和一个大球体上覆盖着程序生成的大理石纹图案——由数学噪声函数（柏林噪声）生成的、旋绕流动且自然多变的色带，而不是一张贴图，这正是计算机图形学和游戏里用来制作逼真云朵、木纹或大理石纹理的常用技术。旁边两个较小的球体使用了同一图案更精细、更密集的版本作为对比，头顶一块发光面板为整个场景带来柔和、逼真的阴影。</translation>
    </message>
    <message>
        ...
        <translation>五块彩色平板以不同角度摆放——朝前、朝侧面、朝上——用来检验无论朝向如何，每一块都能正确接收光照。房间里悬浮着的另一块发光面板是场景中唯一的光源，为其他平板投下柔和的直射光和阴影。</translation>
    </message>
    <message>
        ...
        <translation>沿用了大理石球体场景中的大理石纹地面，但这个场景真正的主角是光本身：一个暖色调发光球体和一块冷色调发光面板，在同一个房间里充当两种形状迥异的光源。由于一个光源是弯曲的球体、另一个是平坦的面板，这里很适合观察不同形状、不同颜色的光是如何在周围表面上扩散、交融的。</translation>
    </message>
    <message>
        ...
        <translation>沿用了熟悉的康奈尔房间，但把两个实心盒子换成了填满彩色雾气的盒子——它们不再像实心物体那样阻挡光线，而是在内部散射光线，呈现出柔和、朦胧、烟雾弥漫的效果，而不是锐利的边缘。这很适合观察渲染器如何处理局限在特定形状和厚度内的雾气和薄雾，而不只是处理平坦的实心表面。</translation>
    </message>
    <message>
        ...
        <translation>整套场景中内容最丰富的一个，几乎把所有花样都集中在了一起：由许多高度随机的小盒子组成的起伏地面、一块发光的天花板面板、一个明显在移动的球体（运动模糊）、一个玻璃球、一个带柔和拉丝质感的金属球、一个内部封着彩色雾气的玻璃球（像雪花球一样）、弥漫在整个房间里的一层极淡薄雾、一个贴着地球贴图的球体、一个大理石纹球体，以及角落里紧密聚集的一小簇白色小球。如果要用一张渲染图展示这个渲染器的全部能力，那就是这一张。</translation>
    </message>
    <message>
        ...
        <translation>五个金色球体排成一排，彼此完全相同，只有一点不同：表面的粗糙程度。最左边的几乎和镜面一样光滑，带有一个又小又锐利的反射高光；越往右，表面越粗糙，高光也逐渐扩散成柔和、宽广的光晕，就像拉丝或缎面处理的金属看起来比抛光金属更暗淡、更模糊一样。这是一次干净利落的并排对比，展示了仅仅改变表面粗糙度就能如何改变金属的外观。</translation>
    </message>
    <message>
        ...
        <translation>与“粗糙金属”那一排相同的拉丝金属质感——一个拉丝铝盒和一个拉丝金球——不过这次被放进了康奈尔房间里，需要在柔和、带色彩的反弹光照下表现，而不是一盏干净的摄影棚顶灯。留意那些模糊、有光泽的反射是如何映出房间彩色墙壁的痕迹的。</translation>
    </message>
    <message>
        ...
        <translation>康奈尔盒里常见的透明玻璃球，被换成了磨砂版本——光线穿过它时会散射成柔和的模糊效果，而不是锐利地弯折，就像磨砂或喷砂玻璃相比干净玻璃窗的效果一样。它表面的反射也同样呈现出这种柔和、模糊的处理效果。</translation>
    </message>
    <message>
        ...
        <translation>抛光的金和铝表面，其颜色和光泽来自各自金属真实测量得到的光学数据，而不是由美术人员随手挑选一个金色或银色色调——因此你看到的颜色，正是该金属真实反射光线的物理效果，而不是一种近似模拟。</translation>
    </message>
    <message>
        ...
        <translation>一个球体和一个盒子看起来像是蘸过一层透明清漆：表面覆盖着一层玻璃般、有反光的透明涂层，下面则是普通的哑光底色。仔细观察高光就会发现——一部分光直接从光亮的外层涂层反弹出去，另一部分则穿透涂层、染上下方的哑光颜色，还有一些光会在两层之间来回反弹几次才最终逸出。</translation>
    </message>
    <message>
        ...
        <translation>一块玻璃把康奈尔房间一分为二，被当作一片理想化的零厚度薄片处理，因此光线穿过时会径直通过，不会像穿过一整块厚玻璃那样发生弯折。它被特意倾斜摆放——当你正对着玻璃看过去时，它几乎不会反射任何东西，但随着视线角度变得越来越倾斜，它会明显变得更像镜子，就像汽车挡风玻璃在斜视角度下会变得反光、而正面看去却几乎“消失”一样。</translation>
    </message>
    <message>
        ...
        <translation>一个金球和一个铜盒都呈现出上过漆的展厅级质感：一层玻璃般的透明涂层覆盖在金属本身真实的反射色之上。这是这组材质中层次最丰富的一种，把金属天然带有色彩的光泽，与涂层自身独立的高光叠加在了一起。</translation>
    </message>
    <message>
        ...
        <translation>一个球体呈现出柔和、蜡质、如烛光般的质感：光不仅仅是从它表面反弹，还有一部分会穿透它、从另一侧散射着透出来，而不是清晰锐利地射出——这是一种简单、低成本的手法，用来模拟蜡烛或灯罩这类半透明材质的外观，而不需要真正模拟光线在物体内部的弹射过程。</translation>
    </message>
    <message>
        ...
        <translation>一个水晶质感的球体，边缘处的发光效果比正面看去更强烈——这正是一杯水或一个肥皂泡边缘看起来比中心更明亮、更有反光感的那种角度相关效应，这里被用在了材质允许内部散射光逸出的方式上。</translation>
    </message>
    <message>
        ...
        <translation>七个球体排成一排，展示了一种灵活、万能的材质，从纯哑光平滑过渡到光泽塑料，再到带清漆涂层的塑料、半金属质感、粗糙拉丝金属，最终变成完美如镜的光滑效果（分别有和没有透明涂层）。不需要为每种外观单独准备一种材质，只要调节金属感、粗糙度和涂层光泽这几个参数，这一种材质几乎可以变成任何表面效果。</translation>
    </message>
    <message>
        ...
        <translation>五个球体使用了与单根发丝相同的光散射模型进行着色——这里应用在球体上而不是真正的发丝细纤维上，方便你单独研究这种材质本身的外观效果（真正的发丝状几何体可参见“曲线纤维”场景，用的是同一原理）。这种材质散射光线的方式非常强烈、集中，几乎像激光一样，以至于这个房间里的灯光不得不调得很暗，以免高光完全过曝成一片死白。</translation>
    </message>
    <message>
        ...
        <translation>两种在不真正建模的情况下伪造精细表面细节的手法：后墙和盒子看起来带有凹凸不平的大理石纹理，但它们实际的形状完全是平的；球体看起来表面被刻上了棋盘格图案，但实际上完全光滑。这两种技巧的原理都是悄悄改变光线“以为”表面朝向的方向，而完全不改变实际的几何形状。</translation>
    </message>
    <message>
        ...
        <translation>一块乳白色的蜡板和一个玉石球体，都呈现出真实半透明材质那种由内而外柔和发光的效果——光线穿过玻璃般的外表面进入物体内部，在里面散射、弹跳一段时间后才找到出路，而不是直接从表面反弹回来。这正是皮肤、牛奶、玉石等真实次表面材质广为人知的效果，这里的实现方式是把物体内部当作一团会散射光线的雾气来处理，而不是使用专门的皮肤/半透明配方。</translation>
    </message>
    <message>
        ...
        <translation>五个球体展示了对真实世界“实测”材质数据的支持——也就是在实验室里扫描真实物理材质样本得到的那种数据表格，而不是用几个方便美术人员调节的滑块来描述外观。这里用的数据是人工合成生成的，并非真实扫描结果，着色计算本身也做了简化，所以这个场景实际上是在检验实测数据格式能否被正确加载和处理，而不是展示某种具体的真实材质。</translation>
    </message>
    <message>
        ...
        <translation>一个玻璃棱镜，就像科学课堂上用来演示光与色彩的那种：白光从一个面射入，从另一面射出时分散成一道彩虹色的光扇，这是因为玻璃对不同波长的光会产生极其细微不同的弯折，与真实玻璃的表现一致。只有当渲染器运行在特殊的 --spectral 模式（按单独波长追踪光线）下时，才会出现完整的彩虹光谱；否则只会看到一个粗略的三色（红/绿/蓝）近似分解效果。</translation>
    </message>
    <message>
        ...
        <translation>与光滑版本相同的、能分散出彩虹光谱的玻璃棱镜，只是这次表面被磨成了磨砂、喷砂质感，而不是抛光。在 --spectral 模式下运行时，同样的彩虹光扇会出现在后方的屏幕上，只是被磨砂表面模糊、柔化了，而不再锐利分明——这证明了色散效果和磨砂玻璃的模糊效果是两个各自独立、又能正确同时发生的现象，而不是粗糙表面意外把颜色冲淡了。</translation>
    </message>
    <message>
        ...
        <translation>在表面的每一个点上混合一种纯哑光材质和一种光泽金属材质——不是把两者的颜色调成某种中间色，而是为每一个独立的光照采样随机挑选其中一种。仔细观察就会发现，结果带有真正的斑驳颗粒感，有些地方表现为纯哑光，有些地方表现为纯光泽，只有当积累足够多的采样后，才会平均成平滑的中间效果。</translation>
    </message>
    <message>
        ...
        <translation>四种截然不同的材质外观并排展示：一片理想化的纸薄玻璃，能弯折光线却不产生侧向位移；一种覆盖了透明清漆的金属；一种像薄叶片或纸张一样、既能反射光又能透光的材质；以及一个使用内置“大理石”预设的方块，无需任何外部扫描数据即可呈现次表面散射那种柔和的内发光效果。</translation>
    </message>
    <message>
        ...
        <translation>一个金属表面，以及同一种金属涂上清漆的版本，它们的颜色和反射率都来自直接输入的原始物理光学折射数值，而不是选用某个命名好的金属预设——这样得到的是精确、自然带色的镜面高光，而不是美术人员随手选定的一种扁平金属色。</translation>
    </message>
    <message>
        ...
        <translation>使用了与真实电影制作级渲染器相同的基于物理的毛发着色模型，用于呈现逼真的毛发和皮毛效果，这里把它应用在普通球体上，方便与本项目另一个独立的发丝演示场景进行干净、可控的对比。</translation>
    </message>
    <message>
        ...
        <translation>一种材质只定义一次，然后通过名字在多个物体上重复使用，其中一个属性由图像贴图驱动，而不是使用单一的纯色——场景中的光源也被设置为从两个面同时发光，而不是只朝一面，就像有人特意记得一块灯板其实是有正反两面的。</translation>
    </message>
    <message>
        ...
        <translation>七个玻璃球体，各自使用一种不同命名的真实光学玻璃类型（与实际相机镜头目录中出现的名称相同），每一种对光线的弯折程度都略有不同。这展示了镜头设计师对玻璃材质的选择，是如何转化为材质弯折光线方式上细微却真实的差异的。</translation>
    </message>
    <message>
        ...
        <translation>一种上过漆的木质质感材质——透明光泽涂层下是哑光底色——它的底色来自贴到表面上的一张真实照片，而不是单一的纯色。这种具体的搭配组合曾经出现过一个 bug：贴图会被悄悄忽略，改用一片灰色代替，因此这个场景的存在就是为了证明贴图现在能够正确显示了。</translation>
    </message>
    <message>
        ...
        <translation>一种类似薄叶片或灯罩的材质——既能从正面反射光线，也能让光从背后透出——它的反射色和透射色都由真实照片驱动，而不是使用纯色，并且在每一种支持的渲染方式下都做了检验。</translation>
    </message>
    <message>
        ...
        <translation>一个棋盘格图案，它的两种“颜色”实际上各自是一整张独立的照片，而不是纯色色块——也就是一层贴图嵌套在另一层贴图里，展示了贴图可以层层叠加组合使用，而不只是单独使用一张。</translation>
    </message>
    <message>
        ...
        <translation>四块完全相同的面板，使用同一张源照片，各自展示了图像解码和应用的一种不同方式：大多数图像使用的标准效果、跳过大多数相机和屏幕都默认的一道颜色校正步骤后明显更亮的版本、拉伸超出边界时保留边缘像素而不是重复图案的版本，以及颜色通道被翻转的版本。把它们并排放在一起，能让一些平时看不出来的图像处理设置变得一目了然。</translation>
    </message>
    <message>
        ...
        <translation>四块面板，各自展示一种纯由公式生成的程序化图案，而不是照片：一种如风吹拂般的湍流效果；一种带有可调细节的、如云朵般起伏的表面；一种沿着自然波浪状、由噪声绘制的边界分割开的双色图案；以及在四个角落颜色之间平滑过渡的渐变效果。</translation>
    </message>
    <message>
        ...
        <translation>把“棋盘格套棋盘格套照片”——三层图案彼此嵌套——与紧挨着放置的更简单的单层版本进行对比。在 CPU 上渲染时，完整的嵌套细节能够正确呈现；在 GPU 上渲染时，则会被有意简化为一个平均颜色（并附带一条警告），这是一个已知且可接受的简化处理，而不是 bug——所以这个场景就是特意用来并排展示这种差异的。</translation>
    </message>
    <message>
        ...
        <translation>一个开放场景，只由天空照明——从头顶的蓝色平滑过渡到地平线附近的暖色光晕，代表的是整片环境，而不是一个单独的灯泡或面板。一个纯哑光球体、一个近乎完美的镜子和一个玻璃球放置在天空下方，展示了同样的天空在不同材质上呈现出的完全不同效果：在哑光球体上柔和、如阴影般，在镜子上是一份锐利的天空反射，在玻璃球中则是经过弯折、折射后的天空景象。</translation>
    </message>
    <message>
        ...
        <translation>一个空房间，由天花板上一盏笔直向下照射的聚光灯照明，光斑边缘柔和、逐渐淡出，而不是一个边界锐利的光圈——就像精心设计的舞台射灯或轨道射灯的效果，而不是刺眼的手电筒光束。</translation>
    </message>
    <message>
        ...
        <translation>一个空房间，其照明效果就像置身户外、沐浴在直射阳光下：光线来自一个固定的方向，且完全平行；与灯具或灯泡不同，它不会随距离变暗——无论把墙壁移得多远，光照强度都始终保持不变。</translation>
    </message>
    <message>
        ...
        <translation>一个空房间，由最简单的一种光源照明：空间中一个点，朝所有方向均匀发光，表面离它越远就越暗，类似一个裸露的小灯泡的光随距离迅速衰减的效果。</translation>
    </message>
    <message>
        ...
        <translation>一个空房间，由一种发光强度不均匀、带有自定义形状的光源照明——某些方向更亮，某些方向更暗，遵循的是一份真实测量得到的光照分布数据，而不是朝所有方向均匀发光，类似真实台灯或灯具常常因为灯罩或反光板的形状，导致某个方向的光更强。</translation>
    </message>
    <message>
        ...
        <translation>一个空房间，由一种能投射图案的光源照明，就像老式幻灯机一样，在墙上投出一道清晰的棋盘格光束，而不是一个普通、均匀照亮的锥形或球形光斑。</translation>
    </message>
    <message>
        ...
        <translation>一个康奈尔风格的房间，天花板上完全没有灯光——取而代之的是后墙上开出的一个真正的窗户形状的洞，所有光线都来自透过这个洞看到的天空。这比一块就摆在房间里的灯板要棘手得多，因为渲染器必须想办法让光线穿过一扇狭窄的窗户，瞄准无处不在、充满整个外部空间的天空，而不是瞄准一个近在咫尺、可以直接对准的光源。</translation>
    </message>
    <message>
        ...
        <translation>五种最基本的“单点”光源类型并排放在同一个房间里——普通点光源、聚光灯、类似太阳的远方光源、带自定义方向性光照分布的光源，以及幻灯机式光源——组成一份便于一眼比较所有简单、无实际大小的光源类型的清单。</translation>
    </message>
    <message>
        ...
        <translation>用来检验两种本应能够投射真实图像的光源类型——带自定义方向性光照分布的光源和幻灯机式光源——现在是否真的能读取并显示各自指定的图像，而不是像过去那样悄悄退化成一道平淡无特征的光束。</translation>
    </message>
    <message>
        ...
        <translation>两块发光面板使用不同的色温发光，色温单位是开尔文，就像描述影棚灯泡或相机色温设置时用的那种单位：一块面板发出温暖、舒适、如白炽灯泡般的橙色光，另一块则发出偏冷、如阴天多云天空般的蓝白色光。如果底层的颜色计算出了问题，两块面板就会趋同变成同一种平淡的白色，所以让它们保持明显不同，正是这个场景存在的意义。</translation>
    </message>
    <message>
        ...
        <translation>一个发光形状（球体或面板），它的发光图案来自一张真实图像，而不是发出单一的纯色光，并且这个图案会从它的两个面同时发出，而不是只朝它通常指向的那一面发光。</translation>
    </message>
    <message>
        ...
        <translation>最简单的一种天空替代方案：一种恒定不变的颜色，同时从各个方向照射进来，本身没有任何可见的形状。它只有在开放场景中才有意义——在完全封闭的房间里，它会被完全挡在外面，就像你在一个密封的盒子里看不到天空一样。</translation>
    </message>
    <message>
        ...
        <translation>一个圆盘形和一个圆柱形的光源，都被当作真正的光源来处理，渲染器会有意地朝它们发射采样光线，而不是寄希望于光线随机反弹恰好命中它们。正是这种有意的瞄准，才让场景能够很快变成干净平滑的画面，而不是长时间停留在斑驳带噪点的状态。</translation>
    </message>
    <message>
        ...
        <translation>两个各自独立的发光球体照亮这个简单的房间——这个极简的设置曾经暴露过一个真实的 bug：渲染器的 GPU 代码只能正确记录一个光源的类型，导致第一个光源之后的任何额外光源都会被识别错误、渲染出错。</translation>
    </message>
    <message>
        ...
        <translation>一个形状像不规则五边扇形的光源，而不是简单的矩形——形状足够别扭，以至于渲染器无法照惯例把两个三角形当作一个简化矩形来采样，只能老老实实地处理这个形状怪异的光源。</translation>
    </message>
    <message>
        ...
        <translation>几何结构和暖/冷光源色温都与“黑体光源”场景完全相同，但渲染时使用了一种更宽广、更接近电影摄影机的色彩空间，而不是大多数屏幕使用的日常色彩空间。把这两个场景并排对比，能直观地证明选择不同的色彩空间确实会改变最终的颜色，而不是一个被悄悄忽略的设置。</translation>
    </message>
    <message>
        ...
        <translation>一个康奈尔风格的房间，后墙上开了一扇窗户，透过这扇窗能看到一张真正的高分辨率天空照片，而不是一片平淡的光晕。与本演示库中其他更简单的“传送光源”场景不同，这里透过窗户看到的是真实的天空细节，而不只是从一个洞里透出的单调颜色光。</translation>
    </message>
    <message>
        ...
        <translation>三个完全相同的球体，各自由一种不同类型的光源照明——裸露的点光源、聚光灯和发光面板——但每个光源的亮度都是按总发光功率设定的，就像灯泡包装盒上印的瓦数，而不是一个原始的亮度数值。要把这个功率公平地转换成实际可见的亮度，对每种光源形状的计算方式都不一样，所以只要三个球体中有任何一个显得过亮或过暗，都会立刻显现出来。</translation>
    </message>
    <message>
        ...
        <translation>一个幻灯机式光源把一张宽幅、非方形的图像（宽度是高度的两倍）投射到场景中，而不是其他投影光源场景常用的方形测试图案。如果处理正确，投射出的光斑应该明显呈矩形，与幻灯片本身的比例一致；如果存在 bug，就会表现为光斑被挤压成正方形，或者被拉伸填满整个光束。</translation>
    </message>
    <message>
        ...
        <translation>三盏颜色浓烈的灯——红、绿、蓝——在近距离内重叠照射在一个普通表面上，正好让混合后的颜色逼近普通屏幕能显示的极限。在它们交融的地方，你应该能看到向白色和柔和色调平滑、自然地过渡；这里曾经出现过的一个渲染 bug，表现为颜色混合处出现浑浊、发暗的斑块，或是明显的色带。</translation>
    </message>
    <message>
        ...
        <translation>一片棋盘格地面，上面散布着位于不同距离的玻璃和金属球体，使用逼真的相机对焦效果渲染。只有恰好位于对焦距离上的球体是刀锋般清晰的，其余更近或更远的部分都逐渐虚化成柔和的模糊——就像透过一台真实相机的镜头观看一样。</translation>
    </message>
    <message>
        ...
        <translation>一排球体位于棋盘格地面上方，透过一台完全没有透视效果的相机观看——光线完全平行传播，而不是汇聚到一个视点，因此不论实际距离多远，每个球体在画面上看起来都是一样大小。这是建筑制图式平面视图的典型特征，与普通照片的效果截然不同。</translation>
    </message>
    <message>
        ...
        <translation>一圈彩色球体环绕着中心一个发光球体，由一台 360 度全景相机拍摄——周围各个方向的完整景象，会同时被压平进一张环绕式图像里，而不是通常那种边界分明的照片画框。</translation>
    </message>
    <message>
        ...
        <translation>五个球体沿棋盘格地面依次向远处排开，通过一个模拟真实相机镜头拍摄——这个镜头由具备各自曲率和厚度的真实玻璃镜片元件组成，而不是一个简化的数学替代模型。画面中的失焦模糊，以及画面边缘的轻微暗角，都是光线穿过真实镜片组时产生的真实副作用，而不是走捷径模拟出来的效果。</translation>
    </message>
    <message>
        ...
        <translation>这组相机对比场景中反复使用的同一个康奈尔盒，这次呈现出柔和、如摄影般的景深虚化效果——便于清楚看到这种效果具体是什么样子，同时你还可以用附近其他相机类型观察同一个房间，除此之外场景的其他部分都没有改变。</translation>
    </message>
    <message>
        ...
        <translation>同一个共用的康奈尔盒，这次透过一台平行投影相机沿其长度方向正对着观看。由于没有透视汇聚效应，盒子的边缘一直到画面边界都保持完全平行，而不会像普通相机视角那样向一个消失点收窄。</translation>
    </message>
    <message>
        ...
        <translation>同一个共用的康奈尔盒，这次由一台放置在正中央的 360 度全景相机环视拍摄——天花板灯光、全部五面墙壁和玻璃球都被包裹进一张连续的图像里，仿佛你正站在房间中央同时望向四面八方。</translation>
    </message>
    <message>
        ...
        <translation>同一个共用的康奈尔盒，用与独立镜头演示场景相同的模拟真实镜头拍摄，光圈按比例调大以适配房间更大的尺寸——这样即便场景尺度更大，模糊失焦的视觉效果强度也能保持相近。</translation>
    </message>
    <message>
        ...
        <translation>同一个共用的康奈尔盒，不过这次相机本身会在快门开启期间从一侧滑向另一侧，产生真实的运动模糊，而不是一帧静止画面。这种效果只有在使用标准路径追踪器或基于光子的 SPPM 模式渲染时才会出现——其他渲染模式以及 GPU，都只会显示停留在起始位置的相机。</translation>
    </message>
    <message>
        ...
        <translation>相机逼真的对焦模糊效果——光圈大小和对焦距离——从外部场景描述文件中加载，而不是内置在程序里。与本项目自带的“景深”场景是同一种光学效果，只是这次是通过加载文件得到的。</translation>
    </message>
    <message>
        ...
        <translation>从场景文件中加载的一台平行投影相机——光线完全平行传播，而不是汇聚到一个视点，因此两个位于不同距离、大小相同的球体在屏幕上看起来一样大，而不是较近的那个显得更大，这与普通照片的效果正好相反。</translation>
    </message>
    <message>
        ...
        <translation>从场景文件中加载的一台 360 度全景相机——光线从单一一个点向各个方向发散，而不是穿过一个普通的平面画框，让一张图像就能捕捉整个周围环境。它被放置在一个封闭房间内，确保无论朝哪个方向看都总有内容可看。</translation>
    </message>
    <message>
        ...
        <translation>一枚真正的多镜片相机镜头——不是其他相机场景使用的那种简化理想化镜头——从外部镜头设计文件中加载，光线会逐条穿过它实际的玻璃镜片元件进行追踪，从而产生真实的景深虚化和镜头像差。</translation>
    </message>
    <message>
        ...
        <translation>一个从边到边都填满均匀雾气的康奈尔盒——内部每一点的浓度和散射方式完全相同，呈现出光线在房间中散射所能产生的最简单、最均匀的朦胧效果。</translation>
    </message>
    <message>
        ...
        <translation>一朵外观逼真的云，它那飘渺不定的形状来自多层叠加、自然变化的柏林噪声，而不是一整块均匀固定的雾气——底部更浓密，向上逐渐变薄，就像真实的云一样。渲染这种浓度逐点变化的雾气，需要专门为逼真处理这种变化密度而设计的技术，这正是这个场景要测试的内容。</translation>
    </message>
    <message>
        ...
        <translation>几个玻璃球体，每个内部都填充着浓度各不相同的雾气。光线必须先在穿过玻璃表面时发生弯折，再被内部的雾气散射或吸收，最后在离开时再次弯折——而不是像遇到不透明外壳那样简单地反弹回去。</translation>
    </message>
    <message>
        ...
        <translation>一团云状体积，由三维网格上每个格子的真实密度数据构建而成，红、绿、蓝三个通道各自使用独立变化的图案，使颜色在云朵内部不断漂移、变幻，而不是呈现为单一、均匀的色调。</translation>
    </message>
    <message>
        ...
        <translation>一朵从外部场景文件加载的云，它的形状由与真实云渲染相同的那种自然变化的噪声图案驱动，避免看起来像一整块明显假的平淡雾块。</translation>
    </message>
    <message>
        ...
        <translation>一层简单均匀的雾气包裹在一个圆柱形状外面——这种具体的组合过去在 GPU 上会被悄悄渲染成一个没有雾气的普通圆柱，所以这个场景的存在就是为了确保雾气效果在 GPU 上也能正确显示出来。</translation>
    </message>
    <message>
        ...
        <translation>一个基于体素的体积，其中每个小格子都携带自己的颜色，而不只是一个密度数值——渲染出的效果是一团柔和、色彩斑斓、如星云般的云朵，而不是一片单一均匀色调的雾气。</translation>
    </message>
    <message>
        ...
        <translation>一个基于体素的体积，每个格子只有一个密度数值，没有各自的颜色——渲染出的效果是一团更简单、柔和发光的雾状团块，是色彩斑斓的网格云场景更朴素的姊妹版本。</translation>
    </message>
    <message>
        ...
        <translation>一个真实的稀疏云数据文件，采用专业视觉特效软件所使用的同一种格式，被加载并渲染成一个柔和的雾状球体。像这样的稀疏数据需要一套与普通等间距网格完全不同的查找方式，因此它是较难良好支持的格式之一。在 CPU 上能够完整工作；在 GPU 上目前会回退为一层普通的、厚度均匀的雾气，这是一个已知的简化处理，而不是 bug。</translation>
    </message>
    <message>
        ...
        <translation>相机本身就置身于一片没有任何包围形状的雾气之中，仿佛镜头本身蒙上了雾——这与这里其他所有雾气场景的设置真正不同，那些场景都是让观察者从外部看着一个带雾的物体。这个效果在 CPU 和两条 GPU 渲染路径中的一条上能正常工作；另一条 GPU 路径目前还不支持它，只会渲染出完全没有雾气的画面。</translation>
    </message>
    <message>
        ...
        <translation>两个弯曲的表面——一个马鞍形和一个平滑的斜坡——直接由各自的四个角点构建而成，而不是用平面三角形来近似。这里特意使用了柔和模糊的金属质感，让明暗渐变揭示出表面真实的弯曲程度；如果用完美的镜面效果，就只会显示一个锐利的高光，掩盖了这个表面根本不是平的这一事实。</translation>
    </message>
    <message>
        ...
        <translation>一个手工搭建的 20 面骰子形状（二十面体），由真正的三角形网格拼装而成。每个平坦的三角面都呈现出各自独立的明暗效果，而不是与相邻面平滑融合，因此呈现出多面、低多边形的外观，而不是光滑圆润的效果。</translation>
    </message>
    <message>
        ...
        <translation>真正弯曲、逐渐变细的发丝——根部粗、尖端细——散布在一个表面上，由真实弯曲的几何体构建而成，而不是另一个“毛发纤维”场景中使用的、用平面球体着色伪装成毛发的手法。</translation>
    </message>
    <message>
        ...
        <translation>一个从外部加载的三维模型，内置了真实的纹理坐标数据，使图像能够正确地贴合在它的表面上。这个具体的场景过去在其中一条 GPU 渲染路径上会渲染成一片纯黑，因为那份纹理坐标数据被悄悄丢弃了。</translation>
    </message>
    <message>
        ...
        <translation>一个从头到尾完整加载的真实外部三维模型文件，其中原始文件里有一个面并不是由三角形构成的——加载器必须自己把它拆分成三角形，而不能假设文件里已经做好了这件事。</translation>
    </message>
    <message>
        ...
        <translation>真正弯曲的纤维发丝，表面平滑圆润，而不是由平面三角形构成，与本项目自带的、以同样方式构建的发丝演示场景并排展示——直接对比两种方法实现同一视觉效果的差异。</translation>
    </message>
    <message>
        ...
        <translation>真正弯曲的纤维发丝首次搭配上恰当的毛发质感材质——两者结合，暴露出一个 bug：每根发丝上的光泽被当作平面来计算，而不是当作弯曲的纤维，而正是这一点，才让一根真实的头发呈现出沿其长度延伸的那道特征性高光条纹，而不是暗淡的哑光效果。</translation>
    </message>
    <message>
        ...
        <translation>一个基于三角形的三维形状，每个角点都明确设置了纹理坐标，使图像能够正确贴合到它上面。在具备这项纹理坐标支持之前，两条 GPU 渲染路径中的一条曾经把这个具体场景渲染成一片纯黑。</translation>
    </message>
    <message>
        ...
        <translation>这个场景特意使用一种更简单、更块状的方式，把大量采样合成为最终的像素颜色，使边缘和轮廓比默认的平滑效果略显生硬、锯齿感更明显。可以拿它和其他场景通常那种柔和边缘的渲染效果做对比，观察其中的差异。</translation>
    </message>
    <message>
        ...
        <translation>一个球体在模拟相机快门开启的起点和终点被赋予了两个不同的位置，因此它应该渲染成两点之间一道柔和的方向性拖影——真正的运动模糊——而不是一个清晰的静止球体，或者一个奇怪的重影副本。旁边放置了一个静止的参照球体用于对比。这个效果在本项目支持的每一种渲染后端上都能正确工作。</translation>
    </message>
    <message>
        ...
        <translation>与“球体运动模糊”场景相同的起点/终点运动模糊手法，这次应用在圆盘和圆柱上——这两种形状底层存储运动信息的方式略有不同。这个效果目前只在 CPU 上渲染时才能生效；两条 GPU 渲染路径目前都只会把这些形状显示为静止在起始姿态，而不会产生模糊。</translation>
    </message>
    <message>
        ...
        <translation>两块完全相同的平板由同一份几何数据构建而成，按默认设置，两者都会背对相机、作为光源时保持昏暗。只把第二块的朝向翻转过来，就能让它发光，而第一块依然保持昏暗——这是一次直观的检验，用来确认物体的朝向被正确地考虑在内，因为如果这里出了错，就会表现为两块面板要么都暗、要么都亮。</translation>
    </message>
    <message>
        ...
        <translation>四个形状由一种较旧的锥形/碟形（抛物面）图元构建而成，这种图元是为了兼容旧版场景文件而保留下来的：一个普通圆锥、一个普通碟形、一个用作发光光源而非普通表面的圆锥，以及一个开口朝向相机的碟形，用作雾气的边界，使光线能够通过开口漂入雾气内部，而不是硬生生穿透实心材质。仅在 CPU 上渲染——两条 GPU 路径都会完全跳过圆锥和碟形。</translation>
    </message>
    <message>
        ...
        <translation>一个经典、精细的雕像模型，以一个完整自包含的场景文件形式呈现——几何体、材质、灯光和相机都在文件内部定义——走的是和用户自己的自定义场景文件完全相同的加载路径。</translation>
    </message>
    <message>
        ...
        <translation>与最开始的第一个演示场景相同的康奈尔盒，这次故意用远少于平常的采样数渲染，使画面明显保持带噪点的状态，而不是干净地收敛。可以尝试在“渲染选项”标签页切换“采样器”的不同选项（Sobol、Z-Sobol、Padded Sobol、Stratified、PMJ02BN、Halton）并重新渲染——每一种都会以不同的方式把采样分布到像素内以及朝向光源，因此柔和阴影边缘的噪点效果在每种选项下都会略有不同，尽管场景的其他部分完全没有变化。这只影响 CPU 的默认渲染器——在 GPU 上渲染时，“采样器”选项没有任何效果。</translation>
    </message>
    <message>
        ...
        <translation>与“光谱色散”场景相同的玻璃棱镜，专门用来展示“渲染选项”标签页上的“光谱渲染”选框。关闭它时，渲染器只追踪红、绿、蓝三色，因此每种颜色的光穿过玻璃时弯折程度都一样。开启后，渲染器会追踪单独的光波长，因此玻璃对每个波长的弯折都会有极其细微的差异——把白光分散成一道清晰可见的彩虹，就像真实的玻璃棱镜一样。仅限 CPU。</translation>
    </message>
    <message>
        ...
        <translation>与“HDRI 天空”演示相同的明亮天空与阴影场景，其真实亮度范围远远超出任何屏幕能一次性显示的范围。“渲染选项”标签页上的两个控件以不同方式处理这个范围：“曝光”是最先施加的一个简单亮度乘数，会均匀地把一切（天空和球体）都调亮或调暗。“色调映射”则是之后施加的一条曲线，把这个巨大的亮度范围压缩到屏幕能够显示的范围内——ACES 会柔和地压低天空最亮的部分，Reinhard 压缩得更激进，“无”则直接把最亮的区域截断为纯白。可以尝试几种不同的“色调映射”选项，配合几档不同的“曝光”数值，观察这两个控件是如何各自独立发挥作用的。CPU 和 GPU 均可使用。</translation>
    </message>
    <message>
        ...
        <translation>与最开始的第一个康奈尔盒演示相同的场景，用非常低的采样数渲染，使画面呈现出真正的噪点感——先关闭“渲染选项”标签页上的 OptiX AI 降噪器渲染一次，再开启它渲染一次。降噪器是一个机器学习模型，能识别出这种斑驳的噪点模式并将其抹平，而不需要多追踪一条光线，用一点点细节换来渲染成本不变、却明显更干净的画面。仅限 GPU。附近的“OptiX 验证模式”选框也值得了解一下，但它的作用不同——它只会开启额外的内部安全检查，带来一定性能开销，绝不会改变画面的最终效果，所以那里没有什么视觉上的差异可比较。</translation>
    </message>
    <message>
        ...
        <translation>与“康奈尔粗糙玻璃”场景相同的磨砂玻璃康奈尔盒，专门用来展示“渲染方式”下拉菜单中的 SPPM 选项。用相同的采样数，先用默认的路径追踪器渲染一次，再用 SPPM（渲染选项标签页）渲染一次——粗糙玻璃球下方那块明亮聚焦的光斑（一种焦散效果），正是普通路径追踪很难快速清理干净的那类棘手光照。SPPM 会先从光源追踪光线并记录它们落在哪里，再从相机一侧把这些信息收集起来——这是专门针对这类光照设计的方法，因此焦散效果应该会明显更快变得清晰锐利。</translation>
    </message>
    <message>
        ...
        <translation>与最开始的第一个康奈尔盒演示相同的场景，专门用来展示“渲染方式”下拉菜单中的 BDPT 和 MLT 选项。可以在“渲染选项”标签页尝试其中任意一个来代替默认的路径追踪器：两者都会同时从相机和光源出发追踪光路，再在中途把它们连接起来——这与默认渲染器或 SPPM（只从相机端追踪）在策略上有根本性的不同。在这种带有明显反弹光照的场景中，这种不同的策略会带来不同的收敛效果，有时更好，有时只是噪点的表现方式不一样。</translation>
    </message>
    <message>
        ...
        <translation>与最开始的第一个康奈尔盒演示相同的场景，用来带你逐一了解“渲染方式”下拉菜单中的参考和调试模式。RandomWalk 只是让每条光线朝随机方向反弹，完全不尝试瞄准光源，也不做任何策略上的智能混合——是最简单、变干净最慢，但也最“老实”的基准方式。SimplePath 恰好为这两个要素分别提供了两个独立的选框：只开启“采样光源”时，直接光照能便宜地获得，但反弹恰好指向旁边一个明亮光源就纯属运气；只开启“采样材质反射”时则正好相反；两者都开启时，两种策略会结合起来得到更干净的结果——这其实也正是默认路径追踪器在背后一直在做的事情。用相同的低采样数分别渲染，观察它们在同一个房间里收敛效果的差异有多大。</translation>
    </message>
    <message>
        ...
        <translation>专门为这个演示搭建的康奈尔盒，天花板上有五盏灯而不是通常的一盏，而且各自的亮度被特意设置得极不均匀（大致比例为 1:2:6:15:80）。“均匀”光源采样器（渲染选项标签页）会把瞄准光源的努力平均分成五份，分别用在这五盏灯上，而不考虑它们各自实际的重要程度——因此它把大部分精力浪费在四盏几乎没什么贡献的灯上，导致真正明亮的那盏灯的噪点比本该有的更多。而“按功率”和 BVH 都会把精力更多地倾向那盏主导光源（BVH 还会考虑距离和方向，“按功率”只考虑总亮度）——用相同的低采样数分别渲染这三种方式，比较明亮角落那盏灯周围的噪点消退速度有多大差别。</translation>
    </message>
    <message>
        ...
        <translation>与最开始的第一个康奈尔盒演示相同的场景，用来展示“渲染方式”下拉菜单中的“环境光遮蔽”模式。AO 根本不是一种正常的带光照渲染——切换到它（渲染选项标签页）之后，场景中每个表面的实际颜色以及每一个真实光源都会被完全忽略，取而代之的是一个简单的灰度值，衡量每个点被周围几何体包围的程度。剩下的只有盒子本身的接触阴影和墙角相接处的暗部，这正是 AO 能够快速预览场景形状和几何复杂度、而完全不用操心光照的原因所在。</translation>
    </message>
    <message>
        ...
        <translation>与“康奈尔粗糙玻璃”和 SPPM 演示场景相同的磨砂玻璃康奈尔盒——粗糙玻璃球下方那块明亮聚焦的光斑，正是普通路径追踪下杂散、孤立的过亮像素（“萤火虫噪点”）的典型来源。“正则化”和“萤火虫钳制”（都在“渲染选项”标签页）用两种不同的方式解决这个问题：“正则化”会在第一次反弹之后，柔和地增加玻璃自身的粗糙度，让这类棘手的光照从一开始就更容易被干净地采样到；“萤火虫钳制”则任由过亮的像素先出现，再限制单个采样所能达到的最大亮度，用一点点精度换取对杂散亮点的确定性上限。分别用相同的采样数渲染原始场景，以及单独开启每一个选项的版本，进行对比。</translation>
    </message>
    <message>
...
or video opens in its own tab here, so past renders stay
around while you compare or tweak settings.</source>
        <translation type="vanished">请从“基本设置”开始一次渲染——每个渲染完成的图像
或视频都会在此处以独立标签页打开,方便你在比较或
调整设置时保留以往的渲染结果。</translation>
    </message>
</context>
</TS>
