...
This app ships with dozens of built-in scenes covering the basics (a simple Cornell box) up through complex conductor/dielectric materials, volumetric fog, and real photogrammetry-scale models - pick one to render, or browse by category using the tabs above.</source>
        <translation>每次渲染都从一个场景开始 - 它描述了世界中的一切：几何体（形状与网格）、材质（表面由什么构成）、光源，以及相机。
...
仅使用 CPU、分辨率较低 - 首次运行需要一些时间。</translation>
    ...
    <message>
        ...
        <translation>将采样器/积分器/光源采样器(渲染选项标签页)设置为此场景自身推荐的值。</translation>
    </message>
    <message>
...
including the handful the GPU backend does not implement.</source>
        <translation>GPU：基于 OptiX 的硬件光线追踪 — 通常快上几个数量级。
...
GPU (OptiX) uses NVIDIA&apos;s dedicated ray-tracing cores to trace thousands of rays in parallel, typically far faster. CPU uses ordinary processor cores instead: much slower, but works on any machine and supports every material this app implements, including a couple the GPU path hasn&apos;t caught up to yet.</source>
        <translation>两者追踪的是完全相同的光线，生成完全相同的图像 - 区别只在于速度和所用硬件，而非物理原理。
...
Only applies when Renderer is set to GPU.</source>
        <translation>递归式：每像素一个线程，是默认的 GPU 路径追踪器 — 覆盖面广，经过充分验证。
...
Recursive traces one ray per thread from start to finish, bouncing recursively - simple and battle-tested. Wavefront instead groups all rays currently doing the same kind of work (e.g. &quot;just hit glass&quot;) into a batch and processes them together - better use of the GPU&apos;s parallel hardware on complex scenes with lots of different materials, at the cost of being a newer, less-tested code path.</source>
        <translation>在 GPU 上组织同一份光线追踪工作的两种不同方式。
...
Hover any item in the dropdown for details on that specific integrator.</source>
        <translation>渲染算法本身,而不仅是渲染速度。路径追踪器(默认)是本应用其他地方通用的、经过充分验证的选择。
...
the render time. Setting it here switches Quality to Custom.</source>
        <translation>每像素追踪的光线数。这是控制质量/时间的主要旋钮：噪点会随该值的
...
this checkbox only ever adds the request, never removes it.</source>
        <translation>在路径的第一次非镜面反弹后加宽粗糙 BSDF 的 GGX alpha 值——
可以抑制来自高难度焦散路径的萤火虫噪点,
...
Off by default, matching pbrt-v4&apos;s own default. If a loaded .pbrt scene&apos;s file already requests this itself, it&apos;s applied either way - this checkbox can only add the request on top, never take it away.</source>
        <translation>某些光路对路径追踪器来说天生就很难被干净地采样到——光线在一个粗糙(但并非完全镜面)的表面上反弹,再穿过另一个粗糙表面,最终射入一个很小很亮的光源。这类路径会在图像中表现为明亮、孤立的斑点(“萤火虫噪点”),需要极长时间才能被平均掉。
...
Off by default (effectively unbounded, matching pbrt-v4&apos;s own default). CPU default path tracer only.</source>
        <translation>蒙特卡洛路径追踪器偶尔会采样到一条本身正确、但极其明亮的光路——比如一条光线恰好以某个角度擦过一个很小但很强的光源——这样的单个样本可能在很长一段时间内主导某个像素的平均值,直到有足够多的其他样本进来将其平滑掉。这类样本会在图像中表现为明亮、孤立的斑点(“萤火虫噪点”)。
...
default Path Tracer - see each control&apos;s own tooltip.</source>
        <translation>选择使用哪种渲染算法。路径追踪器(默认)是经过充分验证的通用选择——下方的备选项则以牺牲通用性为代价换取特定技术(光子映射、双向/Metropolis 光传输,或若干无偏的参考/调试积分器)。除 SPPM 外,所有备选项均仅限 CPU,且都无法与生成视频模式组合使用。
...
than silently rendering wrong colors. Noticeably slower per-sample.</source>
        <translation>使用真正的主波长(hero-wavelength)光谱渲染,而非平面 RGB。
...
effect under BDPT/MLT/SPPM/the debug integrators.</source>
        <translation>在应用 sRGB 曲线之前使用哪种色调映射算子。
...
ACES rolls off bright highlights gently, the way film does; Reinhard is a simpler, older compression; None just clips anything too bright to flat white, which can look harsh.</source>
        <translation>光线追踪场景的真实亮度值是无上限的——一个灯泡可能比墙壁亮上百倍——但屏幕只能显示有限的亮度范围。色调映射就是把这个巨大范围压缩到可显示范围内所使用的曲线。
...
never changes the rendered image.</source>
        <translation>渲染结束时在“日志”标签页打印一小段统计信息(发射光线数、
...
Purely informational: it never changes the rendered image, just tells you what the renderer actually did.</source>
        <translation>渲染完成后打印一段简短的统计摘要——发射了多少光线、发生了多少次反弹、追踪了多少条阴影光线,以及每秒样本数。
...
path tracer only.</source>
        <translation>将渲染范围限制在画面的一个矩形区域内,该区域以
...
Off by default (the full frame). If a loaded .pbrt scene&apos;s file already requests its own cropwindow/pixelbounds, checking this overrides it with the rectangle below; leaving it unchecked lets the scene&apos;s own request (if any) stand.</source>
        <translation>只渲染完整画面中的一个矩形区域——区域之外的部分保持黑色——而不是渲染整张图像。该矩形以完整画面宽/高的四个分数给出,从 0(左/上边缘)到 1(右/下边缘),因此无论分辨率如何,形状都保持不变。
...
The alternates below trade that generality for a specific technique - photon mapping, bidirectional/Metropolis light transport, or a handful of unbiased reference and debug integrators. All are CPU-only except SPPM, and none can be combined with Generate Video mode. Sampler/Spectral/Exposure/Tonemap/Stats above only affect this default Path Tracer.</source>
        <translation>本应用其他地方通用的重要性采样路径追踪器——下一事件估计加上 BSDF 重要性采样,并通过幂启发式(MIS)进行组合。这是经过充分验证的默认选项,除非有特殊理由,否则应从这里开始。
...
GPU: capability-checked per scene - Lambertian/DiffuseLight, RoughDielectric, Metal, Dielectric, Conductor, RoughMetal, and DiffuseTransmission are supported (area lights only); an unsupported scene falls back to an error - use CPU SPPM instead.</source>
        <translation>随机渐进光子映射(pbrt-v4 风格)。最适合普通路径追踪难以解决的强焦散/玻璃场景。
...
CPU only. Area lights only (no punctual/sky-light NEE yet). Verified end-to-end on the Cornell Box scene only; other scenes are unverified.</source>
        <translation>双向路径追踪——同时从相机和光源追踪子路径并将每一对路径连接起来,对某些单靠路径追踪难以处理的困难光照配置更有效。
...
CPU only. Same area-lights-only scope and single-scene (Cornell Box) verification as BDPT.</source>
        <translation>Metropolis 光传输,直接构建在 BDPT 的子路径机制之上——使用马尔可夫链将采样集中到已经产生贡献的光路上,适用于存在难以找到的明亮光路的场景。
...
CPU only.</source>
        <translation>pbrt-v4 的无偏参考路径追踪器——采用均匀球面采样,不使用下一事件估计或多重重要性采样。比默认路径追踪器更简单也更嘈杂;可作为验证其他积分器的基准参考。
...
CPU only.</source>
        <translation>一种可视化/调试模式,而非真实光照渲染——衡量每个点被附近几何体遮蔽的程度,完全忽略材质颜色与间接光照。
...
CPU only. NEE, when enabled, is area-lights-only, the same scope as BDPT/MLT.</source>
        <translation>pbrt-v4 的标准参考路径追踪器——可选的下一事件估计与可选的 BSDF 重要性采样,两者默认均为开启(见下方开关)。
...
CPU only.</source>
        <translation>pbrt-v4 中最简单的体积路径追踪器——纯粹的 delta tracking,不使用 NEE/MIS/表面 BSDF。
...
CPU only. Area lights only.</source>
        <translation>一种纯粹的光源追踪器——与其他所有积分器方向相反:每个样本都从光源出发,并将与相机的连接贡献投射到胶片上,而不是从相机出发。
...
<context>
    ...
    <message>
        ...
        <translation>该场景基于共享的康奈尔盒数据构建:五面 lambertian 漫反射墙壁、一块作为 diffuse_light 的天花板矩形光源、一个旋转过的纯白 lambertian 轴对齐盒子,以及一个使用 dielectric BSDF、进行完整菲涅尔加权反射/折射的球体。它是渲染器的标准收敛测试场景——墙壁间的漫反射互反射、用于验证 BSDF 采样正确性的镜面/透射物体,以及用来检验下一事件估计(NEE)的矩形面光源,因此大多数跨材质或跨光源采样的问题都会最先在这里暴露出来。</translation>
    </message>
    <message>
        ...
        <translation>经典的《Ray Tracing in One Weekend》压轴场景:一个棋盘格纹理的 lambertian 地面大球,上方是由 BVH 加速的一大片小球,大多数是漫反射球体,部分球体带有随时间偏移的第二个球心,使球体本身带有线性运动模糊,一些是带随机模糊半径的金属球,还有少量是电介质(dielectric)球。前方摆放三个大型代表球体——一个清晰的电介质球、一个纯 lambertian 漫反射球,以及一个零模糊、如镜面般完美的金属球——作为镜面/漫反射的参照端点。这个场景主要用于压力测试 BVH 的构建,以及运动模糊光线与球体相交检测在模糊金属反射模型下的表现。</translation>
    </message>
    <message>
        ...
        <translation>两个巨大的球体(半径为 10,一个位于原点上方,一个位于下方)共用同一张通过球面 UV 坐标采样的程序化棋盘格纹理,并以纯 lambertian 方式着色——图案是根据三维位置解析生成的,而非查表得到的图像纹理。下方球体的顶部还摆放了几个较小的点缀球体(漫反射、带轻微模糊的光泽金属,以及一个电介质球),既作为比例参照,也为画面带来真实的镜面/折射效果。这个场景主要测试程序化纹理的三维到棋盘格映射,在大曲率球面上是否依然表现正确。</translation>
    </message>
    <message>
        ...
        <translation>地面与一个大球均使用基于 Perlin 湍流的程序化大理石纹理,直接根据世界空间位置计算并输入 lambertian 材质——不涉及任何图像纹理。另外两个较小的球体以更高的噪声频率复用同一技术作为对比,由一块通过下一事件估计(NEE)采样的 diffuse_light 矩形光源照亮。该场景测试的是 Perlin 噪声湍流/大理石输出能否在真实 NEE 光照(而非平坦环境光)下正确接入 BSDF 管线。</translation>
    </message>
    <message>
        ...
        <translation>五个矩形图元,各自通过不同的基向量呈现不同朝向(朝前、朝侧、朝上),每个都使用纯色 lambertian 材质——这里测试的是矩形可命中体(quad hittable)的平面相交与各朝向下法线/UV 的计算,而非着色的复杂度。房间中还悬浮着一块独立的 diffuse_light 矩形光源,作为场景中唯一的发光体,为下一事件估计提供了具体的重要性采样目标。该场景主要用于在真实面光源采样下,检验矩形几何体及其取决于朝向的法线计算是否正确。</translation>
    </message>
    <message>
        ...
        <translation>复用了大理石纹理场景中的 Perlin 噪声地面/球体设置,但真正的主题是发光:一个赋予 diffuse_light 材质(暖色调)的球体充当了弯曲、非平面的面光源发光体,与之并存的还有一块独立的冷色调 diffuse_light 矩形光源。让球形和矩形两种发光体共存于同一场景,是为了测试光源采样是否能在不同发光体几何形状下正确处理立体角采样,而不仅仅是常见的平面矩形情形——冷暖色温的区分也让人很容易看出哪盏光在哪里起作用。</translation>
    </message>
    <message>
        ...
        <translation>重建了标准的康奈尔盒墙壁,并配有自己那块超大的天花板 diffuse_light 光源,然后将常见的实心盒子替换为两个经过旋转/平移、包裹着 constant_medium 的盒子——这是一种低密度、带有自身色调的均匀各向同性散射参与介质,而非经典的单色烟雾。这是一个体积介质测试场景:在其他场景中被用作实心 lambertian 表面的盒子几何体,在这里则改为定义各向同性内散射体积的边界,用以检验介质采样与边界形状相交计算能否正确配合。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation>一排五个球体,依次呈现同一种金色调 rough_metal 材质(GGX/Trowbridge-Reitz 微表面分布,配合简单的 RGB 色调反射率,而非完整的复数折射率菲涅尔计算)在不同粗糙度下的表现。场景中只有一块地面和一盏面光源,画面中唯一变化的变量就是随着粗糙度增加,镜面高光是如何扩散、变暗的。</translation>
    </message>
    <message>
        ...
        <translation>使用与“粗糙金属球体”场景相同的 rough_metal GGX BxDF(拉丝铝盒、粗糙度不同的拉丝金球),但被放入了熟悉的康奈尔盒场景,而不是开放式的工作室布光,因此这种光泽微表面反射需要在真实的间接反弹光照和墙壁颜色渗透下依然保持正确,而不只是单一顶光下的表现。</translation>
    </message>
    <message>
        ...
        <translation>康奈尔盒中的玻璃球从完美电介质换成了 rough_dielectric——反射和透射两个波瓣都应用了 GGX 微表面模型,因此折射光会散射成磨砂般的模糊效果,而不是遵循斯涅耳定律的清晰光线,同时每个微表面上仍然遵循菲涅尔的反射/透射分配比例。</translation>
    </message>
    <message>
        ...
        <translation>抛光的金和铝表面使用 conductor 材质,基于实测的 eta/k 光谱数据、按通道计算真实的复数菲涅尔反射率,而非简单的 RGB 反照率近似——因此金属的颜色来自真实的反射率光谱,而不是美术人员手动挑选的色调。</translation>
    </message>
    <message>
        ...
        <translation>一个使用 coated_diffuse 材质的涂层漫反射球体与盒子:在 Lambertian 漫反射基底上叠加一层粗糙电介质涂层。这里考验的是分层 BSDF 的求解——光线可以在涂层上发生镜面反射,也可以透射进入并带上漫反射基底的颜色,两层之间还存在内部反射。</translation>
    </message>
    <message>
        ...
        <translation>一块使用 thin_dielectric 材质的竖直玻璃隔板,将康奈尔盒一分为二:这是零厚度玻璃,采用解析的多次反弹菲涅尔公式,而非真实的折射界面,因此透射光会直接穿过而不发生弯折。隔板被刻意倾斜放置,因为折射率为 1.5 的菲涅尔反射率只有在接近掠射角时才明显可见。</translation>
    </message>
    <message>
        ...
        <translation>通过 coated_conductor 呈现漆面金属效果:一个金球和一个铜盒,均是在按通道计算复数菲涅尔的 GGX 导体(conductor)基底上,叠加一层粗糙电介质涂层。这是该类别中层叠最多的 BxDF——它把光谱化的金属反射率置于一层无色光泽涂层之下,呈现出涂层的菲涅尔光泽叠加在金属自身色调之上的效果。</translation>
    </message>
    <message>
        ...
        <translation>一个使用 diffuse_transmission 材质的蜡质球体,反射率与透射率分别设置为不同颜色——光线会同时向同侧半球(反射)和对侧半球(透射)发生漫散射,这是一种廉价的双参数半透明近似方案,无需体积随机游走,与“次表面平板”场景为达到同样视觉目标所采用的方法不同。</translation>
    </message>
    <message>
        ...
        <translation>一个使用 normalized_fresnel 材质的水晶球体——一种菲涅尔加权的漫射出射模型,其 BSDF 值在接近掠射角时升高(此时菲涅尔透射率较低,因此有更多内部散射的光得以逸出)。采样方式仍是余弦半球采样,但散射 PDF 本身携带了菲涅尔加权,并经过归一化处理,以确保在 MIS(多重重要性采样)下总反射能量依然正确。</translation>
    </message>
    <message>
        ...
        <translation>一排七个球体,逐一扫过 principled(原则化)材质的参数空间:哑光漫反射、低粗糙度塑料、带清漆层的塑料、半金属、粗糙近金属、光滑全金属,以及带清漆层的全金属——分别独立改变金属度、粗糙度和清漆强度,展示单一的统一 BSDF 如何跨越从电介质到导体的整个范围,而这个范围在本类别的其他场景中是由多种独立材质分别覆盖的。</translation>
    </message>
    <message>
        ...
        <translation>五个使用 hair 材质(HairBxDF)的球体,各自具有不同的吸收/粗糙度参数,针对的是一种强烈偏向前向/镜面散射的纤维 BSDF,而非漫反射式的 BSDF——由于头发 BSDF 的峰值响应在正常面光源强度下会过曝,这里的光照强度不得不比该系列其他场景暗得多。该材质被应用在球体表面而非真正的曲线几何体上,以便将 BxDF 的着色行为与曲线相交计算机制分离开来(曲线相交部分见“曲线纤维”场景)。</translation>
    </message>
    <message>
        ...
        <translation>在纯 Lambertian 表面上应用了两种扰动技术:凹凸贴图(bump mapping)根据 Perlin 大理石噪声纹理偏移着色法线(后墙与盒子),而法线贴图(normal mapping)则根据棋盘格图案扰动法线(球体)——几何表面本身仍是平面/球面,但用于光照计算的着色法线被扭曲,从而伪造出精细的表面细节。</translation>
    </message>
    <message>
        ...
        <translation>这并非真正的 BSSRDF(次表面散射反射分布函数)——进入乳白蜡质平板和玉石球体的光,会先穿过一层电介质外壳,进入一个 constant_medium(具有自身密度与色调的均匀参与介质),因此这种半透明的辉光效果来自物体内部真实的体积随机游走,其边界由表面的折射菲涅尔效应界定,而不是基于扩散模型的次表面项。</translation>
    </message>
    <message>
        ...
        <translation>该场景考验的是 pbrt-v4 的实测 BRDF 数据管线——一排五个球体使用的是合成生成的表格化 NDF、sigma、VNDF、亮度以及按波长的光谱表(而非真实采集的材质数据)。BSDF 的实际求值被简化为带纯色调的余弦半球采样,而非完整的五维重要性采样映射链,因此该场景更多是在测试表格化数据的接入管线,而非追求逼真的实测材质渲染效果。</translation>
    </message>
    <message>
        ...
        <translation>一个真正的三棱玻璃棱镜,使用色散电介质材质——其折射率依据柯西方程随波长变化,与真实的冕牌玻璃一致——而非固定折射率。白光进入棱镜后,出射时会按波长分开,像真实的玻璃棱镜一样在承接屏上散开成扇形,但这一效果只有在 --spectral(它让渲染器按波长逐一追踪光线)开启时才会出现;若不开启,每个 RGB 通道只是按各自固定且略有差异的折射率折射,而不会形成连续的色散分布。</translation>
    </message>
    <message>
        ...
        <translation>与 B23 相同的色散棱镜场景,但改用磨砂玻璃(rough_dielectric)而非光滑玻璃。在 --spectral 下,承接屏幕上会出现同样的彩虹扇形,只是因表面粗糙度而变得模糊,不再锐利——其背后的物理原理(折射率随波长变化)完全相同,只是叠加了微表面散射的差异。要让磨砂玻璃正确产生色散,不能只是简单地对 B23 换皮:与光滑玻璃不同,粗糙表面需要通过显式的下一事件估计才能采样到该场景中偏轴的定向光源,因此采样过程本身也必须具备波长感知能力,而不仅仅是初始折射。</translation>
    </message>
    <message>
        ...
        <translation>pbrt 的“mix”材质会在每个着色点上对两种材质进行混合,采用真正的逐样本随机选择——每次命中都按概率加权做出选择,而不是简单地混合颜色——因此哑光漫反射与金属导体的混合会呈现出真实的斑驳颗粒感:部分样本落在纯漫反射上,部分落在纯镜面反射上,经过大量样本后正确地平均出结果。该场景在 CPU、GPU 递归式和 GPU 波前式后端上表现一致。</translation>
    </message>
    <message>
        ...
        <translation>这个场景集中了本应用其他地方都没有一起出现过的四种材质:thindielectric(零厚度玻璃片,会发生折射但不会使光线产生位移,不同于普通电介质)、coatedconductor(金属基底上覆盖一层透明涂层)、diffusetransmission(光既发生反射也会透射过去,类似薄叶片或纸张的效果),以及通过命名预设(“Marble”,大理石)实现的次表面散射——无需外部实测数据文件即可获得真实的次表面散射参数。</translation>
    </message>
    <message>
        ...
        <translation>pbrt 的 CoatedDiffuse 材质(在透明电介质涂层下是一个漫反射基底,类似上过漆的木材),其反射率从真实的图像纹理中读取,而非使用纯色——这种组合过去在两个后端上都会悄悄忽略纹理,回退为纯灰色。</translation>
    </message>
    <message>
        ...
        <translation>一个由显式 RGB eta/k(赋予金属带色调、随波长变化反射率的复数折射率)驱动的 conductor 材质,而非使用命名预设——同时,coatedconductor 的金属基底层也应用了相同的复数折射率计算,从而产生准确的彩色镜面高光,而不是单一的镜面色调。</translation>
    </message>
    <message>
        ...
        <translation>DiffuseTransmission(光既发生漫反射也发生漫透射,是薄叶片或灯罩之类物体的模型)材质,其反射率和透射率分别绑定到真实纹理,而非纯色,在两个后端上均是如此。</translation>
    </message>
    <message>
        ...
        <translation>pbrt 的“hair”材质——Marschner/Chiang 纤维散射模型,与真实电影级生产渲染器所使用的基于物理的头发 BSDF 相同——在这里被应用到普通球体上,以便与本项目自带的“毛发纤维”演示场景进行受控的并排对比。</translation>
    </message>
    <message>
        ...
        <translation>一张 pbrt 棋盘格纹理,其两种颜色本身各自绑定到一张真实图像(即在棋盘格/mix 纹理内部再嵌套一层 imagemap 纹理),而不只是简单的纯色——测试纹理引用是否可以嵌套组合,而不仅仅是独立使用。</translation>
    </message>
    <message>
        ...
        <translation>pbrt 的 NamedMaterial 被直接引用到某个形状上(只声明一次、按名称复用,而不只是作为“mix”材质的子成分出现),并且该材质的其中一个参数绑定了纹理,同时场景中还有一个设置了 twosided 标志的 AreaLightSource,使其两面都能发光。</translation>
    </message>
    <message>
        ...
        <translation>七个球体，分别对应一个命名玻璃折射率(IOR)预设(BK7、BAF10、FK51A、LASF9、F5、F10、F11)，通过 FindGlassPreset() 在渲染的参考波长下解析为固定的电介质折射率——这与真实光学玻璃目录所使用的查找表完全相同，此前只能从单元测试中触达，在这个场景之前从未被真正渲染过。</translation>
    </message>
    <message>
        ...
        <translation>四个四边形，使用相同的源图像和 UV 缩放，各自单独测试一个 imagemap 纹理选项：默认的 sRGB 解码加环绕重复(wrap-repeat)作为参考情形，一个线性解码四边形读取相同的字节数据但显得更亮(跳过了通常会压缩中间调的伽马曲线)，一个钳制环绕(clamp-wrap)四边形在超出 [0,1] 范围时保持边缘纹素而不是平铺，以及一个反转通道的四边形——这些路径在其他任何内置场景的默认纹理设置下都无法触达。</translation>
    </message>
    <message>
        ...
        <translation>四个四边形，从左到右分别由一个不同的 pbrt-v4 程序化纹理类驱动——这些类都已接入构建器，但此前从未被任何内置场景使用过：windy(无参数湍流)、wrinkled(可调节倍频/粗糙度的 Perlin 湍流)、dots(由 Perlin 噪声边界分隔的两种纯色)，以及 bilerp(四个角的颜色在 UV 上平滑过渡混合)。</translation>
    </message>
    <message>
        ...
        <translation>两个四边形对比了嵌套两层的纹理(外层棋盘格自身的 tex1 是另一个棋盘格，而后者的 tex1 才是真正的图像)与旁边同时呈现的更简单的单层情形。在 CPU 上是真实计算的——同一张照片的两种不同棋盘格频率确实相互嵌套平铺；GPU 则有意将整棵嵌套树展平为单一平均颜色并给出警告，这是一种有文档记录的近似处理而非缺陷，因此用两种后端分别渲染这个场景会显示出真实的差异。</translation>
    </message>
    <message>
        ...
        <translation>一个开放场景,完全由程序化生成的 HDR 天空渐变(蓝色天顶渐变为暖色地平线)照亮,该渐变以基于图像的无限光形式接入,而非任何局部发光体。场景中放置了一个漫反射球体、一个近乎镜面的金属球体和一个电介质球体,用以展示这种环境光照在不同 BSDF 下的表现,从类似环境光遮蔽的柔和阴影,到清晰反射/折射出的天空渐变副本。</translation>
    </message>
    <message>
        ...
        <translation>光秃秃的康奈尔盒墙壁,完全由一盏聚光灯式的点光源照亮,该光源从天花板正对向下照射,总锥角较宽,内层衰减起始角较窄,因而形成柔和的半影,而非边缘生硬的光斑。与使用面光源的康奈尔盒场景不同,这是一个没有表面可供采样的 delta 光源——锥形与衰减正是这个场景要展示的重点。</translation>
    </message>
    <message>
        ...
        <translation>光秃秃的康奈尔盒墙壁,由一盏平行、类似太阳光的远光源(distant light)照亮:它是一个固定方向向量,没有 1/r² 衰减,因此辐亮度的比例尺直接就是入射辐照度,而不是为距离做补偿的强度值。这是一个刻意设计的对照案例,用于验证点光源管线在处理这种方向性、不衰减的光源时,能否与会衰减的点光源/聚光灯/goniometric 光源一样正确工作。</translation>
    </message>
    <message>
        ...
        <translation>康奈尔盒墙壁由头顶上方一盏各向同性的点光源照亮,具有经典的 1/r² 强度衰减——这是该类别中最简单的点光源:没有锥形限制、没有方向性分布、没有平行光线,只是从天花板附近一个位置发出、随距离衰减的光。</translation>
    </message>
    <message>
        ...
        <translation>康奈尔盒墙壁由一盏 goniometric(测光)光源照亮:这是一种点光源,其发光强度按方向变化,依据一张表格化图像而非均匀发光,同时叠加与普通点光源相同的 1/r² 衰减,并显式设置了旋转以在世界空间中调整该强度分布的朝向。这展示的是任意方向强度分布的情形,与聚光灯简单的锥形分布或点光源的均匀发光形成对比。</translation>
    </message>
    <message>
        ...
        <translation>康奈尔盒墙壁由一盏投影光源(projection light)照亮:这是一种点光源,通过透视视锥投射出一张棋盘格幻灯片图像,像真实的幻灯投影仪一样在场景上投出清晰的图案光束,而不是简单地发出锥形或均匀球形的光。</translation>
    </message>
    <message>
        ...
        <translation>一个康奈尔盒风格的房间,天花板上根本没有矩形光源——取而代之的是后墙上开出的一个真实矩形洞口,天空光只有透过这扇“窗户”才可见。该场景考验的是通过“传送门”(portal)采样的无限光照:环境光的采样被限制在透过传送门孔径实际可见的方向范围内,而非整个球面方向,这是无限光下一事件估计中更困难、更容易出错的情形。</translation>
    </message>
    <message>
        ...
        <translation>该场景集齐了 pbrt-v4 全部五种点光源(delta 分布)类型:点光源(point)、聚光灯(spot)、远光源(distant)、goniometric 光源,以及投影光源(projection)——是一个用于并排比较每种零面积光源类型的单一参考场景。</translation>
    </message>
    <message>
        ...
        <translation>pbrt 的 goniometric 光源和投影光源现在具备真正的图像解码功能——此前它们曾悄悄忽略自身的 filename 参数,回退为均匀光束;这个场景就是用来验证它们现在确实会读取并应用图像的回归测试。</translation>
    </message>
    <message>
        ...
        <translation>pbrt 的“blackbody L”光源颜色,以开尔文温度而非 RGB 三元组来指定,并通过真实的普朗克轨迹黑体转 RGB 数学模型进行换算。两块除色温外完全相同的面板并排放置,一块设为暖色、类似白炽灯的色温,另一块设为冷色、类似阴天天空的色温,一旦颜色计算出现回归错误,两块面板都会立刻变成平淡的纯白色,非常容易察觉。</translation>
    </message>
    <message>
        ...
        <translation>在非三角形形状(球体或矩形)上设置的 AreaLightSource,既通过 filename 使用图像纹理(其发光图案来自图像,而非纯色),又设置了 twosided(两面都发光,而非仅朝表面法线方向的那一面)——两个后端上均是如此。</translation>
    </message>
    <message>
        ...
        <translation>pbrt 的“infinite”(无限)光源——一种没有任何形状的恒定颜色光源,同时从各个方向照亮场景,是天空最简单的替代方案。需要开放式的几何结构才能看到效果,因为封闭的房间无论如何都会从各个方向挡住它。</translation>
    </message>
    <message>
        ...
        <translation>圆盘与圆柱形状作为真正的下一事件估计(NEE)光源使用——每次反弹都朝光源方向进行立体角采样,而不是单纯依靠光线偶然命中——在两个 GPU 后端上都能像 CPU 路径一样干净地收敛,而不是像非 NEE 光源那样,靠“碰运气让光线命中”的方式收敛,噪点会更多。</translation>
    </message>
    <message>
        ...
        <translation>一个场景中包含两个独立的球形面光源——这是一个最小化的用例,曾经用来定位一个真实的 GPU bug:光源类型查找表当时只按一盏光源的大小分配,导致第一盏之后的每一盏光源都悄悄读错了自己的类型,渲染结果因此出错。</translation>
    </message>
    <message>
        ...
        <translation>一个形状为不规则五三角形扇面的面光源——它不是一个简单的平行四边形,也无法被渲染器的“矩形合并”优化(该优化会把两个三角形重新拼合为一个矩形以降低采样成本)重新拼合——因此这个场景考验的是 GPU 上真正的逐三角形光源采样,而非更常见的矩形捷径。</translation>
    </message>
    <message>
        ...
        <translation>pbrt 的 &quot;ColorSpace&quot; 指令,用于选择&quot;blackbody&quot;色温所转换到的工作 RGB 色彩空间(此处使用色域更宽的 Rec.2020,而非默认的 sRGB)。与 Blackbody Light 示例相同的几何结构、相同的 2500K/9000K 色温——唯一的区别在于所声明的色彩空间——因此这两个场景的面板颜色可以直接并排比较,从而证明该指令确实被应用,而不只是被解析后忽略。</translation>
    </message>
    <message>
        ...
        <translation>真正的 pbrt-v4 PortalImageInfiniteLight：一个等面积环境贴图无限光，被限制在一扇窗口四边形之内，因此只有那扇开口能看到真实的天空细节，而不是让整个环境光从四面八方透进来。原生的“传送门无限光”场景(C7)只是在一面墙上、一个平坦 sky_light 背后开出一个几何洞口，从未真正构建过这个类——这是第一个真正做到这一点的内置场景。</translation>
    </message>
    <message>
        ...
        <translation>三个原本完全相同的球体，分别只由一种光源类型照亮——点光源、聚光灯和面光源——各自以 &quot;float power&quot;(以瓦特为单位的总光通量)而非通常直接给出的强度/辐亮度来指定亮度。渲染器必须针对每种光源类型，通过不同的立体角公式将功率反向换算为强度(点光源为 4*pi，聚光灯为其锥体立体角，朗伯发光体的面光源为 area*pi)——因此任何一种类型换算错误，都会表现为该球体亮度明显不匹配，而不是整个场景均匀出错。</translation>
    </message>
    <message>
        ...
        <translation>一张故意做成非正方形(8x4，而非其他投影光场景通常使用的正方形 uv-checker.bmp)的投影光幻灯片图像，是第一个真正考验图像与光源自身视场之间宽高比不匹配情形的内置场景。正确保持宽高比的投影会投射出一个明显偏宽的矩形光斑，与图像本身 2:1 的形状相符；若这里出现问题，光斑就会被压缩成正方形，或被拉伸以填满光源的视场。</translation>
    </message>
    <message>
        ...
        <translation>三盏饱和、近距离的彩色光源照射在一个纯漫反射表面上，其摆放位置使重叠区域恰好落在 --spectral 渲染下 sRGB 色域边界附近。这正是此前真实存在的一个逐样本 XYZ 到 RGB 色域钳制缺陷会导致错误变暗、褪色的确切条件——修复方案将该转换推迟到逐像素而非逐样本进行，因此这里的重叠区域应该平滑地过渡到白色/粉彩色，而不会出现色带或浑浊的欠饱和色斑。</translation>
    </message>
    <message>
        ...
        <translation>一个开放场景(棋盘格地面加上位于不同深度的若干球体,一个玻璃、一个金属),使用默认透视相机内置的薄透镜离焦模糊渲染,因此只有处于对焦距离上的球体是清晰的,而近处和远处的球体则会模糊——这是最直接的“光圈加对焦距离”景深路径,不涉及其他相机类别。</translation>
    </message>
    <message>
        ...
        <translation>一排位于棋盘格地面上方的球体,通过显式的正交(orthographic)相机渲染。光线以平行方式投射,而非从单一视点汇聚发出,因此球体不会出现透视缩短效果——等大的球体无论深度如何都保持等大,这正是平行投影最典型的视觉特征。</translation>
    </message>
    <message>
        ...
        <translation>围绕一个中心发光球体排列的一圈彩色球体,由显式的球面(等距圆柱投影)相机拍摄,因此整个 360 度周边环境都被映射进一张全景图像,而非有限视场角的视锥。</translation>
    </message>
    <message>
        ...
        <translation>五个深度依次递增的球体位于棋盘格地面上方,通过显式的“真实”(realistic)相机渲染,该相机基于真实的多镜片处方(每个镜片表面的曲率、厚度、折射率与光圈)构建,而非理想化的薄透镜模型——光线会逐个镜片元件地穿过整套镜片组进行追踪,产生真实的镜头虚化(bokeh)与暗角效果,而不是闭式解的离焦圆盘近似。</translation>
    </message>
    <message>
        ...
        <translation>使用与普通康奈尔盒场景以及其他相机对比康奈尔盒场景相同的经典几何结构,采用默认透视相机的薄透镜离焦模糊渲染,而非其他相机类别。这样一来,在与其他相机对比场景相同的房间中,景深就成为唯一的变量,使该技术的效果可以直接对比,而不会与不同的几何结构混淆。</translation>
    </message>
    <message>
        ...
        <translation>与其他相机对比场景共用同一个康奈尔盒,通过显式的正交相机渲染,视角沿盒子的深度轴正对拍摄。由于是平行投影而非透视投影,盒子的边缘会一直保持平行延伸到画面边缘,而不会像透视/景深版本那样发生汇聚——特意使用同一个房间,是为了让这种对比清晰可辨。</translation>
    </message>
    <message>
        ...
        <translation>同一个共享的康奈尔盒,由放置在盒子中心的显式球面(等距圆柱投影)相机环视拍摄。全景图从一个内部视点出发,将全部五面墙壁、天花板光源和玻璃球都包裹进一张 360 度图像中——这与独立的“球面相机”场景采用的是同一种技术,只是这里应用到了共享的对比几何结构上。</translation>
    </message>
    <message>
        ...
        <translation>同一个共享的康奈尔盒,使用与独立“真实相机”场景完全相同的多镜片镜头处方进行渲染,但光圈按比例放大以适配盒子大得多的尺度——这样即便场景尺度不同,离焦锥的角度大小、进而可见的镜头虚化效果仍能保持可比性。</translation>
    </message>
    <message>
        ...
        <translation>与之前相同的康奈尔盒场景,但相机本身会在曝光期间移动——通过真正的 AnimatedTransform 插值(pbrt-v4 的运动模糊技术)在两侧位置之间设置关键帧,每条光线都在快门时间区间内采样各自独立的时间点,而非使用单一固定相机位置。仅在默认路径追踪器与 SPPM 下生效(BDPT/MLT/调试积分器会改为渲染一帧静止的首关键帧画面)。GPU 同样只渲染首关键帧的静止画面。</translation>
    </message>
    <message>
        ...
        <translation>透视相机的薄透镜景深效果——镜头半径与对焦距离——直接从 pbrt 文件的 Camera 指令中加载,两个后端上均是如此。这与本项目自带的“景深”场景使用的是同一种光学模型,只是这里考验的是文件加载路径。</translation>
    </message>
    <message>
        ...
        <translation>从文件中加载的 pbrt 正交(平行投影)相机——所有光线都是平行的,而不是汇聚于一个视点,因此两个大小相同、深度不同的球体在屏幕上呈现出相同的大小,而不是近处的显得更大,这与普通透视缩短效果正好相反。</translation>
    </message>
    <message>
        ...
        <translation>从文件中加载的 pbrt 球面(等面积)相机——光线从单一点向各个方向发散,而不是穿过一个平面成像平面,因此一次渲染就能捕捉完整的 360 度环绕视野。该相机被放置在一个封闭房间内,以确保各个方向都确实有可看的内容。</translation>
    </message>
    <message>
        ...
        <translation>pbrt 的“真实”(realistic)相机——一套真正的多镜片系统,而非其他相机场景所使用的理想化薄透镜/针孔模型——从镜头处方文件中加载,让光线穿过实际的玻璃镜片元件,以获得真实的景深和镜头像差,其中也包含了内置编译场景从未测试过的镜头文件加载路径。</translation>
    </message>
    <message>
        ...
        <translation>在康奈尔盒内填充了一种均匀参与介质——一个具有统一散射系数和 Henyey-Greenstein 相函数的边界盒。边界内每一点的密度都相同,因此这个场景测试的是基础的自由程采样/单一散射系数介质路径,而非任何空间变化的密度模型。</translation>
    </message>
    <message>
        ...
        <translation>一个真正的异质体积,其密度是逐点通过多倍频 Perlin 噪声、叠加丝缕状扰动和高度衰减程序化计算出来的,而不是查表读取预先烘焙的体素数据。渲染使用 delta 追踪(空碰撞采样)来处理这种非均匀密度场,呈现出如同真实云朵一样底部更浓密、顶部逐渐变薄的效果。</translation>
    </message>
    <message>
        ...
        <translation>把“电介质加内部介质”这一组合作为独立主题展示:多个玻璃球,每个都由一个折射性电介质表面构成,内部填充着密度各不相同的均匀参与介质。该场景考验的是散射介质由真正的折射表面(而非不透明外壳)界定的情形,因此光线必须先折射进入,穿过内部雾气发生散射/吸收,再折射出来。</translation>
    </message>
    <message>
        ...
        <translation>一种异质介质,其真实的逐体素散射数据被烘焙进一个三维网格,红/绿/蓝三个通道以不同频率独立生成,使颜色在空间上互不相关,而不是呈现为均匀染色的云雾——采样时使用了“主导网格”(majorant grid)加速的 delta 追踪。与逐点解析计算的程序化介质不同,这里是离散的体素数据,从渲染空间的盒子映射到网格自身的坐标空间中。</translation>
    </message>
    <message>
        ...
        <translation>pbrt 的 MakeNamedMedium “cloud”——一种异质散射体积,其密度来自三维 Perlin 噪声,这与真实云朵渲染所采用的噪声驱动方法相同,用以避免呈现出一块平淡、明显假的雾块——从文件中加载,两个后端上均支持。</translation>
    </message>
    <message>
        ...
        <translation>一种均匀参与介质(统一的雾气/散射密度)附加在一个圆柱形状上——这种组合过去在 GPU 后端上会悄悄渲染成普通的空几何体,完全没有雾气效果;这个场景就是用来验证介质效果现在在 GPU 上也真实生效的回归测试。</translation>
    </message>
    <message>
        ...
        <translation>pbrt 的 MakeNamedMedium “rgbgrid”——一个三维体素网格,其中每个体素单元都带有自己的 RGB 散射颜色,而不仅仅是一个密度标量——渲染效果呈现为柔和的、带彩色的星云状体积,两个后端上均是如此。</translation>
    </message>
    <message>
        ...
        <translation>pbrt 的 MakeNamedMedium “uniformgrid”——一个单通道密度体素网格,不像 RGB 网格介质场景那样带有逐体素颜色——渲染效果呈现为一团柔和发光的团状物,是那个场景更简单的姊妹版本。</translation>
    </message>
    <message>
        ...
        <translation>pbrt 的 MakeNamedMedium “nanovdb”：从外部 .nvdb 文件读取的真正 NanoVDB 格式稀疏密度网格，渲染为一个柔和的雾状体积球体——这是本项目所填补的单项最大 pbrt-v4 缺口，因为稀疏体素数据所需的查找方式与其他介质场景所用的致密网格截然不同。仅限 CPU；GPU 完全没有实现 NanoVDB 查找，而是回退为均匀的平坦雾效，这是一种有文档记录的近似处理而非缺陷。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation>测试双线性面片(bilinear patch)图元——这是一种真正的四角弯曲曲面,有自己专属的相交计算方法,而不是由一对三角形拼成——具体做法是放置两个面片,通过让四角高度错开,分别形成一个双曲抛物面鞍形和一个弯曲坡面。这里刻意使用了带有轻微粗糙度的金属材质,以便着色渐变能够描绘出曲面的连续曲率——如果用镜面材质,则只会显示单一的清晰高光,掩盖了曲面并非平面这一事实。</translation>
    </message>
    <message>
        ...
        <translation>构建了一个程序化生成的正二十面体,作为真正的带索引三角网格,考验的是渲染器中每个加载网格所使用的“无缝”(watertight)光线-三角形相交路径。该网格没有提供逐顶点法线,因此每个三角形会退化为其平坦的逐面几何法线,呈现出多面体式的低多边形观感,而非平滑着色的插值法线效果。</translation>
    </message>
    <message>
        ...
        <translation>该场景考验的是真正的曲线几何体及其专属的光线-曲线相交测试,不同于另一个“毛发纤维”场景那样,只是用 hair BSDF 给普通球体着色,而没有真正让表面本身弯曲。带有从根部到梢部渐细效果的贝塞尔发丝以散点方式扎根在表面上,因此这个场景测试的是曲线图元真实的扫掠带状几何,而非头发着色模型本身。</translation>
    </message>
    <message>
        ...
        <translation>pbrt 的 Shape “plymesh” 携带的真实逐顶点 UV 数据,已贯通两个后端——此前 GPU 递归式后端会把这个场景渲染成一片纯黑,原因就是这些 UV 数据被悄悄丢弃了。</translation>
    </message>
    <message>
        ...
        <translation>pbrt 的 Shape “plymesh” 完整加载一个真实的外部 .ply 网格文件,包括对源文件中一个尚未三角化的面进行扇形三角化处理——加载器不能假定每个面都已预先三角化。</translation>
    </message>
    <message>
        ...
        <translation>pbrt 的 Shape “curve”——真正的三次贝塞尔纤维几何体,在底层被镶嵌为适合 GPU 处理的三角形,而不是任何三角网格近似——与本项目自带的、以同样方式构建的“曲线毛簇”演示场景进行了直接对比。</translation>
    </message>
    <message>
        ...
        <translation>这是本项目中首次将真正的曲线几何体与 Material “hair” 材质结合使用——正是这种组合当初暴露出一个 bug:头发 BSDF 使用的是平面法线,而非纤维自身的切线方向,而切线方向正是让每根发丝上的高光看起来像“一根发丝”、而不是一团哑光绒毛的关键所在。</translation>
    </message>
    <message>
        ...
        <translation>pbrt 的 Shape “trianglemesh” 的 “point2 uv” 参数——三角网格上显式的逐顶点纹理坐标——已贯通两个后端;在这条数据通路建立之前,GPU 递归式后端会把这个场景渲染成一片纯黑。</translation>
    </message>
    <message>
        ...
        <translation>pbrt 的 PixelFilter 指令选用了盒式滤波器(box filter)——像素内的每个样本被等权重计入,在像素边界处硬性截断——而非更平滑的默认高斯滤波器。可将此处更明显锯齿化、边缘更生硬的轮廓,与其他任何使用默认滤波的场景渲染结果进行对比。</translation>
    </message>
    <message>
        ...
        <translation>一个球体的 ActiveTransform StartTime/EndTime 区间，为同一个形状烘焙进两个不同的世界空间中心，由相机自身的快门区间逐条光线采样——运动中的球体应当渲染成一道柔和的方向性拖影，而不是清晰的单一副本或重影，与旁边完全没有区间设置的静态参考球体形成对比。在全部三个后端(CPU、GPU 递归式、GPU Wavefront)上都是真实实现的，这一点不同于下面的圆盘/圆柱情形。</translation>
    </message>
    <message>
        ...
        <translation>与“物体运动模糊”场景相同的 ActiveTransform 运动模糊技术，但应用到了 Shape &quot;disk&quot;/&quot;cylinder&quot; 而非球体——这在内部走的是一条不同的代码路径(烘焙进形状自身的起止变换，而不是球体的 center1/center2 一对参数)。仅限 CPU：两个 GPU 后端都会将这些形状固定渲染在其起始姿态上并给出警告，而不会产生模糊效果，这一点不同于 GPU 确实支持的球体运动模糊。</translation>
    </message>
    <message>
        ...
        <translation>两个四边形共享完全相同的点数组和索引绕序，这本身会产生一个背对相机的法线——以这种绕序构成的单面面光源从正面是不可见的。紧接在第二个四边形之前发出的裸 ReverseOrientation 指令，会将其有效法线翻转为朝向相机，因此一个四边形保持暗淡，另一个则被点亮；ReverseOrientation 中的缺陷会表现为两个四边形都暗淡或都被点亮，而绝不会是恰好一明一暗。</translation>
    </message>
    <message>
        ...
        <translation>四个形状测试本项目自有的 pbrt-v3 兼容性扩展(Shape &quot;cone&quot;/&quot;paraboloid&quot;，并非真正 pbrt-v4 的一部分)：一个普通漫反射圆锥和抛物面作为基础形状，一个用作 AreaLightSource 发光体(而非普通材质)的圆锥，以及一个用作 MediumInterface 雾边界的抛物面，其开口边缘朝向相机，使光线从开口端而非实体表面进入介质。仅限 CPU——GPU(两个后端)会发出警告并完全丢弃任何圆锥/抛物面形状。</translation>
    </message>
    <message>
        ...
        <translation>经典的 pbrt-v4 “killeroo” 雕像示例场景——这是一个完全从自己的 .pbrt 文件端到端加载的场景(几何体、材质、光源、相机全部由文件驱动),而非直接编译进本渲染器,使用的是与用户自己的自定义 pbrt 场景相同的文件格式和加载路径。</translation>
    </message>
    <message>
        ...
        <translation>与 A1 相同的康奈尔盒场景,但故意以较低的每像素 16 次采样(而非通常的 100 次)进行渲染——采样数过少,难以干净收敛。在渲染选项标签页切换采样器(Sobol、Z-Sobol、Padded Sobol、Stratified、PMJ02BN、Halton)后重新渲染:每种选择都是在像素与光源上分布这 16 个样本的不同低差异序列,因此即便场景其他部分完全不变,柔和阴影半影区中的噪点/聚簇模式也会随之改变。仅对 CPU 默认路径追踪器生效——采样器控件对 GPU 没有任何影响。</translation>
    </message>
    <message>
        ...
        <translation>与 B23(光谱色散)相同的玻璃棱镜,在此重新用作&quot;光谱渲染&quot;复选框自身的演示场景。关闭光谱渲染(渲染选项标签页)时,渲染器只跟踪红/绿/蓝三通道,每种波长在玻璃中的弯折角度都完全相同;开启后,渲染器会跟踪每条光线真实的波长,棱镜的折射率会随波长真正发生变化,将白光散开成一道可见光谱,与真实棱镜的效果一致。仅对 CPU 默认路径追踪器生效。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation>与 B3(康奈尔粗糙玻璃)相同的磨砂玻璃康奈尔盒，重新设计为“积分器”下拉列表自身的 SPPM 演示。以相同的采样数分别用默认路径追踪器和 SPPM(渲染选项标签页)各渲染一次：粗糙电介质球体下方地板上的焦散——正是普通路径追踪难以收敛的、高频率的间接光照困难情形——在 SPPM 的光子映射方法下应当明显更快地变干净，因为这种方法正是为处理这类光传输而设计的。</translation>
    </message>
    <message>
        ...
        <translation>与 A1 相同的康奈尔盒，重新设计为“积分器”下拉列表自身的 BDPT/MLT 演示。尝试使用 BDPT 或 MLT(渲染选项标签页)而非默认路径追踪器：两者都会同时从相机和光源追踪光路并在中途将它们连接起来，而不是像默认积分器和 SPPM 那样只从相机追踪——这是一种根本不同的策略，在这类具有明显间接光照的场景上可能收敛得不同(有时更好，有时呈现不同的噪点特征)。</translation>
    </message>
    <message>
        ...
        <translation>与 A1 相同的康奈尔盒，重新设计为“积分器”下拉列表参考/调试模式的巡览。RandomWalk 在半球上均匀采样下一个反弹方向，完全不进行光源采样，也不使用 MIS——是收敛最慢但偏差最小的基准。SimplePath 为这两个要素各自添加了独立的复选框：仅开启“采样光源”(NEE)时，直接光照获取成本低，但朝向明亮但偏轴光源的反弹仍然依赖运气；仅开启“采样 BSDF”时则相反；两者都开启时，即为将两者结合的多重重要性采样(MIS)——这也正是默认路径追踪器已经在做的事情。以相同的低采样数分别渲染，观察它们在相同几何体上收敛方式的差异。</translation>
    </message>
    <message>
        ...
        <translation>专为本示例构建的康奈尔盒：天花板上有五盏灯，而不是 A1 的一盏，且刻意设置了悬殊的功率(约为 1:2:6:15:80，而不仅仅是尺寸不同)。Uniform(光源采样器，渲染选项标签页)会将每次下一事件估计采样中的五分之一均匀分配给每盏灯，而不考虑其实际对图像的贡献——因此大部分采样都花在了几乎无关紧要的四盏灯上，而那盏真正主导的灯反而保留了不必要的噪点。Power 和 BVH 都会将选择权重偏向那盏主导灯(BVH 还会考虑距离/方向，Power 仅考虑总发光通量)——以相同的低采样数渲染这三种方式，比较明亮角落那盏灯周围的噪点消退速度。</translation>
    </message>
    <message>
        ...
        <translation>与 A1 相同的康奈尔盒，重新设计为“积分器”下拉列表自身的 Ambient Occlusion(环境光遮蔽)演示。AO 刻意完全不是一个有光照的渲染：切换到该模式(渲染选项标签页)后，每个表面自身的颜色以及场景中的每一个真实光源都会被完全忽略，取而代之的是单一的灰度遮蔽项——即每个点上方的半球有多少被附近的几何体遮挡。旋转盒子自身的接触阴影以及两面墙相交的角落是仅剩的结构，这正是 AO 作为快速预览场景几何复杂度(与其光照隔离开来)的有用之处。</translation>
    </message>
    <message>
        ...
        <translation>与 B3 和 I5 相同的磨砂玻璃康奈尔盒——粗糙电介质球体的地板焦散是一个教科书式的硬焦散案例，这种光路(类镜面散射进入一个很小的立体角，然后落在漫反射地板上)在普通路径追踪下会产生孤立的明亮像素“萤火虫”噪点。Regularize 和萤火虫噪点截断(--maxcomponentvalue，均在渲染选项标签页)以两种不同的方式驯服它：Regularize 在第一次非镜面反弹之后加宽粗糙 BSDF 自身的粗糙度，使得焦散本身从一开始就不那么病态地尖锐、难以采样；而截断则允许萤火虫噪点发生，然后将任何最亮通道超过阈值的样本进行缩放，以损失一点能量为代价换取一个有保证的上限。先不启用任何选项渲染一次，然后分别单独启用每一项渲染，使用相同的采样数进行比较。</translation>
    </message>
    <message>
