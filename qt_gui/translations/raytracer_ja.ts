...
(すでにレンダリング中の場合はキューに追加されます)</translation>
    ...
    <message>
        ...
        <translation>&lt;h3&gt;Ray Tracer&lt;/h3&gt;&lt;p&gt;&lt;i&gt;Ray Tracing in One Weekend&lt;/i&gt;シリーズをベースに発展させた、CPUとGPU(OptiX)の並列バックエンドを備える物理ベースのパストレーサーです。pbrt-v4スタイルの機能セットを備えています。&lt;/p&gt;&lt;p&gt;149のシーン、豊富なBxDFライブラリ、複数のライトタイプとカメラタイプ、三角形メッシュとテクスチャのサポート、BVHによる高速化、ボリュームレンダリング、そして標準のパストレーシングに加えて選択可能な各種インテグレータ - SPPMフォトンマッピング、双方向パストレーシング(BDPT)、Metropolis Light Transport、および複数の参照用/デバッグ用インテグレータ(RandomWalk、Ambient Occlusion、SimplePath、SimpleVolPath、LightPath)を備えています。&lt;/p&gt;&lt;p&gt;このウィンドウはサブプロセスとして&lt;code&gt;%1&lt;/code&gt;を実行します。&lt;/p&gt;</translation>
    </message>
    <message>
        ...
        <translation type="vanished">&lt;h3&gt;Ray Tracer&lt;/h3&gt;&lt;p&gt;&lt;i&gt;Ray Tracing in One Weekend&lt;/i&gt;シリーズをベースに発展させた、CPUとGPU(OptiX)の並列バックエンドを備える物理ベースのパストレーサーです。pbrt-v4スタイルの機能セットを備えています。&lt;/p&gt;&lt;p&gt;123のシーン、豊富なBxDFライブラリ、複数のライトタイプとカメラタイプ、三角形メッシュとテクスチャのサポート、BVHによる高速化、ボリュームレンダリング、そして標準のパストレーシングに加えて選択可能な各種インテグレータ - SPPMフォトンマッピング、双方向パストレーシング(BDPT)、Metropolis Light Transport、および複数の参照用/デバッグ用インテグレータ(RandomWalk、Ambient Occlusion、SimplePath、SimpleVolPath、LightPath)を備えています。&lt;/p&gt;&lt;p&gt;このウィンドウはサブプロセスとして&lt;code&gt;%1&lt;/code&gt;を実行します。&lt;/p&gt;</translation>
    </message>
    <message>
...
Please check the render log for ffmpeg errors.</source>
        <translation>フレームは正常にレンダリングされました (%1ファイル) が、動画ファイルが作成されませんでした。
...
Please check the render log for errors.</source>
        <translation>フレームも動画ファイルも作成されませんでした。
...
The render may have failed to create output.</source>
        <translation>フレームディレクトリが見つかりません:
%1
...
Clear it (the small &quot;x&quot; inside the field) to see every scene in the current category again.</source>
        <translation type="vanished">下のシーン一覧/グリッドを、各シーンの名前・ID・説明のいずれかに対する部分一致で絞り込みます - 上にある利用可否タブやカテゴリタブの代わりではなく、それらに加えて適用されます。
...
This app ships with dozens of built-in scenes covering the basics (a simple Cornell box) up through complex conductor/dielectric materials, volumetric fog, and real photogrammetry-scale models - pick one to render, or browse by category using the tabs above.</source>
        <translation type="vanished">すべてのレンダリングはシーンから始まります。シーンとは、世界の中身 - ジオメトリ (形状やメッシュ)、マテリアル (表面の材質)、ライト、カメラ - を記述したものです。
...
scene not already cached. CPU-only, low resolution - takes a while the first time.</source>
        <translation type="vanished">キャッシュがまだない自己完結型のBasics/Materials/Textures/Camerasシーンについて、
...
Generate Video cannot be combined with an alternate Integrator - see the warning below if that combination is picked.</source>
        <translation type="vanished">このレンダリングが1枚の静止画になるか、複数フレームをつなげた動画になるかを選びます。
...
Generate Video renders a camera path frame by frame and assembles an MP4.</source>
        <translation type="vanished">「単一画像」は1フレームをレンダリングします。
...
including the handful the GPU backend does not implement.</source>
        <translation type="vanished">GPU: OptiXによるハードウェアレイトレーシング — 通常は桁違いに高速です。
CPU: 重点的サンプリングによるパストレーサー — GPUバックエンドが
...
GPU rendering is not available in this build.</source>
        <translation type="vanished">重点的サンプリングによるCPUパストレーサー — すべてのシーンとマテリアルに対応します。
...
GPU (OptiX) uses NVIDIA&apos;s dedicated ray-tracing cores to trace thousands of rays in parallel, typically far faster. CPU uses ordinary processor cores instead: much slower, but works on any machine and supports every material this app implements, including a couple the GPU path hasn&apos;t caught up to yet.</source>
        <translation type="vanished">どちらもまったく同じ光線を追跡し、同じ画像を生成します - 違うのは物理計算ではなく、速度とハードウェアです。
...
Only applies when Renderer is set to GPU.</source>
        <translation type="vanished">Recursive: 1ピクセルにつき1スレッドを使う、既定のGPUパストレーサーです — 対応範囲が広く、実績も豊富です。
Wavefront: 各バウンスをキュー渡しの個別カーネル起動に分割します — 複雑・発散的な
シーンでGPUの利用効率が高まりますが、新しく実績の少ないコードパスです。
...
Recursive traces one ray per thread from start to finish, bouncing recursively - simple and battle-tested. Wavefront instead groups all rays currently doing the same kind of work (e.g. &quot;just hit glass&quot;) into a batch and processes them together - better use of the GPU&apos;s parallel hardware on complex scenes with lots of different materials, at the cost of being a newer, less-tested code path.</source>
        <translation type="vanished">GPU上で同じレイトレーシング処理を行うための、2つの異なる方式です。
...
default Path Tracer - see each control&apos;s own tooltip.</source>
        <translation type="vanished">使用するレンダリングアルゴリズムを選択します。パストレーサー(デフォルト)は十分に検証された汎用の選択肢です - 下の代替インテグレータは、その汎用性の代わりに特定の技術(フォトンマッピング、双方向/Metropolis光輸送、あるいはいくつかの不偏な参照用・デバッグ用インテグレータ)を選びます。SPPMを除くすべての代替インテグレータはCPU専用であり、いずれも動画生成モードと組み合わせることはできません。
...
Hover any item in the dropdown for details on that specific integrator.</source>
        <translation type="vanished">レンダリング速度だけでなく、レンダリングアルゴリズムそのものを選びます。パストレーサー(デフォルト)は、このアプリの他の部分でも使われている汎用的で十分に検証された選択肢です。

SPPM(Stochastic Progressive Photon Mapping)は、通常のパストレーシングが苦手とする強いコースティクス/ガラスのシーンを扱えます。BDPTと、それを基に構築されたMLTは、カメラ側と光源側の両方から光路を追跡して接続します - 一部の難しい照明でより有効ですが、対応するのは面光源のみです。RandomWalk、Ambient Occlusion、SimplePath、SimpleVolPath、LightPathは参照用/デバッグ用のインテグレータです - よりシンプルで、しばしばノイズが多いか適用範囲が狭く(例えばAmbient Occlusionはそもそも照明されたレンダリングではありません)、特定の技術が何をもたらすかを切り分けるのに役立ちます。
...
Live Preview renders continuously with an orbitable camera - GPU only.</source>
        <translation type="vanished">「単一画像」は 1 フレームをレンダリングします。
...
「ライブプレビュー」は視点を回転できるカメラで連続的にレンダリングします - GPU 専用。</translation>
    ...
    <message>
        ...
        <translation type="vanished">NVIDIA OptiXによるハードウェアレイトレーシングです。CPUより桁違いに高速なのが一般的ですが、CUDA対応のNVIDIA GPUが必要で、CPU側が実装するすべてのマテリアルにはまだ対応していません。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">完全な重要度サンプリング付きパストレーサーです。あらゆるマシンで動作し、GPUバックエンドがまだ追いついていない一部のものを含め、本アプリが実装するすべてのシーンとマテリアルに対応していますが、その分はるかに低速です。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">1ピクセルにつき1スレッドを使い、各レイをバウンスごとに再帰的に追跡します。既定のGPUパストレーサーであり、シーンとマテリアルに対する対応範囲が広く、実績も豊富です。</translation>
    </message>
    <message>
...
Render time scales roughly linearly with samples per pixel.</source>
        <translation type="vanished">Samples per Pixel（サンプル数）/ Max Ray Depth（最大レイ深度）:
  Draft    25 spp,  depth 10
  Preview  50 spp,  depth 20
  Good    100 spp,  depth 50
  High    500 spp,  depth 50
  Ultra  1000 spp,  depth 100
  Maximum 5000 spp, depth 100
Customを選択すると「詳細パラメーター」タブの値はそのまま変更されません。
...
Clear it (the small &quot;x&quot; inside the field) to see every scene in the current category again.</source>
        <translation>入力した文字が名前・ID・説明文のいずれかに含まれるシーンだけに、下のリストやグリッドを絞り込みます。これは上部の利用可否タブやカテゴリータブによる絞り込みに加えて働くもので、その代わりにはなりません。
...
This app ships with dozens of built-in scenes, ranging from simple starter setups (a plain box-shaped room) up through scenes with realistic metal and glass, fog and smoke effects, and highly detailed 3D-scanned models - pick one to render, or browse by category using the tabs above.</source>
        <translation>レンダリングは必ず「シーン」から始まります。シーンとは、仮想世界の中身、つまり形やオブジェクト、それぞれの表面の素材、照明、そしてカメラをまとめて記述したものです。
...
Generate Video cannot be combined with an alternate rendering method (Integrator) - see the warning below if that combination is picked.</source>
        <translation>このレンダリングで、1枚の静止画を作るか、複数の画像をつなげた動画を作るか、GPUを使った対話的なライブプレビューにするかを選びます。

「静止画」は、このタブで設定したカメラから1回だけシーンをレンダリングします。「動画を生成」はカメラを経路に沿って動かしながら(設定はこのタブの下にある「動画生成設定」)1コマずつレンダリングし、最後につなげてMP4動画にします。所要時間はおおよそフレーム数倍になります。「ライブプレビュー」は固定の低い解像度で継続的にレンダリングし続けるので、ドラッグやスクロールでカメラを自由に動かしながら画像がリアルタイムで少しずつ鮮明になっていく様子を見られます。こちらはファイルを書き出しません。
...
Live Preview renders continuously with a camera you can freely orbit - GPU only.</source>
        <translation>静止画:1枚の画像をレンダリングします。
動画を生成:カメラを動かしながら1コマずつレンダリングし、MP4動画にまとめます。
...
including the handful the GPU option does not implement.</source>
        <translation>GPU:グラフィックカードのレイトレーシング用ハードウェアを使用します。通常は大幅に高速です。
...
GPU uses your NVIDIA graphics card&apos;s dedicated ray-tracing hardware to process thousands of light rays at once, so it&apos;s typically far faster. CPU uses your computer&apos;s regular processor instead: much slower, but works on any machine and supports every material this app implements, including a couple the GPU option hasn&apos;t caught up to yet.</source>
        <translation>どちらを選んでも計算内容と出来上がる画像は同じです。違うのは速度と、どちらのハードウェアが計算を担当するかだけで、物理的な計算そのものが変わることはありません。
...
Only applies when Renderer is set to GPU.</source>
        <translation>Recursive:既定のGPUレンダリング方式です。幅広くテストされており、最も多くの種類のシーンに対応しています。
Wavefront:似た光線同士をまとめることで、複雑なシーンでグラフィックカードをより効率よく使えますが、新しい方式のためRecursiveほど検証が進んでいません。
...
Recursive follows each light ray from start to finish, one ray at a time - simple and thoroughly tested. Wavefront instead groups together all the rays currently doing the same kind of work (e.g. &quot;just hit a glass surface&quot;) and processes them as a batch - this can make better use of the graphics card on complex scenes with lots of different materials, at the cost of being a newer, less-tested option.</source>
        <translation>どちらもグラフィックカード上で行う処理の内容は同じで、出来上がる画像も同じです。違うのは計算の進め方だけです。
...
the samples takes roughly twice as long.</source>
        <translation>1ピクセルあたりのサンプル数(画質のきれいさ)/ 最大レイ深度(許容するバウンス回数):
  Draft    25 spp,  深度 10
  Preview  50 spp,  深度 20
  Good    100 spp,  深度 50
  High    500 spp,  深度 50
  Ultra  1000 spp,  深度 100
  Maximum 5000 spp, 深度 100
Customを選ぶと、下の「サンプル数」「最大深度」欄の値はそのまま変更されません。
レンダリング時間はおおよそサンプル数に比例します。サンプル数を2倍にすると、
...
Each step up roughly doubles the render time in exchange for a cleaner, less noisy image - Draft is for quickly checking a scene looks right, Ultra/Maximum are for a final image you&apos;d actually want to look at closely.</source>
        <translation>Samples per PixelとMax Ray Depthをまとめて設定するショートカットです。この2つは、レンダリング時間と画質を天秤にかける主要なダイヤルです。
...
Higher resolution means more individual pixels to trace - each one independently sampled - so render time scales up roughly in proportion to the pixel count (double the width AND height and you&apos;re tracing about 4x as many pixels), independent of the Samples per Pixel or Max Ray Depth settings.</source>
        <translation>最終的な画像の幅と高さ（ピクセル数）です。
...
Every path starts from the camera position set below.</source>
        <translation>カメラが一連のフレームの中でどう動くか:
  Orbit     — シーンの中心を見つめたまま、周りを一周します
  Linear    — シーンの前を直線的に通り過ぎます
  Figure-8  — 8の字を描くように、途中で中央を横切ります
  Spiral    — 周回しながら少しずつ近づいていきます
  Tour      — 部屋の中を歩き回るように、左右に揺れながら前へ進み、周囲を見回します
  Showcase  — 製品広告のように、なめらかに一周しながら被写体へ寄り、上下にも動きます
...
Orbit circles fully around the scene, always facing its center - the classic &quot;turntable&quot; shot. Linear sweeps past in a straight line. Figure-8 traces a figure-eight loop, crossing back through the middle. Spiral orbits while steadily moving closer. Tour sways side to side and glides forward while its look-at point drifts too, like an actual visitor walking through and looking around a room. Showcase turns once around the subject with a smooth push-in and a gentle rise-and-fall, like a product advertisement&apos;s hero shot. Every path starts from wherever the camera is positioned further down this tab.</source>
        <translation>カメラが一連のフレームの中でどのように動くかを設定します。
...
カメラ経路: %3。速度に関わらず必ず経路全体を最後まで動きます

出力先: フレームは output/frames/ に保存されます

ffmpegが必要です: レンダリングしたフレームを動画にまとめるには、ffmpegという無料のプログラムが必要です。あらかじめパソコンにインストールし、コマンドラインから実行できる状態にしておいてください。レンダリングログで見つからないと表示された場合は、ffmpeg.org から入手してください。

すべてのフレームのレンダリングが終わると、動画は自動的に結合されて開かれます。

手順1: 上の「動画生成設定」(カメラ経路・フレーム数・FPS)を設定し、出力モードを「動画を生成」にします。

手順2: このタブの下の方で画質の設定を行います。

手順3: 「動画レンダリング開始」をクリックして待ちます。

手順4: 完了すると動画が自動的に結合されて開きます!
...
Looking for the image-quality settings (ReSTIR, Exposure, Samples per Frame, Max Bounces, Firefly Clamp)? Those now live on the Render Options tab&apos;s own Live Preview Settings group, next to the Denoiser section.</source>
        <translation>ライブプレビューでの、マウスによる周回・ズームや、キーボード(WASD・上下キー、左右キー・+/-キー)での移動の感度を調整します。実際に効果があるのは、上の出力モードが「ライブプレビュー(対話操作)」のときだけですが、設定自体はどのモードでも編集できます。
...
Type or Browse to a .exr path instead of .png/.ppm to get a high-dynamic-range file that stores the full range of brightness values without compressing them for a normal screen - useful if you plan to edit the image further in other software. If Denoise is also on and GPU Backend is Recursive, two extra helper files (_albedo.exr and _normal.exr, storing surface color and surface direction) are saved alongside it automatically to help with that cleanup (Wavefront doesn&apos;t produce these yet).</source>
        <translation>レンダリングしたファイルの保存先です。動画モードでは正しい拡張子が自動的に付加されます。ライブプレビューではファイルを書き出さないため、この設定は無視されます。
...
it inline.</source>
        <translation>レンダリングした画像の保存先です。生データの.ppmファイルと一緒に必ず.pngファイルも保存され、「プレビュー」タブに表示されるのはこの.pngファイルです。
...
Choosing a .exr path instead skips both: it writes one file that stores the full range of brightness values with no adjustment for a normal screen - the format professional photo/video editing tools expect, and the only way to get the raw brightness data out of this app instead of an image already adjusted to look right on a regular monitor.</source>
        <translation>完成した画像の保存先です。

生データの.ppmファイルは必ず書き出され、それに合わせて.pngのコピーも自動的に作成されます。ほとんどの画像ビューアー(このアプリのプレビューも含む)は.ppmを直接開けないため、「プレビュー」タブには常に.pngの方が表示されます。
...
Every path starts from the camera position set below.</source>
        <translation type="vanished">フレームの連なりの中でカメラがどう動くか：
...
Orbit circles fully around the scene, always facing its center - the classic &quot;turntable&quot; shot. Linear sweeps past in a straight line. Figure-8 traces a lemniscate, crossing back through the middle. Spiral orbits while steadily moving closer. Tour sways side to side and glides forward while its look-at point drifts too, like an actual visitor walking through and looking around a room. Showcase turns once around the subject with an eased push-in and a gentle rise-and-fall, like a product advertisement&apos;s hero shot. Every path starts from wherever the camera is positioned further down this tab.</source>
        <translation type="vanished">フレームの連なり全体でカメラがどう動くか。
...
Tips: use GPU mode for faster rendering. Lower samples/pixel (10-50) for quick previews, higher (100-500) for production quality. Typical render time is 1-5 minutes on GPU, 15-60 minutes on CPU.</source>
        <translation type="vanished">動画の長さ：%1 秒（%2）
...
ヒント：レンダリングを速くするには GPU モードを使用してください。手早いプレビューには少なめのサンプル数/ピクセル（10～50）、本番品質には多め（100～500）を。典型的なレンダリング時間は GPU で 1～5 分、CPU で 15～60 分です。</translation>
    ...
    <message>
        ...
        <translation type="vanished">OptiX AI デノイザー専用。ノイズのある入力と完全にデノイズされた出力の間をブレンドします（0.0 = 100% デノイズ済み、1.0 = 元のノイズ画像）。これは「画像＆動画」サブセクション自体のブレンド設定と同じ意味です。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">OptiX AI デノイザー専用。各デノイズ済みフレームを、以前のフレームとの累積平均に加えるのではなく、そのまま表示します。サンプルを蓄積し続ければ得られたはずの追加の画質を犠牲にする代わりに、常に最新フレームだけを反映するビューが得られます - WASD で動き回っているとき、蓄積された古いフレームはすでに離れたカメラ位置のものなので、これが役立ちます。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">時間方向のブレンド率の下限です——低いほど履歴を長く保持します（ノイズは少ないが、変化するシーンでは遅延が増えます）。高いほど速く適応します（ノイズは増えるが遅延は減ります）。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">収束したピクセルが蓄積できる履歴フレーム数の上限です——どれだけ「固着」するかを制限します。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">この履歴長を下回る間は、分散を単独で信用せず、近傍ピクセルから空間的に事前フィルタリングします——これは、新規に現れたピクセルや遮蔽が解除されたばかりのピクセルが、十分な独自の時間履歴を持つ前のエッジ保持重みを助けます。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">上記の分散事前フィルタで使用するボックス半径（ピクセル単位）です——半径 3 は 7x7 のボックスを意味します。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">シェーディング法線の差に対するエッジ保持の感度です——高いほど、より小さな法線差でも棄却され、曲面やシルエット部分でのぼやけを防ぎます。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">深度差に対するエッジ保持の感度で、局所的な深度勾配に対する相対値です——高いほど、隣接ピクセルを別の面と判定するまでに許容される深度変化が大きくなります。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">輝度差に対するエッジ保持の感度で、そのピクセル自身が推定するノイズレベルに対する相対値です——高いほど、より大きな明るさの差にまたがってぼかしがかかります。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">A-trous パスごとのフィルタ適用範囲の半径です——フィルタ自身のカーネル重みテーブルには 3 項目しかないため、[0,2] にクランプされます（半径 2 = 5x5）。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">色をアルベドで除算（デモジュレーション）する前に適用される下限です——アルベドがほぼゼロのピクセルが発散したり、黒に丸め込まれたりするのを防ぎます。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">A-trous フィルタのパス数です（各パスでステップ幅が倍になります：1、2、4、8……）——パス数が多いほど有効半径は広くなりますが、その分 GPU コストも高くなります。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">ReSTIR DI/GI、放射輝度キャッシュ、露出、フレームあたりのサンプル数/最大バウンス数、そしてファイアフライ抑制——これらはすべて、画像/動画にのみ適用される下の「詳細パラメータ」グループにも、上の「デノイザー」セクションにも依存しません。上の出力モードが「ライブプレビュー（インタラクティブ）」の場合にのみ効果がありますが、どのモードでも編集は可能です。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">リサンプリングされた 1 バウンス間接光（ReSTIR GI）です——上で有効になっているどのデノイザーとも独立しています。無効にすると、ノイズは多いもののフレームあたりのコストが低い、従来の単一サンプル間接推定に戻ります。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">リサンプリングされた直接光サンプリング（ReSTIR DI）——上の ReSTIR GI とは独立しています（あちらは 1 バウンスの間接光をリサンプリングしますが、こちらは古典的な次イベント推定が単一のグローバルなエイリアステーブルのサンプルから行っていた直接光の抽選をリサンプリングします）。無効にすると、その古典的な単一サンプルの抽選に戻ります。光源が多いシーンではノイズが増えますが、フレームあたりのコストは下がります。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">トーンマッピングの前に適用される一律の明るさ倍率で、「レンダリングオプション」タブ自体の「露出」設定と同じ意味を持ちますが、ライブプレビュー用に独立して設定されます。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">ライブプレビューの各呼び出しでレンダリングされるピクセルあたりのサンプル数です——ライブプレビューは、画像/動画にのみ適用される下の「詳細パラメータ」グループとは独立した、独自の値を持ちます。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">ライブプレビュー用の最大光線深度です——画像/動画にのみ適用される下の「詳細パラメータ」グループとは独立しています。</translation>
    </message>
    <message>
...
surfaces look the same well below the maximum.</source>
    ...
    <message>
        ...
        <translation type="vanished">レンダリングされたファイルの保存先です。動画モードでは正しい拡張子が自動的に付加されます。ライブプレビューはファイルを書き出すことが一切ないため、この設定を完全に無視します。</translation>
    </message>
    <message>
        <source>Where the rendered image is written. A .png is always saved alongside
the raw .ppm, and it is the .png the Preview tab displays.</source>
        <translation type="vanished">レンダリング画像の書き出し先です。生の.ppmファイルとあわせて
常に.pngも保存され、プレビュータブに表示されるのはこの.pngです。</translation>
    ...
    <message>
        ...
        <translation type="vanished">PNG画像 (*.png);;PPM画像 (*.ppm)</translation>
    </message>
    <message>
        <source>Where the finished image is saved.
...
A raw .ppm file is always written, and a .png copy is generated alongside it automatically - the Preview tab always shows the .png, since most image viewers (and this app&apos;s own preview) can&apos;t open .ppm directly.</source>
        <translation type="vanished">完成した画像の保存先です。
...
Paired with Height below to set the resolution manually, overriding whatever the Quality preset on the Basic tab would otherwise use.</source>
        <translation type="vanished">画像の幅（ピクセル数）です。
...
Paired with Width above - together they set the resolution manually, overriding the Basic tab&apos;s Quality preset.</source>
        <translation type="vanished">画像の高さ（ピクセル数）です。
...
the render time. Setting it here switches Quality to Custom.</source>
        <translation type="vanished">1ピクセルあたりに追跡する光線の本数です。これは品質と時間を左右する主要なダイヤルで、
ノイズはこの値の平方根に反比例して減少するため、ノイズを半分にするには
...
More samples means a more accurate average, which shows up as less speckly &quot;noise&quot; in the image - but each extra sample costs render time. Doubling this value roughly halves the noise, but takes about twice as long to render.</source>
        <translation>レイトレーシングは、多数のランダムなレイを発射してその結果を平均することで各ピクセルの色を推定します。大勢の人に意見を聞いて平均を取るようなものです。
...
Each extra bounce lets light travel one more surface before giving up, which is what makes glass, mirrors, and soft indirect lighting look correct. Most scenes look &quot;finished&quot; well before the maximum - beyond that, extra depth mostly traces light too dim to matter.</source>
        <translation>深度1では、レイは直接当たったものしか見えず、反射光は一切考慮されません。反射や間接照明のないシーンのようなイメージです。
...
Generate Video cannot be combined with an alternate Integrator - see the warning below if that combination is picked.</source>
        <translation type="vanished">このレンダリングが 1 枚の静止画を生成するのか、動画としてつなぎ合わせるフレーム列を生成するのか、あるいはインタラクティブな GPU プレビューなのか。
...
Custom leaves the Samples/Max Depth fields below untouched.
Render time scales roughly linearly with samples per pixel.</source>
        <translation type="vanished">ピクセルあたりのサンプル数 / 最大光線深度：
  ドラフト    25 spp、  深度 10
  プレビュー  50 spp、  深度 20
  良好       100 spp、  深度 50
  高品質      500 spp、  深度 50
...
Choosing &quot;Custom&quot; unlocks the X/Y/Z fields below so you can fly the camera anywhere you like instead.</source>
        <translation>このシーン向けに厳選されたカメラ位置のプリセット集で、それぞれ特定の見せ方(たとえば正面から覗き込む、あるいはコーネルボックス風の空間内部から見る、など)になるよう構図が組まれています。
...
Only editable when the preset above is set to Custom - the camera always looks toward the scene&apos;s own fixed look-at point, so moving X/Y/Z changes the viewing angle and distance, not just a straight left-right pan.</source>
        <translation>ワールド空間のX軸(左右方向)に沿ったカメラの位置です。
...
Same Custom-preset-only editing rule as Camera X - the camera keeps looking at the scene&apos;s fixed look-at point as you move it.</source>
        <translation>ワールド空間のY軸(上下方向)に沿ったカメラの位置です。
...
Same Custom-preset-only editing rule as Camera X/Y.</source>
        <translation>ワールド空間のZ軸(前後方向、シーンの奥/手前)に沿ったカメラの位置です。
...
The quickest way to zoom in or pull back once you&apos;ve already found an angle you like via the X/Y/Z fields or a preset.</source>
        <translation>カメラの向きは変えずに、現在向いている方向のままシーンの注視点へ近づけたり遠ざけたりします。
...
X/Y/Zフィールドやプリセットで気に入ったアングルを見つけたあと、手早くズームイン・ズームアウトするのに最適です。</translation>
    ...
    <message>
        ...
        <translation type="vanished">光輸送アルゴリズムを選択します——既定のパストレーサー、あるいは SPPM/BDPT/MLT/AO などの代替方式で、選択すると下にそれぞれ専用のサブオプションが表示されます。代替インテグレーターは CPU 専用であり、動画モードと組み合わせることはできません。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">SPPMが実行するカメラパス+フォトンパスのラウンド数です。反復回数を増やすほど結果はよりクリーンに収束しますが、レンダリング時間はほぼ線形に増加します。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">各反復で光源から発射されるフォトンの数です。フォトン数を増やすと間接光/コースティクスのノイズは減りますが、フォトンパスは遅くなります。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">BDPTが接続する2本のサブパス(カメラ側と光源側)それぞれの最大バウンス数です。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">MLTがマルコフ連鎖の種として、各深度ごとに事前にサンプリングする候補光路の数です - 値を増やすほど初期分布の情報量が充実します。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">すべてのチェーンを合わせたMetropolis変異の総数です - レンダリング時間/品質を左右する主要なパラメータで、デフォルトのパストレーサーにおけるピクセルあたりのサンプル数に相当します。</translation>
    </message>
    <message>
        <source>Same meaning as BDPT&apos;s max path depth (MLT is built directly on BDPT&apos;s subpath machinery).</source>
        <translation type="vanished">BDPTの最大パス深度と同じ意味です(MLTはBDPTのサブパス機構の上に直接構築されています)。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">オクルージョンテストレイが、非遮蔽と判定されるまでに到達できる距離です。デフォルト値(100億)は事実上無限大です - この値を下げると、近くのジオメトリのみが遮蔽物としてカウントされるようになります。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">デフォルトでは、オクルージョンレイは表面法線方向に重み付けされてサンプリングされます(コサイン半球分布)。これはランバート面が実際に受光する様子と一致します。半球一様サンプリングは代わりにサンプルを半球上に均等に分布させます - 異なる、重み付けのない推定方法です。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">下のオクルージョンカラーに対する一律の倍率です。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">アンビエントオクルージョンを可視化する色です - 実際に照明されたレンダリングではないため、これは光源の色ではなく可視化上の選択です。デフォルトは白 (1, 1, 1) です。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">デフォルトで有効です。直接光源サンプリングは、各バウンスで影レイを明示的に光源に向けることで、小さく明るい光源があるシーンのノイズを大幅に低減します。無効にすると、純粋な不偏パストレーサーと同様に、偶然によってのみ光源を見つける挙動に戻ります。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">デフォルトで有効です。各バウンスの新しい方向を、表面material が実際に光を反射する方向に重み付けしてサンプリングします。無効にすると、半球一様サンプリングに戻ります。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">サンプラーアルゴリズム、スペクトルレンダリング、適応サンプリング、そして固定サンプル数の代替となる時間制限です。これらはサンプルが「どのように」取得されるかを制御し、「いくつ」（設定タブ）とは別の話です。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">Sobol列をもとにした低差異序列で、ピクセルごとにスクランブルされています。汎用のデフォルトとして最も優れており、目に見える構造を残さず高速に収束します。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">Sobolをモートン(Z-order)曲線に沿って並べ替えたバリアントです。通常のSobolと少なくとも同等に収束し、適応的/プログレッシブサンプリングの下ではより良い挙動を示します。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">追加のパディング次元を持つSobol列で、1ピクセルが基本のSobolで無理なくカバーできる以上の乱数次元を必要とする場合(バウンス数の多いパスなど)に生じる相関アーティファクトを回避します。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">各ピクセルを格子状のサブセルに分割し、セルごとに1サンプルを取得します。Sobol/Haltonほど洗練されてはいませんが、シンプルで予測しやすいカバレッジを持ち、リファレンス/比較用のサンプラーとして役立ちます。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">ブルーノイズ順序を持つプログレッシブ多重ジッタード(0,2)列です。隣接するピクセル間でのサンプルの空間分布(ブルーノイズ)が特に均一です。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">次元ごとに異なる素数を基数として構築される、古典的な低差異序列です。十分に検証されており、通常の層化サンプリングで見られがちな軸に沿った偏りを回避します。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">純粋な無相関の疑似乱数で、低差異構造は一切ありません。読み込んだ .pbrt シーン自体のSamplerディレクティブに忠実であるために用意されているものであり、それ自体として推奨される選択肢ではありません。</translation>
    </message>
    <message>
...
or under BDPT/MLT/SPPM/the debug integrators.</source>
        <translation type="vanished">ランダムな判断を左右するサンプラーを選択します(Independent以外はすべて低差異序列です)。CPUのデフォルトパストレーサーにのみ影響し、GPUおよびBDPT/MLT/SPPM/デバッグ用インテグレータには影響しません。</translation>
    </message>
    <message>
...
Grayed out? This only exists on the CPU renderer&apos;s default path tracer - switch Renderer to CPU on the Settings tab to use it.</source>
        <translation type="vanished">通常のレンダリングは光を 3 つの数値——赤、緑、青——として記録します。これは画面が色を表示する方法と同じです。
...
becomes a ceiling, not a fixed count. CPU default path tracer only.</source>
        <translation type="vanished">収束したピクセルへのサンプリングを、常に全ピクセルに
...
Lower = cleaner but slower. 0.01 matches Blender Cycles&apos; own default.</source>
        <translation type="vanished">ピクセルが収束したとみなすための目標相対ノイズレベルです。
...
Grayed out? This only affects the CPU renderer&apos;s default path tracer - switch Renderer to CPU on the Settings tab to use it.</source>
        <translation type="vanished">モンテカルロ・パストレーサーのノイズはランダム性に由来します——明るく均一に照らされた壁のようなピクセルは、わずかなサンプルできれいな推定値に収束しますが、小さな窓からしか光が入らない薄暗い隅のようなピクセルは、ノイズが落ち着くまでにはるかに多くのサンプルを必要とします。両方に同じ固定サンプル数を費やすと、すでに終わっているピクセルで時間が無駄になります。
...
tracer only.</source>
        <translation type="vanished">常に全スキャンラインが完了するまで実行するのではなく、
...
budget above.</source>
        <translation type="vanished">上の「サンプル数」予算に関係なく、
...
Grayed out? This only affects the CPU renderer&apos;s default path tracer - switch Renderer to CPU on the Settings tab to use it.</source>
        <translation type="vanished">時間内に終わるサンプル数を推測する代わりに、固定のプレビューやレンダーファームの時間予算として使えます——上の「サンプル数」は余裕を持った上限のままにしつつ、実際にいつ止めるかはこちらが決めます。
...
グレーアウトしていますか？これは CPU レンダラーの既定のパストレーサーにのみ影響します——「設定」タブで「レンダラー」を CPU に切り替えて使用してください。</translation>
    ...
    <message>
        ...
        <translation type="vanished">トーンマッピングカーブ、レンダリング統計を出力するかどうか、そして OptiX 検証モード——これらは最終画像がどう処理・報告されるかに関する動作フラグであり、何をレンダリングするかとは無関係です。デノイズにはこのタブの下部に専用のセクションがあります。</translation>
    </message>
    <message>
...
can over-smooth.</source>
        <translation type="vanished">ノイズのある入力と完全にデノイズされた出力の間をブレンドします
...
Grayed out? This needs the GPU recursive backend - switch Renderer to GPU (and GPU Backend to Recursive) on the Settings tab to use it.</source>
        <translation type="vanished">レイトレーシングは本質的にノイズを伴います——サンプル数が少ないと粒状のまだら画像になります。これが、サンプル数が多いほど通常きれいな画像になる理由です。
...
SVGF Denoiser: an alternative, experimental spatiotemporal filter - tracks per-pixel variance over time and uses it to drive an edge-aware spatial filter, which holds up better during camera movement than the AI denoiser + running-mean combination. Always shows the latest filtered frame rather than accumulating (see the SVGF Advanced Tuning group below for its own tunable constants).</source>
        <translation type="vanished">なし：生の累積サンプルのみで、デノイズは行いません。
...
SVGF デノイザー：代替の実験的な時空間フィルタです——ピクセルごとの分散を時間とともに追跡し、それを使ってエッジを保持する空間フィルタを駆動します。カメラが動いている間、AI デノイザー＋移動平均の組み合わせよりも安定します。常に最新のフィルタ済みフレームを表示し、累積は行いません（独自の調整可能な定数については下の「SVGF 詳細設定」グループを参照）。</translation>
    ...
    <message>
        ...
        <translation type="vanished">ワールド空間の放射照度プローブキャッシュです——2 バウンス以上の深さの拡散光を、上の ReSTIR GI とは独立にキャッシュ・リサンプリングします（あちらは最初の間接バウンスのみをリサンプリングしますが、こちらはそれ以降のすべてのバウンスをカバーします）。多数のフレームをかけて収束するため、有効にした直後やカメラを新しい領域に移動した直後の数秒間はまだら模様やノイズが見えることがあります。無効にすると、すべてのバウンスで従来の次イベント推定に戻ります。深い間接光の多いシーンではノイズが増えますが、収束の遅延はありません。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">光沢/金属表面のバウンド方向を、実際の入射光の位置を粗く漸進的に学習した推定値に基づいて重点的にサンプリングします。マテリアル自体の反射ローブサンプリングのみに頼るのではありません。上の「放射輝度キャッシュ」も有効になっている必要があります——そのキャッシュ自身のグリッドと更新パイプラインを再利用するため、無効の場合は何も行いません。放射輝度キャッシュと同様、収束には数秒かかります。無効にすると、マテリアル自身の不偏な反射サンプリングにフォールバックします。</translation>
    </message>
    <message>
...
SVGF Denoiser: an alternative, experimental noise-reduction filter. Instead of blending many frames together, it tracks how much each pixel&apos;s brightness has been changing over time and smooths it out along natural edges - it holds up better than the AI Denoiser while you&apos;re actively moving the camera. Always shows the latest smoothed frame rather than gradually sharpening over time (see the SVGF Advanced Tuning group below for its own fine-tuning options).</source>
        <translation>None(なし): レンダリングされたままの画像を、自然なノイズが残った状態で表示します。スムージングは行いません。

OptiX AIデノイザー: カメラを動かしている間にライブプレビューがざらついて見える状態を、上の「静止画・動画」サブセクションで仕上げのレンダリングに使うのと同じAIによるスムージングで整えます。徐々にきれいになるのを待たなくても、すぐにそれなりにきれいな見た目になります。1フレームごとにわずかにGPUの処理時間が余分にかかります。
...
SVGFデノイザー: もう一つの、実験的なノイズ低減フィルターです。複数のフレームをブレンドする代わりに、各ピクセルの明るさが時間とともにどう変化しているかを追跡し、自然な輪郭に沿ってなめらかにします。カメラを積極的に動かしている間は、AIデノイザーよりも安定した見た目を保てます。時間とともに徐々に鮮明になっていくのではなく、常に直近のスムージング結果を表示します(細かい調整については、下の「SVGF詳細設定」グループを参照してください)。</translation>
    ...
    <message>
        ...
        <translation type="vanished">決定論的なサブピクセルジッターシーケンスを用いて、数フレームにわたってより鮮明な画像を再構成します。ライブプレビュー本来の低解像度画像をウィンドウに合わせて単純に引き伸ばす代わりに行います。1フレームあたりのレンダリングコストは変わりません——2倍/4倍は、鮮明になるまでに必要なフレーム数（カメラ移動が落ち着いた後、それぞれ4フレームまたは16フレーム）が変わるだけです。4倍は2倍（約40～50MB）よりも明らかに多くのメモリ（約150～200MB）を使用します。上の「デノイザー」ドロップダウンのSVGFモードおよび「最新フレームを表示」オプションとは併用できません——両方がオンの場合はそちらが優先されます。</translation>
    </message>
    <message>
...
Grayed out? This is GPU-only - switch Renderer to GPU on the Settings tab to use it.</source>
        <translation type="vanished">GPU レイトレーシングパイプライン自体の内部で追加の正しさチェックを有効にし、そのままでは静かに誤った画像を生成したり予測不能にクラッシュしたりする類のバグを捕捉します。
...
Grayed out? This only affects the CPU renderer&apos;s default path tracer - switch Renderer to CPU on the Basic Settings tab to use it.</source>
        <translation type="vanished">レイトレーシングでは、レイをどの方向にバウンスさせるか、光源上のどの点をサンプリングするかなど、多くの乱数が必要です - そしてその「乱数」がどのように生成されるかによって、画像がクリーンな結果に収束する速さが変わります。

単純な乱数生成器は偏りや隙間を生みます。ここにあるサンプラーのほとんど(Sobol、Haltonなど)は低差異序列であり、サンプリング空間をより均一にカバーするよう意図的に配置されているため、同じサンプル数でも真の乱数よりも速くクリーンな画像に収束します。Independentは例外で、純粋な無相関の乱数です。これは推奨の選択肢としてではなく、読み込んだ .pbrt シーン自体のSamplerディレクティブに忠実であるために含まれています。
...
グレーアウトしていますか? これはCPUレンダラーのデフォルトパストレーサーにのみ影響します - 使用するには、基本設定タブでレンダラーをCPUに切り替えてください。</translation>
    ...
    <message>
        ...
        <translation type="vanished">シーン内の光源に対して空間階層構造を構築し、単一のグローバルな重み付けを使う代わりに、各光源をパワーとシェーディング点への近さの両方で重み付けして、バウンスごとに適応させます。pbrt-v4自体のデフォルトです - わずかな追加の管理コストと引き換えに、一般的に最も収束が優れています。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">固定の選択の代わりに、そのシーン自身のインテグレーターの「string lightsampler」パラメータが要求する値を使用します（要求がなければ BVH、または本プロジェクトが実装していない何かが要求された場合も同様）——CLI 自体の --lightsampler auto に相当します。これを一度選んでそのままにしておくことが、推奨内容の異なるシーン間を切り替えても常に正しい状態を保てる唯一の選択です。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">光源が放つ全パワー(総放射束)に応じた確率で光源を選びます - 明るい光源ほど頻繁にサンプリングされます。光源の明るさに大きな差があるシーンでは一様よりも速く収束しますが、距離や遮蔽は考慮されません。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">シーン内のすべての光源から、明るさや距離に関係なく一様なランダムで光源を選びます。単純で不偏ですが、明るさが大きく異なる光源が多いシーンでは収束が遅くなります - 暗い光源も明るい光源と同じ頻度でサンプリングされてしまいます。</translation>
    </message>
    <message>
...
or under BDPT/MLT/SPPM/the debug integrators.</source>
        <translation type="vanished">各Next Event Estimationバウンスでサンプリングする光源を
どの戦略で選ぶかを指定します。ノイズや収束速度に影響し、
収束後の画像自体は変えません。CPUのデフォルトパストレーサーにのみ
影響し、GPUおよびBDPT/MLT/SPPM/デバッグ用インテグレータには影響しません。</translation>
    </message>
    <message>
...
Grayed out? This only affects the CPU renderer&apos;s default path tracer - switch Renderer to CPU on the Basic Settings tab to use it.</source>
        <translation type="vanished">拡散/光沢バウンスのたびに、Next Event Estimationで直接サンプリングする光源を(候補が複数あり得る中から)1つ選ぶ必要があります - どの光源が選ばれるか、そしてその選び方がどれだけ公平かによって、画像が収束する速さは変わりますが、何に収束するか(最終的な結果)は変わりません。

BVH(デフォルト、pbrt-v4自体と同じ)は、シーン内の光源に対して空間階層構造を構築し、シェーディング点ごとに重み付けを適応させます - 明るい光源と近くの光源の両方が優先されます。パワーは位置を無視して明るさのみで選ぶため、よりシンプルですが、光源までの距離が大きく異なるシーンでは精度が落ちます。一様はどちらも無視し、明るさや距離に関わらずすべての光源を等確率で扱います。主に比較・デバッグ用に含まれています。
...
this checkbox only ever adds the request, never removes it.</source>
        <translation type="vanished">パスが最初に非鏡面バウンスをした後、
粗いBSDFのGGXアルファを広げます - ハードコースティックパスによる
ファイアフライを抑えますが、その分ぼやけが生じます。CPUとGPUの
デフォルトパストレーサーのみに適用されます。すでにこれ自体を要求している
シーンには影響しません - このチェックボックスは要求を追加するだけで、取り除くことはありません。</translation>
    </message>
    <message>
...
Off by default, matching pbrt-v4&apos;s own default. If a loaded .pbrt scene&apos;s file already requests this itself, it&apos;s applied either way - this checkbox can only add the request on top, never take it away.</source>
        <translation type="vanished">光路の中には、パストレーサーがきれいに見つけるのが本質的に難しいものがあります - 粗い(しかし完全な鏡面ではない)面で跳ね返り、別の粗い面を通過して、小さく明るい光源に至るような光です。そうした経路は、平均化して消えるまでに非常に長い時間がかかる、明るく孤立した斑点(&quot;ファイアフライ&quot;)として現れます。

パス正則化は、パスがそれまでに経た非鏡面バウンスのたびに、面の粗さを意図的に少しずつぼかしていきます - これにより小さなバイアス(技術的には誤った答え)が生じますが、その代わりにファイアフライの収束が劇的に速くなり、レンダリング結果の見た目としては通常こちらの方が良いトレードオフになります。

既定では無効で、pbrt-v4自身のデフォルトと一致しています。読み込んだ.pbrtシーンのファイルがすでにこれ自体を要求している場合、どちらにしても適用されます - このチェックボックスは要求を上乗せするだけで、取り除くことはありません。</translation>
    </message>
    <message>
...
per-sample-total.</source>
        <translation type="vanished">最も明るいチャンネルが下の値を超えるピクセルサンプルを
クランプし、色相を保つためにすべてのチャンネルを一緒にスケールダウンします。
CPUと両方のGPUバックエンドに対応 - recursiveはCPUと厳密に一致し、
wavefrontはサンプル合計単位ではなく寄与単位で近似します。</translation>
    </message>
    <message>
...
Off by default (effectively unbounded, matching pbrt-v4&apos;s own default). CPU default path tracer only.</source>
        <translation type="vanished">モンテカルロパストレーサーは、個々には正しいものの極端に明るいパス - ちょうど良い角度で小さく強い光源をかすめたレイ - を時折サンプリングしてしまい、そのような1つのサンプルが、他の十分な数のサンプルが到着して均されるまでの間、長時間にわたってピクセルの平均値を支配してしまうことがあります。これらは明るく孤立した斑点(&quot;ファイアフライ&quot;)として現れます。

このクランプは、平均に加算される前に、個々のサンプルの最も明るいチャンネルがどこまで明るくなれるかを制限し、小さく制御されたバイアスと引き換えに、同じサンプル数でも劇的にクリーンな見た目の画像を得られるようにします - 値を小さくするほど強力にクリーンアップされますが、外れ値ノイズだけでなく本当に明るい小さな光源まで目に見えて暗くしてしまうおそれがあります。

既定では無効です(pbrt-v4自身のデフォルトと同じく、実質的に無制限)。CPUのデフォルトパストレーサーのみに適用されます。</translation>
    </message>
    <message>
...
and wavefront each have their own denoiser).</source>
        <translation type="vanished">レンダリング完了後の画像に対して、アルベド+法線バッファを手がかりに
...
BDPT/MLT/SPPM/the debug integrators.</source>
        <translation type="vanished">確率的な判断に使用する低差異(low-discrepancy)サンプラーを選択します。
...
Grayed out? This only affects the CPU renderer&apos;s default path tracer - switch Renderer to CPU on the Basic Settings tab to use it.</source>
        <translation type="vanished">レイトレーシングでは、レイをどの方向に反射させるか、ライト上のどの点をサンプリングするかなど、多くの乱数が必要です。この「乱数」をどう生成するかによって、画像がきれいな結果に収束する速さが変わります。

単純な乱数生成器は値が偏って集まったり、隙間ができたりします。ここにあるサンプラー(Sobol、Haltonなど)はいずれも低差異(low-discrepancy)列と呼ばれるもので、サンプリング空間をより均一に埋めるように意図的に配置されており、同じサンプル数でも真の乱数より速くきれいな画像に収束します。
...
Grayed out? This only affects the CPU renderer&apos;s default path tracer - switch Renderer to CPU on the Settings tab to use it.</source>
        <translation type="vanished">レイトレーシングには大量の乱数が必要です——光線をどの方向に反射させるか、光源上のどの点をサンプリングするか、等々——そしてその「乱数」が「どのように」生成されるかによって、画像がきれいな結果へ収束する速さが変わります。
...
Grayed out? This only affects the CPU renderer&apos;s default path tracer - switch Renderer to CPU on the Settings tab to use it.</source>
        <translation type="vanished">拡散/光沢反射のバウンスごとに、次イベント推定のために直接サンプリングする光源を（潜在的に多数の中から）1 つ選ぶ必要があります——どの光源が選ばれ、どれだけ公平に選ばれるかによって画像の収束速度は変わりますが、最終的に収束する「結果」自体は決して変わりません。
...
than silently rendering wrong colors. Noticeably slower per-sample.</source>
        <translation type="vanished">単純なRGBの代わりに、実際のhero-wavelength方式によるスペクトルレンダリングを行います。
...
Grayed out? This only exists on the CPU renderer&apos;s default path tracer - switch Renderer to CPU on the Basic Settings tab to use it.</source>
        <translation type="vanished">通常のレンダリングでは、画面が色を表示するのと同じように、光を赤・緑・青の3つの数値として扱います。

実際の光は連続した波長のスペクトルであり、プリズムが白色光を虹色に分解するような物理現象は、波長ごとに屈折の度合いが異なることによって初めて起こります。これはRGBだけでは表現できません。スペクトルレンダリングでは、単なるRGBの代わりに、レイごとに実際の波長をいくつか追跡します。その代償として、1サンプルあたりのノイズが増え、速度も遅くなります。
...
Both CPU and GPU default path tracer only.</source>
        <translation type="vanished">トーンマッピング前のリニアカラーに対する一律の乗数です(1.0で無効化)。
...
1.0 leaves the image unchanged; below 1.0 darkens it, above 1.0 brightens it - useful for a scene that&apos;s rendering correctly but is just too dark or too bright to see clearly, without changing any actual light in the scene.</source>
        <translation>画像全体に一律に適用される明るさの乗数で、カメラの露出設定と同じ役割を持ちます。
...
1.0では画像は変化せず、1.0未満で暗く、1.0を超えると明るくなります。シーン自体の照明を変更せずに、正しくレンダリングされているものの単に明るすぎる・暗すぎて見づらいシーンを調整するのに便利です。</translation>
    ...
    <message>
        ...
        <translation type="vanished">光線とシーンの交差テストを高速化するための、CPU 専用の空間加速構造と分割ヒューリスティックです。既定のパストレーサーだけでなく、すべてのインテグレーターに適用されます——既定値はほぼすべてのシーンでうまく機能します。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">読み込んだ .pbrt シーン自身の Accelerator ディレクティブをそのままにします（指定がなければ bvh、これは実際の pbrt-v4 自体の既定値です）。ネイティブ（非 .pbrt）シーンにはいずれにせよ影響しません。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">下で選択した分割方法によって構築される階層的境界ボリュームです——これは pbrt-v4 自体と一致する、本プロジェクトの従来からの既定アクセラレーターです。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">BVH の代わりに k-d 木を使用します——pbrt-v4 のもう一つの実在するアクセラレーターオプションです。独自の分割方法という概念を持たず（これを選択すると下のコンボボックスは無視されます）、物体のモーションブラーがあるシーンでは BVH/sah にフォールバックします（本プロジェクトの kd 木ラッパーには光線ごとの時間チャンネルがありません）——その際は警告が出力されます。</translation>
    </message>
    <message>
...
scene (has no Accelerator directive to override - see the log).</source>
        <translation type="vanished">シーンのジオメトリを保持するアクセラレーターです。どの選択肢を
...
Only a loaded .pbrt scene has an Accelerator directive to override at all - a native (non-.pbrt) scene always uses its own fixed BVH regardless of this choice (a warning is printed if you pick something else anyway).</source>
        <translation type="vanished">三角形/球のフラットなリストでは、すべての光線がシーン内のすべてのプリミティブをテストすることになってしまいます——加速構造があるからこそ、光線はシーンの大部分を飛ばして、実際に通過する経路の近くにあるわずかなプリミティブだけをテストできます。
...
Accelerator ディレクティブを上書きできるのは、読み込んだ .pbrt シーンだけです——ネイティブ（非 .pbrt）シーンは、この選択にかかわらず常に独自の固定 BVH を使用します（それでも他を選んだ場合は警告が出力されます）。</translation>
    ...
    <message>
        ...
        <translation type="vanished">表面積ヒューリスティック——複数の候補分割の走査コストを見積もり、最も安いものを選びます。Middle/Equal より構築は遅いですが、ほとんどのシーンで最も走査効率の良い木になります——本プロジェクトの従来からの BVH 構築方式です。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">各ノードをその境界ボックスの最も長い軸の中点で分割します。構築は安価で、コスト見積もりは一切行いません——分布が不均一なジオメトリでは走査効率が悪くなることがあります。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">空間的な広がりに関係なく、各ノードを両側のプリミティブ数が等しくなるように分割します。構築は安価ですが、密集したジオメトリでは形の悪いノードを生成することがあります。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">階層的リニア BVH——モートン符号からボトムアップで構築され、非常に多い三角形数に対してこれら 4 つの中で最も高速に構築できますが、SAH に比べて走査品質はいくらか劣ります。</translation>
    </message>
    <message>
...
warning is printed when that happens.</source>
        <translation type="vanished">上の BVH がどのように構築されるか（Accelerator が
...
警告が出力されます。</translation>
    ...
    <message>
        ...
        <translation type="vanished">上のアクセラレーターが BVH に解決される場合にのみ参照されます（Kd 木には分割方法の概念がないため無視されます）。この 4 つはいずれも異なる戦略から同じ系統の木の形を構築します——SAH はより良く走査できる木と引き換えに構築により多くの時間をかけます。Middle/Equal は安価でシンプルなフォールバックです。HLBVH は非常に大きなシーンでの最速の構築と引き換えに、いくらか走査品質を犠牲にします。</translation>
    </message>
    <message>
...
effect under BDPT/MLT/SPPM/the debug integrators.</source>
        <translation type="vanished">sRGBカーブを適用する前に、どのトーンマッピング演算子を使用するかを選択します。
...
ACES rolls off bright highlights gently, the way film does; Reinhard is a simpler, older compression; None just clips anything too bright to flat white, which can look harsh.</source>
        <translation type="vanished">レイトレースされたシーンの真の明るさの値には上限がなく、たとえば電球は壁の100倍も明るいことがあります。しかし画面が表示できる明るさの範囲は限られています。トーンマッピングとは、この広大な明るさの範囲を表示可能な範囲へ圧縮するカーブのことです。
...
never changes the rendered image.</source>
        <translation type="vanished">レンダリング終了後に、発射したレイ数、バウンス数、シャドウレイ数、秒間サンプル数などの簡易統計をログタブに出力します。あくまで観察用の情報であり、レンダリング結果の画像には一切影響しません。</translation>
    </message>
    <message>
...
Purely informational: it never changes the rendered image, just tells you what the renderer actually did.</source>
        <translation type="vanished">レンダリング終了後に、発射されたレイの数、発生したバウンスの数、追跡されたシャドウレイの数、秒間サンプル数といった短い要約を出力します。
...
Grayed out? This needs the GPU recursive backend - switch Renderer to GPU (and GPU Backend to Recursive) on the Basic Settings tab to use it.</source>
        <translation type="vanished">レイトレーシングは本質的にノイズを伴います。サンプル数が少ないと画像に粒状のノイズが残るため、通常はサンプル数を増やすほどきれいな画像になります。

デノイザーとは、このノイズのパターンを認識し、追加でレイを追跡することなく後処理で滑らかにするよう訓練された機械学習モデルです。細部のディテールをある程度犠牲にする代わりに、より短時間できれいに見える画像を得られます。
...
real per-launch cost. GPU only, for debugging, not routine use.</source>
        <translation type="vanished">OptiX検証モードを有効にします。デバイス側で追加のチェックを行うため、起動ごとに実際のコストがかかります。GPU専用のデバッグ機能であり、通常の利用には向きません。</translation>
    </message>
    <message>
        <source>Turns on extra correctness checks inside the GPU ray-tracing pipeline itself, catching certain classes of bugs that would otherwise silently produce a wrong image or crash unpredictably.
...
Grayed out? This is GPU-only - switch Renderer to GPU on the Basic Settings tab to use it.</source>
        <translation type="vanished">GPUレイトレーシングパイプライン内部で追加の正当性チェックを有効にし、放置すれば警告なく誤った画像を生成したり、予測不能にクラッシュしたりする類のバグを検出します。

これはレンダラー自身のGPUコードを開発する人向けのデバッグ支援機能であり、通常のレンダリングでは恩恵はありません。実際にパフォーマンスコストがかかるうえ、正しくレンダリングされた結果の見た目自体は変わりません。
...
グレーアウトしている場合はGPU専用の設定です。ベーシック設定タブでレンダラーをGPUに切り替えると使用できます。</translation>
    ...
    <message>
        ...
        <translation type="vanished">完全なフレームのうち、正規化された 0～1 座標で指定した矩形の一部だけをレンダリングします——シーンの一部を、画像全体のコストをかけずに素早くテストレンダリングするのに便利です。</translation>
    </message>
    <message>
...
path tracer only.</source>
        <translation type="vanished">レンダリングを、フレーム内の矩形領域に制限します。矩形は
画像全体に対する[0,1]の割合で指定します。CPUとGPUの
デフォルトパストレーサーのみに適用されます。</translation>
    </message>
    <message>
...
Off by default (the full frame). If a loaded .pbrt scene&apos;s file already requests its own cropwindow/pixelbounds, checking this overrides it with the rectangle below; leaving it unchecked lets the scene&apos;s own request (if any) stand.</source>
        <translation type="vanished">画像全体ではなく、フレームの矩形の一部分だけをレンダリングします - それ以外の領域は黒のままになります。矩形は、フレーム全体の幅/高さに対する4つの割合として、0(左端/上端)から1(右端/下端)の範囲で指定するため、解像度に関わらず同じ形状を保ちます。

大きく低速なシーンの問題のある一部分について、より速く試行錯誤したい場合に便利です - 同じ総サンプル数でも、フレーム全体ではなく一部分だけをカバーすればよいため、はるかに速く収束します。

既定では無効です(フレーム全体)。読み込んだ.pbrtシーンのファイルがすでに独自のcropwindow/pixelboundsを要求している場合、これを有効にすると下の矩形で上書きされます。無効のままにしておくと、シーン自身の要求(もしあれば)がそのまま有効になります。</translation>
    </message>
    <message>
...
Sampler/Spectral/Exposure/Tonemap/Stats below only affect the
default Path Tracer - see each control&apos;s own tooltip.</source>
        <translation>使用するレンダリング方式を選びます。Path Tracer(既定)は十分にテストされた汎用的な方式です。下にある代替方式は、その汎用性の代わりに特定の技術(フォトンマッピングや、カメラと光源の双方から光を追跡してつなげる方式など)を使うか、テストや比較のために単純化されたバージョンです。SPPMを除くすべての代替方式はCPUでのみ動作し、いずれも「動画を生成」モードとは組み合わせられません。
...
Hover any item in the dropdown for details on that specific mode.</source>
        <translation>レンダリングの速度だけでなく、レンダリング方式そのものを選びます。Path Tracer(既定)は、このアプリの他の場所でも使われている、汎用的で十分にテストされた方式です。

SPPM(フォトンマッピング技術)は、通常のパストレーシングが苦手とする、扱いの難しいガラスや集光効果に強い方式です。BDPTと(それを土台にした)MLTは、カメラと光源の両方から光線を追跡し、途中でつなげる方式です。難しい照明設定に役立ちますが、面光源にしか対応していません。RandomWalk、Ambient Occlusion、SimplePath、SimpleVolPath、LightPathは、参照用・デバッグ用のモードです。よりシンプルな分、ノイズが多かったり表現できる範囲が狭かったりします(例えばAmbient Occlusionは実際の照明入り画像は生成しません)。特定の技術が最終的な画像にどう寄与しているかを個別に確認したいときに便利です。
...
Grayed out? This only affects the CPU renderer&apos;s default path tracer - switch Renderer to CPU on the Settings tab to use it.</source>
        <translation>レンダリングには、光線をどの方向にバウンスさせるか、光源上のどの点を狙うかなど、大量の乱数が必要です。その「ランダムな」選択をどう生成するかによって、画像がどれだけ早くきれいな結果に落ち着くかが変わります。

普通の乱数は、ある場所に偏って固まり、別の場所に隙間を残しがちです。ここにあるほとんどのサンプラー(Sobol、Haltonなど)は、代わりにあらゆる可能性をより均等にカバーするよう意図的に配置されたパターンを使うため、同じサンプル数でも純粋な乱数より早く画像がきれいになります。Independentは例外で、相関のない普通の乱数です。これを明示的に要求する.pbrtシーンファイルを忠実に再現できるように用意されているだけで、推奨される選択ではありません。
...
Grayed out? This only affects the CPU renderer&apos;s default path tracer - switch Renderer to CPU on the Settings tab to use it.</source>
        <translation>つや消しや半光沢の表面でバウンスするたびに、シーン内に複数あり得る光源の中から光線を直接向ける先を1つ選ぶ必要があります。どの光源をどれだけ公平に選ぶかによって、画像がきれいになる速さは変わりますが、最終的な見た目が変わることはありません。

BVH(既定)はシーンの光源を高速検索用のインデックスにまとめ、着色対象の各点ごとに重み付けを調整します。明るい光源と近い光源の両方が優先されます。Autoは読み込んだシーンファイル自体が指定する方式を使用します(何も指定されていなければBVHにフォールバックします)。Powerは位置を無視して明るさだけで選ぶため、よりシンプルですが、光源までの距離に大きな差があるシーンでは不利になります。Uniformは明るさも距離も無視し、すべての光源を均等な確率で選びます。主に比較やデバッグ用です。
...
Grayed out? This only exists on the CPU renderer&apos;s default path tracer - switch Renderer to CPU on the Settings tab to use it.</source>
        <translation>通常のレンダリングでは、画面が色を表示するのと同じように、光を赤・緑・青の3つの数値だけで扱います。

実際の光は連続した波長のスペクトルであり、プリズムが白色光を虹に分けるような現象は、波長ごとに屈折の度合いが異なるために起こります。単純な赤・緑・青ではこれを表現できません。このオプションは、代わりに1本の光線につきいくつかの実際の波長を個別に追跡します。その分、レンダリングはノイズが多く遅くなります。
...
Grayed out? This only affects the CPU renderer&apos;s default path tracer - switch Renderer to CPU on the Settings tab to use it.</source>
        <translation>レンダリングは多数のランダムなサンプルから画像を組み立てていくため、最初はノイズが多く、徐々にきれいになっていきますが、そのきれいになる速さはピクセルごとに異なります。明るく均一に照らされた壁のピクセルは数サンプルだけできれいになるかもしれませんが、小さな窓からの光しか届かない暗い隅は、ノイズが落ち着くまでにずっと多くのサンプルが必要になることがあります。両方に同じ固定サンプル数を使うと、すでに十分きれいなピクセルに無駄な時間を費やしてしまいます。

アダプティブサンプリングは各ピクセルのノイズの残り具合を常に把握し、下の閾値を下回った時点でそのピクセルへのサンプル追加を早めに打ち切ります。これにより「サンプル数」は、すべてのピクセルが必ず使う固定量ではなく上限として働きます。Blender Cyclesのアダプティブサンプリングと同じ考え方です。
...
Grayed out? This only affects the CPU renderer&apos;s default path tracer - switch Renderer to CPU on the Settings tab to use it.</source>
        <translation>時間内に収まりそうなサンプル数を推測する代わりに、決まった時間でのプレビューや共有パソコンでの時間予算を管理したいときに便利です。上の「サンプル数」は余裕を持った上限のままにしておき、実際にいつ停止するかはこちらが決めます。

停止は行単位で行われます。時間切れになった時点ですでに処理中だった行は最後まで通常どおり完了しますが、まだ処理が始まっていない行は、飛ばされるのではなく黒いまま残されます。そのため、壊れたファイルではなく、途中までとはいえ有効な画像が得られます。

動画を生成する場合、これは各フレームではなく動画全体に対する時間予算です。後の方のフレームは残り時間をそのまま使い、時間切れになった時点で残っているフレームはすべて省略されます。
...
request, never removes it.</source>
        <translation>経路の中で鏡面反射でないバウンスが最初に発生した後、粗い・反射性の・ガラスのような表面のシャープさをわずかに和らげます。これにより、追跡が難しい光路が原因のfireflies(孤立した明るい斑点)を抑えられますが、その代わりに少しぼけが増えます。CPU・GPUいずれの既定のパストレーサーにのみ有効です。すでにこの処理を自ら要求しているシーンには影響しません。このチェックボックスは要求を追加することしかなく、取り除くことはありません。</translation>
    </message>
    <message>
...
Off by default. If a loaded .pbrt scene file already asks for this itself, it&apos;s applied either way - this checkbox can only add the request on top, never take it away.</source>
        <translation>一部の光路は、パストレーサーがきれいに見つけ出すことが本質的に困難です。たとえば、粗い(完全な鏡面ではない)表面でバウンスし、さらに別の粗い表面を通って、小さくて明るい光源に達するような経路です。こうした光路はfirefliesとして現れます。たまたま小さくて非常に明るい光源をちょうど良い角度でとらえた1本の光線が、なかなか平均化されずに残る孤立した明るい斑点として見えるものです。

この設定は、光路がすでに鏡面反射でないバウンスをするたびに、表面の粗さを意図的に少しずつ和らげます。厳密には正しくないわずかなバイアスが加わりますが、その代わりにfirefliesが劇的に早く収まり、最終的な画像は多くの場合むしろ見栄えが良くなります。
...
rather than per whole sample).</source>
        <translation>下の値を超える明るさの色チャンネルを持つサンプルについて、色相が変わらないようすべてのチャンネルを一緒に縮小して上限を設けます。CPUと両方のGPUバックエンドに対応していますが、Recursiveモードは CPU と全く同じ挙動になる一方、Wavefrontモードは(サンプル全体ごとではなく光のバウンスごとに適用するという点で)わずかに異なる方法で適用されます。</translation>
    </message>
    <message>
...
Off by default (effectively unlimited). CPU default path tracer only.</source>
        <translation>レイトレーシングでは、理論上は正しいものの極端に明るいサンプル、つまり小さくて強い光源をちょうど良い角度でとらえてしまった光線が、まれに発生します。こうしたサンプルは、それを打ち消すだけの他のサンプルが十分に集まるまで、そのピクセルの平均値を長く支配してしまうことがあります。これがfireflies、画像の中で目立つ孤立した明るい斑点として現れます。

このクランプは、平均化される前の1サンプルの最も明るい色チャンネルの上限を設定し、わずかで制御された不正確さと引き換えに、同じサンプル数でもずっときれいに見える画像を得られるようにします。値を小さくするほど強く抑制されますが、単なるノイズだけでなく、本当に明るい小さな光源まで目に見えて暗くしてしまう危険があります。
...
Only a scene loaded from a .pbrt file has its own accelerator choice to override at all - any other scene always uses its own fixed BVH-style index regardless of this setting (a warning is printed if you pick something else anyway).</source>
        <translation>もしレンダラーが光線1本ごとにシーン内のすべてのオブジェクトを確認しなければならないとしたら、単純なシーンでさえひどく遅くなってしまいます。このインデックスがあることで、光線は明らかに当たらないオブジェクトを素早く読み飛ばし、実際に進む経路の近くにあるわずかなオブジェクトだけを判定すればよくなります。

BVH(近くのオブジェクトを階層状の箱にまとめる方式)とKd-tree(空間そのものを分割する方式)は、同じジオメトリを整理するための2つの異なる実際の戦略です。どちらもまったく同じレンダリング結果になりますが、シーンの形状によって構築とレンダリングの速度が異なります。
...
ACES rolls off bright highlights gently, the way film does, giving a soft, filmic look; Reinhard is a simpler, older way of compressing brightness; None just clips anything too bright straight to flat white, which can look harsh.</source>
        <translation>レンダリングされたシーンの本当の明るさには上限がありません。電球は壁の100倍も明るいことがありますが、画面は決まった範囲の明るさしか表示できません。トーンマッピングは、その膨大な明るさの幅を画面が実際に表示できる範囲に圧縮するためのカーブです。
...
Purely informational: it never changes the rendered image, it just tells you what the renderer actually did.</source>
        <translation>レンダリング終了後に、発射した光線の本数、発生したバウンスの回数、追跡したシャドウレイ(ある点から光源が見えるかどうかを確認する光線)の本数、そして1秒あたりのサンプル数を短くまとめて表示します。
...
debugging, not routine use.</source>
        <translation>GPU側で追加の正当性チェックを行うようにします。GPUが実行されるたびに実際の処理コストがかかります。GPU専用で、通常の使用ではなくデバッグを目的としています。</translation>
    </message>
    <message>
...
Grayed out? This is GPU-only - switch Renderer to GPU on the Settings tab to use it.</source>
        <translation>GPUレンダリング処理自体の内部で、追加の正当性チェックを行うようにします。これにより、放っておけば誤った画像を静かに生成したり、予期しないクラッシュを起こしたりする種類のバグを検出できます。

これはレンダラー自体のGPUコードを開発する人向けのデバッグ補助機能であり、通常のレンダリングには恩恵がありません。実際の処理コストがかかる上、正しくレンダリングされた場合の見た目は変わりません。

グレー表示になっている場合は、これがGPU専用の設定だからです。使用するには「設定」タブでレンダラーをGPUに切り替えてください。</translation>
    </message>
    <message>
...
have their own denoiser).</source>
        <translation>完成したレンダリング結果にAIデノイザーを適用し、ノイズをなめらかにします。各ピクセルの基本色や表面の向きに関する追加情報を利用することで、単純なぼかしよりも高品質な結果が得られます。GPU専用で、両方のGPUモード(RecursiveとWavefrontはそれぞれ専用のデノイザーを持ちます)に対応しています。</translation>
    </message>
    <message>
...
Grayed out? This needs the GPU recursive backend - switch Renderer to GPU (and GPU Backend to Recursive) on the Settings tab to use it.</source>
        <translation>サンプル数が少ないと、レンダリング結果は本質的にノイズが多くなります。そのため、サンプル数を増やすほど画像はきれいになりますが(レンダリングは遅くなります)。

デノイザーは、そのノイズを認識して後から取り除くよう学習されたAIモデルで、追加の光線を追跡することなく、素早くきれいな画像を得る手段です。ただし、細部が多少犠牲になります。右側の数値は、ノイズが残った元の画像と完全にデノイズされた結果との間をブレンドします。0が完全にデノイズ(既定値)で、値を上げるほど元のノイズを残します。フル強度のデノイズで残したかったテクスチャまで消えてしまう場合に便利です。

グレー表示になっている場合は、これがGPUのRecursiveバックエンドを必要とする設定だからです。使用するには「設定」タブでレンダラーをGPUに(GPUバックエンドをRecursiveに)切り替えてください。</translation>
    </message>
    <message>
...
default path tracer only.</source>
        <translation>フレーム全体に対する0から1の割合で指定した矩形領域に、レンダリングを限定します。CPU・GPUいずれの既定のパストレーサーにのみ有効です。</translation>
    </message>
    <message>
...
Off by default (the full frame). If a loaded .pbrt scene file already requests its own crop region, checking this overrides it with the rectangle below; leaving it unchecked lets the scene&apos;s own request (if any) stand.</source>
        <translation>画像全体ではなく、フレームの中の矩形領域だけをレンダリングします。それ以外の部分は黒いままになります。矩形は、フレームの幅・高さに対する0(左端・上端)から1(右端・下端)までの4つの割合で指定するため、解像度が変わっても同じ形を保ちます。

大きくて処理の重いシーンの、問題のある一部分だけを素早く繰り返し確認したいときに便利です。同じ総サンプル数でも、フレーム全体ではなく一部だけを対象にすれば、はるかに早くきれいになります。
...
same seed. Both CPU and GPU default path tracer only.</source>
        <translation>同じシードで再実行した際に、このレンダリングをバイト単位で
再現できるようにします。CPUとGPUのデフォルトパストレーサーのみに適用されます。</translation>
    </message>
    <message>
...
Off by default (genuinely random every render).</source>
        <translation>レンダリングは通常、毎回異なる乱数列を使用するため、同じ設定であっても同じシーンを2回実行した結果がピクセル単位で一致することはありません。これをチェックすると乱数シードが固定され、同じシード値であれば常にまったく同じ画像が再現されます - シーンを編集する前後を比較したり、見た目の違いがコードの変更によるものか単なるランダムノイズによるものかを切り分けたりする際に役立ちます。
...
other controls on this tab afterward is fine - they simply stop matching
the preset, the same as if you had built the same settings by hand.</source>
        <translation type="vanished">レイトレーシングでよく知られたリファレンスシーンとカメラモーションのプリセットです。シーン、カメラパス、フレーム数、fps、速度を個別に手動設定する必要がないよう、あらかじめ調整されています。
...
Picking one fills in every field below (and the scene on the Basic tab) - you can still change anything afterward, it just stops matching the preset once you do.</source>
        <translation type="vanished">シーン、カメラパス、フレーム数、fps、速度がひとまとめになった既製のプリセットで、すべての設定を自分で細かく選ばなくても見栄えの良い動画になるよう調整済みです。
...
Every path starts from the camera position on the Advanced tab.</source>
        <translation type="vanished">フレームのシーケンス全体でカメラがどう動くかを指定します:
  オービット — シーンの中心を常に見ながら周囲を一周する
  リニア    — シーンの前を直線的に横切る
  8の字     — 連珠形を描き、中央を通過して交差する
  スパイラル — 周回しながら徐々に近づいていく
...
Orbit circles fully around the scene, always facing its center - the classic &quot;turntable&quot; shot. Linear sweeps past in a straight line. Figure-8 traces a lemniscate, crossing back through the middle. Spiral orbits while steadily moving closer. Every path starts from wherever the camera is positioned on the Advanced tab.</source>
        <translation type="vanished">フレームのシーケンス全体でカメラがどのように動くかを指定します。
...
Paired with Frames Per Second below to determine the video&apos;s total length in seconds.</source>
        <translation type="vanished">動画を構成する個々の画像の枚数です。各フレームは完全に独立した1回のレンダリングなので、この値が総レンダリング時間に直接比例します(同じ設定で100フレームなら、1枚の画像のおよそ100倍の時間がかかります)。
...
Doesn&apos;t change how many frames get rendered (that&apos;s Frame Count above) - only how fast they play back, and therefore how many seconds long the finished video is (Frame Count divided by FPS).</source>
        <translation type="vanished">レンダリングされたフレームのうち、動画の1秒間に何枚を再生するかを指定します。
...
Speed 0.5x renders twice as many frames to cover the same journey more slowly and smoothly; speed 2x renders half as many frames, covering the same journey faster.</source>
        <translation>カメラの全経路を何フレームに分けて描くかを調整する倍率です。経路自体が変わるわけではなく、常に同じ完全な移動を行います。
...
速度0.5xでは、同じ経路をよりゆっくり滑らかにたどるため、レンダリングするフレーム数が2倍になります。速度2xでは、同じ経路をより速くたどるため、フレーム数は半分になります。</translation>
    ...
    <message>
        ...
        <translation type="vanished">&lt;b&gt;動画の長さ:&lt;/b&gt; %1 秒(%2)&lt;br&gt;&lt;b&gt;カメラパス:&lt;/b&gt; %3、速度に関わらず常に全経路を完走します&lt;br&gt;&lt;b&gt;出力先:&lt;/b&gt; フレームは &lt;code&gt;output/frames/&lt;/code&gt; に保存されます</translation>
    </message>
    <message>
        <source>ℹ️ Requirements</source>
        <translation type="vanished">ℹ️ 必要要件</translation>
    </message>
    <message>
        ...
        <translation type="vanished">&lt;b&gt;ffmpegが必要です:&lt;/b&gt; 動画のエンコードにはffmpeg(libx264)を使用します。インストール済みで、PATHが通っている必要があります。&lt;br&gt;&lt;small&gt;レンダリングログで「見つからない」と表示された場合は、&lt;a href=&quot;https://ffmpeg.org/download.html&quot;&gt;ffmpeg.org&lt;/a&gt; から入手してください。&lt;/small&gt;&lt;br&gt;&lt;br&gt;&lt;b&gt;自動結合:&lt;/b&gt; 全フレームのレンダリング完了後、動画は自動的に結合され、自動的に開かれます。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation type="vanished">&lt;b&gt;手順1:&lt;/b&gt; 動画設定を行う(カメラパス、フレーム数、FPS)&lt;br&gt;&lt;b&gt;手順2:&lt;/b&gt; ベーシック/詳細設定タブで品質設定を行う&lt;br&gt;&lt;b&gt;手順3:&lt;/b&gt; 「動画レンダリング開始」をクリックして待つ&lt;br&gt;&lt;b&gt;手順4:&lt;/b&gt; 完了すると動画が自動的に結合・オープンされます!&lt;br&gt;&lt;br&gt;&lt;b&gt;ヒント:&lt;/b&gt;&lt;br&gt;• より高速にレンダリングするにはGPUモードを使用&lt;br&gt;• クイックプレビューにはサンプル数/ピクセルを少なめに(10~50)&lt;br&gt;• 本番品質にはサンプル数/ピクセルを多めに(100~500)&lt;br&gt;• 目安のレンダリング時間: 1~5分(GPU)、15~60分(CPU)</translation>
    </message>
    <message>
        <source>Video Settings</source>
        <translation type="vanished">動画設定</translation>
    </message>
    <message>
...
The alternates below trade that generality for a specific technique - photon mapping, bidirectional/Metropolis light transport, or a handful of unbiased reference and debug integrators. All are CPU-only except SPPM, and none can be combined with Generate Video mode. Sampler/Spectral/Exposure/Tonemap/Stats above only affect this default Path Tracer.</source>
        <translation type="vanished">このアプリの他の場所でも使われている汎用の重要度サンプリング付きパストレーサーです - Next Event EstimationとBSDF重要度サンプリングを、パワーヒューリスティック(MIS)で組み合わせています。十分に検証されたデフォルトです。特別な理由がない限り、まずはここから始めてください。
...
GPU: capability-checked per scene - Lambertian/DiffuseLight, RoughDielectric, Metal, Dielectric, Conductor, RoughMetal, and DiffuseTransmission are supported (area lights only); an unsupported scene falls back to an error - use CPU SPPM instead.</source>
        <translation type="vanished">Stochastic Progressive Photon Mapping(pbrt-v4スタイル)です。通常のパストレーシングでは解決が難しい強いコースティクス/ガラスのシーンに最適です。

CPU: Cornell Rough Glassシーンでエンドツーエンドの検証済みです。それ以外のシーンは未検証で、lambertianとdelta-BSDFマテリアルのみをサポートします。
...
CPU only. Area lights only (no punctual/sky-light NEE yet). Verified end-to-end on the Cornell Box scene only; other scenes are unverified.</source>
        <translation type="vanished">双方向パストレーシング - カメラと光源の両方からサブパスを追跡し、すべてのペアを接続します。通常のパストレーシングだけでは苦手とする、一部の難しい照明構成でより有効です。
...
CPU only. Same area-lights-only scope and single-scene (Cornell Box) verification as BDPT.</source>
        <translation type="vanished">BDPTのサブパス機構の上に直接構築されたMetropolis Light Transportです - マルコフ連鎖を使って、すでに寄与している光路にサンプルを集中させます。見つけにくい明るい光路を持つシーンに有効です。
...
CPU only.</source>
        <translation type="vanished">pbrt-v4の不偏な参照用パストレーサーです - 一様球面サンプリングを使用し、Next Event Estimationも多重重要度サンプリングも使用しません。デフォルトのパストレーサーよりシンプルでノイズが多く、他のインテグレータを検証するための正解基準として有用です。
...
CPU only.</source>
        <translation type="vanished">実際に照明されたレンダリングではなく、可視化・デバッグ用のモードです - マテリアルの色や間接光を完全に無視して、各点が近くのジオメトリによってどれだけ遮蔽されているかを測定します。
...
CPU only. NEE, when enabled, is area-lights-only, the same scope as BDPT/MLT.</source>
        <translation type="vanished">pbrt-v4の標準的な参照用パストレーサーです - Next Event Estimationのオン/オフとBSDF重要度サンプリングのオン/オフを選択でき、いずれもデフォルトで有効です(下のトグルを参照)。
...
CPU only.</source>
        <translation type="vanished">pbrt-v4の最もシンプルなボリュームパストレーサーです - 純粋なデルタトラッキングのみで、NEE/MIS/表面BSDFは使用しません。

この実装では到達可能ですがメディア(媒質)には対応していないため、通常の固体ジオメトリのシーンではカメラレイが直接光源に当たる場合を除き、ほぼ黒くレンダリングされます - これはメディアのないシーンにおけるpbrt-v4本家の挙動と一致します。
...
CPU only. Area lights only.</source>
        <translation type="vanished">純粋なライトトレーサーです - ここにある他のすべてのインテグレータとは逆方向に動作します。各サンプルはカメラではなく光源から出発し、カメラとの接続による寄与をフィルムにスプラットします。
...
The alternates below trade that general-purpose approach for a specific technique - simulating light as bouncing particles, tracing extra paths starting from the light itself, or a handful of plain reference/debug modes used mainly for testing. All of them run on the CPU only except one (SPPM), and none can be combined with Generate Video mode. The Sampler/Spectral/Exposure/Tonemap/Stats settings above only affect this default Path Tracer.</source>
        <translation>このアプリがプログラム全体で使う、汎用的な光のシミュレーション方式です。各バウンスで、光源に直接光線を向ける処理(単純な照明を素早くきれいにします)と、表面が実際に光を反射する方向に合わせた方向へ光線を送る処理の両方を行い、その2つの結果をブレンドすることで、どちらか片方だけよりも斑点状のノイズが少なく収束します。十分にテストされた既定の方式なので、特に理由がなければまずここから始めてください。
...
GPU: only certain materials are supported, checked per scene - matte surfaces and plain light sources, plus the Rough Dielectric, Metal, Dielectric, Conductor, Rough Metal, and Diffuse Transmission material types (and only with lights that have a physical size or shape, not point or sky lights). A scene using anything else shows an error - use the CPU version of this mode instead.</source>
        <translation>光を、シーン内を跳ね回りながらカメラの近くに落ち着いていく粒子の集まりとしてシミュレートします。ガラスを通した光や水中の光(プールの底に揺れる光の模様など)のような、明るく集光したパターンのレンダリングに特に強く、通常のパストレーシングではきれいにするのが難しいシーンに向いています。

CPU版: Cornell Rough Glassシーンで正しく動作することを確認済みです。それ以外のシーンは未確認で、対応するのはつや消し表面と、完全な鏡面またはガラスのようなマテリアルのみです。
...
CPU only. Only works with lights that have a physical size or shape (point lights and a sky/environment light aren&apos;t supported yet). Confirmed to work correctly on the Cornell Box scene only; other scenes haven&apos;t been checked.</source>
        <translation>カメラと光源の両方から部分的な光路を構築し、それらすべての組み合わせをつなぎ合わせます。狭い隙間を通り抜ける光のような、扱いの難しい照明設定を、カメラだけから追跡するよりもうまく処理できます。
...
CPU only. Same lights-must-have-a-physical-size-or-shape limitation and single-scene (Cornell Box) verification as Bidirectional Path Tracing above.</source>
        <translation>上のBidirectional Path Tracingと同じ経路接続の考え方を基盤としていますが、実際に寄与する光路を見つけると、その近くで小さくランダムな変化を加えながら似たような光路をさらに探し続けます。ほとんどの光が、見つけにくいごく一部の経路からしか届かないようなシーンで役立ちます。
...
CPU only.</source>
        <translation>既定のPath Tracerがノイズを早くきれいにするために使っているような工夫を一切使わず、光線を表面で完全にランダムな方向へバウンスさせるだけの、最も基本的なレンダラーです。仕組みはシンプルですが、画像がノイズだらけの状態から抜け出すまでにずっと長くかかります。主に、他のモードが正しい結果を出しているかを確認するための、信頼できる基準として使われます。
...
CPU only.</source>
        <translation>完成した画像というより可視化・デバッグ用のモードです。周囲のオブジェクトによる遮蔽をもとに、表面上の各点がどれだけ囲まれているか、あるいは開けているかを示します。部屋の隅に見られる柔らかい影に似ています。マテリアルの色やバウンスした光は一切考慮しません。
...
CPU only. When &quot;aim at lights&quot; is on, it only works with lights that have a physical size or shape - the same limitation as Bidirectional Path Tracing and Metropolis Light Transport above.</source>
        <translation>2つの任意の工夫(既定ではどちらもオン。下のトグルを参照)を備えた、シンプルな基準用パストレーサーです。工夫の内容は、光源に直接一部の光線を向けてノイズを早くきれいにすることと、表面が光を反射する上で重要な角度にバウンス方向を偏らせることです。
...
CPU only.</source>
        <translation>煙や霧のような透過するボリュームをレンダリングするための、最もシンプルなモードです。何もない空間を進み続け、偶然何かに当たるまで処理を続けます。光源に光線を向けることも、既定のPath Tracerが使うようなノイズ低減のブレンドも行わず、不透明な表面にはまったく対応していません。

このアプリには現在このモードでレンダリングできる煙や霧が用意されていないため、通常の不透明オブジェクトのシーンでは、カメラの光線がたまたま光源を直接見ている場合を除き、ほとんど黒い画像になります。これは、レンダリングするものが何もないときのレンダラー本来の挙動どおりです。
...
CPU only. Only works with lights that have a physical size or shape.</source>
        <translation>ここにある他のどのモードとも逆の手順で動作します。各光線をカメラから始めるのではなく、光源から始めて外側へ向かって追跡し、経路がたまたまカメラに接続したときにその寄与を画像に加えます。
...
<context>
    ...
    <message>
        ...
        <translation type="vanished">共有のコーネルボックスデータからボックスを構築します。5枚のlambertian壁、diffuse_lightの天井クアッド、白いlambertianの回転軸並行ボックス、そして完全なFresnel加重の反射/屈折を行うdielectric BSDFの球体で構成されます。このレンダラーの基準となる収束テストであり、壁面同士の拡散相互反射、BSDFサンプリングの正しさを確認する鏡面/透過オブジェクト、そしてNext Event Estimation(NEE)を働かせるエリアライトクアッドを備えているため、マテリアルやライトサンプリングに関わる横断的な不具合の多くがまずここで最初に現れます。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">「Ray Tracing in One Weekend」の締めくくりとして有名なシーンです。チェッカーテクスチャのlambertian地面球体の上に、BVHで高速化された小球体の群れが並びます。ほとんどは拡散反射ですが、一部は時間方向に2つ目の中心をオフセットして持たせることで、球体自体がリニアなモーションブラーを持ちます。一部は球体ごとにランダムなfuzz半径を持つメタル、残りはdielectricです。手前には基準となる鏡面/拡散の両極端として、シャープなdielectric、平坦なlambertian、fuzzゼロの完全な鏡面メタルという3つの大きな特徴球体が置かれています。主にBVH構築と、fuzzyメタル反射モデルと並ぶモーションブラー付きレイ-球体交差判定の負荷テストです。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">巨大な2つの球体(半径10、原点の上下にそれぞれ配置)が、球面UV座標を通してサンプリングされる1つのプロシージャルなチェッカーテクスチャを共有し、単純なlambertianでシェーディングされます。このパターンは画像参照ではなく、3D位置から解析的に生成されます。下側の球体の頂部には、拡散反射・わずかなfuzzを持つ光沢メタル・dielectricという数個の小さなアクセント球体が置かれ、スケール感を出すとともに、フレーム内に実際の鏡面/屈折の振る舞いを与えています。主に、大きな球面曲率のもとでもプロシージャルテクスチャの3D-to-チェッカーマッピングが破綻しないかを確かめるテストです。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">地面と大きな球体はどちらも、ワールド空間座標から直接評価されるPerlinノイズ/タービュランスベースのプロシージャルな大理石テクスチャを使用し、lambertianに供給されます。画像テクスチャは一切使用していません。対比のために、より高いノイズ周波数で同じ手法を再利用した2つの小さな随伴球体があり、Next Event Estimation(NEE)でサンプリングされるdiffuse_lightクアッドによって照らされます。Perlinノイズによるタービュランス/大理石の出力が、単純な環境光ではなく実際のNEE照明のもとでBSDFパイプラインに正しく供給されるかをテストします。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">それぞれ異なる基底ベクトルによって異なる向き(正面向き、側面向き、上面向き)を持つ5枚のクアッドプリミティブがあり、それぞれ単色のlambertianを持ちます。これは複雑なシェーディングではなく、クアッドの平面交差判定と向きごとの法線/UV計算をテストするものです。部屋の中には独立したdiffuse_lightクアッドが浮かんでおり、シーン唯一の発光体としてNext Event Estimation(NEE)の重点サンプリング対象になります。主に、実際のエリアライトサンプリングのもとでのクアッド形状と向き依存の法線の正しさを確認するテストです。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">大理石テクスチャシーンで使われたPerlinノイズの地面/球体構成を再利用していますが、本当の主題は発光です。diffuse_lightマテリアル(暖色)を与えられた球体が、曲面で非平面のエリアライト発光体として機能し、それとは別に寒色のdiffuse_lightクアッドも配置されています。球体とクアッドという2種類の発光体が同じシーンに存在することで、ライトサンプリングが通常の平坦なクアッドの場合だけでなく、異なる発光体形状にわたって立体角サンプリングを正しく扱えるかをテストします。色温度を分けることで、どちらの光がどこに寄与しているかが一目でわかるようになっています。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">標準的なコーネル壁と、その専用の大きめの天井diffuse_lightクアッドを再構築したうえで、通常の中実ボックスの代わりに、constant_mediumで包まれた回転・平行移動済みの2つのボックスに置き換えています。constant_mediumは、低密度かつ独自の色味を持つ均質な等方散乱の参加媒質で、古典的なモノクロの煙ではありません。これはボリュームメディウムのテストです。他のシーンでは中実のlambertian面として使われるボックス形状が、ここでは代わりに等方的な内部散乱ボリュームの境界を定義しており、メディウムサンプリングと境界形状の交差判定が正しく連携するかを確認します。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">このセットの中で最も機能が詰め込まれたシーンです。ランダムな高さのlambertianボックスが並び、起伏のあるBVH構築された地面を形成し、天井クアッドのエリアライト、モーションブラー付きlambertian球体、dielectricのガラス球体、fuzzyなメタル球体、dielectric球体の内部に閉じ込められた色付きconstant_medium(独立した局所的な霧のボリューム)、巨大な包囲球体を覆うほぼ密度ゼロの第2のconstant_medium(かすかな全体的大気)、ミップマップされた画像テクスチャの地球球体、Perlinノイズの大理石球体、そして回転したBVIサブツリーに詰め込まれた小さな白い球体の集まりで構成されています。モーションブラー、dielectric/metal/lambertianの各BSDF、局所的および全体的な参加媒質、画像テクスチャのミップマッピング、そして多階層BVH構築を、1つのレンダリングの中ですべて確認できます。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">同じ金色のrough_metalマテリアル(GGX/Trowbridge-Reitzマイクロファセット分布に、複素IORによる完全なFresnelではなく単純なRGB色付き反射率を組み合わせたもの)で、粗さ(roughness)の値だけを変化させた5つの球体が一列に並んでいます。1つのエリアライトの下、地面プレーン上に単独で置かれているため、フレーム内で変化する唯一の要素は、粗さが増すにつれて鏡面ハイライトがどう広がり弱まっていくかだけです。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">rough-metal-spheresシーンと同じrough_metalのGGX BxDF(異なる粗さのブラッシュドアルミニウムのボックスとブラッシュドゴールドの球体)を使用していますが、開放的なスタジオ設定ではなく、おなじみのコーネルボックスの中に配置されています。そのため、光沢のあるマイクロファセット反射は、単一のオーバーヘッドライトではなく、実際の間接バウンス照明や色付きの壁からの色滲みのもとで整合性を保つ必要があります。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">コーネルボックスのガラス球体を、完全なdielectricからrough_dielectricに置き換えたシーンです。GGXマイクロファセットが反射ローブと透過ローブの両方に適用されるため、屈折光は鋭いスネルの法則によるレイではなく、すりガラスのようにぼやけて散乱します。それでも各マイクロファセットではFresnelによる反射/透過の分配が正しく守られています。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">研磨された金とアルミニウムの表面に、conductorマテリアルを使用しています。RGBアルベドによる近似ではなく、実測されたeta/kスペクトルに基づくチャンネルごとの実際の複素Fresnelを使用しているため、メタルの色はアーティストが選んだ色味ではなく、実際の反射率スペクトルから生じています。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">coated_diffuseを使用した、コート層付き拡散反射の球体とボックスです。Lambertianのベース層の上に粗いdielectricコートが重ねられています。これは層状BSDFの解法をテストするもので、光はコート表面で鏡面反射することも、コートを透過してベースの拡散色を拾うこともでき、2つの層の間では内部反射も起こります。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">thin_dielectricを使用した垂直なガラス板が、コーネルボックスを2つに分割しています。これは厚みゼロのガラスで、実際の屈折面の代わりに解析的な多重反射Fresnel計算式を用いるため、透過するレイは屈曲せずまっすぐ通過します。IOR 1.5のFresnel反射率はほぼ視線に対して斜め(グレージング角)に近い角度でしか目立たないため、この板は意図的に傾けて配置されています。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">coated_conductorによる漆塗りメタルの質感です。金の球体と銅のボックスはそれぞれ、チャンネルごとの複素Fresnelを持つGGX conductorベースの上に粗いdielectricコートを重ねています。このカテゴリの中で最も層が重なったBxDFであり、無彩色の光沢コートの下にスペクトルによるメタル反射率を組み合わせることで、メタル自体の色味の上にコートのFresnelによる艶が乗る様子を示しています。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">diffuse_transmissionを使用した、蝋のような質感の球体で、反射色と透過色が別々に指定されています。光は同じ半球(反射)と反対側の半球(透過)の両方に拡散的に散乱します。これは、subsurfaceスラブのシーンが同じ見た目のゴールに対してボリュームによるランダムウォークを必要とするのに対し、それを必要としない、半透明性の安価な2パラメータの代替表現です。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">normalized_fresnelを使用した水晶のような球体です。これはFresnel加重の拡散射出であり、視線に対して斜めの角度(グレージング角、Fresnel透過率が低いため内部で散乱した光がより多く抜け出す方向)に近づくほどBSDFの値が上昇します。サンプリング自体は余弦半球サンプリングのままですが、散乱PDF自体がFresnel加重を担っており、MIS(多重重点サンプリング)のもとで反射エネルギーの総量が正しく保たれるよう正規化されています。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">principledマテリアルのパラメータ空間を一列で網羅する7つの球体です。マットな拡散反射、低粗さのプラスチック、クリアコート付きプラスチック、半メタリック、粗さのある準メタリック、滑らかな完全メタル、クリアコート付き完全メタルという構成で、metallic・roughness・clearcoatをそれぞれ独立に変化させ、このカテゴリの他のシーンで個別のマテリアルとして扱われているdielectricからconductorまでの全範囲を、1つの統一されたBSDFがどうカバーするかを示しています。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">hairマテリアル(HairBxDF)を、それぞれ異なる吸収率/粗さのパラメータで使用した5つの球体です。拡散的というより、強く前方/鏡面方向に散乱するファイバーBSDF向けに調整されています。hairのBSDFのピーク応答は通常のエリアライト強度では白飛びしてしまうため、このセットの他のシーンよりもかなり暗く光量を再調整する必要がありました。実際のカーブ形状ではなく球体の表面に適用することで、BxDFのシェーディング挙動をカーブ交差判定の仕組みから切り離してテストしています(カーブ交差判定の方はCurve Fibersシーンを参照してください)。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">単純なLambertian面に対する2種類の摂動技法です。バンプマッピングはPerlinノイズによる大理石テクスチャ(背面壁とボックス)からシェーディング法線をずらし、法線マッピングはチェッカーパターン(球体)からシェーディング法線を摂動させます。幾何形状自体は平坦/球形のままですが、照明計算に使われるシェーディング法線が曲げられることで、細かい表面のディテールを疑似的に表現しています。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">厳密には真のBSSRDFではありません。乳白色の蝋のスラブと翡翠の球体に入った光は、dielectricの殻を通過してconstant_medium(独自の密度と色味を持つ均質な参加媒質)へと入っていきます。そのため、この半透明な輝きは拡散ベースのサブサーフェス項ではなく、表面での屈折Fresnelに境界づけられた物体内部での実際のボリュームによるランダムウォークから生じています。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">pbrt-v4の実測BRDFデータパイプラインをテストするシーンです。5つの球体の列にわたって、実際に計測されたマテリアルではなく、合成生成されたテーブル化NDF、sigma、VNDF、輝度、波長ごとのスペクトルテーブルを使用しています。BSDFの評価自体は、完全な5次元重点サンプリングのワープ連鎖ではなく、単色の余弦半球サンプリングに簡略化されているため、このシーンは現実的な実測マテリアルの結果というより、テーブル化データの配線経路そのものをテストするものです。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">文字通りの三角ガラスプリズムで、分散性のあるdielectricを使用しています。これは一定のIOR値の代わりに、実際のクラウンガラスに合わせて波長ごとに変化するコーシーの式による屈折率を持ちます。プリズムに入った白色光は出射時に波長ごとに分かれ、実物のガラスプリズムのように受光スクリーン上で扇状に広がります。ただしこれは、レンダラー内で波長ごとにレイを追跡する--spectralを有効にした場合に限られます。無効の場合は、RGBの各チャンネルがそれぞれわずかに異なる固定量で屈折するだけで、連続的な広がりにはなりません。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">B23と同じ分散プリズムのシーンですが、滑らかなガラスの代わりにすりガラス(rough_dielectric)にしたものです。--spectral では、キャッチャースクリーン上に同じ虹色の扇が現れますが、表面の粗さによって鮮明ではなくぼやけて見えます - 背後の物理法則(波長依存の屈折率)は同一で、上乗せされたマイクロファセット散乱だけが異なります。すりガラスで正しく分散させるには、B23を単純に見た目だけ変えるだけでは不十分でした。滑らかなガラスと異なり、粗い表面の光サンプリングは、このシーンの指向性ビームのような軸外の光源に明示的なNext Event Estimationで到達する必要があるため、初期の屈折だけでなく、そのサンプリング処理自体も波長を意識したものにする必要がありました。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">pbrtの「mix」マテリアルは、単純に混合した色を出すのではなく、各シェーディング点においてサンプルごとに実際の確率的選択を行うことで2つのマテリアルをブレンドします。各ヒットで確率に応じた選択が行われるため、マットな拡散反射とメタリックなconductorの混合は、本物の粒状のノイズとして表れます。あるサンプルは純粋な拡散反射、別のサンプルは純粋な鏡面反射となり、多数のサンプルにわたって正しく平均化されます。CPU、GPU再帰型、GPU wavefront型のいずれでも同一の動作をします。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">このアプリの他のどこにもまとまって登場しない4種類のマテリアルです。thindielectric(通常のdielectricと異なり、レイの位置をずらさずに屈折させる厚みゼロのガラス板)、coatedconductor(メタルベースの上のクリアコート)、diffusetransmission(薄い葉や紙のように、光が反射だけでなく透過もする)、そして名前付きプリセット(「Marble」)によるsubsurface scatteringです。外部の実測データファイルを必要とせずに、本物のサブサーフェスパラメータを実現しています。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">pbrtのCoatedDiffuseマテリアル(ニス塗りの木材のように、クリアなdielectricコートの下に拡散反射ベースを持つマテリアル)が、単色ではなく実際の画像テクスチャから反射率を読み取っています。以前はこの組み合わせで、両方のバックエンドともテクスチャが警告なく無視され、単色グレーにフォールバックしてしまっていました。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">名前付きプリセットではなく、明示的なRGBのeta/k(金属特有の色味を持つ波長依存の反射率を生み出す複素屈折率)によって駆動されるconductorマテリアルです。加えて、同じ複素IOR計算がcoatedconductorのメタルベース層にも適用されており、単純な鏡面色ではなく正確な色付きの鏡面ハイライトを生み出しています。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">DiffuseTransmission(薄い葉やランプシェードのように、光が拡散的に反射も透過もするモデル)を使用しています。反射率と透過率がそれぞれ、単色ではなく実際のテクスチャに紐づけられており、両方のバックエンドで動作します。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">pbrtのMaterial「hair」です。これは実際の映画制作用レンダラーでも使われている、物理ベースのMarschner/Chiangファイバー散乱モデルによるhair BSDFです。ここでは単純な球体に適用することで、このプロジェクト独自のHair Fibersデモとの、条件を揃えた並列比較を可能にしています。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">pbrtのチェッカーボードテクスチャで、2色それぞれが単純な固定色ではなく、実際の画像(checker/mixテクスチャの内側に1階層ネストされたimagemapテクスチャ)に紐づけられています。テクスチャ参照が単独で存在するだけでなく、合成・入れ子にできることをテストします。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">pbrtのNamedMaterialが、「mix」マテリアルの構成要素としてだけでなく、形状に直接参照されています(一度宣言し、名前で再利用する形)。加えて、そのマテリアルのパラメータの1つにテクスチャが紐づけられており、さらにtwosidedフラグが設定されたAreaLightSourceにより両面が発光します。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">7つの球体を、名前付きガラスIORプリセット(BK7、BAF10、FK51A、LASF9、F5、F10、F11)ごとに1つずつ用意したシーンです。各プリセットはFindGlassPreset()によって、レンダリングの基準波長における固定のdielectric IOR値に解決されます。これは実在の光学ガラスカタログで使われているのと同じ参照テーブルであり、以前はユニットテストからのみ到達可能で、このシーンが登場するまで実際にレンダリングされたことはありませんでした。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">同じソース画像と同じUVスケールを持つ4枚のクアッドで、それぞれimagemapテクスチャの1つのオプションだけを切り分けています。基準となるデフォルトのsRGBデコード+wrap-repeat、同じバイト列をより明るく読み取るlinearデコード(通常は中間調を圧縮するガンマカーブを飛ばす)のクアッド、[0,1]を超えてタイリングする代わりに端のテクセルを保持するclamp-wrapのクアッド、そしてチャンネルを反転させたクアッドです。これらの経路はいずれも、他のどの同梱シーンのデフォルトのテクスチャ設定でも到達できないものでした。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">左から右へ並んだ4枚のクアッドは、それぞれビルダーには組み込み済みでありながら、これまでどの同梱シーンでも使われたことのなかった異なるpbrt-v4プロシージャルテクスチャクラスによって駆動されています。windy(パラメータのない乱流)、wrinkled(オクターブと粗さを調整可能なPerlin乱流)、dots(Perlinノイズの境界で分かれた2つの単色)、bilerp(UV全体になめらかにブレンドされる4隅の色)です。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">2枚のクアッドで、2階層の入れ子になったテクスチャ(外側のチェッカーボードのtex1がさらに別のチェッカーボードで、そのtex1が実際の画像である)を、並べて再現したより単純な1階層のケースと比較しています。CPUでは本物の入れ子で、同じ写真の異なる2つのチェッカー周波数が実際に互いの内側でタイリングされます。GPUは意図的に入れ子構造全体を1つの平均色に平坦化し、代わりに警告を出します - これはバグではなく文書化された近似であるため、両方の方式でこのシーンをレンダリングすると、実際の違いが明らかになります。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">局所的な発光体ではなく、プロシージャルなHDR空のグラデーション(青い天頂から暖色の地平線へのフェード)が画像ベースの無限光として組み込まれ、それのみで照らされる開放的なシーンです。拡散反射の球体、ほぼ鏡面のメタル球体、dielectricの球体が下に置かれ、この環境照明がソフトなアンビエントオクルージョン風のシェーディングから、空のグラデーションが鋭く反射・屈折して映り込む様子まで、異なるBSDFでどう見えるかを示しています。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">何もないコーネル壁が、天井から真下に向けられた単一のスポットライト風の点光源のみによって照らされています。全体のコーン幅は広く、内側の減衰開始角はより狭く設定されているため、輪郭のはっきりした光の円盤ではなく、滑らかな半影が生まれます。エリアライトを使うコーネルシーンとは異なり、これはサンプリングすべき面を持たないデルタライトです。コーン形状と減衰の仕方こそがこのシーンの要点です。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">何もないコーネル壁が、太陽光のような平行光線であるdistant lightによって照らされています。これは固定された方向ベクトルを持ち、1/r^2の減衰がないため、放射輝度のスケールは距離を補正した強度ではなく、直接入射照度そのものになります。減衰するpoint/spot/gonioライトと並んで、指向性を持ち減衰しないライトも正しく扱えるかを検証するための、意図的な対比シーンです。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">コーネル壁が、頭上に置かれた単一の等方性point lightによって照らされています。古典的な1/r^2の強度減衰を持つ、このカテゴリの中で最も単純な点光源です。コーンもなく、方向依存のプロファイルもなく、平行光線でもなく、天井付近の1点から距離のみに基づいて減衰するだけの光です。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">コーネル壁が、goniometricライトによって照らされています。これは一様に発光するのではなく、テーブル化された画像に従って方向ごとに強度が変化する点光源で、通常のpoint lightと同じ1/r^2の減衰と、プロファイルをワールド空間で向けるための明示的な回転を組み合わせています。スポットライトの単純なコーンやpoint lightの一様な発光とは対照的な、任意方向強度のケースです。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">コーネル壁が、projectionライトによって照らされています。これは単純なコーンや一様な球状の光を放つのではなく、透視錐台を通してチェッカーボードのスライド画像を投影する点光源で、実物のスライドプロジェクターのようにシーンにくっきりしたパターンのビームを投げかけます。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">天井のライトクアッドを一切持たないコーネル風の部屋です。代わりに背面の壁に実際に長方形の穴が開けられており、その窓を通してのみ空の光が見えます。これはポータルサンプリングされた無限光をテストするもので、環境光のサンプリングは全球方向ではなく、実際にポータル開口部を通して見える方向のみに限定されます。無限光のNext Event Estimation(NEE)にとって、より難しく破綻しやすいケースです。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">pbrt-v4の点(デルタ分布)光源5種類すべて、point、spot、distant、goniometric、projectionを1つのシーンにまとめています。面積ゼロの光源タイプをすべて並べて比較できる、単一のリファレンスシーンです。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">pbrtのgoniometricライトおよびprojectionライトの、実際の画像デコード機能をテストするシーンです。以前はfilenameパラメータが警告なく無視され、一様なビームにフォールバックしてしまっていました。このシーンは、これらのライトが現在では実際に画像を読み込んで適用していることを確認する回帰テストです。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">pbrtの「blackbody L」ライトカラーで、RGBの三つ組ではなくケルビン単位の色温度で指定され、実際のプランク軌跡に基づく黒体放射からRGBへの変換計算を通して求められます。それ以外は同一の2枚のパネルが、白熱灯のような暖色の色温度と、曇り空のような寒色の色温度でそれぞれ並べて置かれているため、色変換に不具合があれば両パネルが単なる白になってしまい、すぐに気づけるようになっています。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">三角形以外の形状(球体やクアッド)に設定されたAreaLightSourceで、filenameによってテクスチャ化されている(発光パターンが単色ではなく画像から来る)と同時に、twosidedでもある(表面法線が向く側だけでなく両面から発光する)ものを、両方のバックエンドでテストします。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">pbrtの「infinite」ライトです。形状を一切持たない単色のライトで、あらゆる方向から一度にシーンを照らします。空の代わりとして考えられる中で最も単純なものです。閉じた部屋ではどの方向からも遮られてしまうため、この効果を見るには開放的な形状が必要です。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">diskとcylinderの形状が、本物のNext Event Estimation(NEE)光源として機能します。バウンスのたびに偶然当たるのを待つのではなく、ライトに向けた立体角サンプリングを行います。両方のGPUバックエンドで、NEEに対応しないライトが受けるノイズの多い「レイが偶然当たることを期待する」フォールバックよりも、CPU側と同じくらいきれいに収束します。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">1つのシーンに2つの独立した球体エリアライトを配置した、最小限のケースです。かつて実在したGPU側のバグを特定したシーンで、ライトタイプの参照テーブルが1つのライト分のサイズしか確保されていなかったため、2つ目以降のライトはすべて自身のタイプを警告なく誤って読み取り、誤ってレンダリングされていました。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">不規則な5三角形のファン形状をしたエリアライトです。単純な平行四辺形ではなく、レンダラーのクアッド結合最適化(2つの三角形を1つのクアッドに再結合し、より安価にサンプリングする仕組み)でも結合できない形状です。そのため、より一般的なクアッド用のショートカットではなく、GPU上での本物の三角形単位のライトサンプリングをテストします。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">pbrtの「ColorSpace」ディレクティブは、「blackbody」の色温度がどの作業用RGB色空間(ここではデフォルトのsRGBではなく、より広い原色を持つRec.2020)を通して変換されるかを選択します。ジオメトリと2500K/9000Kの色温度はBlackbody Lightの例とまったく同一で、宣言された色空間のみが異なります - そのため両シーンのパネルの色を直接見比べることで、このディレクティブが単に解析されて無視されているのではなく、実際に適用されていることを直接証明できます。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">本物のpbrt-v4 PortalImageInfiniteLightです。等面積の環境マップ無限光を、ウィンドウ(開口部)のクアッドに制限したもので、あらゆる方向から環境全体が漏れ込むのではなく、その開口部だけが実際の空のディテールを映し出します。ネイティブの「Portal Infinite Light」シーン(C7)は、平坦なsky_lightの背後の壁に幾何学的な穴を開けるだけで、このクラス自体は実際には構築していません - このシーンが、実際にこのクラスを構築する最初の同梱シーンです。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">それ以外は同一な3つの球体を、それぞれpoint、spot、areaの1種類のライトだけで照らしたシーンです。明るさは通常のintensity/radianceを直接指定するのではなく、「float power」(ワット単位の総光束)として指定されています。レンダラーは、ライトの種類ごとに異なる立体角の公式(点光源では4π、スポットライトでは円錐の立体角、ランバート発光体では面積×πなど)を通してpowerからintensityへの変換を逆算する必要があります。そのため、いずれか1種類の変換が誤っていれば、シーン全体が一様に間違うのではなく、その球体の明るさだけが目に見えて食い違うという形で現れます。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">projectionライトのスライド画像として、他のすべてのprojectionライトシーンが使う通常の正方形のuv-checker.bmpではなく、意図的に非正方形(8x4)の画像を使用しています。画像とライト自身の視野角との間で実際のアスペクト比の不一致が生じる、最初の同梱シーンです。アスペクト比を正しく保った投影であれば、画像自身の2:1の形状に一致する、はっきりと横長の長方形の投影範囲が映し出されます。ここにバグがあれば、投影範囲が正方形に押しつぶされたり、ライトのfovいっぱいに引き伸ばされたりしてしまいます。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">単純な拡散反射面の上に、彩度の高い近距離の色付きライトを3つ配置したシーンです。--spectralレンダリング下で、それらの重なり合う領域がちょうどsRGBの色域境界に位置するように配置されています。これはまさに、サンプルごとのXYZからRGBへの色域クランプに実在したバグが、誤って暗く・彩度を落としてしまっていた条件です。修正では、この変換をサンプルごとではなくピクセルごとに遅延させるようにしたため、重なり合う領域はここでは白/パステル調へなめらかにブレンドされるはずで、バンディングや彩度不足のくすんだパッチにはならないはずです。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">チェッカーの地面と、複数の奥行きに配置された球体(ガラス1つ、メタル1つ)からなる開放的なシーンを、デフォルトの透視投影カメラに内蔵された薄レンズによるデフォーカスブラーでレンダリングしています。ピント距離にある球体のみがシャープにレンダリングされ、手前と奥の球体はぼやけます。別のカメラクラスを使わない、単純な絞り+ピント距離による被写界深度の経路です。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">チェッカーの地面の上に並んだ球体の列を、明示的な正投影(orthographic)カメラでレンダリングしています。レイは1つの視点から収束するのではなく平行に発射されるため、球体には遠近感による見かけの縮小が生じません。同じ大きさの球体は奥行きに関係なく同じ大きさに見えます。これは平行投影の見た目上の決定的な特徴です。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">中央の発光球体を取り囲むように配置された色付きの球体の輪を、明示的な球面(正距円筒図法)カメラで撮影しています。限られた視野角の錐台ではなく、360度全方位の周囲が1枚のパノラマ画像にマッピングされます。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">チェッカーの地面の上に、奥行きを変えて配置された5つの球体を、理想化された薄レンズの代わりに実際の多素子レンズ処方(面ごとの曲率、厚み、IOR、絞り)から構築された明示的なrealisticカメラでレンダリングしています。レイはレンズスタックを1素子ずつ順にたどって追跡され、閉形式のデフォーカス円近似ではなく、本物のレンズに起因するボケとビネットを生み出します。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">通常のコーネルボックスシーンや他のカメラ比較用コーネルシーンと同じ古典的なコーネルボックス形状を、別のカメラクラスではなくデフォルトの透視投影カメラの薄レンズによるデフォーカスブラーでレンダリングしています。他のカメラ比較シーンと同じ部屋を使うことで被写界深度のみを変数として切り分けており、異なる形状による混乱を招くことなく、この技法の効果を直接比較できるようにしています。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">他のカメラ比較シーンと同じ共有のコーネルボックスを、明示的な正投影(orthographic)カメラで、ボックスの奥行き軸に沿って真正面から見た構図でレンダリングしています。投影が透視ではなく平行投影であるため、透視/被写界深度版のように収束するのではなく、ボックスの辺はフレームの端まで平行なままです。この対比をわかりやすくするために、あえて同じ部屋が使われています。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">同じ共有のコーネルボックスを、ボックスの中心に配置した明示的な球面(正距円筒図法)カメラで巡っています。パノラマは、単一の内部視点から、5枚の壁すべて、天井のライト、ガラス球体を1枚の360度画像にラップします。単独のSpherical Cameraシーンと同じ技法を、共有の比較用形状に適用したものです。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">同じ共有のコーネルボックスを、単独のRealistic Cameraシーンで使われているのと同一の多素子realisticカメラのレンズ処方でレンダリングしていますが、ボックスのはるかに大きなスケールに合わせて絞りを拡大しています。これにより、シーンのスケールが異なっていても、デフォーカスコーンの角度的な大きさ、ひいては見た目のレンズボケが比較可能な状態に保たれています。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">共有される同じCornell Boxですが、露光中にカメラ自体が動きます - 実際のAnimatedTransform補間(pbrt-v4のモーションブラー技術)によって片側からもう片側へキーフレームが設定され、単一の固定カメラ位置ではなく、各レイがシャッター区間内の独自の時刻をサンプリングします。デフォルトのパストレーサーとSPPMでのみ適用されます(BDPT/MLT/デバッグ用インテグレータでは、代わりに最初のキーフレームの静止画がレンダリングされます)。GPUも同様に最初のキーフレームの静止画をレンダリングします。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">透視投影カメラの薄レンズによる被写界深度(レンズ半径とピント距離)を、pbrtファイルのCameraディレクティブから直接読み込んでいます。両方のバックエンドで動作します。このプロジェクト独自のDepth of Fieldシーンと同じ光学モデルですが、こちらはファイル読み込みの経路をテストするものです。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">pbrtのorthographic(平行投影)カメラをファイルから読み込んだシーンです。レイは1つの視点に収束するのではなくすべて平行になるため、異なる奥行きにある同じ大きさの2つの球体は、手前の方が大きく見えるのではなく、画面上で同じ大きさに見えます。これは通常の透視投影による遠近縮小とは逆の効果です。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">pbrtのspherical(等面積)カメラをファイルから読み込んだシーンです。レイは平坦な画像平面を通過するのではなく、単一の点からあらゆる方向に扇状に広がるため、1回のレンダリングで360度全方位の周囲を捉えることができます。どの方向にも実際に何かが見えるよう、閉じた部屋の中に配置されています。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">pbrtのrealisticカメラです。他のすべてのカメラシーンが使用する理想化された薄レンズ/ピンホールモデルではなく、実際の多素子レンズシステムをレンズ処方ファイルから読み込んでいます。レイは実際のガラス素子を通して1つずつ追跡され、本物の被写界深度とレンズ収差を生み出します。これには、組み込みシーンでは決してテストされることのなかったレンズファイル読み込みの経路も含まれます。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">コーネルボックスの内部を均質な参加媒質で満たしています。境界ボックスは一様な散乱係数とHenyey-Greenstein位相関数を持ちます。境界内のすべての点で密度が同一であるため、空間的に変化する密度モデルではなく、基本的な自由行程サンプリング/単一散乱係数によるメディウム経路をテストします。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">焼き込まれたボクセルデータの参照ではなく、点ごとに密度が手続き的に評価される、本物の不均質ボリュームです。マルチオクターブのPerlinノイズに、もやもやとした揺らぎの摂動と高度による減衰を組み合わせて評価されます。レンダリングには、不均一な密度場を扱うためにdelta tracking(ヌルコリジョンサンプリング)を用いており、実際の雲のように底部が濃く、上部が薄くなる様子を再現しています。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">dielectricと内部メディウムの組み合わせ自体を主題としたシーンです。それぞれ屈折するdielectric表面で作られたガラス球体の内部に、異なる密度の均質な参加媒質が詰め込まれています。散乱メディウムが不透明な殻ではなく、実際に屈折する表面に境界づけられているケースをテストします。レイは内部に屈折して入り、内部の霧を散乱・吸収しながら通過し、再び屈折して外へ出る必要があります。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">本物のボクセル単位の散乱データが3Dグリッドに焼き込まれた不均質メディウムです。赤・緑・青の各チャンネルが独立して異なる周波数で生成されているため、一様に色付いた雲のようには見えず、色が空間的に分離して見えます。majorantグリッドによって高速化されたdelta trackingを用いてサンプリングされます。点ごとに解析的に評価されるプロシージャルメディウムとは異なり、これはレンダー空間のボックスからグリッド自身の座標空間へマッピングされた離散的なボクセルデータです。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">pbrtのMakeNamedMedium「cloud」です。3D Perlinノイズによって密度が決まる不均質な散乱ボリュームで、平坦で明らかに偽物とわかる霧の塊を避けるために、実際の雲のレンダリングでも使われるのと同じノイズ駆動の手法です。ファイルから読み込まれ、両方のバックエンドで動作します。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">cylinder形状に付与された均質な参加媒質(一様な霧/散乱密度)です。この組み合わせは以前、GPUバックエンドでは霧の効果が一切なく、警告なく単なる空の形状としてレンダリングされてしまっていました。このシーンは、GPU側でもこのメディウムが実際に機能するようになったことを確認する回帰テストです。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">pbrtのMakeNamedMedium「rgbgrid」です。単なる密度スカラーではなく、各セルが独自のRGB散乱色を持つ3Dボクセルグリッドで、柔らかい色付きの星雲のようなボリュームとしてレンダリングされます。両方のバックエンドで動作します。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">pbrtのMakeNamedMedium「uniformgrid」です。RGBグリッドメディウムのシーンとは異なり、ボクセルごとの色を持たない単一チャンネルの密度ボクセルグリッドで、柔らかく光る塊としてレンダリングされます。あちらのシーンよりも単純な、対になるシーンです。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">pbrtのMakeNamedMedium「nanovdb」です。外部の.nvdbファイルから読み込まれる本物のNanoVDB形式のスパース密度グリッドで、柔らかい霧のボリューム球体としてレンダリングされます。スパースなボクセルデータは、他のメディウムシーンが使う密なグリッドとは根本的に異なる参照方法を必要とするため、このプロジェクトが埋めたpbrt-v4のギャップの中で単独最大のものです。CPUのみで動作します。GPUはNanoVDBの参照処理を一切実装しておらず、代わりに一様な均質フォグにフォールバックします - これはバグではなく文書化された近似です。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">pbrt-v4のカメラメディウムのイディオムです。Cameraディレクティブより前に発行されたMediumInterfaceディレクティブにより、境界形状を一切持たない霧の中にカメラ自体が置かれます。これは、他のすべての同梱メディウムシーン(いずれもMediumInterfaceを、カメラが外側から見る形状に付与している)とは本質的に異なる仕組みです。このレンダラーにはレイが持ち回る形でのメディウム状態が存在しないため、BVHのhittableとしてではなく、カメラコード内の明示的なヒット後処理として実装されています。CPUとGPU再帰型では本物として動作しますが、GPU wavefront型はこれを一切実装しておらず、警告なく霧なしでレンダリングします - これは、CPU/GPUのパリティテストにおいて専用の許容誤差の除外設定が必要になるほど、実質的なバックエンド間のギャップです。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">bilinear patchプリミティブをテストするシーンです。これは三角形のペアではなく、独自の交差判定ルーチンで定義された真の4隅の曲面です。角の高さをオフセットした2枚のパッチを配置することで、双曲放物面のような鞍型の面と、湾曲したスロープを形成しています。シェーディングの階調が面の連続的な曲率をたどるように、意図的にやや粗めのメタルマテリアルが使われています。鏡面仕上げだと単一の鋭いハイライトしか見えず、面が平坦でないことがわかりにくくなってしまいます。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">プロシージャルな正二十面体を、本物のインデックス付き三角形メッシュとして構築しています。これは、レンダラーに読み込まれるすべてのメッシュが使用する、水密なレイ-三角形交差判定の経路をテストします。頂点ごとの法線は与えられていないため、各三角形は面ごとの平坦なジオメトリック法線にフォールバックし、スムーズシェーディングされた補間法線ではなく、面取りされたローポリ調の見た目になります。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">独自のレイ-カーブ交差判定を持つ、本物のカーブ形状をテストするシーンです。表面自体を曲げるのではなく単純な球体にhair BSDFを適用する別のHair Fibersシーンとは異なります。根元から先端にかけて先細るベジエストランドが表面全体に散らばって植え付けられており、hairシェーディングモデルではなく、カーブプリミティブ自体のスイープリボン形状をテストします。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">pbrtのShape「plymesh」の、本物の頂点ごとのUVデータを、両方のバックエンドに通したシーンです。以前は、これらのUVが警告なく破棄されていたため、GPU再帰型ではこのシーンがまさに真っ黒にレンダリングされていました。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">pbrtのShape「plymesh」が、実際の外部.plyメッシュファイルを最初から最後まで読み込むシーンです。ソースファイル中の三角形ではない面をファン三角形分割することも含まれています。ローダーは、すべての面が事前に三角形分割されているとは仮定できません。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">pbrtのShape「curve」です。三角形メッシュによる近似ではなく、内部でGPU向きの三角形にテッセレーションされる本物の3次ベジエファイバー形状です。同じ方式で構築された、このプロジェクト独自のcurve-tuftデモと直接比較できるようになっています。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">本物のカーブ形状とMaterial「hair」を組み合わせた、このプロジェクトで初めてのシーンです。まさにこの組み合わせによって、hair BSDFがファイバー自身の接線方向ではなく平坦な面の法線を使ってしまっているバグが明らかになりました。各ストランドに沿ったハイライトを、単なるマットなふわつきではなく1本の毛のように見せているのは、この接線方向です。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">pbrtのShape「trianglemesh」の「point2 uv」パラメータ、すなわち三角形メッシュ上の明示的な頂点ごとのテクスチャ座標を、両方のバックエンドに通したシーンです。この配線が存在する前は、GPU再帰型ではこのシーンがまさに真っ黒にレンダリングされていました。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">pbrtのPixelFilterディレクティブでbox filterを選択したシーンです。よりなめらかなデフォルトのガウシアンフィルタの代わりに、ピクセル境界で急に打ち切られる形で、ピクセル内のすべてのサンプルを等しく数えます。このシーンのよりエイリアシングの目立つ、輪郭の硬いシルエットを、他のデフォルトフィルタでレンダリングされたシーンと比較してみてください。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">球体のActiveTransformによるStartTime/EndTimeの区間指定により、同一の形状に2つの異なるワールド空間上の中心が焼き込まれ、カメラ自身のシャッター区間に応じてレイごとにサンプリングされます。動く球体は、くっきりとした像や二重像としてではなく、柔らかい方向性のあるストリークとしてレンダリングされるはずで、その隣には区間指定を一切行わない静的な参照用の球体が置かれています。下記のdisk/cylinderのケースとは異なり、CPU、GPU再帰型、GPU wavefront型の3つすべてのバックエンドで本物として動作します。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">Object Motion Blurシーンと同じActiveTransformによるモーションブラー技法を、球体ではなくShape「disk」/「cylinder」に適用したものです。内部的には異なるコード経路をたどります(球体のcenter1/center2の組ではなく、形状自身の開始/終了トランスフォームに焼き込まれます)。CPUのみで動作します。GPUが対応している球体のモーションブラーとは異なり、両方のGPUバックエンドは、これらの形状を開始姿勢のまま静止させてレンダリングし、ブラーの代わりに警告を出します。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">2枚のクアッドは同一の点配列と同一のインデックスの巻き方向を共有しており、それ自体ではカメラとは逆を向く法線が生成されます - この巻き方向のままの片面発光エリアライトは、正面からは見えません。2枚目のクアッドの直前に発行された単独のReverseOrientationディレクティブにより、実効的な法線がカメラの方を向くように反転され、その結果、一方のクアッドは暗いまま、もう一方は光ります。ReverseOrientationにバグがあれば、両方のクアッドが暗いか、両方とも光ってしまうはずで、片方だけが光るという状態には決してなりません。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">このプロジェクト独自のpbrt-v3互換拡張(本物のpbrt-v4には含まれないShape「cone」/「paraboloid」)をテストする4つの形状です。基本形状としての単純な拡散反射のconeとparaboloid、通常のマテリアルの代わりにAreaLightSourceの発光体として使われるcone、そしてMediumInterfaceの霧の境界として使われるparaboloidで、開いた縁がカメラを向いているため、レイは固体表面ではなく開口端からメディウムに入ります。CPUのみで動作します - GPU(両方のバックエンド)はcone/paraboloid形状を検出すると警告を出し、完全に取り除きます。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">pbrt-v4の定番サンプルシーンである「killeroo」像です。このレンダラーに直接組み込まれたものではなく、独自の.pbrtファイル(形状、マテリアル、ライト、カメラすべてがファイル駆動)から読み込まれる、最初から最後まで完結したシーンです。ユーザー自身のカスタムpbrtシーンが通るのと同じファイル形式・読み込み経路を使用します。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">A1と全く同じCornell Boxですが、通常の100サンプルではなく、あえてピクセルあたり16サンプルという低い値でレンダリングしています - きれいに収束するには少なすぎる数です。レンダリングオプションタブでサンプラー(Sobol、Z-Sobol、Padded Sobol、Stratified、PMJ02BN、Halton)を切り替えて再レンダリングしてみてください。それぞれの選択は、ピクセルと光源にこの16サンプルを分布させるための異なる低差異序列であり、シーンの他の要素は何も変わらなくても、ソフトシャドウの半影部分におけるノイズ/かたまりのパターンがそれに応じて変化します。CPUのデフォルトパストレーサーにのみ適用され、サンプラーコントロールはGPUには影響しません。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">B23(スペクトル分散)と同じガラスプリズムを、ここでは「スペクトルレンダリング」チェックボックス自体のデモとして再構成したものです。スペクトルレンダリング(レンダリングオプションタブ)をオフにすると、レンダラーは赤/緑/青の3チャンネルのみを追跡し、すべての波長がガラス中で同じ固定量だけ屈折します。オンにすると、レンダラーはレイごとの実際の波長を追跡し、プリズムの屈折率が波長によって本当に変化するようになり、物理的なプリズムと同じように白色光を可視スペクトルへと分散させます。CPUのデフォルトパストレーサーにのみ適用されます。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">C1(HDRIスカイ)と同じHDRの空のグラデーションです。明るいプロシージャルな空を背景に、自分自身の影の中に置かれた拡散球体があり、ほぼ真っ黒から1枚の画像で表示できる範囲を大きく超えるところまで、実際の明るさの値を持っています。レンダリングオプションタブの2つのコントロールが、この範囲に対して異なる働き方をします。露出は他のすべての処理の前に適用される一律の倍率で、上げると球体と空の両方が均等に明るくなります。トーンマッピングはその倍率の後に適用されるカーブで、結果を表示可能な範囲に圧縮します - ACESは空の最も明るいハイライトを穏やかに丸め、Reinhardはより強く圧縮し、Noneは単純に純白でクリップします。いくつか異なる露出の値で各トーンマッピングを試し、2つのコントロールが独立して働く様子を確認してください。CPUとGPUの両方で動作します。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">A1と同じCornell Boxですが、あえてピクセルあたり32サンプルという低い値でレンダリングしており、後処理を行う前の時点で本当に粒状感があります - OptiX AIデノイザー(レンダリングオプションタブ)をオフにした状態とオンにした状態で、それぞれ一度ずつレンダリングしてみてください。このデノイザーは機械学習モデルで、シーン自体のアルベドバッファとノーマルバッファに導かれながら、そのスペックル(斑点)パターンを認識し、追加のレイを1本も追跡することなく滑らかに除去します。細部をわずかに犠牲にする代わりに、同じサンプル数で見た目が劇的にクリーンな画像が得られます。GPUのみで、recursiveとwavefrontの両バックエンドがそれぞれ独自のデノイザーを持っています。隣にあるOptiX検証モードのチェックボックスも、ここで知っておく価値のある別種のコントロールです - 実際の性能コストを伴う追加のデバイス側正当性チェックをオンにするだけで、レンダリング画像そのものを変えることは決してないため、この項目については視覚的に比較できるものはありません。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">B3(Cornell Rough Glass)と同じすりガラスのコーネルボックスを、インテグレータのドロップダウン自体のSPPMデモとして再構成したものです。同じサンプル数で、デフォルトのパストレーサーで1回、SPPM(レンダリングオプションタブ)で1回レンダリングしてみてください。rough_dielectricの球体の下にできる床のコースティクス - 通常のパストレーシングが収束に苦労する、まさに難しく高周波な間接光のケース - が、まさにこの種の光の伝播のために作られたSPPMのフォトンマッピング方式では、目に見えて速くきれいになるはずです。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">A1と同じコーネルボックスを、インテグレータのドロップダウン自体のBDPT/MLTデモとして再構成したものです。デフォルトのパストレーサーの代わりに、BDPTまたはMLT(レンダリングオプションタブ)を試してみてください。どちらも、デフォルトのインテグレータやSPPMのようにカメラからのみ光路を追跡するのではなく、カメラと光源の両方から光路を追跡し、それらを途中で接続します。これは根本的に異なる戦略であり、このシーンのように意味のある間接照明を持つシーンでは、収束の仕方が異なってくることがあります(良くなることもあれば、ノイズの性質が変わることもあります)。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">同じA1のコーネルボックスを、インテグレータのドロップダウンにある参照/デバッグモードを巡るツアーとして再構成したものです。RandomWalkは光サンプリングもMISも一切行わず、半球上で一様に次のバウンス方向をサンプリングします - 最も収束が遅いものの、バイアスが最も少ない基準です。SimplePathはこの2つの要素それぞれに独立したチェックボックスを追加します。「ライトをサンプリング」(NEE)のみをオンにすると、直接光は安価に得られますが、明るくても軸外にあるライトへのバウンスは運任せのままです。「BSDFをサンプリング」のみをオンにすると逆になります。両方をオンにすると、それが両者を組み合わせる多重重点サンプリング(MIS)であり、デフォルトのパストレーサーがすでに行っていることでもあります。同じ低いサンプル数でそれぞれをレンダリングし、同一のジオメトリ上でどれほど異なる収束をするか比較してください。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">このデモ専用に構築されたコーネルボックスです。A1の1つの代わりに天井に5つのライトがあり、意図的に不均衡なパワー(サイズが違うだけでなく、おおよそ1:2:6:15:80の比率)を持たせています。Uniform(ライトサンプラー、レンダリングオプションタブ)は、各ライトが実際に画像へどれだけ寄与しているかに関係なく、すべてのnext-event-estimationサンプルの5分の1を各ライトに費やします - そのため、ほとんどのサンプルはほとんど重要でない4つのライトに向けられ、本当に支配的な1つのライトは必要以上にノイズが残ります。PowerとBVHはどちらもその支配的なライトへ選択を重み付けします(BVHは距離/方向も考慮し、Powerは放出される総光束のみを考慮します) - 同じ低いサンプル数で3つすべてをレンダリングし、明るい角のライト周辺のノイズがどれだけ速く収まるか比較してください。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">A1と同じコーネルボックスを、インテグレータのドロップダウン自体のAmbient Occlusionデモとして再構成したものです。AOは意図的にまったく照明されたレンダリングではありません。それに切り替える(レンダリングオプションタブ)と、各サーフェス自身の色とシーン内のすべての実際のライトが完全に無視され、単一のグレースケールのオクルージョン項に置き換えられます - 各点の上の半球のうちどれだけが近くのジオメトリによって遮られているか、というものです。回転したボックス自身のコンタクトシャドウと、2つの壁が接する角だけが残る構造であり、これこそがAOをシーンの照明から切り離した幾何学的複雑さの高速プレビューとして役立たせている理由です。</translation>
    </message>
    <message>
        ...
        <translation type="vanished">同じ曇りガラスのコーネルボックスをB3およびI5と共有しています - ラフな誘電体球の床のコースティックは教科書的なハードコースティックの例であり、通常のパストレーシングで孤立した明るいピクセルのファイアフライを生む光路の種類(小さな立体角へのほぼ鏡面的な散乱の後、拡散反射する床に着地する)です。RegularizeとFirefly clamp(--maxcomponentvalue、どちらもレンダリングオプションタブ)は、これを2つの異なる方法で抑えます。Regularizeは最初の非鏡面バウンスの後にラフなBSDF自身の粗さを広げるため、そもそもコースティック自体が病的に鋭くサンプリングされにくくなります。一方、クランプはファイアフライの発生を許し、その後最も明るいチャンネルが閾値を超えるサンプルをスケールダウンし、わずかなエネルギー損失と引き換えに確実な上限を得ます。何も有効にせずにレンダリングし、次にそれぞれを個別に、同じサンプル数でレンダリングして比較してください。</translation>
    </message>
    <message>
    ...
    <message>
        ...
        <translation>Cornell Boxは、コンピューターグラフィックスの世界でもっとも有名なテストシーンの1つです。色付きの側面の壁を持つ小さな部屋に、天井に埋め込まれた光るパネル、真っ白な箱、そして周囲の光を反射しながら屈折もさせるガラスの球が置かれています。赤と緑の壁が、バウンスした光で近くの白い面をどう色づけするか、そして天井の光がノイズの目立つ斑点状ではなく柔らかくきれいな影をどう作り出すかに注目してください。バウンスした光、光源、ガラスのオブジェクトが1つの小さな部屋に詰め込まれているため、このシーンは照明やマテリアルの問題が最初に現れる場所になることがよくあります。</translation>
    </message>
    <message>
        ...
        <translation>古典的な書籍「Ray Tracing in One Weekend」の最終章へのオマージュです。チェッカーボード模様の地面が地平線まで広がり、その上につや消し・金属・ガラスの仕上げのさまざまな小さな球が散らばっています。よく見ると、いくつかの球がわずかに筋を引いているのがわかります。これは撮影中にフレーム内を動いており、実際のカメラと同じようにモーションブラーで捉えられているためです。金属の球は表面の粗さに応じて、鏡のようにくっきりしたものから、ブラシがけしたように柔らかいものまで揃っています。手前にある3つの大きな球(透明なガラスの球、つや消しの球、完璧な鏡)は、それぞれの仕上げを並べて比較するための基準です。</translation>
    </message>
    <message>
        ...
        <translation>2つの巨大な球が上下に重なり、同じ青白のチェッカーボード模様に包まれています。これは絵として描かれたものではなく数式で生成されたパターンで、伸びや継ぎ目なく曲面にきれいに巻き付いています。上に乗ったいくつかの小さな球(つや消し、柔らかくぼやけた金属、ガラス)がスケール感を与えると同時に、同じ光が異なるマテリアルとどう違って作用するかを示しています。</translation>
    </message>
    <message>
        ...
        <translation>地面と大きな球には、大理石のようなプロシージャルパターンが施されています。渦を巻く自然な色の帯は、絵として描いた画像ではなく数式によるノイズ関数(パーリンノイズ)で生成されたもので、コンピューターグラフィックスやゲームで説得力のある雲・木目・大理石を作るのと同じ技術です。近くにある2つの小さな球には、対比のためによりきめ細かく複雑なバージョンの同じパターンが使われています。上にある光るパネルがシーン全体を照らし、柔らかくリアルな影を作り出しています。</translation>
    </message>
    <message>
        ...
        <translation>5枚のフラットな色付きパネルが、正面向き・横向き・上向きなど異なる角度に配置されており、向きに関わらずそれぞれが正しく光をとらえるかを確認できます。部屋に浮かぶ別の光るパネルがこのシーンの唯一の光源で、他のパネルに柔らかい直接光と影を落としています。</translation>
    </message>
    <message>
        ...
        <translation>大理石の球のシーンで使われている大理石模様の地面を再利用していますが、ここでの主役は光そのものです。暖かく光る球と、より冷たい色調で光るパネルが、同じ部屋の中で形の大きく異なる2つの光源として働きます。一方は曲面の球、もう一方は平らなパネルであるため、形や色の異なる光が周囲の表面にどう広がり混ざり合うかを観察するのに適したシーンです。</translation>
    </message>
    <message>
        ...
        <translation>おなじみのCornellの部屋を使い、2つの不透明な箱を色付きの霧で満たした箱に置き換えています。不透明なオブジェクトのように光を遮るのではなく、内部で光を散乱させるため、くっきりした輪郭ではなく柔らかく煙のようなかすんだ見た目になります。単なる平らな不透明面ではなく、決まった形と厚みの中に閉じ込められた霧やかすみを、レンダラーがどう扱うかがよくわかるシーンです。</translation>
    </message>
    <message>
        ...
        <translation>シーン全体の中でもっとも要素が詰め込まれたシーンで、ほとんどすべての表現をいっぺんに見せてくれます。ランダムな高さの小さな箱を多数並べた起伏のある地面、光る天井パネル、目に見えて動いている球(モーションブラー)、ガラスの球、柔らかくブラシがけされた金属の球、スノードームのように内部に色付きの霧を閉じ込めたガラスの球、部屋全体に漂うごくかすかなかすみ、地球の画像がテクスチャとして貼られた球、大理石模様の球、そして隅に密集した小さな白い球の集まりが一度に見られます。もし1枚のレンダリングでこのレンダラーの実力をすべて見せるとしたら、まさにこのシーンです。</translation>
    </message>
    <message>
        ...
        <translation>金色の球が5つ並んでおり、違いは表面の粗さだけです。左端はほぼ鏡のようになめらかで、小さくくっきりとした反射のハイライトがあります。右へ行くほど表面は徐々に粗くなり、ハイライトは柔らかく広いグロウへと広がっていきます。これは、ブラシ仕上げやサテン仕上げの金属が、磨き上げたものよりくすんでぼやけて見えるのと同じ現象です。表面の粗さだけで金属の見た目がどう変わるかを、並べてわかりやすく比較できます。</translation>
    </message>
    <message>
        ...
        <translation>粗い金属の列と同じブラシ仕上げの見た目(ブラシ仕上げのアルミの箱とブラシ仕上げの金の球)を、今度はCornellの部屋の中に置いたものです。1つのクリーンなスタジオ照明ではなく、柔らかく色づいたバウンス光の下でどう見えるかが試されます。ぼやけた光沢のある反射に、部屋の色付きの壁のヒントがどう映り込むかに注目してください。</translation>
    </message>
    <message>
        ...
        <translation>Cornellボックスの通常の透明なガラス球を、すりガラス風のものに置き換えています。通ってきた光はくっきりと屈折する代わりに柔らかくぼやけて散乱します。これは、すりガラスやサンドブラスト加工のガラスが普通の窓ガラスに比べてどう見えるかと同じです。表面の反射も同じように柔らかくぼやけた処理になります。</translation>
    </message>
    <message>
        ...
        <translation>磨かれた金とアルミの表面は、アーティストが単に金色や銀色を選んだのではなく、それぞれの金属を実際に測定した光学データから色と輝きを得ています。そのため見えている色は近似ではなく、その金属が実際に光を反射する物理的な様子そのものです。</translation>
    </message>
    <message>
        ...
        <translation>球と箱が、透明なニスやラッカーに浸けたかのような見た目をしています。ガラスのように反射するクリアコートが、下にある普通のつや消しの色の表面の上に重なっています。ハイライトをよく見てください。一部の光はそのまま光沢のある外側のコートで反射し、残りは内部を通り抜けて下のつや消しの色を拾い、さらに一部の光は外に出るまでに2つの層の間を行き来します。</translation>
    </message>
    <message>
        ...
        <translation>1枚のガラス板がCornellの部屋を2つに仕切っています。理想化された厚みゼロの板として扱われているため、通過する光は厚いガラスの塊を通るときのように屈折することなく、そのまま真っすぐ進みます。あえて角度をつけて傾けてあります。ガラスは正面から見るとほとんど何も反射しませんが、見る角度が浅くなるほど目に見えて鏡のようになります。これは車のフロントガラスが斜めから見ると反射して見え、正面から見るとほとんど見えなくなるのと同じ現象です。</translation>
    </message>
    <message>
        ...
        <translation>金の球と銅の箱に、ラッカー仕上げのショールームのような見た目を与えています。下にある金属本来の反射色の上に、ガラスのようなクリアコートが重なっています。この一連のシーンの中でもっとも層の多いマテリアルで、金属本来の色づいた輝きと、その上に別に乗るコート自体のハイライトを組み合わせています。</translation>
    </message>
    <message>
        ...
        <translation>ろうそくのように柔らかく、ろうのような質感を持つ球です。光は表面で反射するだけでなく、一部は内部を通り抜けて反対側からくっきりとではなく散乱しながら光り出ます。これは、物体内部で実際に光が跳ね回る様子をシミュレートすることなく、ろうやランプシェードのような半透明な素材の見た目を安価に再現する、シンプルな方法です。</translation>
    </message>
    <message>
        ...
        <translation>正面よりも縁の方が強く光る、クリスタルのような球です。これはコップの水やシャボン玉が、中心よりも縁の方が明るく反射して見えるのと同じ、角度に依存する現象です。ここでは、内部で散乱した光がどのように外へ抜け出すかにこの効果が適用されています。</translation>
    </message>
    <message>
        ...
        <translation>7つの球が一列に並び、1つの柔軟で万能なマテリアルが、単純なつや消しから、光沢のあるプラスチック、透明なニスのようなコートをまとったプラスチック、半金属、ブラシ仕上げの粗い金属、そして最後にガラスのようになめらかな完璧な鏡面仕上げ(クリアコートあり・なし)まで滑らかに変化する様子を示しています。見た目ごとに異なるマテリアルを用意する代わりに、この1つのマテリアルは「金属らしさ」「粗さ」「光沢コートの強さ」を調整するだけで、ほぼどんな表面にもなります。</translation>
    </message>
    <message>
        ...
        <translation>5つの球に、1本1本の毛髪に使われるのと同じ光散乱モデルが適用されています。実際の髪の毛のような細い繊維ではなく球に適用することで、マテリアルの見た目だけを切り離して観察できます(同じ考え方を実際の毛髪の形状に使った例は、Curve Fibersシーンを参照してください)。このマテリアルは非常に強く、レーザーのように狭い範囲に光を散乱させるため、ハイライトが完全に白く飛んでしまわないよう、この部屋の照明はかなり暗めに設定されています。</translation>
    </message>
    <message>
        ...
        <translation>実際に形状をモデリングすることなく、細かい表面のディテールを見せかける2つの方法です。奥の壁と箱は、実際の形は完全に平らなのにでこぼこした大理石模様のテクスチャがあるように見え、球は完全になめらかなのに表面にチェッカーボード模様が刻まれているように見えます。どちらの手法も、実際のジオメトリはまったく変えずに、光が「表面はこちらを向いている」と感じる方向をわずかに曲げることで成り立っています。</translation>
    </message>
    <message>
        ...
        <translation>乳白色のろうの板と翡翠の球は、どちらも実際の半透明素材が持つ、内側から柔らかく光っているような見た目をしています。光は表面でそのまま反射するのではなく、ガラスのような外側の表面から内部に入り、しばらく散乱・バウンスしてから外へ抜け出します。これは肌・牛乳・翡翠のような実際のサブサーフェス素材で知られる見た目と同じもので、ここでは専用の肌・半透明用の計算式ではなく、物体の内部を光を散乱させる霧のように扱うことで実現しています。</translation>
    </message>
    <message>
        ...
        <translation>5つの球で、実世界の「測定済み」マテリアルデータ、つまりアーティスト向けのスライダーで見た目を記述するのではなく、実際の物理的な素材サンプルを研究室でスキャンして得られるようなデータ表への対応を示しています。ここで使われているデータは実際のスキャンではなく合成的に生成されたもので、シェーディング自体も簡略化されているため、これは特定の実在素材を披露するというより、測定データ形式が正しく読み込まれて反映されるかを確認するためのシーンです。</translation>
    </message>
    <message>
        ...
        <translation>理科の授業で光と色を説明するのに使われるのと同じような、ガラスのプリズムです。一方の面から入った白色光は、出てくるときに扇状の虹色に分かれます。これは、ガラスが波長ごとにわずかに異なる度合いで光を屈折させるためで、実際のガラスの挙動と一致します。この完全な虹の広がりは、光の個々の波長を追跡する特別な--spectralモードでレンダラーを実行したときにのみ現れます。それ以外の場合は、大まかな3色(赤・緑・青)による近似的な分裂しか表示されません。</translation>
    </message>
    <message>
        ...
        <translation>なめらかなバージョンと同じ、虹色に分ける効果を持つガラスのプリズムですが、磨き上げる代わりにすりガラス・サンドブラスト仕上げにしてあります。--spectralモードで実行すると、背景のスクリーンに同じ虹の扇が現れますが、すりガラスの表面によってくっきりとではなく、ぼやけて柔らかくなります。これは、色を分ける効果とすりガラスによるぼけという2つの独立した現象が、正しく同時に起きていることの証明です。粗い表面が偶然色を薄めてしまっているわけではありません。</translation>
    </message>
    <message>
        ...
        <translation>表面のあらゆる点で、単純なつや消しマテリアルと光沢のある金属マテリアルをブレンドしています。ただし色を中間の色合いに混ぜ合わせるのではなく、光のサンプル1つ1つについてどちらか一方をランダムに選ぶという方法です。よく見ると、結果には本物の斑点状のノイズがあり、ある場所は純粋につや消しに、別の場所は純粋に光沢のあるように振る舞います。これは十分なサンプルが積み重なって初めて、なめらかな中間的な見た目に平均化されます。</translation>
    </message>
    <message>
        ...
        <translation>4つの異なるマテリアルの見た目を並べています。横方向にずれることなく光を屈折させる、理想化された紙のように薄いガラス板。透明なラッカーコートをまとった金属。薄い葉や紙のように光を反射しつつ透過もさせるマテリアル。そして、外部のスキャンデータを一切使わずにサブサーフェス散乱による柔らかな輝きを再現する、内蔵の「Marble」プリセットを使ったブロックです。</translation>
    </message>
    <message>
        ...
        <translation>1つの金属表面と、その金属にラッカーコートを施したバージョンは、あらかじめ用意された金属のプリセットを選ぶのではなく、光の屈折に関する生の物理的な数値を直接入力することで色と反射率を得ています。これにより、アーティストが選んだ平坦な金属色ではなく、正確で自然な色合いのスペキュラーハイライトが生まれます。</translation>
    </message>
    <message>
        ...
        <translation>実際の映画制作用レンダラーがフォトリアルな髪や毛皮に使うのと同じ、物理ベースの毛髪シェーディングモデルを使用しています。ここでは単純な球に適用することで、このプロジェクト独自の毛髪デモシーンと、条件をそろえて比較できるようにしています。</translation>
    </message>
    <message>
        ...
        <translation>1つのマテリアルを一度だけ定義し、名前で複数のオブジェクトから再利用しています。そのプロパティの1つは、単色ではなく画像テクスチャによって制御されています。また、このシーンの光源は片面だけでなく両面から光るように設定されています。表と裏があることを忘れなかった、光るパネルのようなイメージです。</translation>
    </message>
    <message>
        ...
        <translation>7つのガラス球は、それぞれ異なる実在の光学ガラスの種類(実際のカメラレンズのカタログにも載っている名前)を使っており、光を屈折させる度合いがそれぞれごくわずかに異なります。レンズ設計者がどのガラスを選ぶかによって、光の屈折の仕方にどれほど小さいながらも実際の違いが生まれるかを見ることができます。</translation>
    </message>
    <message>
        ...
        <translation>ニス塗りの木材のようなマテリアル(透明な光沢コートの下につや消しのベースカラーがある構成)で、そのベースカラーを単色ではなく、表面に貼り付けた実際の写真から得ています。この組み合わせには、かつて画像が静かに無視され、単なるグレーになってしまうバグがありました。このシーンは、画像が今では正しく表示されることを確認するために存在します。</translation>
    </message>
    <message>
        ...
        <translation>薄い葉やランプシェードのようなマテリアル(表面で光を反射しつつ、裏側からの光も透過して光らせるもの)について、反射色と透過色の両方を単色ではなく実際の写真から得ています。これを、対応しているすべてのレンダリング方法で確認しています。</translation>
    </message>
    <message>
        ...
        <translation>2つの「色」がそれぞれ単色のスウォッチではなく、まるごと別々の写真になっているチェッカーボードパターンです。テクスチャの中に別のテクスチャが入れ子になっており、テクスチャを1つずつ使うだけでなく、重ねたり組み合わせたりできることを示しています。</translation>
    </message>
    <message>
        ...
        <translation>同じ元写真を使った4枚の同一のパネルで、それぞれ画像の読み込みと適用の異なる方法を示しています。ほとんどの画像で使われる標準的な見た目、ほとんどのカメラや画面が想定する色補正の工程を省いた目に見えて明るいバージョン、境界を超えて引き伸ばされたときにパターンを繰り返す代わりに端のピクセルを保持し続けるバージョン、そして色チャンネルを反転させたバージョンです。並べて見ることで、普段は目に見えない画像処理の設定の違いを実際に確認できます。</translation>
    </message>
    <message>
        ...
        <translation>4枚のパネルには、それぞれ写真ではなく数式だけで生成されたプロシージャルパターンが表示されています。風に吹かれたような乱流の見た目、細部を調整できるしわの寄った雲のような表面、ノイズによって描かれた自然に波打つ境界で分かれた2色のパターン、そして4隅の異なる色をなめらかにブレンドしたものです。</translation>
    </message>
    <message>
        ...
        <translation>写真の中にチェッカーボード、その中にさらにチェッカーボードという、3層に入れ子になったパターンを、隣に置いたよりシンプルな単層バージョンと比較しています。CPUでレンダリングすると入れ子の詳細がすべて正しく表示されますが、GPUでレンダリングすると、意図的に1つの平均化された色に簡略化されます(警告付き)。これはバグではなく、既知で許容されているショートカットです。このシーンは、その違いをあえて並べて見せるためのものです。</translation>
    </message>
    <message>
        ...
        <translation>空だけに照らされた開放的なシーンです。頭上の青から地平線付近の暖かい輝きへとなめらかに変化するグラデーションが、単一の電球やパネルではなく環境全体の代わりを果たしています。その下には、つや消しの球、ほぼ完璧な鏡、そしてガラスの球が置かれており、まったく同じ空がマテリアルによってどれほど違って見えるかを示しています。つや消しの球では柔らかく影のように、鏡では空をくっきりと反射したコピーとして、ガラスでは屈折して曲がった姿として現れます。</translation>
    </message>
    <message>
        ...
        <translation>天井からまっすぐ下に向けられた1つのスポットライトだけで照らされた、何もない部屋です。くっきりした円形の光ではなく、縁が柔らかく徐々に薄れていく光になっています。きついフラッシュライトのビームというより、よく設計された舞台照明やスポットライトのような見た目です。</translation>
    </message>
    <message>
        ...
        <translation>屋外で直射日光の下に置かれているかのように照らされた、何もない部屋です。光は1つの決まった方向から、完全に平行な光線として届きます。ランプや電球と違い、距離が離れても暗くなりません。壁をどれだけ離しても、光の明るさは変わりません。</translation>
    </message>
    <message>
        ...
        <translation>考えられる中でもっともシンプルな種類の光、つまりあらゆる方向へ均等に光る空間内の1つの点だけで照らされた、何もない部屋です。小さな裸電球の光が距離とともに急速に弱まるのと同じように、光源から離れた表面ほど暗くなります。</translation>
    </message>
    <message>
        ...
        <translation>むらのある、独自の形をした光で照らされた、何もない部屋です。あらゆる方向に均等に光るのではなく、実際に測定されたパターンに従って、ある方向は他の方向より明るくなります。実際のデスクランプや照明器具が、シェードや反射板の形によって方向ごとに光の強さが違うのと似ています。</translation>
    </message>
    <message>
        ...
        <translation>古いスライドプロジェクターのように、模様の入った画像を投影する光で照らされた、何もない部屋です。単純に均等に光る円錐や球状の光ではなく、くっきりしたチェッカーボード模様の光を壁に投げかけます。</translation>
    </message>
    <message>
        ...
        <translation>天井の光をまったく持たない、Cornell風の部屋です。代わりに奥の壁に本物の窓のような穴が開けられており、すべての光はその開口部から覗く空から届きます。これは、部屋の中にある光にまっすぐ向ければよい場合に比べ、はるかに難しい照明の課題です。レンダラーは、狭い窓を通して、その外側のあらゆる場所に存在する空へ向けて光線を狙う方法を見つけなければなりません。</translation>
    </message>
    <message>
        ...
        <translation>基本的な「単一の点」タイプの光源5種類を、1つの部屋の中に並べて配置しています。単純な点光源、スポットライト、太陽のような遠方光源、独自の方向性のある光のパターンを持つ光源、そしてスライドプロジェクターのような光源です。大きさを持たないシンプルな光源のすべてを、一目で比較できる便利なラインナップです。</translation>
    </message>
    <message>
        ...
        <translation>実際の画像を投影するはずの2種類の光源、つまり独自の方向性のある光のパターンを持つ光源と、スライドプロジェクターのような光源が、以前のように何も特徴のない単なる光のビームへ静かにフォールバックすることなく、割り当てられた画像を正しく読み込んで表示しているかを確認します。</translation>
    </message>
    <message>
        ...
        <translation>写真スタジオの電球やカメラの色温度設定と同じように、ケルビンで測定された異なる色温度で光る2枚の光るパネルです。一方は白熱電球のような、暖かく心地よいオレンジ色に、もう一方は曇り空のような、冷たく青みがかった白色に光ります。もし内部の色計算が壊れていれば、両方のパネルは同じ単なる白色になってしまうため、目に見えてはっきり違う色を保つこと自体がこのシーンの目的です。</translation>
    </message>
    <message>
        ...
        <translation>単色で光るのではなく、実際の画像から光のパターンを得ている光る形状(球またはパネル)です。通常は片側の向いている面だけから光るところを、この場合は両面からそのパターンで光ります。</translation>
    </message>
    <message>
        ...
        <translation>考えられる中でもっともシンプルな空の代わりです。それ自体には見える形がなく、あらゆる方向から一様に一定の色で光ります。これが役に立つのは開放的なシーンだけで、完全に閉じた部屋の中では完全に遮られてしまいます。密閉された箱の中から空が見えないのと同じです。</translation>
    </message>
    <message>
        ...
        <translation>円盤形とシリンダー形の光源で、どちらもレンダラーがサンプル光線を意図的に狙って向ける、本物の光源として扱われています。光線が偶然当たることを期待するのではありません。この意図的な狙い撃ちがあるからこそ、シーンは長く斑点状のノイズが残ることなく、素早くきれいでなめらかな画像に収束します。</translation>
    </message>
    <message>
        ...
        <translation>2つの独立した光る球が、このシンプルな部屋を照らしています。このごく小さな構成は、かつてレンダラーのGPUコードが1つの光源の種類しか正しく把握できず、2つ目以降の光源が誤認識されて正しくレンダリングされないという、実際のバグを見つけ出しました。</translation>
    </message>
    <message>
        ...
        <translation>単純な平らな長方形ではなく、不規則な五角形の扇のような形をした光源です。形が扱いにくいため、レンダラーは通常のように三角形のペアを1つの単純化された長方形とみなしてサンプリングするショートカットを使えず、この変わった形の光源を実際にそのまま処理せざるを得ません。</translation>
    </message>
    <message>
        ...
        <translation>Blackbody Lightシーンとまったく同じジオメトリと暖色・寒色の色温度を使っていますが、ほとんどの画面が使う日常的な色空間ではなく、より広く映画用カメラに近い色空間でレンダリングされています。この2つのシーンを並べて比較することは、色空間の選択が静かに無視されるような設定ではなく、実際に最終的な色を変えることの直接的で目に見える証明になります。</translation>
    </message>
    <message>
        ...
        <translation>奥の壁に窓が開けられたCornell風の部屋で、その開口部からは単なる一様な光ではなく、本物の高解像度の空の写真が見えます。このギャラリーの他の場所にあるよりシンプルなポータルライトのシーンとは異なり、ここで窓越しに見えるのは、穴から覗く単なる色付きの光ではなく、本物の空のディテールです。</translation>
    </message>
    <message>
        ...
        <translation>3つの同一の球が、それぞれ異なる種類の光源(裸の点光源、スポットライト、光るパネル)で照らされています。それぞれの明るさは、生の明るさの数値ではなく、電球の箱に書かれたワット数のような「総光出力」として設定されています。それを実際に目に見える明るさへ公平に変換する処理は光源の形ごとに異なるため、もし3つの球のうちどれか1つでも他より明るすぎたり暗すぎたりすれば、すぐに目立ってわかります。</translation>
    </message>
    <message>
        ...
        <translation>スライドプロジェクター型の光源が、他のプロジェクターライトのシーンで使われる通常の正方形のテストパターンではなく、縦の2倍の幅を持つ横長の画像をシーンに投影します。正しく処理されていれば、投影されたパッチはスライド自体の縦横比に合わせてはっきりと長方形になるはずです。ここにバグがあれば、パッチが正方形につぶれたり、ビーム全体を埋めるように引き伸ばされたりして現れます。</translation>
    </message>
    <message>
        ...
        <translation>赤・緑・青の鮮やかな3つの光源が、近距離で平らな表面の上に重なり合っています。混ざり合った色が、通常の画面が表示できる限界ぎりぎりのところまで達しています。重なる部分では、白やパステルカラーへと滑らかで自然な移り変わりが見えるはずです。かつてこのシーンには、色が混ざり合う場所にちょうど濁って暗い斑点や、目に見えるバンディングが現れるレンダリングのバグがありました。</translation>
    </message>
    <message>
        ...
        <translation>チェッカーボード模様の床に、さまざまな距離に散らばったガラスと金属の球が置かれ、リアルなカメラのピント合わせでレンダリングされています。ちょうどピント距離にある球だけがくっきりと鋭く写り、それより手前や奥にあるものはすべて柔らかくぼやけていきます。実際のカメラレンズを通して見ているのと同じです。</translation>
    </message>
    <message>
        ...
        <translation>チェッカー模様の床の上に並んだ球を、遠近感がまったくないカメラを通して見たものです。光線は1つの視点に向かって収束するのではなく、完全に平行に進むため、実際の距離に関わらず画面上ではどの球も同じ大きさに見えます。これは建築図面のようなフラットな見え方の典型的な特徴で、普通の写真の見え方とはまったく異なります。</translation>
    </message>
    <message>
        ...
        <translation>中央にある1つの光る球を、色付きの球の輪が取り囲んでいます。これを360度の全天球カメラで撮影しており、通常の枠に収まった写真とは違い、あらゆる方向を同時にとらえた周囲全体の景色が、1枚の巻き付くような画像に平らに変換されています。</translation>
    </message>
    <message>
        ...
        <translation>チェッカー模様の床の上を奥へと連なる5つの球を、単純化された数式上のレンズではなく、実際の曲率と厚みを持つ本物のガラス素子で構成されたシミュレートされたリアルなカメラレンズを通して撮影したものです。ピントの外れたぼけや、フレームの端に向けてわずかに暗くなる現象は、ショートカットの効果ではなく、実際のレンズの積み重ねを光が通り抜けることによる正真正銘の副作用です。</translation>
    </message>
    <message>
        ...
        <translation>このカメラ比較シリーズ全体で使われているのと同じCornell Boxを、今回は写真的な柔らかい被写界深度のぼけを加えて表示しています。シーンの他の部分は一切変えずに、近くにある他のカメラタイプで見た同じ部屋と比較しながら、この効果がどう見えるかを正確に確認できます。</translation>
    </message>
    <message>
        ...
        <translation>同じ共通のCornell Boxを、平行投影カメラを通して奥行き方向にまっすぐ見たものです。遠近感による収束がないため、通常のカメラで見た場合のように消失点に向かって狭まっていくのではなく、箱の輪郭はフレームの端まで完全に平行なまま保たれます。</translation>
    </message>
    <message>
        ...
        <translation>同じ共通のCornell Boxを、今回はその中心に置いた360度の全天球カメラで巡っています。天井の光、5面すべての壁、そしてガラスの球が、まるで部屋の真ん中に立ってあらゆる方向を同時に見ているかのように、1枚の連続した画像へと巻き込まれます。</translation>
    </message>
    <message>
        ...
        <translation>同じ共通のCornell Boxを、単独のレンズデモシーンと同じシミュレートされたリアルレンズカメラで撮影しています。部屋の大きなスケールに合わせて絞りを開いてあるため、規模が大きくなっても、ぼけた見た目の強さが同じくらいに感じられます。</translation>
    </message>
    <message>
        ...
        <translation>同じ共通のCornell Boxですが、今回はシャッターが開いている間にカメラ自体が一方の端からもう一方へと滑るように移動し、静止したフレームではなく本物のモーションブラーを生み出します。この効果は、標準のパストレーサーまたはフォトンベースのSPPMモードでレンダリングしたときにしか現れません。それ以外のレンダリングモードやGPUでは、代わりにカメラが開始位置で静止したまま表示されます。</translation>
    </message>
    <message>
        ...
        <translation>カメラのリアルなピントぼけ(絞りのサイズとピント距離)を、プログラムに組み込むのではなく外部のシーン記述ファイルから読み込んだものです。このプロジェクト独自の被写界深度シーンと同じ光学効果を、ファイル読み込みという別の経路で実現しています。</translation>
    </message>
    <message>
        ...
        <translation>シーンファイルから読み込んだ平行投影カメラです。光線は視点に向かって収束するのではなく完全に平行に進むため、距離の異なる2つの同一の球が、近い方が大きく見えるのではなく画面上で同じ大きさに見えます。これは普通の写真の見え方とは正反対です。</translation>
    </message>
    <message>
        ...
        <translation>シーンファイルから読み込んだ360度の全天球カメラです。光線は通常の平らな画像フレームを通してではなく、1つの点からあらゆる方向へ扇状に広がり、1枚の画像で周囲全体の景色をとらえます。どの方向にも常に何か見えるよう、囲まれた部屋の中に配置されています。</translation>
    </message>
    <message>
        ...
        <translation>他のすべてのカメラシーンが使う単純化された理想的なレンズではなく、外部のレンズ設計ファイルから読み込んだ、複数の素子から成る本物のカメラレンズです。実際のガラス素子を光線ごとに追跡することで、本物らしい被写界深度のぼけとレンズの不完全さを再現しています。</translation>
    </message>
    <message>
        ...
        <translation>端から端まで均一な霧で満たされたCornell Boxです。内部のどの点も厚みと散乱の振る舞いがまったく同じで、部屋の中を光が散乱しながら通り抜けることで生まれる、もっともシンプルで均一なかすみの見た目になります。</translation>
    </message>
    <message>
        ...
        <translation>固定された一様な霧の塊ではなく、重ね合わせた自然な変化のあるパーリンノイズから、もやもやとした形が生まれるリアルな見た目の雲です。実際の雲のように、根元付近は密度が高く、上に行くほど薄くなっていきます。このように場所ごとに厚みが変化する霧をレンダリングするには、その変化する密度をリアルに扱うための専用の技術が必要で、このシーンはまさにそれをテストしています。</translation>
    </message>
    <message>
        ...
        <translation>それぞれ異なる密度の霧を内部に満たした、ガラスの球です。光は不透明な殻でそのまま反射するのではなく、ガラス表面から入るときに屈折し、内部の霧によって散乱または吸収され、そして外へ出るときに再び屈折します。</translation>
    </message>
    <message>
        ...
        <translation>3Dグリッド上の実際のセルごとの密度データから作られた、雲のようなボリュームです。赤・緑・青にそれぞれ独立して変化するパターンが与えられているため、単一の一様な色合いではなく、雲全体で色がゆっくりと移り変わっていきます。</translation>
    </message>
    <message>
        ...
        <translation>外部のシーンファイルから読み込んだ雲で、その形は、平らで明らかに偽物とわかる霧の塊に見えないよう、実際の雲のレンダリングで使われるのと同じ種類の自然な変化を持つノイズパターンによって決まっています。</translation>
    </message>
    <message>
        ...
        <translation>シリンダー形状を包み込む、シンプルで一様な霧です。この組み合わせは以前、GPUでは霧のないただのシリンダーとして静かにレンダリングされてしまうことがありました。このシーンは、GPUでもかすみが実際に表示されることを確認するために存在します。</translation>
    </message>
    <message>
        ...
        <translation>各小さなセルが単なる密度の値だけでなく独自の色も持つ、ボクセルベースのボリュームです。単一の均一な色合いの霧ではなく、柔らかく色鮮やかな、星雲のような雲としてレンダリングされます。</translation>
    </message>
    <message>
        ...
        <translation>各セルにつき密度の値が1つだけで、セルごとの色を持たない、ボクセルベースのボリュームです。色鮮やかなグリッドクラウドのシーンに比べて、よりシンプルな、柔らかく光る霧の塊としてレンダリングされます。</translation>
    </message>
    <message>
        ...
        <translation>プロ向けの映像効果ソフトウェアで使われているのと同じ形式の、実際のスパースクラウドデータファイルを読み込み、柔らかい霧の球としてレンダリングしています。このようなスパースなデータは、通常の均等な間隔のグリッドとはまったく異なる検索方法が必要で、うまく対応するのが特に難しい形式の1つです。CPUでは完全に動作しますが、GPUでは現在、単純で均一な厚みの霧にフォールバックします。これはバグではなく、既知の簡略化です。</translation>
    </message>
    <message>
        ...
        <translation>カメラ自体が、周囲に決まった形をまったく持たないかすみの中にあり、まるでレンズが曇っているかのようになっています。ここにある他のすべての霧のシーンが、霧に包まれたオブジェクトを外側から見る構成なのに対し、これは本質的に異なる構成です。CPUと、2つあるGPUレンダリング経路のうち一方では動作しますが、もう一方のGPU経路はまだ対応しておらず、霧なしでレンダリングされます。</translation>
    </message>
    <message>
        ...
        <translation>2つの曲面(鞍型となめらかなスロープ)は、平らな三角形で近似するのではなく、4つの角の点だけから直接構築されています。あえて柔らかくぼやけた金属仕上げを使うことで、シェーディングのグラデーションが表面の本当の曲がり具合を明らかにします。完璧な鏡面仕上げでは、1つのくっきりしたハイライトしか見えず、表面がまったく平らでないという事実が隠れてしまいます。</translation>
    </message>
    <message>
        ...
        <translation>手作業で作られた20面サイコロの形(正二十面体)で、実際の三角形メッシュとして組み立てられています。それぞれの平らな三角形の面は、隣の面となめらかに溶け合うのではなく、それぞれ独立したシェーディングを見せるため、滑らかで丸みを帯びた見た目ではなく、面取りされたローポリな見た目になります。</translation>
    </message>
    <message>
        ...
        <translation>根元が太く先端に向かって細くなる、実際に湾曲したテーパー状の繊維が、表面全体に散りばめられています。別のHair Fibersシーンで使われている「球を毛髪のように見せるシェーディング」の手法ではなく、本当に湾曲したジオメトリから構築されています。</translation>
    </message>
    <message>
        ...
        <translation>実際のテクスチャ座標データが焼き込まれた、外部から読み込んだ3Dモデルです。これにより画像を表面に正しく巻き付けることができます。このシーンはかつて、そのテクスチャデータが静かに失われてしまうことが原因で、2つあるGPUレンダリング経路の一方で真っ黒にレンダリングされていました。</translation>
    </message>
    <message>
        ...
        <translation>元のファイルに三角形化されていない面が含まれる、実際の外部3Dモデルファイルを最初から最後まで読み込んだものです。ローダーは、ファイル側ですでに三角形化されていると決めつけるのではなく、自ら三角形に分割しなければなりません。</translation>
    </message>
    <message>
        ...
        <translation>平らな三角形から構築するのではなく、滑らかに丸みを帯びた本物の湾曲した繊維を、同じ方法で作られたこのプロジェクト独自の毛髪デモと並べて表示しています。同じ視覚効果に対する2つのアプローチを直接比較できます。</translation>
    </message>
    <message>
        ...
        <translation>本物の湾曲した繊維を、初めて本格的な毛髪らしいマテリアルと組み合わせています。この2つの組み合わせによって、各繊維の輝きが湾曲した繊維ではなく平らな面であるかのように計算されてしまうバグが明らかになりました。これは、髪の毛がつや消しの鈍い見た目ではなく、繊維に沿って走る特徴的な光の筋を持つ理由そのものです。</translation>
    </message>
    <message>
        ...
        <translation>各頂点に明示的なテクスチャ座標が設定された、三角形ベースの3D形状です。これにより画像を正しく適用できます。テクスチャ座標のサポートが実装される前は、2つあるGPUレンダリング経路の一方で、このシーンはちょうど真っ黒にレンダリングされていました。</translation>
    </message>
    <message>
        ...
        <translation>このシーンでは、多数のサンプルを最終的なピクセルの色に変換する際に、あえてよりシンプルでブロック状の方法を使っています。そのため、なめらかな既定の方式に比べて輪郭やシルエットがやや硬く、ギザギザした見た目になります。他のシーンの通常の柔らかい輪郭のレンダリングと比較すると、その違いがわかります。</translation>
    </message>
    <message>
        ...
        <translation>1つの球に、シミュレートされたカメラのシャッターが開いている間の開始位置と終了位置という、2つの異なる位置が与えられています。そのため、くっきりした静止画のような球や、不自然に二重になったコピーではなく、2つの位置の間を結ぶ柔らかい方向性のある筋、つまり本物のモーションブラーとしてレンダリングされるはずです。比較のために、動かない基準用の球が隣に置かれています。これは、このプロジェクトが対応するすべてのレンダリングバックエンドで正しく動作します。</translation>
    </message>
    <message>
        ...
        <translation>球のモーションブラーのシーンと同じ、開始位置と終了位置によるモーションブラーの手法を、内部での動きの保存方法が少し異なる、円盤とシリンダーに適用したものです。これはCPUでレンダリングした場合にのみ動作します。2つのGPUレンダリング経路では現在、これらの形状はぼけることなく開始時のポーズのまま静止して表示されます。</translation>
    </message>
    <message>
        ...
        <translation>2枚の同一のフラットなパネルは、まったく同じジオメトリから作られており、既定ではどちらもカメラに背を向け、光源として暗いままになるはずです。2枚目だけ向きを反転させることで、1枚目は暗いままなのに2枚目だけが光るようになります。これは、形状の向きが正しく尊重されているかを視覚的に確認する簡潔な方法です。ここで間違いがあれば、両方とも暗いか、両方とも光って見えるはずだからです。</translation>
    </message>
    <message>
        ...
        <translation>古いシーンファイルとの互換性のために残されている、円錐・皿形(放物面)の古いプリミティブから作られた4つの形状です。単純な円錐、単純な皿形、通常の表面ではなく光る光源として使われる円錐、そして開いた端をカメラに向け、光線が不透明な素材を突き抜けるのではなく開口部から霧の中へ入っていけるようにした、霧の境界として使われる皿形です。CPUでのみレンダリングされます。両方のGPU経路は円錐・皿形をまったくスキップします。</translation>
    </message>
    <message>
        ...
        <translation>精巧なディテールを持つ古典的な彫像モデルで、ジオメトリ・マテリアル・照明・カメラのすべてがファイル自体の中で定義された、完全に自己完結したシーンファイルとして提供されています。ユーザー自身が作成したカスタムシーンファイルとまったく同じ読み込み経路をたどります。</translation>
    </message>
    <message>
        ...
        <translation>最初のデモシーンと同じCornell Boxを、あえて通常よりずっと少ないサンプル数でレンダリングし、きれいに収束せず目に見えてノイズが残るようにしています。「レンダリングオプション」タブのサンプラーの選択肢(Sobol、Z-Sobol、Padded Sobol、Stratified、PMJ02BN、Halton)を切り替えて再レンダリングしてみてください。それぞれがピクセル内および光源に向けて異なるパターンでサンプルを配置するため、シーンの他の部分はまったく変わらなくても、柔らかい影の縁のノイズの見え方が少しずつ異なります。これはCPUの既定のレンダラーにのみ影響し、GPUでレンダリングする場合、サンプラーの選択は効果がありません。</translation>
    </message>
    <message>
        ...
        <translation>Spectral Dispersionシーンと同じガラスのプリズムで、「レンダリングオプション」タブの「スペクトルレンダリング」チェックボックスの効果を見せるために特別に作られています。オフの場合、レンダラーは赤・緑・青しか追跡しないため、ガラスを通るすべての色の光は同じ度合いで屈折します。オンにすると、レンダラーは光の個々の波長を追跡するようになり、ガラスは波長ごとにごくわずかに異なる度合いで光を屈折させます。これにより、実際のガラスのプリズムと同じように、白色光が目に見える虹として広がります。CPU専用です。</translation>
    </message>
    <message>
        ...
        <translation>HDRI Skyデモと同じ、明るい空と影のシーンです。その本当の明るさの幅は、どんな画面が一度に表示できる範囲よりもはるかに広くなっています。「レンダリングオプション」タブの2つの設定が、この幅を異なる方法で扱います。露出は最初に適用される単純な明るさの倍率で、空も球も含めてすべてを均等に明るくします。トーンマッピングはその後に適用されるカーブで、その広い幅を画面が表示できる範囲に圧縮します。ACESは空の最も明るい部分を緩やかに丸め、Reinhardはより強く圧縮し、Noneは最も明るい部分をそのまま真っ白に切り詰めます。トーンマッピングの選択肢をいくつか変えつつ、露出のレベルも変えて試すことで、この2つの設定がそれぞれ独立して働く様子を確認できます。CPU・GPUの両方で動作します。</translation>
    </message>
    <message>
        ...
        <translation>最初のCornell Boxデモを、本当にノイズが目立つよう非常に少ないサンプル数でレンダリングしています。まず「レンダリングオプション」タブのOptiX AIデノイザーをオフにしてレンダリングし、次にオンにしてレンダリングしてみてください。デノイザーは、その斑点状のノイズパターンを認識し、追加の光線を一切追跡することなくなめらかにする機械学習モデルで、レンダリングコストを追加することなく、細部をわずかに犠牲にする代わりにずっときれいに見える画像を得られます。GPU専用です。近くにある「OptiX検証モード」のチェックボックスも知っておく価値がありますが、これは別物です。内部の追加の安全確認を有効にするだけで処理コストはかかりますが、画像の見た目は一切変わらないため、視覚的に比較できるものはありません。</translation>
    </message>
    <message>
        ...
        <translation>Cornell Rough Glassシーンと同じすりガラスのCornell Boxで、「インテグレーター」ドロップダウンのSPPMオプションを見せるために作られています。同じサンプル数で、まず既定のPath Tracerで、次にSPPM(「レンダリングオプション」タブ)でレンダリングしてみてください。粗いガラスの球の下にできる明るく集光した光の斑点(コースティクス)は、通常のパストレーシングが素早くきれいにするのを苦手とする、まさに扱いの難しい照明です。SPPMはまず光源から光を追跡してその着地点を記録し、その後カメラ側からそれを集めるという、こうした照明のために特別に作られた方法です。そのため、コースティクスはずっと早く目に見えて鮮明になるはずです。</translation>
    </message>
    <message>
        ...
        <translation>最初のCornell Boxデモを、「インテグレーター」ドロップダウンのBDPTとMLTオプションを見せるために使っています。既定のPath Tracerの代わりに、どちらか(「レンダリングオプション」タブ)を試してみてください。どちらも、カメラと光源の両方から同時に光路を追跡し、途中でつなぎ合わせます。これは、カメラからしか追跡しない既定のレンダラーやSPPMとは根本的に異なる戦略です。このシーンのように意味のあるバウンス照明があるシーンでは、この異なる戦略によって収束の仕方が変わり、より良くなることもあれば、単にノイズの性質が変わるだけのこともあります。</translation>
    </message>
    <message>
        ...
        <translation>最初のCornell Boxデモを使って、「インテグレーター」ドロップダウンの参照用・デバッグ用モードを一通り見ていきます。RandomWalkは、光源を狙うことも戦略を賢く組み合わせることもせず、各光線をただランダムな方向にバウンスさせるだけです。もっともシンプルで収束が遅い分、もっとも「正直な」基準となります。SimplePathは、まさにその2つの要素に対応する独立したチェックボックスを追加しています。「光源を直接サンプリング」だけをオンにすると、直接照明は安く得られますが、たまたま横にある明るい光源を向いたバウンスは単なる幸運です。「BSDFをサンプリング」だけをオンにすると、その逆になります。両方をオンにすると、この2つの戦略が組み合わされてよりきれいな結果になります。これはまさに、既定のPath Tracerが裏側ですでに行っていることです。同じ低いサンプル数でそれぞれをレンダリングして、まったく同じ部屋でどれだけ違った収束の仕方をするかを見てみてください。</translation>
    </message>
    <message>
        ...
        <translation>このデモのために特別に作られたCornell Boxで、通常の1つではなく5つの天井の光源があり、それぞれの明るさはあえて大きく偏っています(およそ1:2:6:15:80)。Uniformライトサンプラー(「レンダリングオプション」タブ)は、実際の重要度に関わらず、5つの光源それぞれに均等に5分の1ずつ光の狙い撃ちの労力を割きます。そのため、ほとんど寄与しない4つの光源に労力の大半を無駄にし、本当に明るい1つの光源は必要以上にノイズが残ってしまいます。PowerとBVHはどちらも代わりに、その支配的な光源に労力を重み付けします(BVHは距離と方向も考慮し、Powerは総明るさのみを考慮します)。同じ低いサンプル数で3つすべてをレンダリングし、明るい隅の光源周辺のノイズがどれだけ早く落ち着くかを比較してみてください。</translation>
    </message>
    <message>
        ...
        <translation>最初のCornell Boxデモを使って、「インテグレーター」ドロップダウンのAmbient Occlusionモードを紹介します。AOはそもそも通常の照明入りレンダリングではありません。これに切り替える(「レンダリングオプション」タブ)と、シーン内のすべての表面の実際の色とすべての実在の光源が完全に無視され、代わりに各点が周囲のジオメトリによってどれだけ囲まれているかを示す、単純なグレースケールの尺度に置き換わります。残るのは箱自体の接触影と壁が交わる隅だけで、これこそがAOを、照明を一切気にせずシーンの形状やジオメトリの複雑さを素早くプレビューする手段にしている理由です。</translation>
    </message>
    <message>
        ...
        <translation>Cornell Rough GlassやSPPMのデモシーンと同じすりガラスのCornell Boxです。粗いガラスの球の下にできる明るく集光した光の斑点は、通常のパストレーシングにおいて、迷い込んだ孤立した過度に明るいピクセル(fireflies)の典型的な発生源です。RegularizeとFirefly clamp(どちらも「レンダリングオプション」タブ)は、これを2つの異なる方法で解決します。Regularizeは最初のバウンス以降、ガラス自身の粗さを緩やかにぼかすことで、そもそもこの扱いにくい照明をきれいにサンプリングしやすくします。一方Firefly clampは、明るいピクセルの発生自体は許しつつ、1サンプルがとり得る明るさの上限を設定し、わずかな精度と引き換えに迷い込んだ明るい点に確実な上限を設けます。同じサンプル数で、まず何もオプションを使わずに、次に各オプションを1つずつ使ってレンダリングし、比較してみてください。</translation>
    </message>
    <message>
...
or video opens in its own tab here, so past renders stay
around while you compare or tweak settings.</source>
        <translation type="vanished">ベーシック設定からレンダリングを開始してください。完了した画像や動画は、それぞれここで専用のタブとして開かれるため、設定を比較したり調整したりする間も過去のレンダリング結果を残しておけます。</translation>
    </message>
</context>
</TS>
