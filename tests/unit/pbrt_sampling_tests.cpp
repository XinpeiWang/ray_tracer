// pbrt_sampling_tests.cpp
// Unit tests for pbrt-v4-aligned sphere/cone/triangle sampling functions
// and MIS heuristics in src/shared/mis_sampling.h.
//
// Tests:
//   MIS heuristics:
...
//    16.  Full sphere (cosThetaMax=-1) covers both hemispheres
//
//   SampleUniformTriangleD:
...
TEST(BalanceHeuristicTest2, EqualStrategiesHalfWeight) {
	double w = BalanceHeuristic(1, 0.5, 1, 0.5);
	EXPECT_NEAR(w, 0.5, kEps);
}

TEST(BalanceHeuristicTest2, DominantFApproachesOne) {
	double w = BalanceHeuristic(1, 1000.0, 1, 0.001);
	EXPECT_GT(w, 0.999);
}

TEST(PowerHeuristicTest2, EqualStrategiesHalfWeight) {
	double w = PowerHeuristic(1, 0.5, 1, 0.5);
	EXPECT_NEAR(w, 0.5, kEps);
}

TEST(PowerHeuristicTest2, DominantFGreaterThanBalance) {
	double balance = BalanceHeuristic(1, 10.0, 1, 1.0);
	double power   = PowerHeuristic(1, 10.0, 1, 1.0);
	EXPECT_GT(power, balance);
}

TEST(PowerHeuristicTest2, ZeroGPdfReturnsOne) {
...
TEST(SampleUniformSphereTest2, OutputIsUnitVector) {
	for (int i = 0; i < 100; ++i) {
...
TEST(SampleUniformSphereTest2, PDFIsInv4Pi) {
	EXPECT_NEAR(UniformSpherePDF(), kInv4Pi, kEps);
}

TEST(SampleUniformSphereTest2, RoundtripInverse) {
	for (int i = 1; i < 10; ++i) {
		for (int j = 1; j < 10; ++j) {
...
TEST(SampleUniformSphereTest2, MeanWzNearZero) {
	int N = 1000;
	double sum = 0.0;
	for (int i = 0; i < N; ++i) {
		double u0 = (i + 0.5) / N;
...
TEST(SampleUniformHemisphereTest2, OutputIsUnitVectorWzNonNegative) {
	for (int i = 0; i < 100; ++i) {
		double u0 = (i + 0.5) / 100.0;
		double u1 = ((i * 37) % 100 + 0.5) / 100.0;
		double wx, wy, wz;
...
TEST(SampleUniformHemisphereTest2, PDFIsInv2Pi) {
	EXPECT_NEAR(UniformHemispherePDF(), kInv2Pi, kEps);
}

TEST(SampleUniformHemisphereTest2, RoundtripInverse) {
	for (int i = 1; i < 10; ++i) {
		for (int j = 1; j < 10; ++j) {
			double u0 = i / 11.0, u1 = j / 11.0;
			double wx, wy, wz;
...
TEST(SampleUniformConeTest2, OutputIsUnitVectorWithinCone) {
	double cosThetaMax = 0.5;
	for (int i = 0; i < 100; ++i) {
		double u0 = (i + 0.5) / 100.0;
		double u1 = ((i * 37) % 100 + 0.5) / 100.0;
		double wx, wy, wz;
...
TEST(SampleUniformConeTest2, FullSphereCoversBothHemispheres) {
...
TEST(SampleUniformTriangleDTest, BarycentricsSumToOneAndNonNegative) {
	for (int i = 0; i < 100; ++i) {
		double u0 = (i + 0.5) / 100.0;
		double u1 = ((i * 37) % 100 + 0.5) / 100.0;
		double b0, b1, b2;
		SampleUniformTriangleD(u0, u1, b0, b1, b2);
...
TEST(SampleUniformTriangleDTest, RoundtripInverse) {
	for (int i = 1; i < 10; ++i) {
		for (int j = 1; j < 10; ++j) {
			double u0 = i / 11.0, u1 = j / 11.0;
			double b0, b1, b2;
...
TEST(SampleUniformTriangleDTest, MeanBarycentricNearOneThird) {
	int N = 1000;
