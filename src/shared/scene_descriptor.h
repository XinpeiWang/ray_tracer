...
struct SceneDesc {
	...
	bool        gpu_supported;
	// Recommended camera position (lookfrom) - mirrors this scene's
	// CameraConfig.lookfrom_x/y/z in scene_registry.h (kept in sync by hand;
	// same drift risk as gpu_supported above). The Qt GUI (mainwindow_slots.cpp's
	// onSceneChanged) resets the camera-position spinboxes to this whenever
	// the scene selection changes, so switching scenes doesn't leave a
	// wildly wrong leftover camera position from whatever scene was
...
inline const SceneDesc* get_all_scenes(int* out_count = nullptr) {
	static const SceneDesc kScenes[] = {
		...
		{ 24, SceneNames::HdriSky,
		   ...
		   "Medium",    200, false, true,   0.0, 2.0, 10.0 },
		...
		{ 36, SceneNames::RealisticCamera,
		   ...
		   "Medium",    200, false, true,   0.0, 2.0, -2.0 },
		{ 37, SceneNames::TriangleMesh,
		   ...
		   "Fast",      100, false, true,   0.0, 4.0, 8.0 },
	};
