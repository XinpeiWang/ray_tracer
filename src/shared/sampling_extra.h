...
//
// Previously carried its own byte-for-byte copy of sampling_helpers.h's
// entire contents instead of including it - an unreconciled fork where a fix
// to the shared prefix would silently apply to only whichever of the two
// physical files a given caller happened to include. Collapsed to a plain
// #include; only the genuinely additional content below survives as this
// file's own.
// ---------------------------------------------------------------------------
