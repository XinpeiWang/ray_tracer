...
struct PrincipledBxDF {
		...
		T u1, T u2, T u3) const
	{
		...
		// Diffuse: Lambertian * (1-metallic) * (1-F_diffuse); F at wiÃ‚Â·n per Disney
...
struct HairBxDF {
	...
	// wi_z = cosTheta_i * sin(phi_i)  Ã¢â‚¬â€ the z-component of wi in local frame.
	...
	CPU_GPU void eval_local(T wo_x, T wo_y, T wo_z,
		...
		// Divide by AbsCosTheta(wi) = |wi_z| Ã¢â‚¬â€ matches pbrt-v4 HairBxDF::f()
