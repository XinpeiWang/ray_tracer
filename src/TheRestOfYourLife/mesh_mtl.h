...
#define MESH_MTL_H
//==============================================================================================
// mesh_mtl.h -- MTL-aware OBJ mesh loader (load_obj_mtl/triangle_mesh_mtl) and
// the .mtl-parsing wrappers it's built on.
//
// Split out of mesh.h, which #includes this file at its own end -- see that
// file's header comment for the plain-OBJ loader (load_obj/triangle_mesh)
// this one is a real per-face-material sibling to. Independent of mesh.h's
// own loader: the two parse loops diverge enough (face material tracking,
// per-face material assignment vs. one shared_ptr for the whole mesh) that
// factoring out the overlap would cost more clarity than it saves -- see
// load_obj_mtl()'s own comment.
//
// Usage:
//   auto mesh = std::make_shared<triangle_mesh_mtl>("sponza.obj", fallback_mat);
//   world.add(mesh);
//==============================================================================================
