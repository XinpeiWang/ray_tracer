...
enum class SceneType {
	CORNELL_BOX = 0,        // Classic Cornell box with glass sphere
	BOUNCING_SPHERES = 1,   // Random spheres (final scene from "In One Weekend")
	CHECKERED_SPHERES = 2,  // Two spheres with checker texture
	EARTH = 3,              // Globe with earth texture (requires earthmap.jpg)
	PERLIN_SPHERES = 4,     // Spheres with Perlin noise texture
...
struct SceneConfig {
	...
	bool requires_files;      // Does scene need external files?
	bool gpu_compatible;      // Works with GPU renderer?
	int recommended_spp;      // Recommended samples per pixel
...
static const SceneConfig SCENE_CONFIGS[] = {
	{ SceneType::CORNELL_BOX, "Cornell Box", 
	  ...
	  false, true, 100, "Medium" },

	{ SceneType::BOUNCING_SPHERES, "Bouncing Spheres", 
	  "Random spheres with checker ground (In One Weekend final)", 
	  false, false, 100, "Slow" },
	...
	{ SceneType::PERLIN_SPHERES, "Perlin Spheres", 
	...
	{ SceneType::CORNELL_SMOKE, "Cornell Smoke", 
