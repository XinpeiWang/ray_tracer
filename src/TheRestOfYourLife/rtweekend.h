...
// camera.h's render() calls its own private camera::reseed_render_rng()
// once per scanline to overwrite this initial state with a deterministic
// one (see that method's own comment, camera.h, for why it lives there
// rather than here) - this lazy-init lambda still runs exactly once per
// thread either way, since a per-scanline reseed always follows shortly
// after, on the CPU-only path that actually cares about determinism.
