...
//      As of the BDPT/MLT CLI integration, this layer lives in its own
//      header (bsdf_bridge.h) so bdpt_adapter.h can reuse it without
//      pulling in power_light_sampler.h -- see that header's own file
//      comment for why (a global `class AliasTable` name collision with
//      src/shared/reservoir_sampler.h, which src/shared/mlt.h needs).
