...
//
// This file holds the plain, one-flat-material loader (load_obj/
// triangle_mesh). The real per-face-.mtl-material loader (load_obj_mtl/
// triangle_mesh_mtl) lives in mesh_mtl.h, #included at the bottom of this
// file so existing callers still get both from a single "#include mesh.h".
//
// Usage:
...
// The MTL-aware loader (load_obj_mtl/triangle_mesh_mtl) and its .mtl-parsing
// helpers live in mesh_mtl.h, #included here so every existing "#include
// mesh.h" caller keeps getting both loaders unchanged -- see that file's own
// header comment for why it's a separate, independent sibling rather than
// built on top of load_obj() above.
#include "mesh_mtl.h"
