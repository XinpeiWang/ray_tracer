...
## 179. Closing E4: RGB Grid Medium - a real heterogeneous "nebula" with an independent per-voxel R/G/B scattering grid, same trigger-sphere delta tracking as E2

CPU's `build_rgb_grid_medium_scene()` (`scenes_advanced.h`) wraps a
real pbrt-v4 `RGBGridMedium` (`src/shared/rgb_grid_medium.h`): a
heterogeneous medium whose scattering coefficient is THREE
INDEPENDENT 24x24x24 voxel grids, one per R/G/B channel, generated by
`generate_nebula_channel()` (`src/shared/rgb_nebula_generator.h` - a
real, plain, CPU_GPU-tag-free host function ALREADY SHARED by CPU and
OptiX's own scene builders, so all three backends render the exact
SAME voxel data given the same parameters, not just the same
algorithm). Sampled by delta tracking against a majorant, same shape
as E2's own CloudMedium - but with spatially-varying COLOUR (each
channel's own independent noise pattern), not just spatially-varying
grayscale density.

**Immediately tractable once E2's own infrastructure existed**: same
"invisible trigger sphere, ray/AABB overlap computed as pure math
against an affine world-to-medium transform" pattern E2's own
materialType 29 established, and `gpu/optix/optix_types.h` already
documented (for its own `GpuRgbGridMedium`) the SAME deliberate
simplification this section keeps: a single GLOBAL majorant rather
than CPU's real per-voxel DDA majorant grid, "to keep GPU delta
tracking algorithmically simple" (that enumerator's own words) - not
a new scope cut invented here, one already accepted on the other GPU
backend for this exact scene. `generate_nebula_channel<float>()` is
called directly, host-side, in `buildRgbGridMediumScene()`
(`metal_poc_scenes_e.mm`) - no port needed at all, since it's already
portable plain C++ and only its OUTPUT needs to reach the GPU, not the
generator itself.

**New**: `GpuRgbGridMedium` (bounds/affine transform/nx,ny,nz/
dataOffset/sigmaScale/sigmaMaj/phaseG - no `leDataOffset`/`Le_scale`,
since E4's own scene never sets per-voxel emission, unlike OptiX's
more general struct), materialType 30, a new flat `rgbGridData` buffer
(R-then-G-then-B voxel blocks, concatenated) alongside the metadata
buffer. The kernel's own delta-tracking loop (`metal_poc_kernel.metal`)
mirrors materialType 29's shape closely, with two real differences:
density is a trilinear lookup (`gpuRgbGridTrilinear()`, direct port of
OptiX's own `gpu_rgb_grid_trilinear()`) into the flat voxel buffer
instead of Perlin noise, and the per-scatter "albedo" is computed
DYNAMICALLY from the local R/G/B density ratio at the accepted scatter
point (`scatterColor = (sr,sg,sb)/max(sr,sg,sb)`) rather than a fixed
`TriangleMaterial::color` constant - the whole point of this scene
over E2's own is spatially-varying colour, so a single flat albedo
can't represent it. E2's own NEE-toward-the-constant-background
technique (section 178, majorant-based self-attenuation) is reused
verbatim, just sourcing `sigmaMaj`/`phaseG` from `GpuRgbGridMedium`
instead of `GpuCloudMedium` and weighting by `scatterColor` instead of
a fixed albedo.

**One real bug found via render-and-compare, not assumed correct from
the algorithm**: the first render showed almost NOTHING - no visible
box at all, just the two context spheres and empty sky/ground, with
render time suspiciously fast (a debug print confirmed `sigmaMaj` was
a healthy, non-zero ~3.07, ruling out a degenerate-density
explanation). Root cause: `GpuRgbGridMedium::boundsMin`/`boundsMax`
were populated with the scene's own WORLD-SPACE AABB (copying OptiX's
own `GpuRgbGridMedium` field comment - "world-space AABB" - literally)
but `rgbGridAabbSlabIntersect()`'s own contract (mirroring
`cloudAabbSlabIntersect()`'s own convention) compares these fields
directly against a ray ALREADY TRANSFORMED INTO MEDIUM SPACE, i.e.
expects a `[0,1]` unit cube, not world coordinates. Comparing a
medium-space ray (roughly `[0,1]^3`) against world-scale bounds
(dozens of units wide) meant the slab test essentially never found a
valid segment, so the medium was invisible - not a crash, a silent
near-total pass-through. Fixed by setting `boundsMin={0,0,0}`/
`boundsMax={1,1,1}` (medium space), matching `GpuCloudMedium`'s own
convention exactly - OptiX's own closest-hit code, on inspection,
never actually reads `bounds_min`/`bounds_max` at all in its own slab
test (it hardcodes the `[0,1]` literals directly), so the field
existing with that name/comment there was a latent trap for a generic
port, not evidence the field is meant to be read as world-space
bounds by any caller.

**Verified**: full clean `RT_BUILD_METAL=ON` rebuild, ctest (4/4), a
51-scene GPU smoke sweep (every currently-supported hand-authored
scene ID) with zero crashes, direct `--gpu` vs `--cpu` comparison
(both showing a matching mottled purple/blue/green nebula silhouette,
same size/position, denser and darker toward the box's own base,
translucent at the edges), and a before/after hash comparison (stash/
rebuild-clean/re-render, non-interleaved) across the same 16
representative sphere/medium scenes E2's own verification used
(including E2 itself, since this change also extends
`sphereIntersectionFunction`'s own shadow-ray occlusion-exemption
check to materialType 30) - all 16 byte-for-byte identical, no
regression.

**The honest limitation**: same class of gap E2 already documents -
real delta tracking/trilinear density, not simplified, with the gap
versus CPU entirely in the NEE self-attenuation approximation
(majorant-bound, not CPU's own continuous ratio-tracked transmittance).

With E2 and E4 both closed, every genuinely volumetric scene this
series scoped out in section 174's own closing accounting is now
done. That leaves 16 pre-existing failures, all in the categories
