...
| `none` / `""` (interface) | Full | Full | pbrt-v4's real interface-material idiom for a shape that bounds a participating medium with no BSDF response of its own — the ray should pass straight through, only the medium changes. Both backends build it via the exact same `Dielectric` path with `eta` forced to `1.001` (`1.0` exactly makes `FrDielectric()` divide `0/0` at grazing incidence), the same near-invisible substitute this project's own bundled `cloud-medium.pbrt`/`rgbgrid-medium.pbrt`/`uniformgrid-medium.pbrt` scenes already hand-author via a plain `dielectric` material — now recognized automatically for any scene using the real directive instead. |
