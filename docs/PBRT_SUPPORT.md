...
|---|---|---|---|
| `"float[4] cropwindow"` / `"integer[4] pixelbounds"` | Approx | Approx | Restricts rendering to a sub-rectangle of the frame - pbrt-v4 allows both together (cropwindow as an NDC fraction, pixelbounds in pixel space), each independently narrowing the region via intersection; both resolve here to one NDC-fraction rectangle (`pbrt_flatten::FlatScene::cropX0`/`X1`/`Y0`/`Y1`) rather than pixel indices, since `xresolution`/`yresolution` are only advisory in this codebase (a CLI width/height argument wins, same as `maxdepth`/`Sampler` type above) - a pixel-space bound resolved against the wrong resolution would be wrong, where a fraction stays correct. **Approx, not Full, on both backends**: real pbrt-v4 writes a smaller *output image* sized to just the crop rectangle; this codebase instead still writes the full `xresolution`×`yresolution` frame, with every pixel outside the crop rectangle left explicit black rather than sampled/traced - CPU via the existing per-pixel filter-weight-sum-of-zero path, GPU via an early-return in each backend's own primary-ray-generation kernel (`__raygen__rg`'s explicit black write; `generate_camera_rays`'s skip-the-enqueue, relying on `normalize_framebuffer`'s own pre-existing zero-weight-is-black guard) - a real, deliberate simplification on both, chosen to avoid rippling a genuinely different output image size through the PPM/EXR writers, the PNG conversion step, and the Qt GUI's preview, all of which currently assume the output image is `image_width`×`image_height`. **Scoped to the default path tracer on CPU, and to the two main GPU backends (recursive + wavefront) on GPU** - same "one integrator/backend gets a feature, the rest don't" shape as `regularize` above, but unlike `regularize` (whose gap on BDPT/MLT/SPPM happens to coincide with pbrt-v4's own off-by-default semantics), a crop request on `--bdpt`/`--mlt`/`--randomwalk`/`--ao`/`--simplepath`/`--simplevolpath`/`--lightpath`/`--sppm` (CPU) or GPU SPPM (`sppm_programs.cu`'s own `__raygen__sppm_camera_pass`, a genuinely separate launch-params/camera representation from `GpuCameraParams` - not wired this round) is real, unimplemented divergence - the full frame renders instead, with a console warning on CPU (`cpu_interface_bdpt.cpp`, `cpu_interface.cpp`'s SPPM entry point) and on GPU SPPM (`optix_interface.cpp`'s `optix_render_main_sppm`, checking the already-resolved `cameraExtra.cropX0`/`X1`/`Y0`/`Y1` against a full-frame match). The wavefront backend's own `generate_camera_rays` kernel (launched once per SAMPLE, unlike the recursive backend's single whole-render launch) goes further than an early-return: `wf_launch_generate_camera_rays` (`wavefront_launch.cu`) sizes its CUDA launch grid to just the crop rectangle when one is active, so a cropped-out pixel's GPU thread is never scheduled at all on that backend, not merely skipped after the fact. |
