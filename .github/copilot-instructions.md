...
- The GPU renderer is built via custom MSBuild targets (`build_cuda.targets`) and called in-process through the `gpu_render_main()` C API defined in `gpu/cuda/gpu_interface.h`.
