...
static bool spectral_scan_hittable(const hittable* h, std::string& error_out) {
	...
	if (const auto* ati = dynamic_cast<const animated_transform_instance*>(h))
		return spectral_scan_hittable(ati->get_object().get(), error_out);
...
extern "C" int cpu_render_main(int width, int height, int spp, int max_depth, const char* output_path,
	...
	try {
		...
		cam.time_limit_seconds = options.time_limit_seconds;
		...
		// request, applied AFTER setup_camera() so a loaded .pbrt scene's
		// own directives (set inside that same call - scene_registry.h's
		...
		// crop/seed only override when the CLI value differs from its
		// neutral default, same "explicit value wins outright" shape as
		// exposure (seed has no scene-directive equivalent at all to
		// preserve - see camera_t::seed's own comment).
		...
		if (options.seed >= 0) cam.seed = options.seed;
