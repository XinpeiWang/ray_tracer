			...
			break;
				...
				case MaterialType::CoatedDiffuse: {
					...
					if (random_float(seed) < F_in) {
						...
						// illuminated by BSDF-sampled paths that randomly hit
						// the light - no importance sampling toward it at all,
						// making it render far too dark. T_in (F_in) is fixed
						// for this scattering event; F_out is re-evaluated at
						// the light-sampled direction.
						if (params.numLights > 0) {
							int light_idx;
							float selection_pdf;
							if (params.aliasTable) {
