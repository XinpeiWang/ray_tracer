...
extern "C" __global__ void __miss__ms() {
	...
	// pack_aov_payload()'s own comment (optix_device_helpers.h). A miss has
	// no material/surface, so there's no real albedo/normal to report -
	// pack the sky/background color as the albedo proxy and the camera-
	// facing direction (-rayDir) as a reasonable dummy normal, the common
	// convention for sky pixels in denoiser guide layers.
	pack_aov_payload(color, -rayDir);
}
