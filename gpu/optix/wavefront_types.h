...
struct ShadowRayWorkItem {
	...
	// true for a shadow ray generated by a depth==1, GI-eligible hit's own
	// NEE (see wf_finish_material_scatter's giCandidateRadianceOut parameter
	// comment). accumulate_shadow (wavefront_kernels_accumulate.cu) routes
	// this ray's Ld into the separate per-pixel GI-candidate-radiance buffer
	// instead of the main framebuffer when this is set - every other shadow
	// ray's behavior is completely unchanged. Defaults false so every
	...
	unsigned int seed = 0;
	// Probe cache (Live Preview only, gpu/optix/wavefront_probe_cache.h) -
	// true for a shadow ray generated by probe_cache_shade's own one-light
	// NEE sample at a probe-update ray's hit point. Same redirect pattern as
	// isGiCandidate above: accumulate_shadow adds this ray's Ld into
	// WavefrontPathTracer::d_probeCacheRadianceOut_[pixelIndex] instead of
	// the real framebuffer when set - `pixelIndex` here is NOT a screen
	// pixel, it's this frame's own probe-update batch slot (see
	// ProbeCacheRayWorkItem::batchSlot's own comment). Defaults false so
	// every existing call site keeps going to the real framebuffer.
	bool   isProbeCacheRay = false;
	// Neural Radiance Cache training pipeline (Live Preview only, gpu/optix/
	// wavefront_kernels_nrc.cu) - true for a shadow ray generated by
	// nrc_training_shade_simple/_full's own one-light NEE sample at a
	// training path's vertex. Same redirect pattern as isProbeCacheRay
	// above: accumulate_shadow adds this ray's Ld into
	// WavefrontPathTracer::d_nrcTrainingRecords_[pixelIndex].directRadiance
	// instead of the real framebuffer when set - `pixelIndex` here is NOT a
	// screen pixel, it's this training vertex's own flat record index
	// (depth*kNrcTrainingPathsPerFrame + pathSlot, see NrcTrainingRecord's
	// own comment). Defaults false so every existing call site keeps going
	// to the real framebuffer.
	bool   isNrcTrainingRay = false;
};

// ReSTIR GI's own plain data structs (Live Preview only, gpu/optix/
// wavefront_restir_gi_math.h and wavefront_kernels_restir.cu) - same
// host+device-safe, math-free shape as optix_types.h's GpuLightSample/
// GpuReservoir, but defined here (not there) because they need
// kWFNWavelengths-sized spectral storage, which isn't visible in
// optix_types.h (wavefront_types.h includes THAT file, not the reverse).
//
// Unlike DI's light-sample payload (an AREA-domain pdf, origin-independent
// by construction - see wf_reevaluate_light_geometry's own comment), a GI
// candidate's pdf is a SOLID-ANGLE pdf at the originating primary hit x0
// from BSDF sampling - reusing it at a different origin requires an
// explicit Jacobian (wf_restir_gi_jacobian, wavefront_restir_gi_math.h),
// which is why GpuGiSample stores x0Point/x1Normal in addition to the
// sample point itself. `radiance` is cached as RGB, not spectral -
// deliberately mirroring GpuLightSample's own raw-RGB-emission convention
// (wf_light_raw_emission), re-uplifted to spectral fresh at consumption
// time using the CONSUMING pixel's own wavelengths (GpuGiOriginContext::
// wavelengths below) via the same SampledSpectrumToXYZ/wf_xyz_to_linear_rgb
// round trip addToFramebuffer already does - a cached SPECTRAL value would
// be meaningless once reused by a different pixel/frame with its own,
// independently-sampled hero wavelengths (hero-wavelength importance
// sampling is fundamentally per-path, not something ReSTIR reuse can share
// across paths).
struct GpuGiSample {
	float3 x1Point  = make_float3(0.0f, 0.0f, 0.0f);  // secondary hit point
	float3 x1Normal = make_float3(0.0f, 1.0f, 0.0f);  // secondary hit's own shading normal (needed by the Jacobian)
	float3 x0Point  = make_float3(0.0f, 0.0f, 0.0f);  // the ORIGINATING primary hit's position (needed by the Jacobian)
	float  pdfAtX0  = 0.0f;   // solid-angle pdf of the x0->x1 BSDF-sampled direction
	float3 radiance = make_float3(0.0f, 0.0f, 0.0f);  // cached Lo(x1 -> x0): x1's own NEE + emission-hit contribution, RGB
...
// Per-pixel stash of the ORIGINATING primary hit x0's shading context,
// written during depth==0's wf_finish_material_scatter call for every
// GI-eligible pixel (non-specular x0) and consumed by the GI finalize pass
// after depth 1 fully resolves (wavefront_kernels_restir.cu). By that point
// depth 0's own OptiX hit context (material, throughput, BSDF, this path's
// hero wavelengths...) no longer exists anywhere else - each bounce depth is
// a separate kernel launch over a compacted, reordered queue, and
// RayWorkItem::pixelIndex is the only thing DI-style single-call resampling
// ever needed to carry forward. GI's candidate radiance isn't known until a
// LATER call (depth 1's own NEE/emission resolves asynchronously via
// accumulate_shadow), so x0's context has to be remembered explicitly -
// this is the one piece of GI's design with no DI analog.
struct GpuGiOriginContext {
	float3 x0Point;
	float3 x0Normal;
	float3 dirX0ToX1;
	float  pdfAtX0;                          // 0 = no GI candidate this frame (x0 specular, RR-terminated, no lights, etc.)
	int    materialIdx = -1;
	float  throughputX0[kWFNWavelengths];    // x0's own spectral throughput (SS) at generation time
	// This path's hero wavelengths/pdfs (RayWorkItem::wavelengths/
	// wavelength_pdfs, fixed for the whole path) - needed to re-uplift a
	// reused reservoir sample's cached RGB radiance to spectral at finalize
	// time (GpuGiSample::radiance's own comment), since that pass runs
	// outside any wf_finish_material_scatter call and has no `swl` of its
	// own otherwise.
	float  wavelengths[kWFNWavelengths];
	float  wavelength_pdfs[kWFNWavelengths];
...
// probe_grid_types.h/wavefront_probe_cache.h) - a two-phase design, same
// "dedicated raygen, minimal payload, real shading in a plain CUDA kernel"
// split the shadow pass already uses (a raygen traces, a separate kernel
// shades), because wf_generate_restir_candidate()/wf_sample_*_light() (the
// full, all-light-kind NEE machinery this cache's own update ray wants to
// reuse rather than re-derive) live in wavefront_device_helpers.h's
// translation unit, which wavefront_programs.cu's OptiX raygen/hit programs
// (the only things allowed to call optixTrace()) cannot reach - see this
// project's own plan for the full "why not one dedicated raygen doing
// everything" rationale.
//
// `batchSlot` is NOT a probe index - it's this frame's own position
// (0..kProbesPerFrame-1) in the round-robin update batch, i.e. an index into
// WavefrontPathTracer::d_probeCacheRadianceOut_/d_probeCacheHitDistOut_
// (sized kProbesPerFrame, not totalProbes). The real probe index is
// recovered later, in probe_cache_accumulate, as
// (probeUpdateCursor_ + batchSlot) % totalProbes - the exact same
// round-robin arithmetic the host used to pick this ray's own origin in the
// first place - so nothing here needs to carry the real probe index at all.
struct ProbeCacheRayWorkItem {
	int          batchSlot;
	float3       origin;      // probe world position (host's own GpuProbeGridMeta::probeWorldPos())
	float3       direction;   // wf_rand_unit(seed)-sampled, built host-side - see WavefrontPathTracer::render()'s own comment
	unsigned int seed;
};

// Produced by __raygen__wf_probe_cache (wavefront_probe_cache.h, reuses
// wavefront_probe.h's own wf_trace_probe_ray()/WfProbePayload for the
// intersection query - no new hit/miss programs or SBT needed), consumed by
// probe_cache_shade (wavefront_kernels_restir.cu), which has the NEE
// machinery this needs.
struct ProbeCacheHitWorkItem {
	int    batchSlot;
	bool   hit;
	float3 hitPoint;
	float3 hitNormal;
...
struct WavefrontLaunchParams {
	...
	// Probe cache update rays (Live Preview only, gpu/optix/
	// wavefront_probe_cache.h) - traced via a FOURTH optixLaunch (same
	// intersect pipeline/module as the probe walk above, its own dedicated
	// raygen, no new hit/miss programs - reuses wavefront_probe.h's own
	// probeSBT_ hit groups via wf_trace_probe_ray()). Host-populated each
	// render() call (see WavefrontPathTracer::render()'s own comment) rather
	// than device-populated by a prior kernel, since "which probes to update
	// this frame" is host-side round-robin bookkeeping, not a per-ray
	// decision made during shading.
	WorkQueue<ProbeCacheRayWorkItem> probeCacheRayQueue;
