...
// HitQueue instead of within an OptiX program.
//
// The RNG/texture/light-sampling/camera-ray device helpers these kernels
// share now live in wavefront_device_helpers.h (see that file's own header
// comment for why the split there needed no build-file wiring, unlike
// splitting these __global__ kernels into further .cu files would).
